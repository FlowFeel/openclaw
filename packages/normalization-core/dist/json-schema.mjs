import { isRecord } from "./record-coerce.mjs";
...
function expandJsonSchemaTypeArray(schema) {
	...
	return {
		...
		anyOf: types.map((entry) => Object.assign({}, Object.fromEntries(branchEntries), { type: entry }))
	};
}
...
function isJsonSchemaValueValid(schema, value) {
	if (!isJsonValue(value)) return false;
	try {
		return Value.Check(normalizeJsonSchemaForTypeBox(schema), value);
	} catch {
		return false;
	}
}
//#endregion
export { isJsonSchemaValueValid, jsonSchemaValuesEqual, normalizeJsonSchemaForTypeBox };
