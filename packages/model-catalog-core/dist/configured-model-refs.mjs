import { t as isRecord } from "./record-coerce-B5vvQpkQ.mjs";
...
function collectConfiguredModelRefs(config, options = {}) {
	...
	if (isRecord(agents.entries)) for (const [agentId, entry] of Object.entries(agents.entries)) collectFromAgent(`agents.entries.${agentId}`, entry);
	else if (Array.isArray(agents.list)) for (const [index, entry] of agents.list.entries()) collectFromAgent(`agents.list.${index}`, entry);
...
//#endregion
export { AGENT_MODEL_CONFIG_KEYS, collectConfiguredModelRefValues, collectConfiguredModelRefs, listModelRefsFromConfigValue };
