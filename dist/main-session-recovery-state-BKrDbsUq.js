...
import { Q as hasSqliteSessionEntriesByStatusReadOnly } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { r as listConfiguredSessionStoreAgentIds, s as resolveAllAgentSessionStoreTargetsSync } from "./targets-Bz-meSET.js";
import { n as buildRestartRecoveryClaimCleanupPatch } from "./session-entry-slot-keys-DPRQmSpa.js";
...
//#region src/agents/main-session-restart-recovery-shared.ts
...
async function resolveRestartRecoveryStorePaths(params) {
	...
	if (params.cfg) {
		...
		const configuredStorePaths = new Set(configuredAgentIds.map((agentId) => path.resolve(resolveStorePath(params.cfg?.session?.store, {
			agentId,
			env
		}))));
	...
	return [...storePaths].filter((storePath) => hasSqliteSessionEntriesByStatusReadOnly({
		env,
		storePath
...
//#endregion
export { DEFAULT_RECOVERY_DELAY_MS as a, hasCurrentProcessOwner as c, normalizeStringSet as d, resolveRestartRecoveryStorePaths as f, transitionMainSessionRecovery as i, log as l, isMainSessionRecoveryPending as n, UNRESUMABLE_SESSION_NOTICE as o, normalizeMainSessionRecoveryRunFences as r, buildRestartRecoveryExpectedState as s, isMainRestartRecoveryCandidate as t, normalizeFiniteTimestamp as u };
