...
import { a as parseSystemAgentOperation, i as isPersistentSystemAgentOperation, n as describeSystemAgentPersistentOperation, t as executeSystemAgentOperation } from "./operations-9m6TOUCP.js";
import { n as hashSystemAgentOperation } from "./system-agent-tool-C67001Ey.js";
...
import { t as approvalQuestion } from "./dialogue-DpmYr4gE.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./system-agent-tool-dwACRuH9.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-JgSKs66X.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-C8PdYTWR.js")).runEmbeddedAgent)({
				...shared,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
				systemAgentTool,
...
var SystemAgentChatEngine = class {
	...
	*/
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.opts.planWithAssistant ?? (await import("./assistant-DHITTDLN.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-DHITTDLN.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
			verifiedInference: this.verifiedInference,
			deps: this.opts.deps
		}));
