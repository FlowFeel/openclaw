...
async function createNativeExclusiveFile(targetPath, mode) {
	...
	let created;
	try {
		const opened = binding.openBeneath(parent.fd, basename, nativeOpenFlags(fs.constants.O_WRONLY | fs.constants.O_CREAT | fs.constants.O_EXCL));
		fd = opened.fd;
			...
			while (true) {
				...
				try {
					...
					if (handle) {
						...
						if (!options.lockRoot) await fs$1.rm(lockPath, { force: true }).catch(() => void 0);
...
//#endregion
export { readSidecarLockSnapshotSync as a, serializeSidecarLockPayload as c, syncNativeFileBestEffort as d, writeNativeFd as f, sidecarLockPayloadIsStale as i, sidecarLockSnapshotMatches as l, withSidecarLock as n, relativeSidecarLockPath as o, computeSidecarLockDelayMs as r, removeSidecarLockIfUnchangedSync as s, createSidecarLockManager as t, removeNativeCreatedFileIfStillPinned as u };
