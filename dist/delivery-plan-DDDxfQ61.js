...
function resolveFailureDestination(job, globalConfig) {
	...
	if (hasJobFailureDest) {
		...
		const jobChannel = normalizeChannel(jobFailureDest.channel) ?? (jobTo ? resolveTargetPrefixedChannel(jobTo) : void 0);
		...
		if (hasJobChannelField || jobChannel && jobTo) {
			...
			if (jobChannel && jobChannel !== globalChannel) {
				if (!hasJobToField) to = void 0;
				if (!hasJobAccountIdField) accountId = void 0;
			}
		}
		...
		if (hasJobModeField || jobImpliesAnnounce) {
			...
			if ((mode ?? "announce") !== (effectiveJobMode ?? "announce")) {
				if (!jobToExplicitValue) to = void 0;
				if (!hasJobAccountIdField) accountId = void 0;
			}
