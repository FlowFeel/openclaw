...
import { n as loadStaticManifestCatalogRowsForList, r as resolveManifestCatalogCoverageForList, t as loadManifestCatalogRowsForList } from "./list.manifest-catalog-CiMY1ZqF.js";
//#region src/commands/models/list.scoped-catalog.ts
/** Dependency-light model catalog snapshots for default model-list views. */
const persistedCatalogModuleLoader = createLazyImportLoader(() => import("./list.persisted-catalog-C0RnkJnz.js"));
const preparedScopedCatalogModuleLoader = createLazyImportLoader(() => import("./prepared-model-runtime.scoped-catalog-4UZdaLfY.js"));
...
async function loadScopedListModelCatalogSnapshot(params) {
	...
	const manifestEntries = selectProviderRows(loadManifestCatalogRowsForList(loaderParams), providerIds).map(toCatalogEntry);
	...
	const staticEntries = selectProviderRows(loadStaticManifestCatalogRowsForList(loaderParams), providerIds).map(toCatalogEntry);
