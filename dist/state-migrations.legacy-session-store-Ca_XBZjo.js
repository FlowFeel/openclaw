...
import { a as mergeDeliveryContext, c as normalizeSessionDeliveryState, i as isCanonicalSessionDeliveryState, o as normalizeDeliveryChannelRoute, s as normalizeDeliveryContext, t as deliveryContextFromChannelRoute } from "./delivery-context.shared-DR6KpKlV.js";
...
import { C as resolveAgentHarnessSessionStoreTransitionError, Dt as hydrateSessionStoreSkillPromptRefs, Et as ensureSessionStorePromptBlobsForPersistence, F as enforceSessionDiskBudget, G as pruneStaleEntries, H as capEntryCount, K as pruneStaleModelRunEntries, Ot as projectSessionStoreForPersistence, S as resolveAgentHarnessSessionStoreError, U as getActiveSessionMaintenanceWarning, V as resolveMaintenanceConfig, X as shouldRunModelRunPrune, Z as shouldRunSessionEntryMaintenance, c as normalizeRestartRecoveryEntryFields, k as normalizeSessionRuntimeModelFields, ot as collectSessionMaintenancePreserveKeysForStore, t as normalizeSessionEntrySlotKey, v as isValidAgentHarnessSessionStoreEntry, wt as runExclusiveSessionStoreWrite } from "./session-entry-slot-keys-DPRQmSpa.js";
import fs from "node:fs";
import path from "node:path";
import { Buffer as Buffer$1 } from "node:buffer";
import { deserialize, serialize } from "node:v8";
const MAGIC_BYTES = Buffer$1.from("OC-V8\n", "utf-8");
/**
...
*/
function detectSerializationFormat(bytes) {
	if (bytes.length >= MAGIC_BYTES.length) {
		for (let i = 0; i < MAGIC_BYTES.length; i++) if (bytes[i] !== MAGIC_BYTES[i]) return "json";
		return "v8";
	}
	return "json";
}
...
*/
function isJsonSafeValue(value, seen, depth = 0) {
	if (depth > 200) return false;
	if (value === null || value === void 0) return true;
	const type = typeof value;
	if (type === "string" || type === "number" || type === "boolean") return Number.isFinite(value) || type !== "number";
	if (type === "bigint" || type === "symbol" || type === "function") return false;
	if (type !== "object") return false;
	const obj = value;
	if (obj instanceof Map || obj instanceof Set || obj instanceof ArrayBuffer || ArrayBuffer.isView(obj)) return false;
	if (obj instanceof Date) return false;
	const visited = seen ?? /* @__PURE__ */ new WeakSet();
	if (visited.has(obj)) return false;
	visited.add(obj);
	if (Array.isArray(obj)) {
		for (const item of obj) if (!isJsonSafeValue(item, visited, depth + 1)) return false;
		return true;
	}
	for (const key of Object.keys(obj)) {
		if (typeof key !== "string") return false;
		const prop = obj[key];
		if (!isJsonSafeValue(prop, visited, depth + 1)) return false;
	}
	return true;
}
/**
...
*/
function chooseSerializationFormat(value) {
	if (isJsonSafeValue(value)) return {
		format: "json",
		reason: "all values are JSON-native; JSON is human-readable and backward-compatible"
	};
	return {
		format: "v8",
		reason: "value contains non-JSON-native types (Map/Set/Date/BigInt/binary/circular); v8 is lossless"
	};
}
/**
...
*   format // → "json"
*/
function serializeSessionStore(value) {
	if (chooseSerializationFormat(value).format === "v8") {
		const payload = serialize(value);
		return {
			bytes: Buffer$1.concat([MAGIC_BYTES, Buffer$1.from(payload)]),
			format: "v8"
		};
	}
	return {
		bytes: Buffer$1.from(JSON.stringify(value, null, 2), "utf-8"),
		format: "json"
	};
}
/**
...
*/
function deserializeSessionStore(bytes) {
	if (detectSerializationFormat(bytes) === "v8") {
		try {
			const value = deserialize(bytes.subarray(MAGIC_BYTES.length));
	...
	try {
		const text = Buffer$1.from(bytes).toString("utf-8");
...
const loadTrajectoryCleanupRuntime = createLazyRuntimeModule(() => import("./cleanup-D1g6ctNb.js"));
...
async function persistLegacySessionStore(storePath, store) {
	...
	await replaceFileAtomic({
		filePath: storePath,
		content: storeFormat === "json" && !storeBytes.toString("utf-8").endsWith("\n") ? Buffer.concat([storeBytes, Buffer.from("\n")]) : storeBytes,
		mode: 384,
...
//#endregion
export { ensureMigrationDir as a, parseSessionStoreJson5 as c, updateLegacySessionStore as i, readSessionStoreJson5 as l, normalizeLegacySessionEntryDelivery as n, existsDir as o, saveLegacySessionStore as r, fileExists as s, loadLegacySessionStore as t, safeReadDir as u };
