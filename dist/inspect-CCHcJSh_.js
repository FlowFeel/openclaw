...
import { h as resolveGatewayWindowsTaskName, m as resolveGatewaySystemdServiceName, r as GATEWAY_SERVICE_MARKER, u as resolveGatewayLaunchAgentLabel } from "./constants-Qf4OESGR.js";
import { r as resolveHomeDir } from "./paths-DsnYGLDu.js";
import { a as readLaunchDaemonPlistLabel, f as resolveLaunchAgentLabel, l as parseLaunchdPlistLabel } from "./launchd-system-TrOLshYs.js";
import { r as parseSystemdExecStart } from "./systemd-unit-DACqAHrA.js";
import { t as execSchtasks } from "./schtasks-exec-BRCuFi9x.js";
...
function renderGatewayServiceCleanupHints(services = []) {
	...
	for (const service of services) switch (service.platform) {
		...
		case "linux": {
			...
			hints.push(`${systemctlCommand} disable --now -- ${quotePosixCleanupArgument(service.label)}`);
			if (service.detail.startsWith("unit:")) {
				const unitPath = service.detail.slice(5).trim();
				if (unitPath) {
					const removeCommand = service.scope === "system" ? "sudo rm" : "rm";
		...
		case "win32":
			if (/^[A-Za-z0-9_. ()\\/-]+$/.test(service.label)) hints.push(`schtasks /Delete /TN "${service.label}" /F`);
			break;
	}
	return hints;
}
...
async function scanLaunchdDir(params) {
	...
	for (const { name: labelFromName, fullPath, contents } of candidates) {
		const nativeLabel = params.includeManagedOpenClaw ? await readLaunchDaemonPlistLabel(fullPath) : null;
		const label = nativeLabel?.status === "ok" ? nativeLabel.label : parseLaunchdPlistLabel(contents) ?? labelFromName;
		...
		const marker = label === params.managedLabel || hasGatewayServiceMarker(contents) || executionMarker === "openclaw" ? "openclaw" : executionMarker === "clawdbot" || legacyLabel ? "clawdbot" : null;
		if (!marker) continue;
		if (!params.includeManagedOpenClaw && isIgnoredLaunchdLabel(label)) continue;
		if (!params.includeManagedOpenClaw && marker === "openclaw" && isOpenClawGatewayLaunchdService(label, contents)) continue;
		results.push({
			platform: "darwin",
			label,
