...
import { b as findTaskByRunId, v as publishTaskRecordAfterAtomicStore, x as getTaskById } from "./task-registry-Bs8cA6Tm.js";
import { f as upsertTaskRunRowInDatabase, t as bindTaskRecord } from "./task-registry.store.sqlite-Cr1yxp4f.js";
import "./runtime-internal-grPJIfa5.js";
import { b as subagentRuns, c as upsertSubagentRunRowInDatabase, d as ensureDeliveryState, t as bindSubagentRunRecord } from "./subagent-registry.store.sqlite-CnKtxK9F.js";
import { t as ANNOUNCE_COMPLETION_HARD_EXPIRY_MS, y as resolveSubagentCompletionResultText } from "./subagent-registry-helpers-DG9sluup.js";
import { _ as upsertBoundDeliveryQueueEntryInDatabase, g as loadDeliveryQueueEntryInDatabase, h as bindDeliveryQueueEntry, s as getDeliveryQueueEntryStatus, u as loadDeliveryQueueEntryAnyStatus } from "./delivery-queue-sqlite-C2mJ-TeJ.js";
import { _ as prepareClaimedSessionDelivery, a as SessionDeliveryDeferredError, i as SessionDeliveryDeadLetteredError, m as loadPendingSessionDelivery, p as loadPendingSessionDeliveries, r as SESSION_DELIVERY_QUEUE_NAME, t as drainPendingSessionDeliveries, v as releaseSessionDeliveryClaim } from "./session-delivery-queue-COwVOsBb.js";
import { t as computeBackoffMs } from "./delivery-recovery.shared-CehMLTLc.js";
...
//#endregion
//#region src/agents/subagent-completion-admission.store.ts
...
//#endregion
//#region src/agents/subagent-completion-delivery.ts
...
async function settleCorrelatedSubagentDelivery(queued, outcome) {
	...
	publishCommittedRecords(subagent, projectedTask);
	if (outcome === "recovered") {
		const { resumeSubagentRun } = await import("./subagent-registry-BH3v6WwW.js");
...
async function retrySubagentCompletionDelivery(taskId, databaseOptions) {
	...
	const failed = loadDeliveryQueueEntryAnyStatus(SESSION_DELIVERY_QUEUE_NAME, delivery.queueId);
	if (!failed || failed.kind !== "agentTurn" || failed.owner?.kind !== "subagent_completion") return {
		ok: false,
		reason: "retained delivery route is unavailable"
	};
	const payload = {
		...failed,
	...
	if (admitted.claimed) await releaseSessionDeliveryClaim(admitted.id);
...
function dismissSubagentCompletionDelivery(taskId) {
	const task = getTaskById(taskId);
	const current = task ? findSubagentForTask(task) : void 0;
...
//#endregion
export { settleCorrelatedSubagentDelivery as a, startSessionDeliveryRuntime as c, retrySubagentCompletionDelivery as i, dismissSubagentCompletionDelivery as n, schedulePendingSessionDeliveries as o, resolveCorrelatedSubagentDelivery as r, scheduleSessionDelivery as s, admitCorrelatedSubagentSessionDelivery as t };
