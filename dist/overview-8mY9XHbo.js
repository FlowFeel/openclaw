...
import { d as resolveConfigPath, g as resolveGatewayPort } from "./paths-CL43LNS6.js";
...
function resolveFastTestReferences(env) {
	if (!isFastTestRuntimeEnv(env)) return;
...
async function loadSystemAgentOverview(opts = {}) {
	...
	let gatewayError;
	try {
		const details = (deps.buildGatewayConnectionDetails ?? (await import("./call-DctZOBCQ.js")).buildGatewayConnectionDetails)({
			config: cfg,
			configPath
		});
