...
import { o as getActivePluginChannelRegistryVersion, p as getActivePluginRegistryVersion } from "./runtime-WkCmQTS9.js";
...
import { n as getRegisteredAgentHarness } from "./registry-DkL0MuB5.js";
import { p as stringifyRouteThreadId } from "./channel-route-BmrWdIq2.js";
import { d as sessionDeliveryOrigin, n as deliveryContextFromSession } from "./delivery-context.shared-DR6KpKlV.js";
import { qi as validateToolsEffectiveParams } from "./src-BSn6va4B.js";
...
import { s as peekSessionMcpRuntime, y as resolveSessionMcpConfigSummary } from "./agent-bundle-mcp-manager-api-D2GddaSc.js";
import { i as getPluginToolMeta } from "./tools-sUgO7zqy.js";
import { t as buildBundleMcpToolsFromCatalog } from "./agent-bundle-mcp-materialize-ajBEG2-c.js";
...
import { n as getConnectedNodePluginToolsVersion } from "./node-plugin-tool-snapshot-BycmRmDB.js";
...
import { a as resolveEffectiveToolRawDescription, i as resolveEffectiveToolLabel, o as summarizeEffectiveToolDescription, r as disambiguateEffectiveToolLabels } from "./tools-effective-inventory-build-BD5BugKK.js";
import { i as buildEffectiveToolInventoryGroups, n as resolveEffectiveToolInventory, r as resolveEffectiveToolInventoryRuntimeModelContextAsync } from "./tools-effective-inventory-qqe1xxdf.js";
...
function scheduleBaseToolsEffectiveRefresh(key, context) {
	...
	const task = new Promise((resolve, reject) => {
		setImmediate(() => {
			resolveBaseToolsEffectiveInventory(context).then((value) => {
...
async function resolveBaseToolsEffectiveInventory(context) {
...
async function resolveReadOnlyToolsEffectiveInventory(context) {
	...
	const harness = context.agentHarnessId ? getRegisteredAgentHarness(context.agentHarnessId)?.harness : void 0;
	...
	return await projectMcpCatalog({
		...
		catalog,
		context,
		runtimeModelContext: runtime.workspaceDir === context.workspaceDir ? baseResolution.runtimeModelContext : await resolveEffectiveToolInventoryRuntimeModelContextAsync({
			cfg: context.cfg,
			agentId: context.agentId,
...
function resolveTrustedToolsEffectiveContext(params) {
	const loaded = loadSessionEntryReadOnly(params.sessionKey, params.requestedAgentId ? { agentId: params.requestedAgentId } : void 0);
	if (!loaded.entry) {
	...
	return {
		...
		replyToMode: resolveReplyToMode(loaded.cfg, delivery?.channel ?? origin?.provider, delivery?.accountId ?? origin?.accountId, loaded.entry.chatType ?? origin?.chatType)
	};
}
