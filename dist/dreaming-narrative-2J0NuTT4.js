...
import { n as extractErrorCode, r as formatErrorMessage, s as readErrorName } from "./errors-D-7D3ZtF.js";
...
import { i as updateDreamsFile, n as resolveDreamsPath, t as readDreamsFile } from "./dreaming-dreams-file-B8fjxfWh.js";
import { n as DREAMING_SESSION_KEY_PREFIX, r as scrubDreamingNarrativeArtifacts } from "./dreaming-session-cleanup-BlamTwSw.js";
...
function buildNarrativeAttemptKey(baseKey, attempt) {
...
function buildNarrativeWorkspaceHash(workspaceDir) {
		...
		try {
				...
				try {
					try {
						...
						if (!isRequestScopedSubagentRuntimeError(preCleanupErr)) {
							...
							params.logger.warn(`memory-core: narrative pre-cleanup failed for ${params.data.phase} phase: ${cleanupFailure}`);
						}
					}
			...
			for (const attempt of attempts) {
				...
				try {
					...
					params.logger.warn(`memory-core: narrative session cleanup failed for ${params.data.phase} phase: ${cleanupFailure}`);
				}
			}
			await scrubDreamingNarrativeArtifacts({
				...
				params.logger.warn(`memory-core: dreaming cleanup scrub failed for ${params.data.phase} phase: ${cleanupFailure}`);
			});
		}
	});
...
function runDetachedNarrativeJob(params) {
	queueMicrotask(() => {
		detachedNarrativeLimit(params.job).then((outcome) => {
			if (outcome.status === "degraded") params.logger.warn(`memory-core: detached dreaming narrative degraded for ${params.phase} phase [workspace=${params.workspaceDir}]: ${outcome.error}`);
		}).catch(() => {});
	});
}
/**
...
async function runDreamNarrative(params) {
	...
	const job = agentId ? () => generateAndAppendDreamNarrative({
		...
		await appendFallbackNarrativeEntry({
			...rest,
			nowMs: Number.isFinite(rest.nowMs) ? rest.nowMs : Date.now(),
...
//#endregion
export { runDreamNarrative as a, removeBackfillDiaryEntries as i, dedupeDreamDiaryEntries as n, writeBackfillDiaryEntries as o, readRecentDreamDiaryEntries as r, appendFallbackNarrativeEntry as t };
