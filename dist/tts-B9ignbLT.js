...
import { a as assertProviderBinaryResponseContent, i as assertOkOrThrowProviderError } from "./provider-http-errors-Dm9G78mz.js";
...
import { a as isDebugProxyGlobalFetchPatchInstalled, t as captureHttpExchange } from "./runtime-L0PfyXri.js";
...
async function openaiTTS(params) {
	...
	try {
		...
		const audio = await readResponseWithLimit(response, maxBytes, { onOverflow: ({ maxBytes: maxBytesLocal }) => /* @__PURE__ */ new Error(`OpenAI TTS audio response exceeds ${maxBytesLocal} bytes`) });
		if (audio.byteLength === 0) throw new Error("OpenAI TTS API error: malformed audio response");
		return audio;
	} finally {
		await release();
	}
