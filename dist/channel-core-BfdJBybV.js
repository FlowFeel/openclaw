import { r as createChannelPluginBase$1 } from "./core-79EQWAsk.js";
...
function createChannelProgressUiHints(params) {
	...
	return {
		...
		"streaming.progress.labels": {
			...
			help: params.labels === "openclaw" ? "Candidate labels for streaming.progress.label=\"auto\". Leave unset to use OpenClaw built-in progress labels." : "Candidate labels for streaming.progress.label=\"auto\". Leave unset to use the built-in \"Working\" label."
		},
		...
		"streaming.progress.toolProgress": {
			...
			help: params.titleWording ? "Show compact tool/progress lines in progress mode (default: true). Set false to keep only the title until final delivery." : "Show compact tool/progress lines in progress draft mode (default: true). Set false to keep only the label until final delivery."
		},
...
function createChannelStreamingUiHints(channelLabel, entries) {
	return Object.fromEntries(Object.entries(entries).map(([key, value]) => [key ? `streaming.${key}` : "streaming", typeof value === "string" ? {
		label: `${channelLabel} ${STREAMING_HINT_LABELS[key]}`,
		help: value
	} : value]));
}
...
function createChannelConfigUiHints(params) {
	return {
		...
		...params.streaming ? createChannelStreamingUiHints(params.channelLabel, params.streaming) : {},
