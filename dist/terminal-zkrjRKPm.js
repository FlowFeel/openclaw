...
import { i as NODE_DUPLEX_INVOKE_IDLE_TIMEOUT_MS, p as NODE_TERMINAL_UPLOAD_COMMAND } from "./node-commands-CeG5w7c2.js";
import { t as ErrorCodes } from "./gateway-error-details-mJ5vWsi5.js";
import { $h as validateTerminalResizeParams, Qh as validateTerminalOpenParams, Xh as validateTerminalCloseParams, Yh as validateTerminalAttachParams, Zh as validateTerminalInputParams, eg as validateTerminalTextParams, ng as validateTerminalUploadResult, tg as validateTerminalUploadParams } from "./src-BSn6va4B.js";
...
import { r as resolveSessionCatalogProvider } from "./session-catalog-CON0j9e6.js";
...
const terminalHandlers = {
	...
	"terminal.text": async (opts) => {
		const { params, respond, context } = opts;
		if (!assertValidParams(params, validateTerminalTextParams, "terminal.text", respond)) return;
		if (!requireConnId(opts)) return;
		const p = params;
		if (!context.terminalSessions || !terminalEnabled(context)) {
			respond(false, void 0, errorShape(ErrorCodes.UNAVAILABLE, "terminal is not available"));
