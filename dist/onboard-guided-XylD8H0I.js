...
import { a as requireRiskAcknowledgement } from "./setup.shared-CenLq3Go.js";
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-BMll00Qy.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-DXdl4oSf.js") : void 0;
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-B7R9AdB5.js")).runSetupMemoryImportStep)({
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-tFlXBKZs.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-Cb8CAJV5.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-B7R9AdB5.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-D3q12h4e.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
		...
		if (recommendedConfig !== persistedConfig) {
			...
			const { mergeWizardConfigOntoLatest, writeWizardConfigFile } = await import("./setup.shared-B1YQd8f0.js");
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-DjmEzuUh.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
...
async function launchHatchTui(workspace) {
	const [{ launchTuiCli }, { DEFAULT_BOOTSTRAP_FILENAME }, { restoreTerminalState }, fs, path] = await Promise.all([
		import("./tui-launch-BDe0EceF.js"),
		import("./workspace-yjZWPD2s.js"),
