...
import { at as isJsonObject } from "./shared-client-CDGOYFn5.js";
import { c as fingerprintCodexMirrorSourceMessage, d as attachCodexMirrorIdentity, l as readCodexMirrorSourceFingerprint, m as readUpstreamUserText, p as readMirrorIdentity, t as codexTranscriptMirrorRuntime } from "./transcript-mirror-CcsTV4P4.js";
import { t as runBoundedCodexAppServerTurn } from "./bounded-turn-BJYeonsc.js";
import { n as createAssistantMessage } from "./event-projector-assistant-message-DW9V0uXg.js";
...
function projectUserMessage(message) {
	...
	if (upstreamUserText && typeof record.content === "string") return [{
		type: "message",
		role: "user",
		content: [{
			type: "input_text",
			text: requireBoundedText(upstreamUserText, "upstream user text")
		}]
	}];
	if (typeof record.content === "string") return [{
		...
		content: [{
			type: "input_text",
			text: requireBoundedText(record.content, "user message")
		}]
	}];
	if (!Array.isArray(record.content)) throw new Error("Codex settled-turn projection found unsupported user content");
	const content = [];
	for (const value of record.content) {
		...
		throw new Error(`Codex settled-turn projection does not support user content ${String(value.type)}`);
	}
...
function projectToolResult(message) {
	...
	if (message.isError !== void 0 && typeof message.isError !== "boolean") throw new Error("Codex settled-turn projection found invalid tool result status");
...
async function runCodexSettledTurnFinalization(operation, options) {
	...
	if (!text || isSilentReplyText(text)) throw new Error("Codex settled-turn finalization completed without a visible answer");
	...
	const persistedAssistantRecord = persistedAssistant;
	const assistantTranscriptIdempotencyKey = typeof persistedAssistantRecord.idempotencyKey === "string" ? persistedAssistantRecord.idempotencyKey.trim() : "";
	return {
