...
import { l as redactToolDetail } from "./redact-BGqyyKN9.js";
...
import { t as logDebug } from "./logger-CWK_jOH2.js";
import { y as notifyListeners } from "./agent-events-COCf-9-O.js";
import { d as getActivePluginRegistry } from "./runtime-WkCmQTS9.js";
import { k as getPluginSessionExtensionStateSync } from "./loader-CT1KBBu5.js";
...
import { c as createPluginToolMatcherScope, d as pluginToolMatcherCoversTool, m as cloneHookIsolationValue, t as getGlobalHookRunner, u as normalizePluginToolMatcher, y as getGlobalHookRunnerRegistry } from "./hook-runner-global-BNa5TLJ4.js";
import { i as resolveSandboxInputPath } from "./sandbox-paths-Cwxd9MU0.js";
import { Ct as buildSessionCreationStamp, ct as loadSqliteSessionEntryReadOnly, lt as patchSqliteSessionEntry, o as appendSqliteTranscriptMessage } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { i as listChannelPlugins, t as getChannelPlugin } from "./registry-DOSZ6AfI.js";
...
import "./types-HshsOZMQ.js";
import { c as canonicalizePath } from "./curator-LSeZNHiW.js";
import { c as resolveToolExecutionErrorKind, i as protectNetworkToolExecutionError, l as resolveToolResultFailureKind, r as isTrustedToolExecutionPreflightError, t as formatToolExecutionErrorMessage } from "./tool-result-error-BI56-yEJ.js";
...
import { u as resolveAgentRunAbortLifecycleFields } from "./run-termination-CczpQCyZ.js";
import { a as getCodeModeExecBeforeHookMetadataForToolKind, c as normalizeCodeModeExecBeforeHookParams, i as getCodeModeExecBeforeHookMetadata, l as normalizeCodeModeExecBeforeHookParamsForToolKind, u as reconcileCodeModeExecBeforeHookParams } from "./code-mode-control-tools-Bts-Jy9S.js";
import { i as getPluginToolMeta, n as copyPluginToolMeta } from "./tools-sUgO7zqy.js";
import { _ as BEFORE_TOOL_CALL_HOOK_CONTEXT, d as getToolTerminalPresentation, g as BEFORE_TOOL_CALL_DIAGNOSTIC_OPTIONS, h as setChannelAgentToolMeta, m as getChannelAgentToolMeta, p as copyChannelAgentToolMeta, t as callGatewayTool, u as copyToolTerminalPresentation, v as BEFORE_TOOL_CALL_SOURCE_TOOL, y as BEFORE_TOOL_CALL_WRAPPED } from "./gateway-B5H4NIb8.js";
...
import { a as listMessageActionDiscoveryChannels, c as resolveCurrentChannelMessageToolDiscoveryAdapter, l as resolveMessageActionDiscoveryChannelId, r as createMessageActionDiscoveryContext, u as resolveMessageActionDiscoveryForPlugin } from "./message-action-discovery-BLGLSVzQ.js";
import { n as channelPluginHasNativeApprovalPromptUi, t as NATIVE_APPROVAL_PROMPT_RUNTIME_CAPABILITY } from "./native-approval-prompt-kq8K9dDv.js";
import { n as DEFAULT_PLUGIN_APPROVAL_TIMEOUT_MS, r as MAX_PLUGIN_APPROVAL_TIMEOUT_MS } from "./plugin-approvals-DmWtM_Ej.js";
import { t as resolveCanonicalPluginApprovalRequestAllowedDecisions } from "./plugin-approval-canonical-decisions-CtBpKRPA.js";
import { i as resolveApprovalInitiatingSurfaceState, n as describeNativePluginApprovalClientSetup } from "./exec-approval-surface-DP9YlW0l.js";
import { p as resolvePendingSkillProposal } from "./service-CdkFj-Ch.js";
import { t as buildToolMutationState } from "./tool-mutation-BmnNoynG.js";
import { t as buildOutboundSessionContext } from "./session-context-DdqCb4cM.js";
import { t as resolveSessionDeliveryTarget } from "./targets-session-BaGX-R2K.js";
...
async function deliverClientVoiceMutationDigest(record, config, signal) {
	...
	const { sendDurableMessageBatch } = await import("./runtime-s5Yw1b3o.js");
...
function resolveClientVoiceAgentSessionId(params) {
	return loadSqliteSessionEntryReadOnly(params)?.sessionId?.trim() || void 0;
}
...
async function ensureClientVoiceAgentSessionEntry(params) {
	const created = await patchSqliteSessionEntry(params, (_entry, context) => {
...
async function runBeforeToolCallHook(args) {
	...
	try {
		if (args.ctx?.sessionKey) {
			const { markDiagnosticArgumentChurnObservation, getDiagnosticSessionState, logToolLoopAction, detectToolCallLoop, recordToolCall } = await loadBeforeToolCallRuntime();
			const sessionState = getDiagnosticSessionState({
				sessionKey: args.ctx.sessionKey,
				sessionId: args.ctx.sessionId
			});
			...
			if (loopResult.stuck) {
				if (loopResult.level === "critical") {
					beforeToolCallLog.error(`Blocking ${toolName} due to critical loop: ${loopResult.message}`);
					logToolLoopAction({
						sessionKey: args.ctx.sessionKey,
						sessionId: args.ctx.sessionId,
						toolName,
				...
				if (shouldEmitLoopWarning(sessionState, args.ctx.runId ? `${args.ctx.runId}:${baseWarningKey}` : baseWarningKey, loopResult.count)) {
					beforeToolCallLog.warn(`Loop warning for ${toolName}: ${loopResult.message}`);
					logToolLoopAction({
						sessionKey: args.ctx.sessionKey,
						sessionId: args.ctx.sessionId,
						toolName,
			...
			if (args.ctx.loopDetection?.enabled === true) recordToolCall(sessionState, toolName, params, args.toolCallId, args.ctx.loopDetection, loopScope);
		}
...
function wrapToolWithBeforeToolCallHook(tool, ctx, options = {}) {
	...
	wrappedTool.execute = async (toolCallId, params, signal, onUpdate, ...executionArgs) => {
		...
		try {
			return await executeWithHooks(toolCallId, params, signal, onUpdate, ...executionArgs);
		} finally {
...
//#endregion
export { PluginApprovalResolutions as $, authorizeClientVoiceConfirmation as A, finalizeToolTerminalPresentation as B, ensureClientVoiceAgentSessionEntry as C, resolveOpenClientVoiceSessionId as D, resolveClientVoiceSessionOrigin as E, EmbeddedPluginApprovalBroker as F, resolveChannelPromptCapabilities as G, listChannelAgentTools as H, clearEmbeddedPluginApprovalBroker as I, consumePreExecutionBlockedToolCall as J, resolveChannelReactionGuidance as K, setEmbeddedPluginApprovalBroker as L, getTrustedToolPolicyMatcherScope as M, cancelDeferredPluginToolApproval as N, VOICE_TRANSCRIPT_QUEUE_POLICY as O, requestDeferredPluginToolApproval as P, peekPreExecutionBlockedToolCall as Q, isEmbeddedMode as R, createOrResumeClientVoiceSession as S, resolveClientVoiceAgentSessionId as T, listChannelSupportedActions as U, listAllChannelSupportedActions as V, resolveChannelMessageToolHints as W, consumeTrackedToolExecutionStarted as X, consumeStructuredReplaySafeToolCall as Y, peekAdjustedParamsForToolCall as Z, appendRelayVoiceTranscript as _, isPreExecutionBlockedToolResult as a, closeRelayVoiceSessionRecord as b, recordStructuredReplayTrustForToolCall as c, runWithToolExecutionValidation as d, consumeFinalClientVoiceToolConfirmation as f, appendClientVoiceTranscript as g, runBeforeToolCallHook as h, isBeforeToolCallBlockedError as i, bindAuthorizedClientVoiceConfirmation as j, normalizeVoiceTranscriptText as k, rewrapToolWithBeforeToolCallHook as l, hasBeforeToolCallPolicy as m, finalizeBeforeToolCallExecutionParams as n, prepareBeforeToolCallExecutionParams as o, getBeforeToolCallPolicyDiagnosticState as p, consumeAdjustedParamsForToolCall as q, getBeforeToolCallFailureDisposition as r, recordAdjustedParamsForToolCall as s, buildBlockedToolResult as t, wrapToolWithBeforeToolCallHook as u, assertClientVoiceSessionOpen as v, registerClientVoiceConsultRun as w, closeStaleClientVoiceSessions as x, closeClientVoiceSession as y, setEmbeddedMode as z };
