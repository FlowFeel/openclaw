...
import { n as formatConsoleDiagnosticLine } from "./json-console-line-CHgBRErc.js";
...
import { i as withDoctorSqliteMaintenanceLock, r as isDestructiveDoctorSessionSqliteMode } from "./doctor-sqlite-maintenance-lock-D6TRnK8m.js";
...
async function doctorCommand(runtime, options) {
	if (options?.stateSqlite) {
		...
		const { runDoctorStateSqliteCompact } = await import("./doctor-state-sqlite-compact-B_MfY07k.js");
	...
	await (await import("./doctor-health-9snnZEWz.js")).doctorCommand(runtime, options);
}
async function maybeCreateSessionSqliteGithubIssue(runtime, report, options) {
	...
	if (!approved && options.nonInteractive !== true && options.json !== true) {
		const { promptYesNo } = await import("./prompt-BW-syFKG.js");
