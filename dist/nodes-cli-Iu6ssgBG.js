...
import { E as isNodesMachineOutput } from "./argv-D6QuZiS5.js";
...
import { v as randomIdempotencyKey } from "./call-B_qeDbXh.js";
import { t as quoteCliArg } from "./quote-cli-arg-BriMa9wW.js";
import { c as cameraTempPath, d as resolveCameraClipTarget, f as resolveCameraSnapTargets, l as parseCameraClipPayload, m as writeCameraPayloadToFile, o as writeScreenRecordToFile, p as writeCameraClipPayloadToFile, r as screenRecordTempPath, t as parseScreenRecordPayload, u as parseCameraSnapPayload } from "./nodes-screen-B2pfYP1x.js";
...
import { i as withConsoleLogsRoutedToStderrForJson } from "./json-output-mode-BBbjwPbU.js";
...
function registerNodesPairingCommands(nodes) {
	...
	nodesCallOpts(nodes.command("reject").description("Reject a pending pairing request").argument("<requestId>", "Pending request id").action(async (requestId, opts) => {
		await runNodesCommand("reject", async () => {
			let result;
			try {
				result = await callGatewayCli("node.pair.reject", opts, { requestId });
			} catch (error) {
				rethrowUnknownNodePairRequestId(error, requestId, opts);
			}
			defaultRuntime.writeJson(result);
	...
	nodesCallOpts(nodes.command("remove").description("Remove a paired node entry").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").action(async (opts) => {
		await runNodesCommand("remove", async () => {
			const nodeId = await resolveNodeId(opts, normalizeOptionalString(opts.node) ?? "");
			if (!nodeId) {
				defaultRuntime.error(`--node is required. Run ${formatCliCommand("openclaw nodes pending")} to choose a node request.`);
				defaultRuntime.exit(1);
				return;
			}
	...
	nodesCallOpts(nodes.command("rename").description("Rename a paired node (display name override)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").requiredOption("--name <displayName>", "New display name").action(async (opts) => {
		await runNodesCommand("rename", async () => {
			...
			if (!nodeId || !name) {
				defaultRuntime.error(`--node and --name are required. Run ${formatCliCommand("openclaw nodes pending")} to choose a node, then rerun with --name <displayName>.`);
				defaultRuntime.exit(1);
				return;
			}
...
function formatPathEnv(raw, platform) {
	...
	return shortenHomeInString(parts.length <= 3 ? trimmed : `${parts.slice(0, 2).join(delimiter)}${delimiter}…${delimiter}${parts.slice(-1)[0]}`);
}
...
async function registerNodesCli(program, argv = process.argv) {
	...
	const { registerPluginCliCommandsFromValidatedConfig } = await import("./cli-BZJxGH08.js");
