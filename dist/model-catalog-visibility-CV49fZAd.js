import { i as buildConfiguredModelCatalog, p as modelCatalogLogicalKey, s as dedupeModelCatalogEntries } from "./model-selection-shared-V7VmYFPH.js";
import { n as compareModelCatalogEntries } from "./model-catalog-order-CuLyE0qg.js";
...
function isPickerVisibleCatalogEntry(entry, configuredKeys, routePolicy) {
	return entry.status !== "deprecated" && entry.status !== "disabled" || configuredKeys.has(resolveLogicalKey(entry, routePolicy));
}
...
async function resolveLogicalVisibleModelCatalog(params) {
	...
	return (await projectEntries([
		...
		...retained,
		...routeBacked
	])).filter((entry) => isPickerVisibleCatalogEntry(entry, configuredKeys, params.routePolicy));
}
//#endregion
