...
import { n as listTranscriptSourceProviders, t as getTranscriptSourceProvider } from "./provider-registry-DId12FSS.js";
...
async function startTranscripts(params) {
	...
	if (!provider?.start) throw new Error(`transcripts provider ${providerSource.providerId} cannot start live capture`);
	const session = {
...
async function stopTranscripts(params) {
	...
	if (!session || !ownsTranscriptSession(params.ctx, session)) throw new Error(`transcripts session not found: ${sessionSelector}`);
...
async function importTranscripts(params) {
	...
	const provider = resolveSourceProvider(providerSource.providerId, params.ctx);
	if (!provider?.importTranscript) throw new Error(`transcripts provider ${providerSource.providerId} cannot import transcripts`);
	const session = {
		sessionId: readStringParam(params.rawParams, "sessionId", { trim: true }) ?? createSessionId(),
		title: readStringParam(params.rawParams, "title", { trim: true }),
		source: sanitizeTranscriptSourceLocator(providerSource),
...
async function summarizeExisting(params) {
	...
	if (!entry || !ownsTranscriptSession(params.ctx, entry.session)) throw new Error(`transcripts session not found: ${sessionId}`);
	const { summaryPath, intendedSummaryPath, summary, summaryExportError } = await summarizeAndPersist({
...
async function statusTranscripts(ctx) {
	...
	const active = [...activeSessions.values()].filter((entry) => ownsTranscriptSession(ctx, entry.session)).map((entry) => ({
