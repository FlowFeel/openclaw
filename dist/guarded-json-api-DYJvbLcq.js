...
import { c as readResponseTextPrefix, u as readResponseWithLimit } from "./http-body-DhB9daEt.js";
...
import { n as reserveWebhookReplay, t as createWebhookReplayCache } from "./webhook-replay-DncqsA_e.js";
...
function extractHostname(hostHeader) {
	...
	if (hostHeader.startsWith("[")) {
		...
		try {
			...
			if (!parsedHostname.startsWith("[") || !parsedHostname.endsWith("]")) return null;
...
function buildTwilioVerificationUrl(ctx, publicUrl, urlOptions) {
	...
	try {
		...
		base.search = new URL(ctx.url).search;
...
function verifyTelnyxWebhook(ctx, publicKey, options) {
	...
	const eventTimeSec = /^(?:0|[1-9]\d*)$/.test(timestamp) ? Number(timestamp) : void 0;
	if (eventTimeSec === void 0 || !Number.isSafeInteger(eventTimeSec)) return {
		ok: false,
...
async function readVoiceCallProviderJsonResponse(response, malformedJsonMessage) {
	const body = await readResponseWithLimit(response, PROVIDER_JSON_RESPONSE_MAX_BYTES, { onOverflow: ({ size, maxBytes }) => /* @__PURE__ */ new Error(`provider response body too large: ${size} bytes (limit: ${maxBytes} bytes)`) });
...
async function guardedJsonApiRequest(params) {
	...
	try {
		...
		return await readVoiceCallProviderJsonResponse(response, `${params.errorPrefix}: malformed JSON response`);
	} finally {
		await release();
	}
}
//#endregion
export { reconstructWebhookUrl as a, verifyTwilioWebhook as c, readVoiceCallProviderJsonResponse as i, cancelProviderResponseBody as n, verifyPlivoWebhook as o, readProviderErrorResponseSnippet as r, verifyTelnyxWebhook as s, guardedJsonApiRequest as t };
