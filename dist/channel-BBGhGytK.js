...
import { l as createScopedDmSecurityResolver, u as createTopLevelChannelConfigAdapter } from "./channel-config-helpers-BwgkzH7b.js";
...
import { c as collectStatusIssuesFromLastError, d as createDefaultChannelRuntimeState, u as createComputedAccountStatusAdapter } from "./status-helpers-CSowaqvB.js";
import { i as runPassiveAccountLifecycle } from "./channel-lifecycle.core-C98dobNq.js";
import { r as buildTrafficStatusSummary, t as buildPassiveChannelStatusSummary } from "./extension-shared-CyJFAMlE.js";
...
import { $ as hexToBytes, i as cbc, tt as randomBytes } from "./hkdf-C7rH9AZE.js";
import { a as getPublicKey, c as base64, l as secp256k1, n as SimplePool, r as finalizeEvent, s as verifyEvent } from "./esm-B8-t-Wx3.js";
import { n as NostrProfileSchema, t as NostrConfigSchema } from "./config-schema-CQxoNBYQ.js";
import { o as DEFAULT_RELAYS } from "./setup-adapter-XeWLgdbr.js";
import { a as resolveDefaultNostrAccountId, c as validatePrivateKey, i as listNostrAccountIds, n as nostrSetupContract, o as resolveNostrAccount, r as nostrSetupWizard, s as normalizePubkey } from "./setup-surface-BOICWxdM.js";
...
const NostrIngressAdmissionRejectedError = createChannelIngressError("NostrIngressAdmissionRejectedError", { withReason: true });
...
function createNostrIngress(options) {
	...
	const monitor = createChannelIngressMonitor({
		...
		retention: {
			completedMaxEntries: 1e5,
			failedMaxEntries: 1e5
		},
		drain: {
			...
			for (const relay of relays) {
				...
				subscriptions.push(options.pool.subscribeMany([relay], options.filter, {
					...
					onclose: (reasons) => {
						...
						deadlineTimers.delete(deadlineTimer);
						settleBackfill(relay, "incomplete");
						if (!locallyClosing && !options.abort.aborted) options.onClose(relay, reasons.map(({ reason }) => reason));
					},
...
function normalizeRelayLifecycleKey(relay) {
...
const startNostrGatewayAccount = async (ctx) => {
	...
	let busHandle = null;
	const connectedRelays = /* @__PURE__ */ new Set();
		...
		start: async () => {
			const bus = await startNostrBus({
				...
				onMessage: async (senderPubkey, text, reply, meta, lifecycle) => {
					...
					const { dispatchInboundDirectDm } = await import("./inbound-direct-dm-runtime-Cwr-wdoM.js");
					await dispatchInboundDirectDm({
						...
						deliver: async (payload) => {
							const sanitizedText = sanitizeAssistantVisibleText(payload && typeof payload === "object" && "text" in payload ? payload.text ?? "" : "");
							...
							const message = stripMarkdown(runtime.channel.text.convertMarkdownTables(sanitizedText, tableMode));
				...
				onDisconnect: (relay) => {
					...
					if (connectedRelays.size === 0) ctx.setStatus({
						accountId: account.accountId,
						connected: false,
...
const nostrOutboundAdapter = {
	...
	sendText: async ({ cfg, to, text, accountId }) => {
		...
		if (!message) throw new Error("Nostr send requires non-empty text after markdown stripping.");
