...
import { L as applySqliteSessionEntryLifecycleMutation, ct as loadSqliteSessionEntryReadOnly, lt as patchSqliteSessionEntry } from "./session-accessor.sqlite-B9iW7DOt.js";
...
import { p as toDatabaseOptions, s as resolveSqliteScope } from "./session-accessor.sqlite-scope-BFeKxPFV.js";
import { A as resolveFreshSessionTotalTokens, G as pruneStaleEntries, it as collectActiveSessionWorkAdmissionKeys } from "./session-entry-slot-keys-DR5d2mKt.js";
import { $ as listSessionEntries } from "./session-accessor-t3qUoTeV.js";
import "./delivery-info-D_tbjn1X.js";
import { d as recordSessionGoalChanged } from "./session-state-events-MWtkoPhW.js";
...
async function createSessionGoal(options) {
	...
	const now = nowMs(options.now);
	let created;
	const result = await patchSqliteSessionEntry({
		sessionKey: options.sessionKey,
		storePath: options.storePath
	}, (entry) => {
...
async function runSessionRegistryMaintenanceForStore(params) {
	...
	if (removals.length > 0) {
		const mutation = await applySqliteSessionEntryLifecycleMutation({
			storePath: params.storePath,
			removals,
