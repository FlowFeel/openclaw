...
import { m as normalizeSecretInputString, u as isSecretRef } from "./types.secrets-BvApkFoj.js";
import { t as formatSystemAgentOnboardingWelcome } from "./overview-CDBIuwEP.js";
...
async function buildOnboardingWelcome(params) {
	...
	const localSetup = params.localRecovery === true && overview.config.exists && overview.config.valid && authoredConfig !== void 0 && authoredConfig?.gateway?.mode !== "remote" ? (await import("./local-onboarding-state-DXdl4oSf.js")).readLocalOnboardingStateForConfig(overview.config.path, authoredConfig) : void 0;
	...
	if (hasAuthoredSetup && !pendingSetup && defaultModel && (!requestedWorkspace || requestedWorkspace === authoredWorkspace)) {
	...
	if (!defaultModel) throw new Error("OpenClaw onboarding requires working inference first. Run `openclaw onboard` on the machine running OpenClaw to configure and verify a default model.");
