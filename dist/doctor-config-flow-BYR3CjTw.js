...
import { t as CONFIG_PATH } from "./paths-CL43LNS6.js";
...
import { a as readAgentRosterProperty, n as listAgentEntries } from "./agent-scope-config-Dusa8eSA.js";
import { n as normalizeAgentId } from "./agent-id-DDgUze4y.js";
import { t as DEFAULT_ACCOUNT_ID } from "./account-id-CIVg1QNG.js";
import "./session-key-DtTE9-Tg.js";
import { J as resolveNormalizedProviderModelMaxTokens } from "./io-BsQc3Kgy.js";
...
import { g as parseLegacySecretRefEnvMarker, l as isLegacySecretRefEnvMarker } from "./types.secrets-BvApkFoj.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-BIKs-96s.js";
import { i as getBundledChannelSetupPlugin, o as hasBundledChannelPackageSetupFeature } from "./bundled-Wqb8B5b_.js";
import { s as HeartbeatSchema } from "./zod-schema.agent-runtime-CaWcMFul.js";
import { a as migrateLegacyXSearchConfig, c as migrateLegacyRuntimeModelRef, d as hasOwnKey, f as migratePersistedImplicitMainRoster, i as stripRetiredTuningKnobs, l as cloneRecord, o as legacyRuntimeModelAliasRequiresRuntimePolicy, r as migrateLegacyWebSearchConfig, s as listLegacyRuntimeModelProviderAliases, u as ensureRecord } from "./legacy-C8NOAlee.js";
import { r as normalizeTalkSection } from "./talk-Desj3oxA.js";
import { a as isBlockedLegacyCodexModelRef } from "./codex-route-model-ref-BDwklqCa.js";
import { n as formatConfigIssueLines } from "./issue-format-NBZpPOU4.js";
import { c as mergeMissing, r as isLegacyModelsAddCodexMetadataModel } from "./legacy-config-migrations.runtime.models-D5zvVANI.js";
import { t as configIncludeOwnsAgentRoster } from "./agent-roster-provenance-CfRWZSQN.js";
import { i as setPathExistingStrict } from "./path-utils-DfXesdV4.js";
import { n as discoverConfigSecretTargets } from "./target-registry-query-CnPjWCNY.js";
import "./target-registry-BVDHguMA.js";
import { c as callGateway } from "./call-B_qeDbXh.js";
import { a as runPluginSetupConfigMigrations } from "./setup-registry-BgmsrGpe.js";
...
import { n as resolveSingleAccountPromotion } from "./setup-promotion-helpers-DKzqTbS-.js";
import { a as noteSandboxOriginProxyWarning, i as noteOpencodeProviderOverrides, n as noteImplicitFallbackClobberWarnings, s as stripUnknownConfigKeys } from "./doctor-config-analysis-DBfZ7XGO.js";
import { t as runDoctorConfigPreflight } from "./doctor-config-preflight-DdyrzA6O.js";
import { n as applyChannelDoctorCompatibilityMigrations, t as migrateLegacyConfig } from "./legacy-config-migrate-Bj6UZtJY.js";
import { n as cronCodexRuntimePolicyTargetKey } from "./store-migration-DtxGNOYP.js";
import { n as containsAuthoredInclude, r as isSingleTopLevelIncludeMigration, t as classifyConfigPathMigrationOwnership } from "./include-migration-ownership-BFF9HYFj.js";
...
function applyLegacyCompatibilityStep(params) {
	...
	if (params.snapshot.legacyIssues.some((issue) => issue.path === OTEL_GRPC_PROTOCOL_PATH)) {
		...
		if (ownership.kind === "manual") {
			const targets = ownership.targetPaths.length > 0 ? ` Inspect these candidate source files and remove or replace ${OTEL_GRPC_PROTOCOL_PATH} = "grpc" from every definition: ${ownership.targetPaths.join(", ")}.` : ` Remove or replace ${OTEL_GRPC_PROTOCOL_PATH} = "grpc" in the owning $include directive or included file.`;
			return {
				state: params.state,
				issueLines: [...issueLines, `- ${OTEL_GRPC_PROTOCOL_PATH}: Doctor cannot safely rewrite this $include ownership.${targets} No config files were changed.`],
				changeLines: [],
				blocksWrite: true
			};
		}
...
//#endregion
//#region src/commands/doctor/shared/default-agent-role-materialization.ts
function resolveLegacyMultiAgentDefault(cfg) {
	const entries = listAgentEntries(cfg);
	if (entries.length < 2) return;
	const defaults = entries.filter((entry) => entry.default === true);
	return defaults.length === 1 ? normalizeAgentId(defaults[0].id) : void 0;
}
function listAmbientConfiguredChannelIds(cfg) {
	if (!isRecord(cfg.channels)) return [];
	return Object.entries(cfg.channels).flatMap(([channelId, value]) => {
		if (channelId === "defaults" || isRecord(value) && value.enabled === false) return [];
...
function isChannelWideBinding(binding, channelId) {
	...
	return normalizeRouteBindingChannelId(typeof match.channel === "string" ? match.channel : void 0) === channelId && (typeof match.accountId === "string" ? match.accountId.trim() : void 0) === "*" && match.peer === void 0 && !normalizeOptionalString(typeof match.guildId === "string" ? match.guildId : void 0) && !normalizeOptionalString(typeof match.teamId === "string" ? match.teamId : void 0) && (!Array.isArray(match.roles) || match.roles.length === 0);
}
/**
...
*/
function materializeDefaultAgentRoles(cfg) {
	...
	let next = cfg;
	const changes = [];
	const canMaterializeBindings = cfg.bindings === void 0 || Array.isArray(cfg.bindings);
	const bindings = Array.isArray(cfg.bindings) ? cfg.bindings.filter((binding) => isRecord(binding) && binding.type !== "acp") : [];
	const missingChannelBindings = canMaterializeBindings ? listAmbientConfiguredChannelIds(cfg).filter((channelId) => !bindings.some((binding) => isChannelWideBinding(binding, channelId))) : [];
	if (missingChannelBindings.length > 0) {
		next = {
			...next,
			bindings: [...Array.isArray(next.bindings) ? next.bindings : [], ...missingChannelBindings.map((channel) => ({
		...
		changes.push(`Bound ${missingChannelBindings.join(", ")} unbound account routing to agent "${defaultAgentId}".`);
	}
	const rawDefaults = cfg.agents?.defaults;
	const defaultsConfig = isRecord(rawDefaults) ? rawDefaults : void 0;
	const canMaterializeDefaults = rawDefaults === void 0 || defaultsConfig !== void 0;
	const hasPerAgentHeartbeat = listAgentEntries(cfg).some((entry) => Boolean(entry.heartbeat));
	if (canMaterializeDefaults && !hasPerAgentHeartbeat && defaultsConfig?.heartbeat === void 0) {
		next = {
			...next,
			agents: {
				...next.agents,
	...
	const systemAgentConfig = isRecord(rawSystemAgent) ? rawSystemAgent : void 0;
	if (canMaterializeDefaults && (rawSystemAgent === void 0 || systemAgentConfig !== void 0) && (!systemAgentConfig || !Object.hasOwn(systemAgentConfig, "agentId"))) {
		next = {
			...next,
			agents: {
				...next.agents,
	...
	const talkConfig = isRecord(cfg.talk) ? cfg.talk : void 0;
	if ((cfg.talk === void 0 || talkConfig !== void 0) && (!talkConfig || !Object.hasOwn(talkConfig, "agentId"))) {
		next = {
			...next,
			talk: {
				...talkConfig,
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	const rosterMigrationNeeded = [snapshot.sourceConfigBeforeMigrations, snapshot.parsed].some((source) => source !== void 0 && migratePersistedImplicitMainRoster(source).changed);
	const includeOwnsRoster = configIncludeOwnsAgentRoster(snapshot);
	if (snapshot.exists && rosterMigrationNeeded && !includeOwnsRoster) {
		const migrated = migratePersistedImplicitMainRoster(state.candidate).config;
	...
	applyConfigMutation(materializeDefaultAgentRoles(state.candidate), { fixHint: `Run "${doctorFixCommand}" to persist explicit ambient agent targets.` });
	const { collectBlockedLegacyOpenAICodexProviderPlan } = await import("./legacy-config-migrations.runtime.models-CzXA4AEB.js");
	...
	if (preflight.cronCodexRuntimePolicyTargets?.length) {
		const { repairCronCodexRuntimePolicies } = await import("./runtime-policy-migration-CuxUfwnE.js");
	...
	applyConfigMutation(normalizeCompatibilityConfigValues(state.candidate, {
		blockedModelIdentities: blockedCodexModelIdentities,
		sourceRaw: snapshot.parsed
	}), { fixHint: `Run "${doctorFixCommand}" to apply these changes.` });
	const { prepareRetiredPhoneControlCleanup } = await import("./doctor-retired-phone-control-WEjhIqwj.js");
	...
	const { applyPluginAutoEnable } = await import("./plugin-auto-enable-BUGd6Bn4.js");
	applyConfigMutation(applyPluginAutoEnable({
		config: state.candidate,
		env: process.env
	}), { fixHint: `Run "${doctorFixCommand}" to apply these changes.` });
	if (!shouldRepair) {
		const { repairStaleAgentModelRefs } = await import("./stale-agent-model-ref-repair-Cv28ZiCW.js");
		applyConfigMutation(repairStaleAgentModelRefs(state.candidate, { env: process.env }), {
			fixHint: `Run "${doctorFixCommand}" to remove stale agent model references.`,
			sanitize: true,
			emitWarnings: true
		});
	}
	const { collectPluginToolAllowlistWarnings } = await import("./plugin-tool-allowlist-warnings-CCAW4IMX.js");
	...
	if (hasConfiguredChannels) {
		const channelDoctor = await import("./channel-doctor-ByaFIyYL.js");
		...
		for (const staleCleanup of await channelDoctor.collectChannelDoctorStaleConfigMutations(state.candidate, { env: process.env })) applyConfigMutation(staleCleanup, {
			fixHint: `Run "${doctorFixCommand}" to remove stale channel plugin references.`,
			sanitize: true
		});
	}
	const { repairHooksTokenReuseGatewayAuth } = await import("./hooks-token-reuse-repair-fvei3E4l.js");
	...
	if (shouldRepair) {
		const { runDoctorRepairSequence } = await import("./repair-sequencing-B7LvA_ZC.js");
		...
		const { collectDoctorPreviewNotes } = await import("./preview-warnings-DKxtIEss.js");
	...
	const activeOpencodePluginIds = configuredOpencodePluginIds.length > 0 ? (await import("./providers-DyY3jhMN.js")).resolveEnabledProviderPluginIds({
		config: cfg,
