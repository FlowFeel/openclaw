...
import { lt as patchSqliteSessionEntry, yt as resolveSessionStorePathForScope } from "./session-accessor.sqlite-B9iW7DOt.js";
...
import { s as resolveStableSessionEndTranscript } from "./session-transcript-files.fs-DUSeZC1w.js";
...
import { t as resolveReusableWorkspaceSkillSnapshot } from "./session-snapshot-DbeHrjCP.js";
...
async function incrementCompactionCount(params) {
	const { agentId, sessionEntry, sessionStore, sessionKey, storePath, cfg, now = Date.now(), amount = 1, tokensAfter, newSessionId } = params;
	...
	if (effectiveStorePath) {
		const persistedEntry = await patchSqliteSessionEntry({
			...agentId ? { agentId } : {},
