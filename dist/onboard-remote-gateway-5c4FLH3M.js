...
async function runRemoteGatewayInferenceOnboarding(target, runtime = defaultRuntime, deps = {}) {
	const callGateway = deps.callGateway ?? (await import("./call-DctZOBCQ.js")).callGatewayCli;
	const runGuidedOnboarding = deps.runGuidedOnboarding ?? (await import("./onboard-guided-CNFKCj1N.js")).runGuidedOnboarding;
	...
	await runGuidedOnboarding({}, runtime, {
		...
		runSystemAgentChat: async () => {
			...
			await (deps.runTui ?? (await import("./tui-BavjTmCc.js")).runTui)({
				config: boundConfig,
				deliver: false,
