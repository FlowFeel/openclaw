...
import { f as resolveDefaultAgentId, i as listAgentIds, n as listAgentEntries } from "./agent-scope-config-Dusa8eSA.js";
...
import { C as buildConversationRef, E as parseSqliteSessionEntryRecord, T as hasValidSqliteSessionEntryIdentity, Z as isSameOpenClawAgentDatabasePath, c as deferOpenClawAgentPostCommitPublication, et as listOpenClawRegisteredAgentDatabases, w as normalizeConversationPeerId } from "./openclaw-agent-db--PLC25lY.js";
import { t as normalizeChatType } from "./chat-type-BARlA53h.js";
import { a as mergeDeliveryContext, c as normalizeSessionDeliveryState, d as sessionDeliveryOrigin, f as sessionDeliveryRoute, n as deliveryContextFromSession, p as normalizeAccountId, s as normalizeDeliveryContext, t as deliveryContextFromChannelRoute, u as sessionDeliveryChannel } from "./delivery-context.shared-B-QSuGw_.js";
import { r as isInternalNonDeliveryChannel } from "./message-channel-constants-76XnXM8q.js";
import { l as resolveStorePath, n as resolveAgentsDirFromSessionStorePath } from "./paths-DSnYpBD3.js";
import { t as withOpenClawAgentDatabaseReadOnly } from "./openclaw-agent-db-readonly-zFkIbzhy.js";
import { t as resolveConversationLabel } from "./conversation-label-pgUV7Er9.js";
import { a as normalizeChannelId, n as getLoadedChannelPlugin } from "./registry-B1AiP2IQ.js";
...
import { a as resolveGroupSessionKey, n as projectCanonicalSessionEntryShape, r as buildGroupDisplayName } from "./store-entry-shape-DB6qjwcl.js";
import { a as normalizeStoreSessionKey, n as foldedSessionKeyAliasCandidates, o as resolveDeliveryProvenCanonicalSessionKey, t as collectSessionEntryLookupKeys } from "./store-entry-0o94FtE8.js";
import { i as resolveUnsuffixedSqliteTargetFromSessionStorePath, r as resolveSqliteTargetFromSessionStorePath, t as listDurableSqliteTargetOwnersForSessionStorePath } from "./session-sqlite-target-BlEPGL-w.js";
import { a as normalizeSqliteSessionKey, i as getSessionKysely, t as cloneSessionEntry } from "./session-accessor.sqlite-scope-BFeKxPFV.js";
import { n as deleteSessionTranscriptIndexInTransaction } from "./session-transcript-index-cy-aJty7.js";
...
function readSqliteSessionEntryCache(database, options) {
...
function publishSqliteSessionEntryCacheUpsert(database, row) {
...
function publishSqliteSessionEntryCacheInvalidation(database, row) {
	if (row) {
...
function conversationIdentityFromSessionEntry(entry) {
	...
	const deliveryTarget = routeTarget ?? (kind === "direct" ? normalizeText(origin?.from) : void 0);
...
function bindSqliteSessionWindowEntryProjection(params) {
	return {
...
function bindSqliteSessionNode(params) {
	...
	return {
		...
		parent_session_key: normalizeSqliteText(params.entry.parentSessionKey) ?? normalizeSqliteText(params.entry.spawnedBy),
		spawned_by: normalizeSqliteText(params.entry.spawnedBy),
...
function readSqliteSessionEntryStore(database, options = {}) {
...
function readSqliteSessionEntryCount(database) {
	const db = getSessionKysely(database.db);
	return executeSqliteQuerySync(database.db, db.selectFrom("session_nodes").select("entry_json")).rows.reduce((count, row) => count + (parseSqliteSessionEntryJson(row) ? 1 : 0), 0);
}
function readSqliteSessionEntryKeys(database) {
	const db = getSessionKysely(database.db);
	return executeSqliteQuerySync(database.db, db.selectFrom("session_nodes").select(["entry_json", "session_key"]).orderBy("session_key", "asc")).rows.flatMap((row) => parseSqliteSessionEntryJson(row) ? [row.session_key] : []);
}
function resolveSqliteLifecyclePrimaryEntry(database, target, options = {}) {
...
function readSqliteLifecycleTargetSnapshot(database, target, options = {}) {
	assertCanonicalSqliteSessionKeysCurrent(database);
...
function deleteSqliteSessionEntryRows(database, sessionKey, options = {}) {
	const db = getSessionKysely(database.db);
	...
	for (const window of windows) {
		const survivingNode = survivingNodes.find((node) => {
			...
			const entry = parseSqliteSessionEntryJson(node);
			return entry ? collectSqliteSessionStateIdsForEntry(entry).includes(window.session_id) : false;
		});
...
function rehomeSqliteSessionWindows(database, canonicalKey, previousKeys) {
...
function writeSessionEntry(database, sessionKey, entry, options = {}) {
	...
	if (!hasValidSqliteSessionEntryIdentity(normalizedEntry)) throw new Error("Refusing invalid SQLite session entry identity");
...
function dedupeSessionStoreTargetsBySqliteTarget(targets, options) {
	...
	const resolvePhysicalGroupKey = (groups, pathname) => [...groups.keys()].find((candidate) => isSameOpenClawAgentDatabasePath(candidate, pathname)) ?? path.resolve(pathname);
	...
	for (const [sqlitePath, group] of grouped) {
		...
		const registeredOwners = [...new Set(registeredDatabases.filter((entry) => isSameOpenClawAgentDatabasePath(entry.path, sqlitePath)).map((entry) => normalizeAgentId(entry.agentId)))];
...
function listKnownSessionStoreAgentIds(cfg, params = {}) {
	...
	if (!isPerAgentSessionStoreConfig(cfg.session?.store)) {
		...
		if (durableTarget.shared && durableTarget.agentId && fs.existsSync(durableTarget.path)) try {
			const logicalOwners = withOpenClawAgentDatabaseReadOnly((database) => readSqliteSessionEntryKeys(database).flatMap((sessionKey) => {
				const parsed = parseAgentSessionKey(sessionKey);
	...
	for (const registered of listOpenClawRegisteredAgentDatabases({ env })) {
		...
		const expectedPath = resolveSqliteTargetFromSessionStorePath(resolveStorePath(cfg.session?.store, {
			agentId,
			env
		}), {
			agentId,
		...
		if (isSameOpenClawAgentDatabasePath(registered.path, expectedPath)) ids.add(agentId);
	}
	return [...ids];
}
...
function resolveSessionStoreTargets(cfg, opts, params = {}) {
	...
	if (allAgents) return dedupeSessionStoreTargetsBySqliteTarget(listConfiguredSessionStoreAgentIds(cfg).map((agentId) => ({
		agentId,
...
//#endregion
export { conversationIdentityFromMsgContext as $, sqliteSessionEntriesEqual as A, touchTranscriptMutationInTransaction as B, readSqliteLifecycleTargetSnapshot as C, readSqliteSessionIdentitySnapshot as D, readSqliteSessionEntryStore as E, ensureTranscriptSessionRoot as F, bindSqliteSessionWindowEntryProjection as G, assertCanonicalSqliteSessionKeysCurrent as H, readNextTranscriptSeq as I, collectSqliteSessionStateIdsForEntry as J, createFallbackSessionEntry as K, readTranscriptGenerationInTransaction as L, advanceTranscriptMutationAtInTransaction as M, deleteSqliteTranscriptEventsInTransaction as N, rehomeSqliteSessionWindows as O, ensureTranscriptGenerationInTransaction as P, buildConversationIdentity as Q, readTranscriptMutationStateInTransaction as R, readSessionEntryRow as S, readSqliteSessionEntrySelectionSnapshot as T, canonicalSessionKeyMigrationRequiredError as U, assertCanonicalSessionKeyWrite as V, setCanonicalSqliteSessionMainKey as W, deleteSessionMembersForRepair as X, copySessionNodeArtifactsForRepair as Y, upsertConversationIdentity as Z, deleteSqliteSessionEntryRows as _, resolveAgentSessionStoreTargetsSync as a, deriveSessionMetaPatch as at, readExactSessionEntryRow as b, resolveExistingAgentSessionStoreTargetsSync as c, assertSqliteLifecycleTargetSnapshotUnchanged as d, publishSqliteSessionEntryCacheInvalidation as et, assertSqliteLifecycleTargetUnchanged as f, deleteSqliteLifecycleTargetRows as g, deleteLegacySessionEntryRows as h, listKnownSessionStoreAgentIds as i, deriveLastRoutePatch as it, writeSessionEntry as j, resolveSqliteLifecyclePrimaryEntry as k, resolveSessionStoreTargets as l, createSqliteSessionIdentitySnapshot as m, isPerAgentSessionStoreConfig as n, parseSqliteSessionEntryJson as nt, resolveAllAgentSessionStoreCandidateTargetsSync as o, deriveSessionOrigin as ot, assertSqliteSessionEntrySelectionUnchanged as p, normalizeSqliteNumber as q, listConfiguredSessionStoreAgentIds as r, readSqliteSessionEntriesByStatus as rt, resolveAllAgentSessionStoreTargetsSync as s, isConfiguredSessionStoreAgentId as t, readSqliteSessionEntryCache as tt, dedupeSessionStoreTargetsBySqliteTarget as u, normalizeSqliteLifecycleTarget as v, readSqliteSessionEntryCount as w, readExactSessionEntryRowValidated as x, readExactSessionEntryJsonForCanonicalRepair as y, rotateTranscriptGenerationInTransaction as z };
