...
import { P as normalizeUsage } from "./session-accessor.sqlite-CtCo5VZ6.js";
...
async function runIsolatedAgentRuntimeCompletion(params) {
	...
	try {
		const operation = (async () => {
			const { runIsolatedCompletion } = await import("./isolated-completion-DdBQpX7H.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-BnmXQj8E.js"), Promise.resolve(resolveRuntimeConfig(options))]);
