...
import { _ as fireAndForgetBoundedHook, t as getGlobalHookRunner } from "./hook-runner-global-CRNklGqK.js";
import { A as derivePromptTokens, P as normalizeUsage } from "./session-accessor.sqlite-B9iW7DOt.js";
import { c as getStreamLlmRuntime } from "./stream-Cgu09Ime.js";
import { o as createAnthropicVertexStreamFnForModel } from "./ai-transport-runtime-host-DwMiqDOL.js";
import { l as markDiagnosticRunProgress } from "./diagnostic-run-activity-3mcrQxEA.js";
import { a as diagnosticProviderRequestIdHash, i as diagnosticHttpStatusCode, n as diagnosticErrorFailureKind, t as diagnosticErrorCategory } from "./diagnostic-error-metadata-CxJn_BAC.js";
import { t as cloneDiagnosticContentValue } from "./diagnostic-llm-content-BiTOBguG.js";
...
function emitModelCallCompleted(eventBase, startedAt, state) {
	...
	if (!state.suppressPluginHooks) dispatchModelCallEndedHook(eventBase, {
		durationMs,
		outcome: "completed",
		...sizeTimingFields
	});
}
function emitModelCallError(eventBase, startedAt, state, err) {
	if (state.terminalEventEmitted) return;
	state.terminalEventEmitted = true;
	const durationMs = Date.now() - startedAt;
	const sizeTimingFields = modelCallSizeTimingFields(state);
	...
	emitProviderRequestTimelineEvent(eventBase, startedAt, durationMs, false, state.responseStatus ?? (errorStatus === void 0 ? void 0 : Number(errorStatus)));
	emitTrustedDiagnosticEventWithPrivateData({
