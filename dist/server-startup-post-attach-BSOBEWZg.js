...
function dashboardDataPrewarmItems(cfg, log) {
	...
	return [
		...
		})),
		{
			name: "plugins",
			load: async () => {
				const { listManagedPlugins } = await import("./management-service-i5CtblEf.js");
...
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-n0_gmpg5.js"));
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-Bl2RWrxt.js")).scheduleGatewayUpdateCheck(...args),
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	const sidecarsPromise = params.minimalTestGateway ? Promise.resolve({
		...
		try {
			const { scheduleSubagentRegistrySweep } = await import("./subagent-registry-CjPpjF8W.js");
