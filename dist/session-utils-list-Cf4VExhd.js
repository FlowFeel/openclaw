...
import { c as parseAgentSessionKey, r as isCronRunSessionKey } from "./session-key-utils-02xWdGSz.js";
import { t as isIncognitoSessionKey } from "./incognito-session-key-BwpD1Lwd.js";
import { n as withPinnedActivePluginRegistryWorkspaceDir } from "./runtime-workspace-state-UtLma22Q.js";
...
import { r as buildGroupDisplayName } from "./store-entry-shape-CTwpu-C_.js";
import { i as readAcpSessionMetaBatch } from "./session-meta-jzIthKMh.js";
import "./sessions-BqBqRT1f.js";
import { t as resolveSessionModelIdentityRef } from "./session-model-ref-D_2SCgSv.js";
import { a as shouldKeepSubagentRunChildLink } from "./subagent-run-liveness-CjxLH_UA.js";
import { o as getSessionDisplaySubagentRunByChildSessionKey, r as countActiveDescendantRuns } from "./subagent-registry-read-CIgozRhQ.js";
import { C as loadSessionEntryReadOnly, F as resolveSessionDisplayModelIdentityRefCached, I as deriveSessionTitle, L as isCurrentSessionChildOwner, R as isFinitePositiveTimestamp, a as buildSingleRowStoreChildSessionsByKey, b as isGroupOrChannelDisplaySession, i as buildSessionListRowMetadataContext, j as getSessionDefaults, l as readSessionTitleFieldsFromTranscriptBatch, n as projectSessionActor, o as resolveSessionSelectedModelRef, r as buildSessionListRowContext, t as buildGatewaySessionRow, w as parseGroupKey, z as shouldKeepStoreOnlyChildLink } from "./session-utils-row-Br8x7LNG.js";
...
function populateSessionListAcpMetadata(params) {
	...
	const entries = params.entries.map(([key, entry]) => {
		...
		const agentId = normalizeAgentId(key === "global" && typeof params.opts.agentId === "string" ? params.opts.agentId : parsed?.agentId ?? resolveDefaultAgentId(params.cfg));
		return {
...
function buildSessionsListResult(params) {
	...
	return {
		...
		defaults: getSessionDefaults(params.cfg, params.modelCatalog, { allowPluginNormalization: false }),
		sessions
	};
}
...
async function listSessionsFromStoreAsync(params) {
	return withPinnedActivePluginRegistryWorkspaceDir(async () => {
		...
		const transcriptFields = readSessionTitleFieldsFromTranscriptBatch(list.entries.slice(0, SESSIONS_LIST_TRANSCRIPT_FIELD_ROWS).flatMap(([key, entry]) => {
			...
			const parsed = parseAgentSessionKey(key);
			return [{
				agentId: key === "global" && typeof opts.agentId === "string" ? normalizeAgentId(opts.agentId) : parsed?.agentId ? normalizeAgentId(parsed.agentId) : resolveDefaultAgentId(cfg),
				sessionEntry: entry,
...
//#endregion
export { loadGatewaySessionRow as a, buildGatewaySessionInfo as i, listSessionsFromStore as n, listSessionsFromStoreAsync as r, filterAndSortSessionEntries as t };
