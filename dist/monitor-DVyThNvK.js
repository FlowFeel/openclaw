import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString, d as normalizeStringifiedEntries, o as normalizeNullableString } from "./string-coerce-DW4mBlAt.js";
...
import { S as formatLocationText, w as toLocationContext } from "./reply-payload-BE_j43tQ.js";
import { T as resolveChannelStreamingBlockEnabled, c as formatChannelProgressDraftText, i as buildChannelProgressDraftLineForEntry, o as formatChannelProgressDraftLine } from "./streaming-Byu76Sav.js";
...
import { i as resolveHumanDelayConfig, t as resolveAckReaction } from "./identity-DxC7SNFJ.js";
import { n as listMessageReceiptPlatformIds, t as createMessageReceiptFromOutboundResults } from "./receipt-DSE0BXtY.js";
import { a as deliverWithFinalizableLivePreviewAdapter, n as createPreviewMessageReceipt, r as defineFinalizableLivePreviewAdapter } from "./live-BkLbrfBk.js";
...
import { n as createReplyPrefixOptions } from "./reply-prefix-CZRxYOQO.js";
...
import { l as buildInboundHistoryFromEntries } from "./history-DLKGD0Dj.js";
...
import { g as toInboundMediaFactsWithMetadata } from "./kernel-BcjIFVgI.js";
...
import { t as createChannelInboundEnvelopeBuilder } from "./envelope-CQ9Mj3EE.js";
...
import { i as channelStoppedPatch, n as channelBlockedPatch, o as createTransportActivityStatusPatch, r as channelReadyPatch } from "./gateway-runtime-BOoMLXP7.js";
import "./html-entity-runtime-DaPF1Tq9.js";
import { d as canonicalizeAllowlistWithResolvedIds, l as addAllowlistUserEntriesFromConfigEntry, m as summarizeMapping, p as patchAllowlistUsersInConfigEntries, u as buildAllowlistResolutionSummary } from "./allow-from-CoKxBCLg.js";
...
import { a as resolveMatrixAccountConfig, i as resolveMatrixAccountAllowlistConfig } from "./account-config-BFrkzWp4.js";
import { t as getMatrixRuntime } from "./runtime-Drg4hYqm.js";
import { n as resolveConfiguredMatrixBotUserIds } from "./accounts-oeV3IBG_.js";
import { n as normalizeMatrixUserId, r as resolveMatrixAllowListMatch, t as normalizeMatrixAllowList } from "./allowlist-DYcR_rvX.js";
import { t as isMatrixQualifiedUserId } from "./target-ids-CFN4hb1k.js";
import { n as resolveMatrixRoomConfig } from "./rooms-MA4iSP9b.js";
import { t as resolveMatrixStoredSessionMeta } from "./session-store-metadata-OKaBHN-X.js";
import { g as isPollStartType, h as isPollEventType, l as chunkMatrixText, p as formatPollAsText, r as sendMessageMatrix, v as parsePollStartContent } from "./send-TXyQeVRJ.js";
import { a as readJoinedMatrixMembers, r as isStrictDirectMembership, t as hasDirectMatrixMemberFlag } from "./direct-room-DGg6GySX.js";
import { o as MATRIX_OPENCLAW_FINALIZED_PREVIEW_KEY, r as promoteMatrixDirectRoomCandidate } from "./direct-management-Cnt3QosU.js";
import { t as createMatrixThreadBindingManager } from "./thread-bindings-D800Hhv-.js";
import { i as createMatrixInboundEventDeduper } from "./inbound-dedupe-BcqZkFmI.js";
import { a as resolveMatrixMessageAttachment, c as fetchMatrixPollSnapshot, i as isLikelyBareFilename, n as formatMatrixMediaUnavailableText, o as resolveMatrixMessageBody, r as formatMatrixMessageText, t as formatMatrixMediaTooLargeText } from "./media-text-j7xzeoh0.js";
import { n as setActiveMatrixClient } from "./active-client-Em4bZMXV.js";
import { t as isBunRuntime } from "./runtime-BefyhPWv.js";
import { n as resolveMatrixAuth, r as resolveMatrixAuthContext, t as backfillMatrixAuthDeviceIdAfterStartup } from "./config-CjddSpCO.js";
import { i as throwIfMatrixStartupAborted, r as isMatrixStartupAbortError } from "./startup-abort-BYdCNXCk.js";
import { i as resolveSharedMatrixClient, n as releaseSharedClientInstance } from "./shared-DEDQ-0C0.js";
import "./client-C_n6olO3.js";
import { i as isMatrixMediaSizeLimitError, r as MatrixMediaSizeLimitError } from "./http-client-DohnVrXo.js";
import "./runtime-api-_XLSr0sT.js";
import { t as resolveMatrixTargets } from "./resolve-targets-CJJ6BjYl.js";
import { t as formatMatrixEncryptedEventDisabledWarning } from "./encryption-guidance-BEuC-MZf.js";
import { a as isMatrixAccessTokenInvalidatedError, d as isMatrixDisconnectedSyncState, f as isMatrixReadySyncState, p as isMatrixTerminalSyncState } from "./client-support-Dyvq4T4B.js";
import { n as setBoundedMap, t as createMatrixRoomInfoResolver } from "./room-info-BgwH1Xe1.js";
import { a as EventType, i as resolveMatrixThreadRouting, n as resolveMatrixReplyToEventId, o as RelationType, r as resolveMatrixThreadRootId, t as resolveMatrixInboundRoute } from "./route-CiRnRKaC.js";
...
function createDirectRoomTracker(client, opts = {}) {
	...
	const resolveJoinedMembers = async (roomId) => {
		...
		try {
			...
			setBoundedMap(joinedMembersCache, roomId, {
				members: normalized,
				ts: now
			}, MAX_DM_ROOMS);
			return normalized;
	...
	const resolveDirectMemberFlag = async (roomId, userId) => {
		...
		setBoundedMap(directMemberFlagCache, cacheKey, {
			isDirect,
			ts: now
		}, MAX_DM_MEMBER_FLAGS);
		return isDirect;
	...
	const rememberLocallyPromotedDirectRoom = (roomId, remoteUserId) => {
		...
		setBoundedMap(locallyPromotedDirectRooms, roomId, { remoteUserId: normalizedRemoteUserId }, MAX_DM_ROOMS);
	};
	return {
		...
		rememberInvite: (roomId, remoteUserId) => {
			const normalizedRemoteUserId = remoteUserId.trim();
			if (!normalizedRemoteUserId) return;
			setBoundedMap(recentInviteCandidates, roomId, {
...
function isMatrixVerificationRoomMessage(content) {
	return isMatrixVerificationRequestMsgType(content.msgtype) || trimMaybeString(content.msgtype) === "m.notice" && isMatrixVerificationNoticeBody(content.body);
}
...
const loadMatrixDirectRoomDeps = createLazyRuntimeModule(() => Promise.all([import("./direct-management-D9ttw7Hn.js"), import("./direct-room-CdNnkXix.js")]).then(([directManagementModule, directRoomModule]) => ({
...
function readVerificationSignal(event) {
	...
	const msgtype = normalizeNullableString(content.msgtype) ?? "";
	const relatedEventId = normalizeNullableString(content["m.relates_to"]?.event_id);
	const transactionId = normalizeNullableString(content.transaction_id);
...
function formatVerificationSasNotice(summary) {
	...
	if (!sas) return null;
	const emojiLine = Array.isArray(sas.emoji) && sas.emoji.length > 0 ? `SAS emoji: ${sas.emoji.map(([emoji, name]) => `${normalizeNullableString(emoji) ?? "?"} ${normalizeNullableString(name) ?? "?"}`).join(" | ")}` : null;
...
async function resolveVerificationSummaryForSignal(client, params) {
	...
	const activeRoomId = normalizeNullableString((await inspectMatrixDirectRooms({
		client,
	...
	const activeInRoom = activeByUser.filter((entry) => {
		return normalizeNullableString(entry.roomId) === params.roomId;
	});
...
function createMatrixVerificationEventRouter(params) {
	...
	async function resolveActiveDirectRoomId(remoteUserId) {
		...
		return normalizeNullableString((await inspectMatrixDirectRooms({
	...
	async function resolveSummaryRoomId(summary) {
		const mappedRoomId = normalizeNullableString(summary.roomId) ?? normalizeNullableString(summary.transactionId ? verificationFlowRooms.get(summary.transactionId) : null) ?? normalizeNullableString(verificationFlowRooms.get(summary.id));
	...
	function routeVerificationEvent(roomId, event) {
		...
		const routeTask = async () => {
			...
			const sourceFingerprint = normalizeNullableString(event?.event_id) ?? `${senderId}:${event.type}:${flowId ?? "none"}`;
			if (!await (await loadMatrixDirectRoomDeps()).isStrictDirectRoom({
				client: params.client,
				roomId,
				remoteUserId: senderId
...
function registerMatrixMonitorEvents(params) {
	const { cfg, client, auth, allowFrom, dmEnabled, dmPolicy, readStoreAllowFrom, directTracker, invalidateMemberDisplayName, logVerboseMessage, warnedEncryptedRooms, warnedCryptoMissingRooms, logger, startupGraceMs, getHealthySyncSinceMs, formatNativeDependencyHint, onRoomMessage, runDetachedTask, sasNoticeRetryDelayMs } = params;
	...
	client.on("room.event", (roomId, event) => {
		...
		if (eventType === EventType.RoomMember) {
			...
			if (stateKey) invalidateMemberDisplayName?.(roomId, stateKey);
...
const loadMatrixPreflightAudioRuntime = createLazyRuntimeModule(() => import("./preflight-audio.runtime-Buqsm6PI.js"));
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-helpers.ts
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-runtime.ts
async function redactMatrixDraftEvent(client, roomId, draftEventId) {
	return await client.redactEvent(roomId, draftEventId).then(() => true, () => false);
}
function buildMatrixFinalizedPreviewContent() {
	return { [MATRIX_OPENCLAW_FINALIZED_PREVIEW_KEY]: true };
}
const loadMatrixSendModule = createLazyRuntimeModule(() => import("./send-pqN0Egvx.js"));
...
const loadMatrixReactionEvents = createLazyRuntimeModule(() => import("./reaction-events-Bvfx8MV5.js"));
const loadMatrixDraftStream = createLazyRuntimeModule(() => import("./draft-stream-M2uLg6X8.js"));
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-context.ts
async function resolveMatrixInboundContext(config) {
	const { client, core, cfg, accountId, runtime, logVerboseMessage, roomId, event, eventTs, route: _route, isDirectMessage, isRoom, effectiveRoomUsers, groupPolicy, effectiveGroupAllowFrom, contextVisibilityMode, resolveThreadContext, resolveReplyContext, threadRootId, thread, getRoomInfo, senderId, senderName, bodyText, commandBodyText, roomConfig, messageId, inboundHistory, wasMentioned, effectiveWasMentioned, shouldBypassMention, canDetectMention, shouldRequireMention, commandAuthorized, locationPayload, media, preflightAudioTranscript, historyLimit, hasExplicitSessionBinding, dmSessionScope, sharedDmContextNoticeRooms, resolveStorePath: resolveStorePathImpl, createChannelInboundEnvelopeBuilder: createChannelInboundEnvelopeBuilderImpl, finalizeInboundContext } = config;
	...
	const ctxPayload = buildChannelInboundEventContext({
		...
		media: await toInboundMediaFactsWithMetadata(media ? [{
			path: media.path,
			url: media.path,
	...
	logVerboseMessage(`matrix inbound: room=${roomId} from=${senderId} preview="${truncateUtf16Safe(bodyText, 200).replace(/\n/g, "\\n")}"`);
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-draft-controller.ts
async function createMatrixDraftController(params) {
	const { streaming, previewToolProgressEnabled, replyToMode, messageId, threadTarget, accountConfig, cfg, accountId, roomId, client, logVerboseMessage } = params;
	...
	const renderPreviewPlan = () => formatChannelProgressDraftText({
	...
	const progressDraft = createChannelProgressDraftCompositor({
		...
		seed: progressSeed,
		formatLine: formatMatrixToolProgressMarkdownCode,
		buildProgressEventLine: (input, options) => input.event === "approval" ? formatChannelProgressDraftLine(input, options) : buildChannelProgressDraftLineForEntry(progressConfigEntry, input, options),
		update: (text) => {
			const previewText = !progressDraftStreaming && (previewPlan || previewPlanExplanation) ? renderPreviewPlan() : text.replace(/^• /gmu, "- ");
			draftStream?.update(previewText);
		}
	});
	const resetPreviewToolProgress = () => {
		previewPlan = void 0;
		previewPlanExplanation = void 0;
	...
	const buildPreviewToolProgressReplyOptions = () => {
		...
		return {
			...
			onPlanUpdate: async (payload) => {
				...
				if (!draftStream || previewPlanSuppressed) return;
				previewPlan = payload.steps?.length ? payload.steps.map((step) => ({ ...step })) : void 0;
				previewPlanExplanation = payload.explanation?.replace(/\s+/g, " ").trim() || void 0;
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-ingress-access.ts
async function resolveMatrixIngressAccess(config) {
	const { handler, params: paramsLocal, roomId, event, eventTs, senderId, isReactionEvent, readStoreAllowFrom, shouldSendPairingReply, resolveLiveAccountAllowlists, roomHistoryTracker, commitInboundEventIfClaimed } = config;
	const { core, cfg, accountId, accountConfig, client, logVerboseMessage, groupPolicy, dmEnabled, dmPolicy, dmThreadReplies, threadReplies, roomsConfig, accountAllowBots, configuredBotUserIds, needsRoomAliasesForConfig, getRoomInfo, getMemberDisplayName } = handler;
...
function hasVisibleNativeMatrixUserMention(text, userId) {
	const localpart = resolveMatrixUserLocalpart(userId);
	if (!text || !localpart) return false;
	return new RegExp(String.raw`(?:^|\p{White_Space})(?:${escapeRegExp(userId)}(?=$|\p{White_Space}|:\p{White_Space}|[,!?;](?=$|\p{White_Space}))|${escapeRegExp(`@${localpart}`)}(?=$|\p{White_Space}|:\p{White_Space}))`, "u").test(text);
}
...
function resolveMentions(params) {
	...
	const metadataBackedUserMention = Boolean(params.userId && mentionedUsers.has(params.userId) && (mentionedInFormattedBody || textMentioned || hasVisibleNativeMatrixUserMention(params.text, params.userId)));
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-ingress-content.ts
async function resolveMatrixIngressContent(config) {
	const { handler, params: paramsLocal, access, roomId, event, eventType, isPollEvent, eventTs, senderId, roomHistoryTracker, commitInboundEventIfClaimed } = config;
	const { client, core, cfg, accountId, accountConfig, logger, logVerboseMessage, historyLimit, mediaMaxBytes, dmSessionScope, getMemberDisplayName } = handler;
	const { content: accessContent, messageId, audioPreflightMode, isDirectMessage, isRoom, locationPayload, reservedHistorySlot, threadRootId, thread, historyThreadId, discardReservedHistorySlot, markReservedHistorySlotConsumed, commitInboundEventIfClaimedAndDiscardReserved, roomConfig, allowBotsMode, isConfiguredBotSender, selfUserId, botLoopProtection, roomMatchMeta, getSenderName, accessState, effectiveGroupAllowFrom, effectiveRoomUsers } = access;
	...
	if (reservedHistorySlot && preparedTrigger) markReservedHistorySlotConsumed();
	return {
		route: _route,
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-ingress-prefix.ts
async function readMatrixIngressPrefix(config) {
	const { client, senderId, dropPreStartupMessages, eventTs, eventAge, startupMs, startupGraceMs, event, eventType, eventId, inboundDeduper, roomId, logVerboseMessage, directTracker, claimInboundReplay } = config;
	...
	if (eventId && inboundDeduper) {
		...
		if (claim.kind === "claimed") claimInboundReplay(claim.handle);
...
function joinMatrixVisibleContent(contents) {
	return contents.filter((content) => Boolean(content)).join("\n");
}
function mergeMatrixReplyDeliveryResults(results) {
	...
	for (const result of visibleResults) {
		...
		for (const messageId of result.messageIds ?? []) receiptInputs.push({ messageId });
	}
	const receipt = receiptInputs.length > 0 ? createMessageReceiptFromOutboundResults({ results: receiptInputs }) : void 0;
	return {
		...receipt ? {
...
function toMatrixPartialDeliveryError(error, settled) {
	const failedPartial = isChannelPartialDeliveryError(error) ? error.deliveryResult : void 0;
	const merged = mergeMatrixReplyDeliveryResults([...settled, ...failedPartial ? [failedPartial] : []]);
	return merged.visibleReplySent ? createChannelPartialDeliveryError(error, {
		...merged,
		visibleReplySent: true
	}) : error;
}
function createMatrixReplyDeliveryResult(results) {
	if (results.length === 0) return mergeMatrixReplyDeliveryResults([]);
	const receipt = createMessageReceiptFromOutboundResults({ results: results.map((result) => ({ receipt: result.receipt })) });
	return {
		messageIds: listMessageReceiptPlatformIds(receipt),
		receipt,
		visibleReplySent: true,
...
function resolveVisibleMatrixReplyText(text) {
	if (typeof text !== "string") return;
	const trimmedStart = text.trimStart();
	if (!trimmedStart) return text;
	if (normalizeLowercaseStringOrEmpty(trimmedStart).startsWith("reasoning:")) return;
	const visibleText = stripReasoningTagsFromText(text, {
		mode: "strict",
		trim: "none"
	});
	return visibleText.trim() ? visibleText : void 0;
}
async function deliverMatrixReplies(params) {
	...
	try {
		for (const reply of params.replies) {
			...
			if (reply.isReasoning === true || !hasMedia && reply.text && visibleText === void 0) {
			...
			const shouldIncludeReply = (id) => Boolean(id) && (params.threadId || params.replyToMode === "all" || !hasRepliedRef.value);
			const replyToIdForReply = shouldIncludeReply(replyToId) ? replyToId : void 0;
			const onDeliveryResult = (result) => {
				acceptedResults.push(result);
				if (replyToIdForReply) hasRepliedRef.value = true;
			};
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-reply-dispatcher.ts
function createMatrixReplyDispatcher(config) {
	const { cfg, prefixOptions, humanDelay, typingCallbacks, streaming, draftStream, draftController, client, roomId, runtime, textLimit, replyToMode, threadTarget, replyToEventId, accountId, mediaLocalRoots, tableMode, logVerboseMessage } = config;
	const quietDraftStreaming = streaming === "quiet" || streaming === "progress";
	...
	const { deliver: deliverReply, onError: onReplyError, ...turnDispatcherOptions } = {
		...
		deliver: async (payload, info) => {
			const completeDelivery = async (result) => {
				if (info.kind === "block") {
			...
			const createDraftReceipt = (id) => createPreviewMessageReceipt({
				id,
				...threadTarget ? { threadId: threadTarget } : {},
				...draftController.currentReplyToId() ? { replyToId: draftController.currentReplyToId() } : {}
			});
			const createDraftDeliveryResult = (id, content) => {
			...
			const createSurvivingDraftDelivery = (id, redacted) => {
				const content = redacted ? void 0 : draftStream?.content();
				return content ? createDraftDeliveryResult(id, content) : mergeMatrixReplyDeliveryResults([]);
			};
			if (draftStream && info.kind !== "tool" && !payload.isCompactionNotice) {
				...
				if (draftController.isDraftConsumed()) {
					await draftStream.discardPending();
					return await completeDelivery(await deliverMatrixReplies({
						cfg,
				...
				const payloadReplyMismatch = replyToMode !== "off" && !threadTarget && payloadReplyToId !== draftController.currentReplyToId();
				...
				if (draftEventId && payload.text && !payload.isError && !hasMedia && !payloadReplyMismatch && !mustDeliverFinalNormally && !draftFinalTextNeedsNormalMentionDelivery) {
					...
					return await completeDelivery(previewResult.kind === "preview-finalized" && previewResult.liveState?.receipt ? createDraftDeliveryResult(draftEventId, finalizedDraftContent ?? preparedFinalPreviewContent) : fallbackResult ?? mergeMatrixReplyDeliveryResults([]));
					...
					const preparedPayloadContent = typeof payloadText === "string" ? (await loadMatrixSendModule()).prepareMatrixSingleText(payloadText, {
						cfg,
						accountId,
						preserveWhitespace: true
					}).convertedText : void 0;
					...
					const draftRedacted = reusesDraftAsFinalText ? false : await redactMatrixDraftEvent(client, roomId, draftEventId);
					...
					const previewDelivery = reusesDraftAsFinalText && providerDraftContent ? createDraftDeliveryResult(draftEventId, providerDraftContent) : !draftRedacted && draftContent ? createDraftDeliveryResult(draftEventId, draftContent) : mergeMatrixReplyDeliveryResults([]);
					let mediaDelivery;
					try {
					...
					draftController.markDraftConsumed();
					return await completeDelivery(mergeMatrixReplyDeliveryResults([previewDelivery, mediaDelivery]));
				}
				const shouldRedactDraft = Boolean(draftEventId) && (payload.isError || payloadReplyMismatch || mustDeliverFinalNormally || draftFinalTextNeedsNormalMentionDelivery);
				const draftRedacted = shouldRedactDraft && draftEventId ? await redactMatrixDraftEvent(client, roomId, draftEventId) : false;
				const survivingDraft = shouldRedactDraft && draftEventId ? createSurvivingDraftDelivery(draftEventId, draftRedacted) : mergeMatrixReplyDeliveryResults([]);
				let deliveredFallback;
				try {
					deliveredFallback = await deliverMatrixReplies({
						cfg,
				...
				if (shouldRedactDraft || deliveredFallback.visibleReplySent) draftController.markDraftConsumed();
				return await completeDelivery(mergeMatrixReplyDeliveryResults([survivingDraft, deliveredFallback]));
			}
			return await completeDelivery(await deliverMatrixReplies({
				cfg,
				replies: [payload],
		...
		onError: (err, info) => {
			...
			if (info.kind === "block") draftController.advanceDraftBlockBoundary({ fallbackToLatestEnd: true });
...
//#endregion
//#region extensions/matrix/src/matrix/monitor/handler-state.ts
const ALLOW_FROM_STORE_CACHE_TTL_MS = 3e4;
const PAIRING_REPLY_COOLDOWN_MS = 5 * 6e4;
const MAX_TRACKED_PAIRING_REPLY_SENDERS = 512;
function createMatrixHandlerState(config) {
	const { core, accountId, runtime, allowFromResolvedEntries, groupAllowFromResolvedEntries, resolveLiveUserAllowlist } = config;
	...
	const resolveLiveAccountAllowlists = async () => {
		...
		return {
			...
			liveGroupAllowFrom: await resolveCachedLiveAllowlist({
				cfg: liveCfg,
...
function createMatrixRoomMessageHandler(params) {
	...
	return async (roomId, event) => {
		...
		try {
			...
			const readIngressPrefix = () => readMatrixIngressPrefix({
				client,
				senderId,
				dropPreStartupMessages,
				eventTs: eventTs ?? void 0,
				eventAge: eventAge ?? void 0,
				startupMs,
				startupGraceMs,
				event,
				eventType,
			...
			if (!inboundContext) return;
			const { replyToEventId, threadTarget, storePath, ctxPayload, replyTarget, sharedDmContextNotice } = inboundContext;
			...
			const replyDispatcher = createMatrixReplyDispatcher({
				...
				replyToMode,
				threadTarget,
				replyToEventId: replyToEventId ?? void 0,
				accountId: _route.accountId,
				mediaLocalRoots,
				tableMode,
				logVerboseMessage
				...
				adapter: {
					...
					resolveTurn: () => ({
						...
						replyOptions: {
							...
							onPartialReply: draftStream ? (payload) => draftController.onPartialReply(payload.text ?? "") : void 0,
							...
							onQueuedFollowupAdmitted: draftStream ? draftController.resetDraftDeliveryState : void 0,
			...
			if (draftStream) {
				const draftEventId = await draftStream.stop().catch(() => void 0);
				if (draftEventId && draftControllerRef?.isDraftConsumed() !== true) await redactMatrixDraftEvent(client, roomId, draftEventId);
			}
...
const loadMatrixStartupMaintenanceDeps = createLazyRuntimeModule(() => Promise.all([
	import("./config-update-DHV8Np5k.js"),
	import("./device-health-CA7cIiLv.js"),
	import("./profile-CbslQ84W.js"),
	import("./startup-verification-DJOnzqGN.js")
...
function createMatrixMonitorStatusController(params) {
	...
	const noteDisconnected = (paramsLocal) => {
		...
		if (tokenInvalidated) Object.assign(status, channelBlockedPatch(error ?? "Matrix access token invalidated"));
		else {
	...
	return {
		...
		markStopped(at = Date.now()) {
			if (status.lifecycle !== "blocked" && status.healthState !== "error") Object.assign(status, channelStoppedPatch({
				lastEventAt: at,
				healthState: "stopped"
			}));
			else {
...
function resolveMatrixStreamingMode(streaming) {
	...
	if (mode === "partial" || mode === "quiet" || mode === "progress") return mode;
	return "off";
}
...
async function monitorMatrixProvider(opts = {}) {
	...
	try {
		...
		const { getRoomInfo, getMemberDisplayName, invalidateMemberDisplayName } = createMatrixRoomInfoResolver(client);
		const isExplicitlyConfiguredRoom = async (roomId) => {
