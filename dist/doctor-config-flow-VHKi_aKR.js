...
import { t as runDoctorConfigPreflight } from "./doctor-config-preflight-C-rTIp7u.js";
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	if (!shouldRepair) {
		const { repairStaleAgentModelRefs } = await import("./stale-agent-model-ref-repair-hD7nFaVD.js");
	...
	if (shouldRepair) {
		const { runDoctorRepairSequence } = await import("./repair-sequencing-DQdC-dSz.js");
		...
		const { collectDoctorPreviewNotes } = await import("./preview-warnings-RBrJ2f5e.js");
