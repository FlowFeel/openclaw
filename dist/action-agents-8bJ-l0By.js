...
import { i as sortSubagentRuns, t as formatRunLabel } from "./subagents-utils-Co2UgCFq.js";
import { l as stopWithText } from "./shared-Dd6sVZF-.js";
...
function handleSubagentsAgentsAction(ctx) {
	...
	const numericOrder = [...dedupedRuns.filter((entry) => !entry.execution.endedAt || readIndex.countPendingDescendantRuns(entry.childSessionKey) > 0), ...dedupedRuns.filter((entry) => entry.execution.endedAt && readIndex.countPendingDescendantRuns(entry.childSessionKey) === 0 && entry.execution.endedAt >= recentCutoff)];
	...
	for (const entry of dedupedRuns) {
		if (!(!entry.execution.endedAt || readIndex.countPendingDescendantRuns(entry.childSessionKey) > 0 || resolveSessionBindings(entry.childSessionKey).length > 0)) continue;
