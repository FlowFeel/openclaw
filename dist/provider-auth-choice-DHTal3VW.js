...
async function applyDefaultModelFromAuthChoice(params) {
	...
	if (!preservesDifferentPrimary) {
		const { CODEX_RUNTIME_PLUGIN_ID, ensureCodexRuntimePluginForModelSelection } = await import("./codex-runtime-plugin-install-Bb2yVhvM.js");
		...
		const { ensureCopilotRuntimePluginForModelSelection } = await import("./copilot-runtime-plugin-install-Dpmd956u.js");
