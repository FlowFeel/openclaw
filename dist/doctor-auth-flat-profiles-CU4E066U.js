...
import { h as getNodeSqliteKysely, m as executeSqliteQueryTakeFirstSync, p as executeSqliteQuerySync, t as openNodeSqliteDatabase } from "./node-sqlite-BJTPe7U8.js";
...
import { c as resolveAuthProfileDatabasePath, f as resolveSharedMainAuthAgentDir, i as readPersistedAuthProfileStateRaw, l as runAuthProfileWriteTransaction, n as inspectPersistedAuthProfileStateRaw, r as inspectPersistedAuthProfileStoreRaw } from "./sqlite-DQLy89x1.js";
import { a as clearAuthProfileMigrationDiagnostics, p as resolveLegacyOAuthPath, u as listLegacyAuthProfileArchives } from "./legacy-source-diagnostic-CQwNDL5h.js";
import { n as hasMatchingOAuthIdentity, t as areOAuthCredentialsEquivalent } from "./oauth-shared-B-oYylTU.js";
import { t as loadJsonFile } from "./json-file-vo1Xldgv.js";
import { a as loadPersistedAuthProfileStore, i as coercePersistedAuthProfileStore, l as coerceAuthProfileState, r as coerceLegacyAuthStore, s as parseLegacyCredentialEntry, t as applyLegacyAuthStore } from "./persisted-pDgmg_nM.js";
import { l as isInheritedMainOAuthCredential, r as clearRuntimeAuthProfileStoreSnapshots, v as saveAuthProfileStore } from "./store-C8MGqOG3.js";
import { t as note } from "./note-uiKX_qfX.js";
import { n as resolveLegacyAuthStatePath, r as resolveLegacyFlatAuthPath, t as resolveLegacyAuthProfilesPath } from "./doctor-auth-legacy-paths-U4S5zdNi.js";
import { a as recordLegacyMigrationRun, n as readLegacyMigrationReceipt, o as recordLegacyMigrationSource } from "./state-migrations.receipts-BV0HPXiq.js";
...
function verifyAuthProfileMigrationTarget(receipt) {
	...
	try {
		...
		if (hasExpectedProfiles && receipt.expectedProfileSha256) {
			const row = executeSqliteQueryTakeFirstSync(db, kysely.selectFrom("auth_profile_store").select("store_json").where("store_key", "=", "primary"));
		...
		if (receipt.expectedStateSha256) {
			const row = executeSqliteQueryTakeFirstSync(db, kysely.selectFrom("auth_profile_state").select("state_json").where("state_key", "=", "primary"));
...
function resumePendingAuthProfileMigrationArchives(env) {
	...
	for (const row of rows) {
		...
		if (typeof row.source_sha256 !== "string" || typeof row.source_size_bytes !== "number" || typeof row.source_record_count !== "number" || typeof report.archivePath !== "string" || typeof report.targetDatabasePath !== "string" || row.target_table !== "auth_profile_store" && row.target_table !== "auth_profile_state") throw new Error("invalid pending auth profile migration receipt");
		const receipt = {
			...
			...isRecordOfStrings(report.expectedProfileSha256) ? { expectedProfileSha256: report.expectedProfileSha256 } : {},
...
function normalizeLegacyApiKeyAliasesForImport(raw) {
	...
	for (const profile of Object.values(raw.profiles)) {
		...
		if (readNonEmptyString(profile.key) !== void 0 || coerceSecretRef(profile.key) !== null || coerceSecretRef(profile.keyRef) !== null || profile["api_key"] === void 0) continue;
...
function loadAuthProfileMigrationTargetStore(agentDir, loadStore = loadPersistedAuthProfileStore, database) {
	const inspection = inspectPersistedAuthProfileStoreRaw(agentDir, database);
	const store = loadStore(agentDir, database ? { database } : void 0);
	if (store) return store;
	if (inspection.status !== "missing") throw new Error("canonical auth profile store is unreadable; legacy source left in place");
	if (inspectPersistedAuthProfileStateRaw(agentDir, database).status === "unreadable") throw new Error("canonical auth profile state is unreadable; legacy source left in place");
	return {
		version: 1,
		profiles: {},
...
function migrateLockedLegacyOAuthFile(params) {
	...
	const parsed = coerceLegacyOAuthFile(loadJsonFile(params.oauthPath));
	...
	const loaded = runAuthProfileWriteTransaction(mainAgentDir, (database) => {
		if (!isDeepStrictEqual(loadAuthProfileMigrationTargetStore(mainAgentDir, loadPersistedAuthProfileStore, database), existing)) throw new Error("canonical auth profile store changed during legacy OAuth migration");
		saveAuthProfileStore(next, mainAgentDir, {
...
async function maybeMigrateAuthProfileJsonStoresToSqlite(params) {
	...
	for (const candidate of detected) {
		...
		try {
			...
			let sourceReceipts = candidateSourcePaths.filter(fs.existsSync).map((pathname) => prepareAuthProfileSourceReceipt({
				pathname,
				targetDatabasePath,
				targetTable: pathname === candidate.statePath ? "auth_profile_state" : "auth_profile_store",
				now,
				env
			}));
			...
			if (!canonicalStore && !configCanonicalStore && !legacyStore && !hasAuthProfileState(state) && !awsSdkMarkerStore) {
				if (!unresolvedSidecarRawStore && sourceReceipts.length > 0) {
					...
					result.warnings.push(`Archived unparseable auth profile input without import for ${shortenHomePath(candidate.authPath)} (${archived.map(shortenHomePath).join(", ")}).`);
					continue;
				}
				...
				try {
					...
					verifiedStore = runAuthProfileWriteTransaction(candidate.agentDir, (database) => {
						...
						const dedupedToMainProfileIds = new Set([...importedProfileIds].filter((profileId) => {
							...
							return credential !== void 0 && !loaded?.profiles[profileId] && isInheritedMainOAuthCredential({
								agentDir: candidate.agentDir,
								profileId,
								credential
							});
			...
			const archivalReceipts = unresolvedSidecarRawStore ? sourceReceipts.filter((receipt) => receipt.sourcePath !== candidate.authPath) : sourceReceipts;
			assertAuthProfileMigrationSourcesUnchanged(candidate, sourceReceipts);
			const archives = archivalReceipts.map((receipt) => archiveVerifiedAuthProfileSource(receipt, true));
	...
	if (hasLegacyOAuth && sharedMainCredentialSourceRemains) result.warnings.push(`Deferred shared legacy OAuth migration until higher-priority shared-main credential sources are resolved by ${formatCliCommand("openclaw doctor --fix")}.`);
		...
		result.warnings.push(`Failed to migrate shared legacy OAuth credentials; the source was left in place.`);
	}
...
function recoverArchivedOpenAICodexAuthProfileIdMap(params) {
	...
	const archives = listLegacyAuthProfileArchives({
		agentDirs: params.candidates.flatMap((candidate) => candidate.agentDir ? [candidate.agentDir] : []),
		env: params.env
