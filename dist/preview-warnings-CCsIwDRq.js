...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-Cs_uOTOp.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-ZpoXg6S_.js"));
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-CZsMKsn3.js"), import("./command-secret-targets-mPBlwEGw.js")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	const channelPluginRuntime = await import("./channel-plugin-blockers-C7YQkIDy.js");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-DY8HWtTV.js");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-QOf3n_S5.js");
	...
	const { collectCodexRouteWarnings } = await import("./codex-route-warnings-NdXKn7y8.js");
	...
	if (hasPluginConfig) {
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-BjrGyiJh.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-BUg2cGBu.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-DAx7RgU6.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-Rlc6ZQUk.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-C_7tbNy4.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-Dr41triH.js");
	...
	const { collectStaleConfiguredAuthOrderWarnings } = await import("./stale-auth-order-BJG_Guus.js");
