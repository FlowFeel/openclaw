...
import { F as isPluginRegistryRetired, d as getActivePluginRegistry, k as getPluginRuntimeGatewayRequestScope } from "./runtime-WkCmQTS9.js";
import { d as attachToolAllowlistIntersection, g as readToolAllowlistIntersection, h as normalizeToolName, m as normalizeToolList, p as expandToolGroups } from "./tool-policy-CrjVfI-s.js";
...
function normalizePluginToolMatcher(matcher) {
	...
	for (let index = 0; index < matcher.length; index += 1) {
		...
		if (NON_CANONICAL_TOOL_MATCHER_NAMES.has(canonicalToolName)) throw new TypeError("tool hook matcher entries must use canonical OpenClaw tool ids");
...
function createHookRunner(registry, options = {}) {
	...
	const runSyncHookHandler = (hook, event, ctx) => {
		...
		Promise.resolve(out).catch(() => void 0);
		const msg = `[hooks] ${hook.hookName} handler from ${hook.pluginId} returned a Promise; this hook is synchronous and the result was ignored.`;
	...
	*/
	async function runBeforePromptBuild(event, ctx) {
		...
		return await beforePromptBuildDispatch.run(token, async () => {
			try {
				return await runModifyingHook("before_prompt_build", event, ctx, { mergeResults: mergeBeforePromptBuild });
			} finally {
				token.active = false;
			}
		});
	...
	*/
	function runToolResultPersist(event, ctx) {
		...
		for (const hook of hooks) try {
			const next = runSyncHookHandler(hook, {
				...event,
				message: current
			}, ctx)?.message;
	...
	*/
	function runBeforeMessageWrite(event, ctx) {
		...
		let current = event.message;
		for (const hook of hooks) try {
			const result = runSyncHookHandler(hook, {
				...event,
				message: current
			}, ctx);
...
//#endregion
export { fireAndForgetBoundedHook as _, resetGlobalHookRunner as a, createPluginToolMatcherScope as c, pluginToolMatcherCoversTool as d, createPluginSubagentRequesterContext as f, joinPresentTextSegments as g, resolveBlockMessage as h, initializeGlobalHookRunner as i, mergePluginToolMatcherScopes as l, cloneHookIsolationValue as m, getGlobalPluginRegistry as n, runGlobalGatewayStopSafely as o, resolvePluginSubagentCompletionRequester as p, hasGlobalHooks as r, getToolHookMatcherScope as s, getGlobalHookRunner as t, normalizePluginToolMatcher as u, fireAndForgetHook as v, getGlobalHookRunnerRegistry as y };
