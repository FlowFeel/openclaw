...
import { n as describeSystemAgentPersistentOperation } from "./operations-DStBH-DH.js";
...
import { n as getHealthCache } from "./health-state-CCFf1_L-.js";
import { t as WizardSession } from "./session-BeEA_WOJ.js";
import { n as SystemAgentChatEngine, r as SystemAgentWizardAnswerError } from "./chat-engine-CiYvSk-0.js";
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = new WizardSession(async (prompter, signal) => {
			await runExclusiveSystemAgentSetupActivation(async () => runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-DBDm6msJ.js"), import("./setup.shared-B20Bo9O_.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const inference = params.delegation ? await import("./inference-fallback-oTmMTiF2.js").then(({ verifySystemAgentInferenceWithFallback }) => verifySystemAgentInferenceWithFallback({
