import { v as shouldMigrateStateFromPath } from "./argv-D6QuZiS5.js";
import { o as resolveRequiredHomeDir } from "./home-dir-Cs7bTrwJ.js";
import { C as resolveStateDir, S as resolveOAuthDir, v as resolveIsNixMode, y as resolveLegacyStateDirs } from "./paths-CL43LNS6.js";
...
import { c as resolveExecApprovalsPath } from "./exec-approvals-config-DrfE-I9X.js";
import { i as withSuppressedNotes } from "./note-uiKX_qfX.js";
...
async function ensureConfigReady(params, recoveryDeps) {
	...
	const runStateMigrationPreflight = async () => {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-DMEAXnGp.js")).runDoctorConfigPreflight({
			migrateState: true,
	...
	const configSnapshotOptions = commandName === "logs" ? {
		observe: false,
		skipPluginValidation: true
	} : commandName === "status" || commandName === "gateway" && subcommandName === "call" ? { observe: false } : void 0;
	...
	const { formatConfigIssueLines } = await import("./issue-format-bHy1_ouZ.js");
	...
	const [{ colorize, isRich, theme }, { shortenHomePath }, { formatCliCommand }, { isPluginPackagingRuntimeOutputInvalidConfigSnapshot }, { formatPluginPackagingRuntimeOutputRecoveryHint }] = await Promise.all([
		...
		import("./utils-blFhDTzd.js"),
		import("./command-format-nGsiCAcY.js"),
		import("./recovery-policy-XgXko0ay.js"),
		import("./config-recovery-hints-CtFy2vFu.js")
	]);
	const rich = isRich();
	...
	if (isPluginPackagingFailure || isNixManagedConfig || !shouldOfferRecovery) {
		const fixHint = isPluginPackagingFailure ? formatPluginPackagingRuntimeOutputRecoveryHint() : isNixManagedConfig ? new (await (import("./nix-mode-write-guard-hMHcX3Gw.js"))).NixModeConfigMutationError({ configPath: snapshot.path }).message : commandText(formatCliCommand("openclaw doctor --fix"));
	...
	if (shouldOfferRecovery && !isPluginPackagingFailure) {
		...
		if ((await offerInvalidConfigRecovery({
			...
			retry: async () => {
				configSnapshotPromise = null;
				const { runDoctorConfigPreflight } = await import("./doctor-config-preflight-DMEAXnGp.js");
