...
import { i as GATEWAY_SERVICE_RUNTIME_PID_ENV } from "./constants-Qf4OESGR.js";
import { c as normalizeStateDirEnv, g as resolveGatewayPort, t as CONFIG_PATH } from "./paths-CL43LNS6.js";
...
import { r as flushLogger } from "./logger-CEPY5_jR.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-shoafCYv.js";
import { t as createSubsystemLogger } from "./subsystem-Cr19cPPQ.js";
import { x as findOpenClawStateDatabaseSchemaMigrationRequiredError } from "./openclaw-state-db-BU55lNCH.js";
import { c as hasConfiguredSecretInput } from "./types.secrets-BvApkFoj.js";
import { p as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-DwleMuU3.js";
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-CVIGoBrK.js";
import { t as clearRuntimeConfigSnapshot } from "./runtime-snapshot-DLOCFXOE.js";
import { i as isInvalidConfigError, r as isDoctorRecoverableInvalidConfigError } from "./io.invalid-config-BvEx-KOJ.js";
import "./globals-DHQUG86L.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-BPZGOtDB.js";
import { t as isContainerEnvironment } from "./container-environment-CNsJSTpY.js";
import { h as resolveGatewayBindHost, o as isLoopbackHost, t as defaultGatewayBindMode } from "./net-B22ilI8B.js";
import { n as formatGatewayPidList, t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-Rib2chcH.js";
...
import { r as findOpenClawAgentDatabaseMediaMigrationRequiredError, t as GATEWAY_AGENT_MEDIA_MIGRATION_REQUIRED_REASON } from "./openclaw-agent-db-migration-required-BU5hc3wx.js";
import { t as printClawBanner } from "./claw-banner-fdUSdLjD.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-Cu5YQjxo.js";
import { r as withProgress } from "./progress-BjGUBuxw.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-86BGsxSJ.js";
import { n as isGatewayRunFutureConfigAllowed, t as enforceGatewayRunFutureConfigGuard } from "./future-config-guard-D3I47UZn.js";
import { a as inspectGatewayCrashLoopBreaker, i as formatGatewayCrashLoopManualChannelStartHint, n as GATEWAY_CRASH_LOOP_RECOVERED_REASON, o as recordGatewayBootStart, r as completeGatewayBootLifecycle, s as recordGatewayCrashLoopRecovery, t as GATEWAY_CRASH_LOOP_BREAKER_REASON } from "./gateway-boot-lifecycle-B_02jyRB.js";
...
//#endregion
//#region src/agents/embedded-agent-runner/drain-admission-policy.ts
/**
...
*/
function resolveDrainTimeoutMs(intent, config) {
	if (intent.force === true) return 0;
	if (typeof intent.waitMs === "number" && Number.isFinite(intent.waitMs) && intent.waitMs >= 0) return Math.floor(intent.waitMs);
	...
	if (typeof defaultMs === "number" && Number.isFinite(defaultMs) && defaultMs >= 0) {
		...
		if (typeof max === "number" && Number.isFinite(max) && max >= 0) return Math.min(Math.floor(defaultMs), Math.floor(max));
...
async function runGatewayLoop(params) {
	...
	const exitProcessAfterLogFlush = async (code) => {
		let flushTimer;
	...
	const resolveRestartDrainTimeoutMs = async (restartIntent) => {
		...
		if (pureResult !== void 0) return pureResult === 0 ? void 0 : pureResult;
		try {
	...
	const runAcceptedRequest = (acceptedRequest) => {
		...
		(async () => {
			...
			try {
				if (isRestart) {
					...
					await measureGatewayRestartTrace("restart.drain", async () => {
						...
						const markActiveMainSessionsForRestart = async (reason) => {
							if (hasMarkedActiveMainSessionsForRestart) return;
							...
							try {
								if ((await markRestartAbortedMainSessions({
									cfg: getRuntimeConfig(),
									sessionKeys,
									sessionIds,
									reason
								})).marked > 0) hasMarkedActiveMainSessionsForRestart = true;
							} catch (err) {
				...
				if (!isRestart) try {
					const rootDrain = await eagerLifecycleRuntime.waitForActiveGatewayRootWork(Math.max(0, SHUTDOWN_TIMEOUT_MS - RESTART_CLOSE_REPLY_DRAIN_SHUTDOWN_RESERVE_MS));
					if (!rootDrain.drained) gatewayLog$1.warn(`gateway root transaction drain timeout reached with ${rootDrain.active} root(s) still active; proceeding with shutdown`);
				} catch (err) {
					gatewayLog$1.warn(`gateway root transaction drain failed; proceeding with shutdown: ${formatErrorMessage(err)}`);
				}
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-DXQYkwuA.js");
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	const { createConfigRuntimeEnv } = await import("./env-vars-KZxwXfFD.js");
	const { resolveShellEnvFallbackTimeoutMs, shouldDeferShellEnvFallback, shouldEnableShellEnvFallback } = await import("./shell-env-CTw342Md.js");
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-C2xV2dP2.js");
	return {
		enabled: true,
...
async function loadGatewayRunShellEnvFallback(plan) {
	const { loadShellEnvFallback } = await import("./shell-env-CTw342Md.js");
...
async function clearGatewayRunShellEnvFallback(values) {
	...
	const { clearShellEnvAppliedKeys } = await import("./shell-env-CTw342Md.js");
...
function resolveGatewayStartupFailureExitCode(err) {
	return isInvalidConfigError(err) || findOpenClawAgentDatabaseMediaMigrationRequiredError(err) || findOpenClawStateDatabaseSchemaMigrationRequiredError(err) ? EXIT_CONFIG_ERROR : 1;
}
...
function createConfiguredGatewayHealthProbe(cfg) {
	...
	return async (params) => {
		...
		if (!tlsFingerprint) tlsFingerprint = (await import("./gateway-DI2NxdY8.js").then(({ loadGatewayTlsRuntime }) => loadGatewayTlsRuntime({
			...tlsConfig,
			autoGenerate: false
...
async function maybeWriteGatewayStartupFailureBundle(err, reason = "gateway.startup_failed") {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-BTA5OjHT.js");
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-DeW3uFyz.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-DsmG7DTZ.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-DeW3uFyz.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-BdVeyhWx.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-DeW3uFyz.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-1sV54Imj.js");
		...
		if (stale.length > 0) {
			...
			if (process.platform === "linux") {
				const { findSystemdGatewayInstallation, formatDuelingScopesWarning } = await import("./systemd-CSqbUz-r.js");
	...
	if (opts.force) {
		...
		try {
			const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-pY_xPe7G.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-fF1oL29L.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-CR9p1YvO.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DmON7W5M.js");
		...
		if (findOpenClawAgentDatabaseMediaMigrationRequiredError(err)) try {
			const { parkCurrentLaunchAgentForMaintenance } = await import("./launchd-bdwwBeGC.js");
...
async function runGatewayCommand(opts, hooks = {}, recoveryDeps) {
	try {
		...
		const { offerInvalidConfigRecovery } = await import("./invalid-config-recovery-JNiDPDXn.js");
