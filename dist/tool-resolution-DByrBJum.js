...
import { _ as resolveToolProfilePolicy, a as collectExplicitDenylist, c as mergeAlsoAllowPolicy, h as normalizeToolName, i as collectExplicitAllowlist, l as replaceWithEffectiveToolAllowlist, s as hasRestrictiveAllowPolicy } from "./tool-policy-CrjVfI-s.js";
import "./message-channel-constants-76XnXM8q.js";
import { n as normalizeMessageChannel } from "./message-channel-core-BQfg2juM.js";
...
import { r as resolveEffectiveToolPolicy } from "./agent-tools.policy-7CMM1SfU.js";
import { n as resolveRequesterToolPolicies } from "./requester-tool-policy-Bn0yRFGG.js";
import { t as createOpenClawTools } from "./openclaw-tools-B_OHwLQN.js";
import { n as resolveEventSessionRoutingPolicy } from "./event-session-routing-BY6OxTFh.js";
import { r as GATEWAY_OWNER_ONLY_CORE_TOOLS, t as DEFAULT_GATEWAY_HTTP_TOOL_DENY } from "./dangerous-tools-wzbtc-0z.js";
import { i as resolveExecToolConfig, o as filterToolsByMessageProvider, r as createLazyExecTool, t as createOpenClawCodingTools } from "./agent-tools-Df1_Fel3.js";
import { n as nodeExecSchema } from "./bash-tools.schemas-BR2n1OVf.js";
import { n as buildDefaultToolPolicyPipelineSteps, t as applyToolPolicyPipeline } from "./tool-policy-pipeline-Caz0N3cQ.js";
...
function resolveGatewayScopedTools(params) {
	...
	const mediatedToolNames = new Set(Array.from(params.mediatedToolNames ?? [], (name) => normalizeToolName(name)).filter(Boolean));
	const gatewayToolsCfg = params.cfg.gateway?.tools;
	const defaultGatewayDeny = surface === "http" ? DEFAULT_GATEWAY_HTTP_TOOL_DENY.filter((name) => !gatewayToolsCfg?.allow?.some((allowed) => normalizeToolName(allowed) === normalizeToolName(name))) : [];
	...
	const workspaceDir = params.workspaceDir?.trim() || resolveAgentWorkspaceDir(params.cfg, agentId ?? resolveDefaultAgentId(params.cfg));
	...
	const toolsWithMediatedCoding = [...(nodeExecSurface ? openClawTools.filter((tool) => tool.name.trim().toLowerCase() !== "exec") : openClawTools).filter((tool) => !mediatedToolNames.has(normalizeToolName(tool.name))), ...mediatedCodingTools];
