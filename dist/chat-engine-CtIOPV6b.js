...
import { i as loadSystemAgentOverview } from "./overview-CDBIuwEP.js";
import { a as resolveSystemAgentVerifiedInferenceRoute, i as resolveSystemAgentExpectedAgentHarnessRuntimeArtifact } from "./verified-inference-CRg3rEqZ.js";
import { a as parseSystemAgentOperation, i as isPersistentSystemAgentOperation, n as describeSystemAgentPersistentOperation, t as executeSystemAgentOperation } from "./operations-CPFqirC1.js";
import { n as hashSystemAgentOperation } from "./system-agent-tool-D8Doo-4S.js";
...
import { t as approvalQuestion } from "./dialogue-D-_Bsew4.js";
import { n as classifySystemAgentApprovalText } from "./approval-intent-0sHAQRcs.js";
import { o as normalizeCliModel } from "./helpers-Dc_1XotW.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./embedded-agent-subscribe.tools-BHMWEsYN.js"),
		import("./system-agent-tool-BxF6SjGD.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-BuUcqlQ-.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-CZpATf5T.js")).runEmbeddedAgent)({
				...shared,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
				systemAgentTool,
...
async function runHostedConfigWizard(params) {
	const { readSetupConfigFileSnapshot, writeWizardConfigFile } = await import("./setup.shared-B1YQd8f0.js");
...
async function defaultChannelSetupWizardRunner(channel, prompter, beforePersistentApply) {
	const { createChannelOnboardingPostWriteHookCollector, runCollectedChannelOnboardingPostWriteHooks, setupChannels } = await import("./onboard-channels-yrpTV2ON.js");
...
async function defaultSkillsSetupWizardRunner(prompter, beforePersistentApply) {
	const [{ setupSkills }, { resolveOnboardingAgentTarget }] = await Promise.all([import("./onboard-skills-DLW1szzn.js"), import("./onboard-agent-target-vbLRcyRT.js")]);
	return await runHostedConfigWizard({
		label: "Skills setup",
		beforePersistentApply,
...
async function defaultGatewaySetupWizardRunner(prompter, beforePersistentApply) {
	const [{ resolveGatewayPort }, { configureGatewayForSetup }, { resolveQuickstartGatewayDefaults }] = await Promise.all([
		...
		import("./setup.gateway-config-CWCcBRM9.js"),
		import("./setup.shared-B1YQd8f0.js")
	]);
	return await runHostedConfigWizard({
		label: "Gateway setup",
		beforePersistentApply,
...
var SystemAgentChatEngine = class {
	...
	*/
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.opts.classifyApproval ?? (await import("./approval-intent-DRGGSJNk.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	*/
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.opts.planWithAssistant ?? (await import("./assistant-CQmq5t2T.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-CQmq5t2T.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
			verifiedInference: this.verifiedInference,
			deps: this.opts.deps
		}));
