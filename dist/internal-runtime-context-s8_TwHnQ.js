...
function delimitedTokenLinePattern(token) {
...
function stripStandaloneDelimitedTokenLines(text, token) {
...
function extractDelimitedBlocks(text, begin, end, options = {}) {
	let next = text;
	const blocks = [];
	for (;;) {
...
function stripDelimitedBlock(text, begin, end, options) {
...
function stripInternalRuntimeContext(text, options = {}) {
	if (!text) return text;
	return stripRuntimeContextPromptPreface(stripLegacyInternalRuntimeContext(stripStandaloneDelimitedTokenLines(stripDelimitedBlock(text, INTERNAL_RUNTIME_CONTEXT_BEGIN, INTERNAL_RUNTIME_CONTEXT_END, options), INTERNAL_RUNTIME_CONTEXT_END)));
}
...
function relocateCurrentRuntimeContextCarrierToTail(messages) {
	...
	if (lastIndex < 0 || !messages.some(isOpenClawRuntimeContextCustomMessage)) return messages;
	let firstNonCarrierFromEnd = lastIndex;
	while (firstNonCarrierFromEnd >= 0 && isOpenClawRuntimeContextCustomMessage(messages[firstNonCarrierFromEnd])) firstNonCarrierFromEnd -= 1;
	const rest = messages.filter((message) => !isOpenClawRuntimeContextCustomMessage(message));
	if (!rest.some(isUserMessage)) return messages;
	if (firstNonCarrierFromEnd === rest.length - 1) return messages;
	const carriers = messages.filter(isOpenClawRuntimeContextCustomMessage);
	return [...rest, ...carriers];
}
//#endregion
export { OPENCLAW_RUNTIME_CONTEXT_NOTICE as a, extractInternalRuntimeContext as c, stripHistoricalRuntimeContextCustomMessages as d, stripInternalRuntimeContext as f, OPENCLAW_RUNTIME_CONTEXT_CUSTOM_TYPE as i, hasInternalRuntimeContext as l, INTERNAL_RUNTIME_CONTEXT_END as n, OPENCLAW_RUNTIME_EVENT_HEADER as o, stripRuntimeContextCustomMessages as p, OPENCLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER as r, escapeInternalRuntimeContextDelimiters as s, INTERNAL_RUNTIME_CONTEXT_BEGIN as t, relocateCurrentRuntimeContextCarrierToTail as u };
