...
function buildHttp1EnvHttpProxyAgentOptions(options, timeoutMs) {
	...
	return withHttp1OnlyDispatcherOptions(addIpSafeProxyClientFactory(addUndiciAgentFactory(addActiveManagedProxyTlsOptions(proxyOptions) ?? proxyOptions)), timeoutMs, {
		connect: true,
		proxyTls: true
	});
}
function buildHttp1ProxyAgentOptions(options, timeoutMs) {
	return withHttp1OnlyDispatcherOptions(addIpSafeProxyClientFactory(addUndiciAgentFactory(addActiveManagedProxyTlsOptions({
		proxyTunnel: true,
		...typeof options === "string" || options instanceof URL ? { uri: options.toString() } : { ...options }
	}))), timeoutMs, { proxyTls: true });
}
//#endregion
