...
import { c as redactSensitiveText, u as redactToolPayloadText } from "./redact-DUpJZuMu.js";
...
import { _ as resolvePinnedHostnameWithPolicy, d as isPrivateNetworkAllowedByPolicy, p as matchesHostnameAllowlist } from "./ssrf-C889LYfv.js";
...
import { r as saveJsonFile, t as loadJsonFile } from "./json-store-BVGQY8xv.js";
import "./text-utility-runtime-D52Cj1WO.js";
import { v as DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME } from "./constants-BoWgRtg0.js";
import { i as resolveManagedBrowserHeadlessMode, o as DEFAULT_DOWNLOAD_DIR, t as getManagedBrowserMissingDisplayError } from "./config-Da5ffEih.js";
import { F as CHROME_BOOTSTRAP_PREFS_TIMEOUT_MS, I as CHROME_LAUNCH_READY_WINDOW_MS, J as ensureAbsoluteDirectory, P as CHROME_BOOTSTRAP_EXIT_TIMEOUT_MS, R as CHROME_STDERR_HINT_MAX_CHARS, T as BrowserProfileUnavailableError, W as assertManagedProxyAllowsCdpUrl, a as fetchJson, b as BROWSER_ERROR_REASONS, c as isDirectCdpWebSocketEndpoint, d as openCdpWebSocket, g as withCdpSocket, h as stripCdpUrlCredentials, i as fetchCdpChecked, l as isWebSocketUrl, m as scopeCdpPolicyToConfiguredEndpoint, n as assertCdpEndpointAllowed, q as rawDataToString, t as appendCdpPath, u as normalizeCdpHttpBaseForJsonEndpoints, x as BrowserCdpEndpointBlockedError, z as CHROME_STOP_TIMEOUT_MS } from "./tmp-openclaw-dir-DQI4fBxj.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-B2udiA_j.js";
import { t as createBoundedUtf8Tail } from "./bounded-utf8-tail-Dj_3LqJr.js";
...
function getRoleSnapshotIdentityKeys(refs, mode) {
	return new Set(Object.entries(refs).map(([ref, value]) => mode === "aria" ? ref : `${value.role}\0${value.name ?? ""}\0${value.nth ?? 0}`));
}
...
function annotateRoleSnapshotDelta(params) {
	...
	if (params.previousKeys === void 0) return {
		snapshot: params.snapshot,
		keys
	};
	const keyByRef = new Map(Object.entries(params.refs).map(([ref, value]) => [ref, params.mode === "aria" ? ref : `${value.role}\0${value.name ?? ""}\0${value.nth ?? 0}`]));
	const markedKeys = /* @__PURE__ */ new Set();
	const lines = params.snapshot.split("\n").map((line) => {
		const ref = findSnapshotLineRef(line);
		const key = ref ? keyByRef.get(ref) : void 0;
		if (!key || params.previousKeys?.has(key)) return line;
		markedKeys.add(key);
		return `${line} [new]`;
	});
	const newElements = markedKeys.size;
	if (newElements > 0) lines.push(`${newElements} new element(s) since last snapshot`);
	return {
		snapshot: lines.join("\n"),
		keys,
		newElements
...
function finalizeRoleSnapshot(params) {
	...
	const sourceSnapshot = (params.delta ? annotateRoleSnapshotDelta({
		snapshot: params.snapshot,
		refs: params.refs,
		mode: params.delta.mode,
		previousKeys: params.delta.previousKeys
	}) : void 0)?.snapshot ?? params.snapshot;
	...
	const newElements = params.delta?.previousKeys === void 0 ? void 0 : [...getRoleSnapshotIdentityKeys(refs, params.delta.mode)].filter((key) => !params.delta?.previousKeys?.has(key)).length;
	const result = {
		snapshot,
		refs,
...
async function getMainFrameDocumentIdentityViaCdp(opts) {
	return await withCdpSocket(opts.wsUrl, async (send) => await readCdpMainFrameDocumentIdentity(send), { commandTimeoutMs: opts.timeoutMs ?? 5e3 });
}
...
//#endregion
export { assertBrowserNavigationAllowed as A, finalizeRoleSnapshot as C, INTERACTIVE_ROLES as D, CONTENT_ROLES as E, withBrowserNavigationPolicy as F, readCdpMainFrameDocumentIdentity as I, waitForCdpCommittedNavigationUrl as L, assertBrowserNavigationResultAllowed as M, parseBrowserNavigationUrl as N, STRUCTURAL_ROLES as O, requiresInspectableBrowserNavigationRedirectsForUrl as P, resolveBrowserNavigationProxyMode as R, buildRoleSnapshotFromAriaSnapshot as S, parseRoleRef as T, getMainFrameDocumentIdentityViaCdp as _, isChromeReachable as a, snapshotRoleViaCdp as b, stopOpenClawChrome as c, diagnoseChromeCdp as d, formatChromeCdpDiagnostic as f, formatAriaSnapshot as g, createTargetViaCdp as h, isChromeCdpReady as i, assertBrowserNavigationRedirectChainAllowed as j, InvalidBrowserNavigationUrlError as k, ensureOutputDirectory as l, captureScreenshot as m, getChromeWebSocketUrl as n, launchOpenClawChrome as o, AX_REF_PATTERN as p, isChromeCdpOwnedByPid as r, resolveOpenClawUserDataDir as s, ManagedChromeCleanupError as t, usesOpenClawMockKeychain as u, normalizeCdpWsUrl as v, getRoleSnapshotIdentityKeys as w, buildRoleSnapshotFromAiSnapshot as x, snapshotAria as y };
