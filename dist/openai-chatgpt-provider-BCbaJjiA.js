...
import { d as normalizeProviderId, m as matchesExactOrPrefix, p as cloneFirstTemplateModel } from "./provider-model-shared-D9A9VaW7.js";
import { f as findCatalogTemplate } from "./provider-catalog-shared-BFBHaHNZ.js";
import { a as isOpenAIApiBaseUrl, n as OPENAI_CODEX_RESPONSES_BASE_URL, o as isOpenAICodexBaseUrl } from "./base-url-CTylAPKq.js";
import { t as OPENAI_CODEX_DEFAULT_MODEL } from "./default-models-C6fZfunJ.js";
...
import { n as resolveOpenAIUsageAuth, t as fetchOpenAIUsage } from "./usage-BR4qb8VM.js";
...
const OPENAI_CODEX_GPT_56_NATIVE_CONTEXT_TOKENS = 372e3;
	...
	try {
		const creds = await loginOpenAICodexDeviceCode({
			...
			onVerification: async ({ verificationUrl, userCode, expiresInMs }) => {
				...
				const deviceCodeMessage = [ctx.isRemote ? "Open this URL in your LOCAL browser and enter the code below." : "Open this URL in your browser and enter the code below.", `URL: ${verificationUrl}`].join("\n");
				if (ctx.isRemote) await ctx.openUrl(verificationUrl);
				if (ctx.prompter.deviceCode) await ctx.prompter.deviceCode({
					title: "OpenAI Codex device code",
					code: userCode,
					expiresInMinutes,
					message: deviceCodeMessage
				});
				else await ctx.prompter.note([
					deviceCodeMessage,
					`Code: ${userCode}`,
