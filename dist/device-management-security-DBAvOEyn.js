...
function resolveDeviceSessionAuthz(client) {
	...
	return {
		callerDeviceId: (client?.isDeviceTokenAuth || isDeviceAuthMigrationCaller) && typeof rawCallerDeviceId === "string" && rawCallerDeviceId.trim() ? rawCallerDeviceId.trim() : null,
		callerScopes,
		isAdminCaller: !isDeviceAuthMigrationSession && !isDeviceAuthMigrationCaller && callerScopes.includes("operator.admin"),
...
function deniesCrossDeviceManagement(authz) {
	if (authz.isDeviceAuthMigrationSession && !authz.callerDeviceId) return true;
