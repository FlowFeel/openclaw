...
import { a as asRecord, o as isRecord, r as asOptionalObjectRecord } from "./record-coerce-DHZ4bFlT.js";
...
import { Y as normalizeSqliteNumber } from "./openclaw-state-db-BU55lNCH.js";
import { n as normalizeCronJobInput } from "./normalize-CupkFhHE.js";
import { i as coerceFiniteScheduleNumber } from "./normalize-BHdv6zWT.js";
...
import { t as parseCronPacingBounds } from "./pacing-BxtLSQjv.js";
...
function replaceCronRows(db, storeKey, store) {
	...
	for (const [index, job] of store.jobs.entries()) upsertCronJobRow(db, storeKey, job, index);
...
function loadedCronStoreFromRows(rows) {
	...
	for (const [index, row] of rows.entries()) {
		...
		const configJob = mergeFailureDestinationProjection(asOptionalObjectRecord(safeParseJson(row.job_json)) ?? (job ? stripJobRuntimeFields(job) : {}), job);
		const runtimeEntry = {
		...
		if (!job) {
			invalidConfigRows.push({
				sourceIndex: index,
				reason: getInvalidPersistedCronJobReason(configJob) ?? (scheduleFromRow(row) ? "invalid-payload" : "invalid-schedule"),
				job: configJob,
...
//#endregion
export { projectCronJobThroughStorageCodec as a, upsertCronJobRow as c, normalizeCronJobIdentityFields as d, loadedCronStoreFromRows as i, getCronStoreKysely as l, deleteStaleCronJobFamilyRows as n, replaceCronRows as o, loadCronRows as r, updateCronRuntimeRows as s, assertCronStoreCanPersist as t, cronSchedulingInputsEqual as u };
