...
async function validateLmstudioNonInteractive(ctx) {
	...
	const dockerSetup = [
		...
		"yes",
		"on"
	].includes(process.env.OPENCLAW_DOCKER_SETUP?.trim().toLowerCase() ?? "");
	const baseUrl = resolveLmstudioInferenceBase(configuredBaseUrl || (dockerSetup ? LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL : LMSTUDIO_DEFAULT_INFERENCE_BASE_URL));
	...
	const resolvedApiKey = await ctx.resolveApiKey({
		...
		flagName: providerApiKey === void 0 ? "--custom-api-key" : "--lmstudio-api-key",
		envVar: LMSTUDIO_DEFAULT_API_KEY_ENV_VAR,
		envVarName: LMSTUDIO_DEFAULT_API_KEY_ENV_VAR,
		required: false
	});
	const { fetchLmstudioModels } = await import("../../models.fetch-Co8BnZln.js");
	...
	if (!discovery.reachable) {
		ctx.runtime.error(`LM Studio could not be reached at ${baseUrl}.\nStart LM Studio (or run lms server start) and re-run setup.`);
		ctx.runtime.exit(1);
		return false;
	}
	if (discovery.status !== void 0 && discovery.status >= 400) {
		ctx.runtime.error(`LM Studio returned HTTP ${discovery.status} while listing models at ${baseUrl}.\nCheck the base URL and API key, then re-run setup.`);
		ctx.runtime.exit(1);
		return false;
	}
	const installedModels = discovery.models.filter((model) => model.type === "llm").map((model) => model.key?.trim()).filter((model) => Boolean(model));
	const loadedModels = discovery.models.filter((model) => model.type === "llm" && Boolean(model.key?.trim()) && resolveLoadedContextWindow(model) !== null).map((model) => model.key?.trim()).filter((model) => Boolean(model));
	...
	if (requestedModel && !installedModels.includes(requestedModel)) {
		ctx.runtime.error(`LM Studio model ${requestedModel} was not found at ${baseUrl}.\nAvailable models: ${installedModels.join(", ")}`);
		ctx.runtime.exit(1);
		return false;
	}
	if (requestedModel && !loadedModels.includes(requestedModel)) {
		ctx.runtime.error(`LM Studio model ${requestedModel} is installed but not loaded at ${baseUrl}.\nLoad that model in LM Studio, then re-run setup.`);
		ctx.runtime.exit(1);
		return false;
	}
	if (loadedModels.length === 0) {
		ctx.runtime.error(`No loaded LM Studio LLM models were found at ${baseUrl}.\nLoad a model in LM Studio (or run lms load <model>), then re-run setup.`);
		ctx.runtime.exit(1);
		return false;
	}
	return true;
