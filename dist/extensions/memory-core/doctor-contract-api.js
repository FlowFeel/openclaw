...
import { n as normalizeMemoryHostEventRecordForStorage } from "../../event-store-DHBPY7jU.js";
import { i as resolveMemoryHostEventLogPath } from "../../memory-host-events-TpcF_kI3.js";
...
import { i as normalizeSessionIngestionState, n as normalizeDailyIngestionState } from "../../dreaming-ingestion-state-C81c5Z66.js";
import { C as normalizeShortTermRecallStore, S as normalizeShortTermPhaseSignalStore, b as SHORT_TERM_PHASE_SIGNAL_RELATIVE_PATH, x as SHORT_TERM_STORE_RELATIVE_PATH } from "../../short-term-promotion-BzzPra8m.js";
...
const LEGACY_DAILY_INGESTION_STATE_RELATIVE_PATH = path.join("memory", ".dreams", "daily-ingestion.json");
const LEGACY_SESSION_INGESTION_STATE_RELATIVE_PATH = path.join("memory", ".dreams", "session-ingestion.json");
		...
		const candidates = [
			{
				...
				relativePath: LEGACY_DAILY_INGESTION_STATE_RELATIVE_PATH
			},
			{
				label: "session ingestion",
				relativePath: LEGACY_SESSION_INGESTION_STATE_RELATIVE_PATH
			},
			{
...
const MEMORY_INDEX_META_KEY$1 = "memory_index_meta_v1";
...
function readMemoryIndexMetaVectorDimensions(db, schema, tableName) {
	...
	const meta = db.prepare(`SELECT value FROM ${schema}.${tableName} WHERE key = ?`).get(MEMORY_INDEX_META_KEY$1);
...
function readAgentMemorySearch(config, agentId) {
	...
	const keyedSearch = asRecord(asRecord(asRecord(keyedEntries ? Object.entries(keyedEntries).find(([id]) => normalizeAgentId(id) === agentId)?.[1] : void 0)?.memory)?.search);
	if (keyedSearch) return keyedSearch;
	return asRecord(asRecord((Array.isArray(agents?.list) ? agents.list : []).map(asRecord).find((candidate) => normalizeAgentId(typeof candidate?.id === "string" ? candidate.id : void 0) === agentId)?.memory)?.search);
}
...
async function inspectConfiguredProvider(params) {
	const [{ resolveAgentConfig }, foundation, embeddings, providerState] = await Promise.all([
		...
		import("../../embeddings-Dd_aIIYz.js"),
		import("../../manager-provider-state-VX-0VE2u.js")
	]);
	let settings;
	try {
...
const testing = {
	setInspectConfiguredProviderForTest(inspect) {
...
if (process.env.VITEST || false) globalThis[Symbol.for("openclaw.memoryCoreVectorIndexProviderDiagnosticTestApi")] = testing;
...
function readExistingVectorModel(databasePath) {
	...
	try {
		...
		const model = parsed && typeof parsed === "object" && typeof parsed.model === "string" ? parsed.model.trim() : "";
...
async function collectVectorProviderFindings(params) {
	if (params.config.memory?.backend === "qmd") return [];
	const findings = [];
	for (const agentId of listConfiguredAgentIds(params.config)) {
		...
		if (!model) continue;
		if (hasConfiguredMemorySecretRef(params.config, agentId)) continue;
...
const stateMigrations = [
	...
	qmdLocksStateMigration,
	{
		...
		async migrateLegacyState(params) {
			return {
				changes: [],
				warnings: (await collectVectorProviderFindings(params)).map(formatFinding)
			};
		}
	}
];
