...
import { a as createWebhookInFlightLimiter, i as beginWebhookRequestPipelineOrReject, s as readJsonWebhookBodyOrReject } from "../../webhook-request-guards-cY_xufKf.js";
...
function parseBeamUpload(value) {
	...
	const hookEvent = value.hookEvent === void 0 ? void 0 : optionalString(value.hookEvent, 64);
...
function catalogSessionUrl(beamId, basePath) {
...
function createBeamMirrorRunner(params) {
	...
	const buildUpload = async (catalog, candidate, completed) => {
		const reduced = buildBeamMirrorItems((await catalog.read({
	...
	const tick = async () => {
		...
		try {
			...
			const candidates = [];
			for (const catalog of catalogs) try {
			...
			for (const candidate of selected) {
				...
				try {
					...
					if (await upload(mirror.endpoint, token, payload)) tracked.set(key, {
						candidate,
						fingerprint
					});
				} catch (error) {
