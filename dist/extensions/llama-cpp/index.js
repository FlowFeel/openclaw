...
import { h as createAssistantMessageEventStream, u as parseStreamingJson } from "../../llm-jtdG_sAo.js";
import { d as createPlainTextToolCallCompatWrapper } from "../../provider-stream-shared-DdEotuZs.js";
import "../../error-runtime-Nqb-RQG4.js";
import { t as definePluginEntry } from "../../plugin-entry-DjIG8BVe.js";
import { t as createLocalEmbeddingProvider } from "../../embeddings-BrDLPCH_.js";
import "../../memory-core-host-engine-embeddings-B-1J_oQS.js";
...
function buildLlamaCppProviderConfig(existing) {
	...
	return {
		...existing,
		baseUrl: existing?.baseUrl ?? "local://llama-cpp",
...
async function resolveLlamaCppResponseGrammar(params) {
	...
	if (Object.keys(responseFormat).length === 0) return await params.llama.getGrammarFor("json");
	if (responseFormat.type === "json_object") return await params.llama.getGrammarFor("json");
	if (responseFormat.type === "text") return;
	if (responseFormat.type === "json_schema") {
		const schema = normalizeArguments(normalizeArguments(responseFormat.json_schema).schema);
		return Object.keys(schema).length > 0 ? await params.llama.createGrammarForJsonSchema(schema) : await params.llama.getGrammarFor("json");
	}
	return await params.llama.createGrammarForJsonSchema(responseFormat);
}
...
function resolveContextSize(model, providerConfig) {
	...
	const advertisedCap = typeof model.contextWindow === "number" && model.contextWindow > 0 ? Math.floor(model.contextWindow) : DEFAULT_LLAMA_CPP_CONTEXT_SIZE;
	return { max: typeof model.contextTokens === "number" && model.contextTokens > 0 ? Math.floor(model.contextTokens) : Math.min(advertisedCap, DEFAULT_LLAMA_CPP_CONTEXT_SIZE) };
}
		...
		const run = async () => {
			...
			try {
				...
				const appendThinkingChunk = (chunk) => {
					...
					if (chunk.segmentEndTime) closeThinkingBlock();
				};
				const appendTextDelta = (delta) => {
					...
					streamedContent[activeText.contentIndex] = {
						type: "text",
				...
				const appendFunctionCallParamsChunk = (chunk) => {
					closeThinkingBlock();
					closeTextBlock();
				...
				try {
					const grammar = functions || !options?.responseFormat ? void 0 : await resolveLlamaCppResponseGrammar({
						llama: loaded.llama,
					...
					const result = await chat.generateResponse(mapContextToLlamaChatHistory(context), generationOptions);
					...
					closeThinkingBlock();
					closeTextBlock();
					const toolCalls = (result.metadata.stopReason === "maxTokens" ? [] : result.functionCalls ?? []).map((call, callIndex) => {
						...
						if (!state) throw new Error("llama.cpp native tool call stream state is missing");
						state.toolCall = {
							...state.toolCall,
					...
					const content = streamedContent.filter((block) => block.type !== "toolCall" || confirmedToolCallIds.has(block.id));
					const reason = result.metadata.stopReason === "maxTokens" ? "length" : toolCalls.length > 0 ? "toolUse" : "stop";
...
async function runLlamaCppSetup(ctx) {
	...
	if (!cachedPath || !await isFile(cachedPath)) {
		...
		if (!meetsLlamaCppDefaultModelRamFloor(totalmemBytes)) {
			await ctx.prompter.note(`This Gateway has ${formatRamGb(totalmemBytes)} GB RAM; the recommended model needs 16 GB+. Use Ollama or LM Studio with a smaller model, configure an existing GGUF, or choose a cloud provider.`, "Setup skipped");
			return { profiles: [] };
		}
		if (!await ctx.prompter.confirm({
			message: "OpenClaw will download Gemma 4 E4B IT Q4_K_M (about 5.0 GB) and run it directly inside this Gateway. Continue?",
			initialValue: false
		})) {
	...
	register(api) {
		...
		api.registerProvider({
			...
			createStreamFn: ({ config, model, provider }) => {
				if (model.baseUrl !== "local://llama-cpp") return;
				return createLlamaCppStreamFn({ providerConfig: config?.models?.providers?.[provider] });
			},
