...
import { s as normalizePluginTargetConfig } from "./config-state-DrPZVhOu.js";
...
import { i as legacyModelKey, s as normalizeProviderId } from "./model-ref-shared-BEKlpylb.js";
...
import { a as readCodexCliActiveApiKey, i as readClaudeCliCredentialsCached, o as readCodexCliCredentialsCached, s as readGeminiCliCredentialsCached } from "./external-cli-sync-BpsZiSFL.js";
...
import { a as resolveCliBackendLiveSessionRequirement, i as resolveCliBackendConfig } from "./cli-backends-CEDdjYXx.js";
import { t as areRuntimeModelRefsEquivalent } from "./model-runtime-aliases-D1mCKu2G.js";
import { t as loadAgentRuntimePluginRegistryHandle } from "./runtime-plugins-DeZ9hK27.js";
import { u as normalizeAuthProfileCredential } from "./profiles-Hfbx6aWI.js";
...
import { a as probeLocalCommand } from "./overview-CDBIuwEP.js";
...
import { n as resolveManifestProviderAuthChoice, r as resolveManifestProviderAuthChoices } from "./provider-auth-choices-_vLe6VPC.js";
import { n as loadAuthoredSetupConfig } from "./onboarding-welcome-C6-wAHoi.js";
import { a as resolveSystemAgentVerifiedInferenceRoute, n as createSystemAgentVerifiedInferenceBinding, r as hasCurrentSystemAgentOwnerPluginArtifacts, t as captureSystemAgentOwnerPluginArtifacts } from "./verified-inference-CRg3rEqZ.js";
import { t as CliExecutionAuthProfileError } from "./cli-execution-auth-DCtMEh3g.js";
import { a as sameDefaultInferenceRoute, n as projectInferenceRoute, r as resolveSystemAgentConfiguredRouteFromConfig, t as projectDefaultInferenceRoute } from "./inference-route-Co_fOQeJ.js";
...
import { i as appendSystemAgentAuditEntry } from "./audit-CDmyzGIA.js";
import { i as createSystemAgentModelSelectionUpdater, r as createQuickstartNotePrompter, t as applySystemAgentModelSelection } from "./setup-apply-BFK8i28p.js";
import { a as describeFailoverError } from "./failover-error-DBKQENP4.js";
import { a as runProviderPluginAuthMethodUnpersisted, n as applyProviderPluginAuthMethodResultConfig } from "./provider-auth-choice-CpMO0DVE.js";
...
async function resolveSetupInferenceWorkspace(params) {
	...
	const { DEFAULT_WORKSPACE } = await import("./onboard-helpers-CKVn1bCG.js");
	return {
...
async function cleanupSetupInferenceTempDir(params) {
	try {
		(params.deps.disposeOpenClawAgentDatabaseByPath ?? (await import("./openclaw-agent-db-Eyvs3qpa.js")).disposeOpenClawAgentDatabaseByPath)(path.join(params.tempDir, "agent", "openclaw-agent.sqlite"));
	} catch {
...
async function isCodexInstallRecordPersisted(record, deps) {
	try {
		const currentInstallRecords = await (deps.readPersistedInstalledPluginIndexInstallRecords ?? (await import("./installed-plugin-index-records-BJ1P8R7e.js")).readPersistedInstalledPluginIndexInstallRecords)();
...
async function clearUnownedCodexInstallCaches(deps) {
	try {
		(deps.clearLoadInstalledPluginIndexInstallRecordsCache ?? (await import("./installed-plugin-index-records-BJ1P8R7e.js")).clearLoadInstalledPluginIndexInstallRecordsCache)();
	} catch {
	...
	try {
		await (deps.invalidatePluginRuntimeDiscoveryAfterConfigMutation ?? (await import("./registry-refresh-BJY8rroB.js")).invalidatePluginRuntimeDiscoveryAfterConfigMutation)({ logger: log });
	} catch {
...
async function reloadCodexRegistryAfterActivation(params) {
	...
	try {
		await (params.deps.refreshPluginRegistryAfterConfigMutation ?? (await import("./registry-refresh-BJY8rroB.js")).refreshPluginRegistryAfterConfigMutation)({
			config: sourceConfig,
	...
	try {
		(params.deps.ensurePluginRegistryLoaded ?? (await import("./runtime-registry-loader-DC8VWxk_.js")).ensurePluginRegistryLoaded)({
			scope: "all",
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-BuUcqlQ-.js")).runCliAgent)({
			sessionId,
			sessionKey,
			sessionManager,
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-CZpATf5T.js")).runEmbeddedAgent)({
			sessionId,
			sessionKey,
			sessionManager,
			agentId: effectiveAgentId,
...
async function persistActivatedSetupInference(input) {
	...
	const { stripPendingPluginInstallRecords } = await import("./install-record-commit-CqVY1ClZ.js");
	...
	const transformConfig = deps.transformConfigWithPendingPluginInstalls ?? (await import("./install-record-commit-CqVY1ClZ.js")).transformConfigWithPendingPluginInstalls;
...
async function activateSetupInferenceUnredacted(params, codexCliApiKey) {
	...
	try {
		...
		if (params.kind === "codex-cli") {
			...
			const ensured = await (deps.ensureCodexRuntimePlugin ?? (await import("./codex-runtime-plugin-install-CgPwwJDr.js")).ensureCodexRuntimePluginForModelSelection)({
			...
			const refreshPluginRegistry = deps.refreshPluginRegistryAfterConfigMutation ?? (await import("./registry-refresh-BJY8rroB.js")).refreshPluginRegistryAfterConfigMutation;
...
async function redactSetupInferenceError(message, ...apiKeys) {
	...
	const { redactToolPayloadText } = await import("./redact-C4u7sR5N.js");
