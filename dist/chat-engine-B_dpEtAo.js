...
import { a as parseSystemAgentOperation, i as isPersistentSystemAgentOperation, n as describeSystemAgentPersistentOperation, t as executeSystemAgentOperation } from "./operations-BtNayaH5.js";
import { n as hashSystemAgentOperation } from "./system-agent-tool-DDe482kB.js";
...
import { t as approvalQuestion } from "./dialogue-lOdQ0U-h.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./system-agent-tool-CtEhhyad.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-DDsvZuYF.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-D3IzfMqr.js")).runEmbeddedAgent)({
				...shared,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
				systemAgentTool,
...
async function defaultChannelSetupWizardRunner(channel, prompter, beforePersistentApply) {
	const { createChannelOnboardingPostWriteHookCollector, runCollectedChannelOnboardingPostWriteHooks, setupChannels } = await import("./onboard-channels-CRCe6eBh.js");
...
var SystemAgentChatEngine = class {
	...
	*/
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.opts.planWithAssistant ?? (await import("./assistant-CqNE3uFQ.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-CqNE3uFQ.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
			verifiedInference: this.verifiedInference,
			deps: this.opts.deps
		}));
