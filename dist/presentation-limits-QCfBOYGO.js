...
function fallbackTextBlocks(params) {
	return splitPresentationText(params.text, params.limits).map((text, index) => {
		...
		if (index > 0) Object.defineProperty(block, PRESENTATION_FALLBACK_CONTINUATION, { value: true });
		return block;
	});
}
...
function appendAdaptedButtonsBlock(blocks, block, limits, budget, fallbackBlockType, buttonSelection, textLimits) {
	blocks.push(...adaptButtonsBlock(block, limits, budget, fallbackBlockType, buttonSelection, textLimits));
}
...
function adaptTextBlock(block, limits) {
	if (block.type === "text" || block.type === "context") {
		...
		if (Object.getOwnPropertyDescriptor(block, PRESENTATION_FALLBACK_CONTINUATION)?.value === true) Object.defineProperty(adapted, PRESENTATION_FALLBACK_CONTINUATION, { value: true });
		return adapted;
	}
	return block;
}
...
function adaptMessagePresentationForChannel(params) {
	...
	for (const block of params.presentation.blocks) {
		...
		if (block.type === "buttons") {
			...
			appendAdaptedButtonsBlock(blocks, block, limits?.actions, actionBudget, fallbackBlockType, buttonSelection, limits?.text);
			continue;
		}
