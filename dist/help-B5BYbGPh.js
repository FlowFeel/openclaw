import { L as getCoreCliCommandsWithSubcommands, f as isRootVersionInvocation, n as getCommandPathWithRootOptions, y as getSubCliCommandsWithSubcommands } from "./argv-D6QuZiS5.js";
...
import { t as formatConsoleDiagnosticBlock } from "./json-console-line-CHgBRErc.js";
import { i as tryParseLogLevel, t as ALLOWED_LOG_LEVELS } from "./levels-CLdqNCQ3.js";
...
import { n as formatCliBannerLine, r as hasEmittedCliBanner } from "./banner-DsjE7vkA.js";
import { n as getCommanderErrorCommandPath } from "./commander-parse-facts-BpRwmsnA.js";
...
function resolveHelpCommand(argv, options) {
	if (options?.root) return formatCliCommand("openclaw --help");
...
function formatCliParseErrorOutput(raw, options = {}) {
	...
	if (unknownOption) {
		...
		return lines(theme.error(`OpenClaw does not recognize option ${quote(option)}.`), formatHelpHint(options.argv, { commandPath: options.commandPath }));
	}
	const missingArgument = message.match(/^missing required argument ['"`](.+?)['"`]/i);
	if (missingArgument) {
		const argument = missingArgument[1] ?? "";
		return lines(theme.error(`Missing required argument ${quote(argument)}.`), formatHelpHint(options.argv, { commandPath: options.commandPath }));
	}
	const missingOption = message.match(/^required option ['"`](.+?)['"`] not specified/i);
	if (missingOption) {
		const option = missingOption[1] ?? "";
		return lines(theme.error(`Missing required option ${quote(option)}.`), formatHelpHint(options.argv, { commandPath: options.commandPath }));
	}
	...
	return lines(theme.error(`OpenClaw could not parse this command: ${message}`), formatHelpHint(options.argv, { commandPath: options.commandPath }));
}
//#endregion
...
function configureProgramHelp(program, ctx, options) {
	...
	program.configureOutput({
		...
		outputError: (str, write) => write(formatCliParseErrorOutput(str, {
			argv: process.argv,
