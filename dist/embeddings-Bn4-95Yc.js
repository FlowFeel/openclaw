...
import { r as sanitizeAndNormalizeEmbedding } from "./embeddings-BrDLPCH_.js";
import { T as withRemoteHttpResponse, w as buildRemoteBaseUrlPolicy } from "./memory-core-host-engine-embeddings-DD6bd85Y.js";
...
import { i as resolveGithubCopilotDomain } from "./domain-DI7ThHsj.js";
import { t as CopilotRuntimeAuthError } from "./runtime-auth-error-CQZde1c0.js";
import { n as resolveCopilotRuntimeAuth } from "./runtime-auth-BpgRbyvw.js";
...
async function resolveGitHubCopilotEmbeddingSession(client) {
	const auth = client.runtimeAuth ?? await resolveCopilotRuntimeAuth({
	...
	return {
		baseUrl: client.baseUrl?.trim() || auth.baseUrl || "https://api.individual.githubcopilot.com",
		headers: {
			...COPILOT_HEADERS_STATIC,
			...client.headers,
...
async function createGitHubCopilotEmbeddingProvider(client) {
	...
	const embed = async (input, signal) => {
		...
		return await withRemoteHttpResponse({
			url: `${initialSession.baseUrl.replace(/\/$/, "")}/embeddings`,
			fetchImpl: client.fetchImpl,
