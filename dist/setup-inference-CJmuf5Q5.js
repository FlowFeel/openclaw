...
import { i as createSystemAgentModelSelectionUpdater, r as createQuickstartNotePrompter, t as applySystemAgentModelSelection } from "./setup-apply-Ph1ZGlfT.js";
import { a as describeFailoverError } from "./failover-error-U3al4tnA.js";
import { a as runProviderPluginAuthMethodUnpersisted, n as applyProviderPluginAuthMethodResultConfig } from "./provider-auth-choice-DHTal3VW.js";
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-DDsvZuYF.js")).runCliAgent)({
			sessionId,
			sessionKey,
			sessionManager,
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-D3IzfMqr.js")).runEmbeddedAgent)({
			sessionId,
			sessionKey,
			sessionManager,
			agentId: effectiveAgentId,
