...
async function ensureConfigReady(params, recoveryDeps) {
	...
	const runStateMigrationPreflight = async () => {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-Cl6HPmxw.js")).runDoctorConfigPreflight({
			migrateState: true,
	...
	if (shouldOfferRecovery && !isPluginPackagingFailure) {
		...
		if ((await offerInvalidConfigRecovery({
			...
			retry: async () => {
				configSnapshotPromise = null;
				const { runDoctorConfigPreflight } = await import("./doctor-config-preflight-Cl6HPmxw.js");
