...
import { c as parseAgentSessionKey, f as parseThreadSessionSuffix, i as isCronSessionKey } from "./session-key-utils-02xWdGSz.js";
...
import { ut as beginSessionWorkAdmission } from "./session-entry-slot-keys-DPRQmSpa.js";
import { n as stripTargetKindPrefix, r as stripTargetProviderPrefix } from "./channel-target-prefix-VFVVF5j7.js";
...
import { s as projectOutboundPayloadPlanForMirror, t as createOutboundPayloadPlan } from "./payloads-6G4VluOK.js";
import "./inbound.runtime-4y2wIo8L.js";
import { t as resolveDeliveryTarget } from "./delivery-target-DAbGZLUZ.js";
import { c as resolveCronLifecycleRevisionIdentity, d as pickLastNonEmptyTextFromPayloads, f as pickSummaryFromOutput } from "./run-session-state-Bgn-NKP_.js";
import { t as loadCronSessionEntryLatest } from "./session-BcmdPD5R.js";
import { t as cleanupCronRunSessionAfterRun } from "./session-cleanup-ClTSKZdk.js";
import { _ as waitForCompletedDirectCronDelivery, a as isStaleCronDelivery, c as logCronDeliveryErrorDeferred, d as normalizeDeliveryTarget, f as normalizeSilentReplyText, g as retryTransientDirectCronDelivery, h as resolveCronDeliveryStartDelayMs, i as isCompletedDirectCronDelivery, l as logCronDeliveryWarn, m as resolveCronDeliveryScheduledAtMs, n as buildDirectCronDeliveryIdempotencyKey, o as loadDeliverySubagentRegistryRuntime, p as resolveCronDeliveryBestEffort, r as cleanupDirectCronSession, s as logCronDeliveryError, t as DIRECT_CRON_DELIVERY_COMPLETION_RETENTION, u as maybeApplyTtsToCronPayloads } from "./delivery-dispatch-policy-DYlwnRgY.js";
...
function selectCronRouteCurrentSessionKey(job, agentSessionKey, deliveryProvider, deliveryTarget) {
	...
	const targetPeerId = stripTargetKindPrefix(stripTargetProviderPrefix(deliveryTarget, deliveryProvider));
...
const outboundSessionRuntimeLoader = createLazyImportLoader(() => import("./outbound-session-C8NPQE9y.js"));
const transcriptRuntimeLoader = createLazyImportLoader(() => import("./transcript.runtime.js"));
async function loadDeliveryOutboundRuntime$1() {
	return await deliveryOutboundRuntimeLoader$1.load();
}
...
async function queueCronAwarenessSystemEvent(params) {
	try {
		const { enqueueSystemEvent } = await loadDeliveryOutboundRuntime$1();
...
async function dispatchCronDelivery(params) {
	...
	const deliverViaDirect = async (delivery, options) => {
		...
		try {
			...
			const normalizedSummaryFallback = summaryFallbackText ? normalizeSilentReplyText(summaryFallbackText) : void 0;
			const normalizedSummaryFallbackText = normalizedSummaryFallback?.strippedTrailingSilentToken === true ? void 0 : normalizedSummaryFallback?.text;
			const normalizeDirectPayload = (payload) => {
			...
			const normalizedDeliveryPayloads = deliveryPayloads.map(normalizeDirectPayload).filter((payload) => hasReplyPayloadContent(payload, { trimText: true }));
			const existingFallbackSourceIndex = resolveDirectCronFallbackSourceIndex(normalizedDeliveryPayloads, normalizedSummaryFallbackText);
			const needsFallbackSource = Boolean(normalizedSummaryFallbackText) && normalizedDeliveryPayloads.some(shouldAttachDirectCronFallbackText) && existingFallbackSourceIndex === void 0;
			const fallbackSourceIndex = needsFallbackSource ? 0 : existingFallbackSourceIndex;
			const directPayloads = needsFallbackSource ? [{ text: normalizedSummaryFallbackText }, ...normalizedDeliveryPayloads] : normalizedDeliveryPayloads;
			let normalizedPayloads = [];
			for (const payload of directPayloads) normalizedPayloads.push(shouldAttachDirectCronFallbackText(payload) && normalizedSummaryFallbackText ? {
				...payload,
				fallbackText: {
					text: normalizedSummaryFallbackText,
					...fallbackSourceIndex !== void 0 ? { replacesPayloadIndex: fallbackSourceIndex } : {}
				}
			} : payload);
			if (normalizedPayloads.length === 0 && normalizedSummaryFallbackText) normalizedPayloads = [{ text: normalizedSummaryFallbackText }];
			...
			if (delivered && !deliveryWillReachAwarenessMainSession && !mirrorWouldBypassIsolatedAwarenessPolicy) {
				...
				const transcriptMirror = {
					...
					text: mirrorText,
					mediaUrls: void 0,
					storePath: resolveStorePath(params.cfgWithAgentDefaults.session?.store, { agentId: params.agentId }),
