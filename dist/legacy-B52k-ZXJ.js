...
import { n as DEFAULT_GATEWAY_PORT } from "./paths-CL43LNS6.js";
import { r as normalizeProviderId } from "./provider-id-BIcU_2-A.js";
import { a as readAgentRosterProperty } from "./agent-scope-config-Dusa8eSA.js";
...
import { l as normalizeStaticProviderModelId } from "./model-ref-shared-BCBRWGJh.js";
import { a as ensureRecord$2, c as mergeMissing, i as defineLegacyConfigMigration, o as getRecord, s as mapLegacyAudioTranscription, t as LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS } from "./legacy-config-migrations.runtime.models-iwZrvJ3V.js";
...
import { _ as resolveToolProfilePolicy, c as mergeAlsoAllowPolicy, p as expandToolGroups } from "./tool-policy-CrjVfI-s.js";
import { r as isToolAllowedByPolicyName } from "./tool-policy-match-BJgxicXr.js";
import { a as resolveGatewayPortWithDefault, i as isGatewayNonLoopbackBindMode, r as hasConfiguredControlUiAllowedOrigins, t as buildDefaultControlUiAllowedOrigins } from "./gateway-control-ui-origins-JQOYrYru.js";
import { i as resolveOpenClawMcpTransportAlias, n as isKnownCliMcpTypeAlias, t as canonicalizeConfiguredMcpServer } from "./mcp-config-normalize-UWWF1cpp.js";
//#region src/config/legacy.roster.ts
/** Every missing or empty roster is the shipped implicit-main shape. */
function migratePersistedImplicitMainRoster(raw) {
	...
	if (!rosterProperty || entries && typeof entries === "object" && !Array.isArray(entries) && Object.keys(entries).length === 0) return {
		config: {
			...root,
			agents: {
				...agents,
	...
	const defaultIds = validIds.filter((id) => roster[id].default === true);
	if (defaultIds.length === 1) return convertedLegacyList ? {
		config: {
			...root,
			agents
		},
	...
	const repaired = Object.fromEntries(Object.entries(roster).map(([id, entry]) => {
		if (!entry || typeof entry !== "object" || Array.isArray(entry)) return [id, entry];
		const next = { ...entry };
		if (id === effectiveId) next.default = true;
	...
	return {
		...
		changed: true,
		diagnostics: [...convertedLegacyList ? ["Moved agents.list to keyed agents.entries."] : [], defaultIds.length === 0 ? `Migrated agents.entries by marking "${effectiveId}" as default.` : `Migrated agents.entries by keeping "${effectiveId}" as default and clearing ${defaultIds.length - 1} duplicate marker(s).`]
	};
}
//#endregion
...
function applyLegacyAudioTranscriptionModel(params) {
	...
	const media = ensureRecord$2(ensureRecord$2(params.raw, "tools"), "media");
	const mediaAudio = ensureRecord$2(media, "audio");
...
function migrateRoutingGroupChatMessages(params) {
	const migrateMessageGroupField = (field) => {
		...
		const messagesGroup = ensureRecord$2(ensureRecord$2(params.raw, "messages"), "groupChat");
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_AGENTS = [
	...
	defineLegacyConfigMigration({
		...
		apply: (raw, changes) => {
			...
			if (legacyDefaults || legacyTopLevel) {
				...
				ensureRecord$2(raw, "memory").search = target;
			...
			for (const [index, rawAgent] of agents.list.entries()) {
				...
				const agentMemory = ensureRecord$2(agent, "memory");
	...
	defineLegacyConfigMigration({
		...
		apply: (raw, changes) => {
			...
			const defaults = ensureRecord$2(ensureRecord$2(raw, "agents"), "defaults");
...
function migrateWhatsAppDebounce(raw, changes) {
	...
	const byChannel = ensureRecord$2(ensureRecord$2(ensureRecord$2(raw, "messages"), "inbound"), "byChannel");
...
function migrateWebEnabled(raw, changes) {
	...
	if (Object.hasOwn(web, "enabled")) {
		const whatsapp = ensureRecord$2(ensureRecord$2(raw, "channels"), "whatsapp");
...
function migrateFinalLayoutRenames(raw, changes) {
	...
	if (legacyMedia) {
		mergeMissing(ensureRecord$2(raw, "attachments"), legacyMedia);
		delete raw.media;
		changes.push("Moved media → attachments.");
	}
	const audit = getRecord(raw.audit);
	if (audit) {
		const logging = ensureRecord$2(raw, "logging");
...
function migrateFinalLayoutKills(raw, changes) {
	...
	visitChannelEntries(raw, "whatsapp", (entry, path) => {
		...
		if (!ack) return;
		messages ??= ensureRecord$2(raw, "messages");
...
function migratePluginWebSearchConfig(params) {
	const entry = ensureRecord$1(ensureRecord$1(ensureRecord$1(params.root, "plugins"), "entries"), params.pluginId);
...
//#endregion
export { migrateLegacyXSearchConfig as a, migrateLegacyRuntimeModelRef as c, hasOwnKey as d, migratePersistedImplicitMainRoster as f, stripRetiredTuningKnobs as i, cloneRecord$1 as l, LEGACY_CONFIG_MIGRATIONS as n, legacyRuntimeModelAliasRequiresRuntimePolicy as o, migrateLegacyWebSearchConfig as r, listLegacyRuntimeModelProviderAliases as s, findLegacyConfigIssues as t, ensureRecord$1 as u };
