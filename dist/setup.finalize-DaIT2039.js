...
async function startSessionGatewayForOnboarding(params) {
	...
	try {
		const { startGatewayServer } = await import("./server-CNt1cRa2.js");
...
const loadSearchSetupModule = createLazyRuntimeModule(() => import("./search-setup-BpgHiIq3.js"));
/**
...
async function finalizeSetupWizard(options) {
	...
	try {
		...
		const [{ resolveDefaultModelAuthStatus, resolveDefaultModelCatalogFacts }, { loadPreparedModelCatalogSnapshot }] = await Promise.all([import("./auth-choice-CPnnrHXp.js"), import("./prepared-model-catalog-BddDieHg.js")]);
