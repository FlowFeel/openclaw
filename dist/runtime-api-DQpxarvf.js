...
import { f as resolveDefaultAgentId, i as listAgentIds, o as resolveAgentConfig, u as resolveAgentWorkspaceDir } from "./agent-scope-config-Dusa8eSA.js";
...
import { t as createWorkboardSqliteStores } from "./sqlite-store-BT7Wh2BZ.js";
...
function isWorkboardClaimReclaimable(claim, now) {
	...
	async refreshDiagnostics(now = Date.now()) {
		return await this.enqueueMutation(async () => {
			...
			for (const card of cards) {
				...
				if (!latest || latest.metadata?.archivedAt) continue;
...
function selectStartableCards(cards, limit, candidates, ownerOverride, now) {
	...
	for (const card of cards) {
		const claim = card.metadata?.claim;
		if (!(!isWorkboardClaimReclaimable(claim, now) && (card.status === "running" || card.status !== "done" && cardHasActiveClaim(card, now) || card.execution?.status === "running")) || cardIsArchived(card)) continue;
	...
	for (const card of candidates.filter((entry) => entry.status === "ready" && !cardHasActiveClaim(entry, now) && !cardIsArchived(entry)).toSorted(sortReadyCards)) {
