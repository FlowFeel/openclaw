...
import { h as tryResolveDefaultAgentId, u as resolveAgentWorkspaceDir } from "./agent-scope-config-Dusa8eSA.js";
...
import { o as normalizePluginId } from "./config-state-DrPZVhOu.js";
import { m as resolveOfficialExternalPluginId, u as listOfficialExternalPluginCatalogEntries } from "./official-external-plugin-catalog-CWuAzOfx.js";
...
function collectPluginRegistryState(cfg, env) {
	...
	const registry = loadManifestMetadataSnapshot({
		config: cfg,
		workspaceDir: (defaultAgentId ? resolveAgentWorkspaceDir(cfg, defaultAgentId) : void 0) ?? void 0,
		env: environment
	}).manifestRegistry;
...
function collectDependentChannelConfigHits(cfg, channelIds) {
	...
	for (const { agent, path } of listMutableCodexRouteAgentEntries(cfg)) {
		const target = asObjectRecord(agent.heartbeat)?.target;
