...
import { Y as normalizeSqliteNumber, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, r as closeOpenClawStateDatabase } from "./openclaw-state-db-BU55lNCH.js";
import { u as onAgentEvent } from "./agent-events-COCf-9-O.js";
import { m as runWithGatewayIndependentRootWorkAdmission, o as isGatewayRestartDraining } from "./gateway-work-admission-D_DdbtmL.js";
...
import { d as sessionDeliveryOrigin, s as normalizeDeliveryContext } from "./delivery-context.shared-B-QSuGw_.js";
...
import { h as parseDeliveryContextJson, m as parseTaskNotifyPolicy } from "./task-registry.store.sqlite-7VY-WxbM.js";
import { i as buildAgentRunTerminalOutcomeFromLifecycleEvent, o as classifyAgentRunTerminalOutcome } from "./agent-run-terminal-outcome-9kJP3Yn1.js";
import { i as formatTaskStatusTitleText, o as sanitizeTaskStatusText } from "./task-status-Bo-Cq_rI.js";
import { i as resolveSourceReplyDeliveryMode } from "./source-reply-delivery-mode-DBkd7D2I.js";
import { t as channelSupportsThreadDelivery } from "./thread-addressing-CEiTb1uv.js";
import { i as resetTaskRegistryRuntimeForTests, n as getTaskRegistryObservers, r as getTaskRegistryStore } from "./task-registry.store-D-o1SBrp.js";
...
const deliveryRuntimeLoader = createLazyPromiseLoader(() => import("./task-registry-delivery-runtime-PFOfdcyT.js"), { cacheRejections: true });
...
function taskMatchesRelatedSession(task, sessionKey) {
	if (!sessionKey) return true;
	return [
		task.requesterSessionKey,
		task.childSessionKey,
		task.ownerKey
	].some((candidate) => normalizeOptionalString(candidate) === sessionKey);
}
function taskMatchesAgent(task, agentId) {
	...
	return [
		...
		task.childSessionKey,
		task.ownerKey
	].some((candidate) => parseAgentSessionKey(candidate)?.agentId === agentId);
}
...
function listTaskRecordPage(params) {
	...
	const sessionKey = normalizeOptionalString(params.sessionKey);
	const matching = [...tasks.values()].filter((task) => (!statuses || statuses.has(task.status)) && taskMatchesAgent(task, agentId) && taskMatchesRelatedSession(task, sessionKey)).toSorted((left, right) => {
...
//#endregion
export { listTaskFlowRecords as $, listTasksForRelatedSessionKey as A, isTerminalFlowStatus as B, listFreshTasksForOwnerKey as C, listTasksForAgentId as D, listTaskRecordsUnsorted as E, isProvisionalSubagentKillTask as F, createTaskFlowForTask as G, isTerminalTaskStatus as H, isTaskFlowCancellationPending as I, failFlow as J, deleteTaskFlowRecordById as K, completionRequiresMessageToolDelivery as L, resolveTaskForLookupToken as M, ensureTaskRegistryReady as N, listTasksForFlowId as O, reloadTaskRegistryFromStore as P, getTaskFlowRegistryRestoreFailure as Q, resolveDurableCompletionDeliveryMode as R, hasActiveTaskForChildSessionKey as S, listTaskRecords as T, SUBAGENT_KILL_TASK_ERROR as U, mapAgentRunTerminalOutcomeToTaskStatus as V, createManagedTaskFlow as W, finishFlow as X, findLatestTaskFlowForOwnerKey as Y, getTaskFlowById as Z, maybeDeliverTaskTerminalUpdate as _, linkTaskToFlowById as a, setFlowWaiting as at, findTaskByRunId as b, markTaskTerminalById as c, setTaskRunDeliveryStatusByRunId as d, listTaskFlowsForOwnerKey as et, updateTaskNotifyPolicyById as f, isBackgroundExecTask as g, BACKGROUND_EXEC_TASK_KIND as h, finalizeTaskRunByRunId as i, resumeFlow as it, listTasksForSessionKey as j, listTasksForOwnerKey as k, recordTaskProgressByRunId as l, CRON_TASK_KIND as m, cancelTaskById as n, requestFlowCancel as nt, markTaskLostById as o, updateFlowRecordByIdExpectedRevision as ot, isHarnessOwnedSubagentTask as p, ensureTaskFlowRegistryReady as q, createTaskRecord as r, resolveTaskFlowForLookupToken as rt, markTaskRunningByRunId as s, assertTaskCancellationReadyById as t, reloadTaskFlowRegistryFromStore as tt, setTaskCleanupAfterById as u, publishTaskRecordAfterAtomicStore as v, listTaskRecordPage as w, getTaskById as x, deleteTaskRecordById as y, isParentFlowLinkError as z };
