...
import { c as startSessionDeliveryRuntime, o as schedulePendingSessionDeliveries } from "./subagent-completion-delivery-DFUMUB9K.js";
import { n as resolveHeartbeatIntervalMs } from "./heartbeat-summary-S-VZc34p.js";
import "./embedded-agent-B188BQ_l.js";
import { n as startHeartbeatRunner, o as resolveHeartbeatAgents } from "./heartbeat-runner-Cpvmcnf4.js";
...
function startPendingOutboundDeliveryRecovery(params) {
	...
	const recover = (startup) => {
		...
		const settled = runWithGatewayIndependentRootWorkAdmission(async () => {
			if (stopped) return;
			const { drainPendingDeliveries, recoverPendingDeliveries } = await import("./delivery-queue-6a5XDJP6.js");
			const { deliverOutboundPayloadsInternal } = await import("./deliver-DDZSzJ1V.js");
			if (stopped) return;
...
function startPendingSessionDeliveryRuntime(params) {
	...
	const timer = setTimeout(() => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { deliverQueuedSessionDelivery, recoverPendingRestartContinuationDeliveries, settleQueuedSessionDelivery } = await import("./server-restart-sentinel-n0_gmpg5.js");
			if (stopped) return;
