...
function resolveCommitmentsConfig(_cfg) {
	return {
		enabled: false,
...
async function markCommitmentsStatus(params) {
	...
	return runOpenClawStateWriteTransaction(({ db }) => {
		const commitmentsDb = getNodeSqliteKysely(db);
		const rowsById = new Map(executeSqliteQuerySync(db, commitmentsDb.selectFrom("commitments").selectAll().where("id", "in", ids).where("status", "in", [...ACTIVE_STATUSES])).rows.map((row) => [row.id, row]));
		const updatedIds = [];
		for (const id of ids) {
