import { n as DEFAULT_PREAUTH_HANDSHAKE_TIMEOUT_MS } from "./timeouts-Dbj-IGQf.js";
import { n as computeBackoff, s as sleepWithAbort } from "./src-DKBD8PDy.js";
import "./version-CwNT1gaY.js";
import "./backoff-CCtTkmwj.js";
import { W as WorkerProtocolCloseReasonSchema, h as WorkerAdmissionResponseFrameSchema, j as WorkerTranscriptCommitResponseFrameSchema, w as WorkerLiveEventResponseFrameSchema, y as WorkerHeartbeatResponseFrameSchema, z as WORKER_PROTOCOL_MAX_PAYLOAD_BYTES } from "./worker-admission-cYaTLkLk.js";
import { a as WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES, d as WorkerInferenceStartResponseFrameSchema, h as validateWorkerInferenceTerminalFrame, p as validateWorkerInferenceEventFrame, s as WorkerInferenceCancelResponseFrameSchema } from "./worker-inference-jzapa-pT.js";
...
import { i as isWorkerTranscriptMessageFrameSafe } from "./transcript-message-oguPQDoF.js";
import { n as parseWorkerLaunchDescriptor, t as buildWorkerConnectParams } from "./launch-descriptor--JhiDKGo.js";
...
var WorkerConnectionFrameDispatcher = class {
	...
	request(kind, params, beforeResolve) {
		const id = randomUUID();
		const frame = {
			type: "req",
			id,
	...
	resolvePendingFrame(id, pending, frame) {
		...
		try {
			...
			completed.reject(toError$1(error));
			return true;
		}
	...
	sendRequest(id, frame, value) {
		...
		try {
			...
			return Promise.reject(toError$1(error));
		}
		const payloadLimit = value.kind === "inference-start" ? WORKER_PROTOCOL_MAX_INFERENCE_PAYLOAD_BYTES : WORKER_PROTOCOL_MAX_PAYLOAD_BYTES;
		...
		try {
			...
			this.pending.take(id, pending)?.reject(new WorkerConnectionInterruptedError(toError$1(error).message));
			this.options.interruptReadySocket(readySocket);
		}
...
async function runWorkerDescriptor(descriptor, options = {}) {
	...
	try {
		...
		const [{ runWorkerEmbeddedTurn }, { createWorkerInferenceStreamAdapter }] = await Promise.all([import("./embedded-agent.runtime-eloFMnai.js"), import("./inference-stream.runtime.js")]);
