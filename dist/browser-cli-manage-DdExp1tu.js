...
import { n as info, t as danger } from "./globals-DHQUG86L.js";
...
import { n as formatBrowserGraphicsSummary } from "./routes-BDQes0bG.js";
import "./core-api-Bbh-282r.js";
import { n as callBrowserRequest, s as parseBrowserPositiveIntegerValue, t as BROWSER_TAB_REFERENCE_HELP } from "./browser-cli-shared-CG4QYGp5.js";
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	browser.command("doctor").description("Check browser plugin readiness").option("--deep", "Run a live snapshot probe").action(async (opts, cmd) => {
		...
		await runBrowserCommand(async () => {
			...
			if (!printJsonResult(parent, result)) defaultRuntime.log(result.checks.map(formatDoctorLine).join("\n"));
