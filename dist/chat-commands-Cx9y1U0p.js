...
import { a as isPathInsideWithRealpath } from "./path-D8zNGPJM.js";
...
import { S as resolveEffectiveAgentSkillFilter } from "./agent-scope-DyEposw2.js";
import { o as mergeBundlePathLists, s as normalizeBundlePathList, t as CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH } from "./bundle-manifest-BVamVGYB.js";
import { c as normalizePluginsConfig, r as hasExplicitPluginConfig, u as resolveEffectivePluginActivationState } from "./config-state-DrPZVhOu.js";
import { n as loadPluginManifestRegistryForPluginRegistry } from "./plugin-registry-contributions-B0602V_s.js";
...
import { i as parseFrontmatterBool } from "./frontmatter-CRq-4JMr.js";
import { n as resolveSkillTelemetrySource } from "./source-9Jdpd6BI.js";
import { et as filterUserInvocableSkillEntries, tt as isSkillPromptVisible } from "./store-wPr8539E.js";
import { t as getChatCommands } from "./commands-registry.data-CxG6KLOu.js";
import { a as loadVisibleWorkspaceSkillEntries, r as filterWorkspaceSkillEntriesWithOptions } from "./workspace-D7s0sLcF.js";
...
function hasSkillReferenceCandidate(text) {
	for (const match of skillReferenceMatches(text)) {
		...
		if (name && index !== void 0 && !isEscapedReference(text, index) && !isShellVariableReference(name)) return true;
	}
	return false;
}
...
function resolveSkillReferenceInvocations(params) {
	...
	for (const match of skillReferenceMatches(params.text)) {
		const name = match[1]?.replace(/:+$/gu, "");
		const index = match.index;
		if (!name || index === void 0 || isEscapedReference(params.text, index) || isShellVariableReference(name)) continue;
		const command = findSkillCommand(params.skillCommands, name);
		if (!command || command.modelVisible === false || seen.has(command.name)) continue;
...
//#endregion
export { resolveSkillCommandInvocation as a, listReservedChatSlashCommandNames as i, listSkillCommandsForWorkspace as n, resolveSkillReferenceInvocations as o, hasSkillReferenceCandidate as r, listSkillCommandsForAgents as t };
