...
import { ot as loadExactSqliteSessionEntryReadOnly } from "./session-accessor.sqlite-B9iW7DOt.js";
import "./session-accessor-t3qUoTeV.js";
import { s as listAmbientGroupWatchTargets } from "./session-state-events-MWtkoPhW.js";
...
import { s as resolveSandboxSessionToolsVisibility } from "./session-visibility-uYeEbYS1.js";
...
function prepareWatchedSessionsPrompt(params) {
	...
	const sessions = targets.slice(0, WATCHED_SESSIONS_PROMPT_LIMIT).map((key) => {
		const row = { key };
		const entry = loadExactSqliteSessionEntryReadOnly({
			sessionKey: key,
			clone: false
		})?.entry;
