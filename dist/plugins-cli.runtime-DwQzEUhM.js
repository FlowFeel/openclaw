...
import { m as shortenHomeInString } from "./utils-Bs67j6-3.js";
import { r as defaultRuntime } from "./runtime-DOr96aVu.js";
import { l as readConfigFileSnapshot, r as getRuntimeConfig } from "./io-BsQc3Kgy.js";
import { r as theme } from "./theme-vjDs9tao.js";
import { n as emitDiagnosticsTimelineEvent } from "./diagnostics-timeline-BxwfDTJ-.js";
...
import { a as formatMissingPluginMessage } from "./error-format-BZ5G9eas.js";
import { t as withPluginLifecycleLease } from "./plugin-lifecycle-lease-Cmmxy1R1.js";
import { a as resolveConfiguredRuntimePluginInstallCandidate, i as collectConfiguredRuntimePluginIds } from "./configured-runtime-plugin-installs-Du-iScG6.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-CfQlhTJT.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-ZNqyvmhV.js"));
const loadPluginSlotSelection = createModuleLoader(() => import("./slot-selection-DfIfsw2U.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-YL5xBbPf.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./registry-refresh-BJY8rroB.js"));
...
async function runPluginsEnableCommandUnlocked(idInput) {
	let id = idInput;
	assertConfigWriteAllowedInCurrentMode();
	const { enableExplicitlySelectedPluginInConfig } = await import("./enable-C6R09f8p.js");
...
async function runPluginsDisableCommandUnlocked(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-BkDIkaq_.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-NZW6wM30.js");
...
async function runPluginMarketplaceEntriesCommand(opts) {
	const catalog = await import("./official-external-plugin-catalog-Ds4TaBIj.js");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceRefreshCommand(opts) {
	const { loadConfiguredHostedOfficialExternalPluginCatalogEntries } = await import("./official-external-plugin-catalog-Ds4TaBIj.js");
	const cfg = getRuntimeConfig();
	...
	const { clearManagedPluginOfficialCatalogCache } = await import("./management-service-Dm9rmdZW.js");
	clearManagedPluginOfficialCatalogCache();
	let gatewayRefreshed = true;
	if (result.source !== "bundled-fallback") {
		const { notifyGatewayPluginMetadataChanged } = await import("./plugins-update-gateway-signal-ONHid77L.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-LV-1Lv3X.js");
