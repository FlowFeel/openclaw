...
import { r as flushLogger } from "./logger-Cv9z4NYi.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-DRDfhKk-.js";
import { t as createSubsystemLogger } from "./subsystem-Ess1Ww-N.js";
import { x as findOpenClawStateDatabaseSchemaMigrationRequiredError } from "./openclaw-state-db-D9eH245j.js";
import { c as hasConfiguredSecretInput } from "./types.secrets-BvApkFoj.js";
import { p as ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV } from "./config-env-vars-Dh6msnxE.js";
import { r as CONFIG_AUDIT_STORE_LABEL } from "./io.audit-CRLY6_C7.js";
import { t as clearRuntimeConfigSnapshot } from "./runtime-snapshot-Bzqj8IgJ.js";
import { i as isInvalidConfigError, r as isDoctorRecoverableInvalidConfigError } from "./io.invalid-config-Chuupwwv.js";
import "./globals-Cw62Mq_M.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-bSCpyKdU.js";
...
import { n as formatGatewayPidList, t as findVerifiedGatewayListenerPidsOnPortSync } from "./gateway-processes-CwKuXMZo.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-wEbhk9Tt.js";
import { r as withProgress } from "./progress-BjGUBuxw.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-86BGsxSJ.js";
import { n as isGatewayRunFutureConfigAllowed, t as enforceGatewayRunFutureConfigGuard } from "./future-config-guard-Dot7uJp9.js";
import { a as inspectGatewayCrashLoopBreaker, i as formatGatewayCrashLoopManualChannelStartHint, n as GATEWAY_CRASH_LOOP_RECOVERED_REASON, o as recordGatewayBootStart, r as completeGatewayBootLifecycle, s as recordGatewayCrashLoopRecovery, t as GATEWAY_CRASH_LOOP_BREAKER_REASON } from "./gateway-boot-lifecycle-aj9sYWV-.js";
...
async function resolveGatewayRunShellEnvFallbackPlan(cfg) {
	const { createConfigRuntimeEnv } = await import("./env-vars-DF5fp442.js");
	const { resolveShellEnvFallbackTimeoutMs, shouldDeferShellEnvFallback, shouldEnableShellEnvFallback } = await import("./shell-env-DtnVbC-V.js");
	...
	const { resolveShellEnvExpectedKeys } = await import("./shell-env-expected-keys-CCiO4RqS.js");
	return {
		enabled: true,
...
async function loadGatewayRunShellEnvFallback(plan) {
	const { loadShellEnvFallback } = await import("./shell-env-DtnVbC-V.js");
...
async function clearGatewayRunShellEnvFallback(values) {
	...
	const { clearShellEnvAppliedKeys } = await import("./shell-env-DtnVbC-V.js");
...
function createConfiguredGatewayHealthProbe(cfg) {
	...
	return async (params) => {
		...
		if (!tlsFingerprint) tlsFingerprint = (await import("./gateway-mxpwATm2.js").then(({ loadGatewayTlsRuntime }) => loadGatewayTlsRuntime({
			...tlsConfig,
			autoGenerate: false
...
async function maybeWriteGatewayStartupFailureBundle(err, reason = "gateway.startup_failed") {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-_DrXf5UB.js");
...
async function runGatewayCommandOnce(opts, hooks = {}) {
	...
	const { clearGatewayRunConfigEnvironment } = await import("./pre-bootstrap-DMSxPlaw.js");
	...
	const serverImportMeasurement = startupTrace.startMeasure("cli.server-import", () => import("./server-Bg4NfXeN.js"));
	...
	if (devMode) {
		if (opts.reset) {
			const { recheckGatewayRunReset } = await import("./pre-bootstrap-DMSxPlaw.js");
			if (!await recheckGatewayRunReset({
				opts,
				runtime: defaultRuntime
			})) return;
		}
		const { ensureDevGatewayConfig } = await import("./dev-CtL01CJr.js");
	...
	if (snapshot) {
		const { applyFinalGatewayRunConfigEnv } = await import("./pre-bootstrap-DMSxPlaw.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-BumMllRK.js");
		...
		if (stale.length > 0) {
			...
			if (process.platform === "linux") {
				const { findSystemdGatewayInstallation, formatDuelingScopesWarning } = await import("./systemd-D1WJK-gC.js");
	...
	if (opts.force) {
		...
		try {
			const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-BfMEaMfi.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-vDp73-aL.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DznTuKti.js");
		...
		if (findOpenClawAgentDatabaseMediaMigrationRequiredError(err)) try {
			const { parkCurrentLaunchAgentForMaintenance } = await import("./launchd-C-0BDrM8.js");
...
async function runGatewayCommand(opts, hooks = {}, recoveryDeps) {
	try {
		...
		const { offerInvalidConfigRecovery } = await import("./invalid-config-recovery-lr6QVnUD.js");
