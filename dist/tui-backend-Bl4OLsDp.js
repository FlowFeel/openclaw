...
import { x as runTui } from "./tui-vZgq2V2i.js";
...
import { t as executeSystemAgentOperation } from "./operations-BtNayaH5.js";
...
import { i as assertLocalGatewaySetupMode, n as SystemAgentChatEngine, t as GATEWAY_SETUP_AFTER_WRITE } from "./chat-engine-B_dpEtAo.js";
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	if (handoff.target === "search") {
		...
		const [{ runSearchSetupFlow }, { createClackPrompter }, { readSetupConfigFileSnapshot, writeWizardConfigFile }] = await Promise.all([
			import("./search-setup-B0GaBaJC.js"),
			import("./clack-prompter-DyhRpF9c.js"),
			import("./setup.shared-BQDwhL8U.js")
		]);
		const snapshot = await readSetupConfigFileSnapshot();
	...
	await (opts.runChannelsAdd ?? (await import("./add-DFJJuDsb.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
		beforePersistentEffect
	});
}
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (route) {
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-BddDieHg.js"), import("./model-thinking-default-B14nlDbu.js")]);
