...
import { o as normalizePluginProviderBaseUrl, r as loadPluginMetadataSnapshot } from "./plugin-metadata-snapshot-DtW_P3kZ.js";
...
let fallbackProviderMetadataOwnersMemo;
function clearFallbackProviderMetadataOwnersMemo() {
	fallbackProviderMetadataOwnersMemo = void 0;
}
registerPluginMetadataProcessMemoLifecycleClear(clearFallbackProviderMetadataOwnersMemo);
function resolveFallbackProviderMetadataOwners() {
	if (fallbackProviderMetadataOwnersMemo) return fallbackProviderMetadataOwnersMemo;
	...
	fallbackProviderMetadataOwnersMemo = {
		...
		providerRequests: fallback.providerRequests ?? /* @__PURE__ */ new Map()
	};
...
function resolveProviderMetadataOwners(prepared) {
	if (prepared) return {
		...
		providerRequests: prepared.providerRequests ?? /* @__PURE__ */ new Map()
	};
	const current = getCurrentPluginMetadataSnapshot({ allowWorkspaceScopedSnapshot: true });
	if (current) return {
		providerEndpoints: current.owners?.providerEndpoints ?? [],
		providerRequests: current.owners?.providerRequests ?? /* @__PURE__ */ new Map()
	};
...
function resolveManifestProviderEndpoint(params) {
	for (const endpoint of resolveProviderMetadataOwners(params.providerMetadataOwners).providerEndpoints) {
		...
		if (params.normalizedBaseUrl && (endpoint.baseUrls ?? []).includes(params.normalizedBaseUrl)) return buildManifestEndpointResolution(endpoint, params.host);
	}
}
