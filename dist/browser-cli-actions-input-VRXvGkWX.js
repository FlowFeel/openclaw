...
import { c as resolveBrowserActExecutionBudgetMs, n as ACT_MAX_VIEWPORT_DIMENSION } from "./act-policy-9AEfR1NQ.js";
import { l as resolveExistingUploadPaths } from "./config-B2k8sZ1m.js";
import { c as normalizeBrowserFormFieldValue, s as normalizeBrowserFormField } from "./snapshot-urls-XEHa-0UE.js";
import "./core-api-DlefJ9Ou.js";
import { i as parseBrowserNonNegativeIntegerOption, n as callBrowserRequest, o as parseBrowserPositiveIntegerOption, s as parseBrowserPositiveIntegerValue, t as BROWSER_TAB_REFERENCE_HELP } from "./browser-cli-shared-D4JKvg3l.js";
import { t as runBrowserResizeWithOutput } from "./browser-cli-resize-t_rOs_8W.js";
...
const ACTIONS_STDIN_MAX_BYTES = 1e6;
/** Reads stdin to a UTF-8 string, throwing once the byte cap is exceeded. */
async function readStdinText(stream = process.stdin, maxBytes = ACTIONS_STDIN_MAX_BYTES) {
	const chunks = [];
	let total = 0;
	for await (const chunk of stream) {
		const buf = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
		total += buf.length;
		if (total > maxBytes) throw new Error(`actions stdin exceeds ${maxBytes} bytes.`);
		chunks.push(buf);
	}
...
async function readActionsPayload(opts) {
	...
	if (opts.actionsFile) return opts.actionsFile === "-" ? await readStdinText() : await readFile$1(opts.actionsFile);
	return opts.actions ?? "";
}
//#endregion
	...
	browser.command("batch").description("Run a batch of browser actions in one call (default: stop on first error)").option("--actions <json>", "JSON array of act requests").option("--actions-file <path>", "Read JSON array from a file (- for stdin)").option("--continue", "Continue through all actions instead of stopping on first error").option("--target-id <id>", BROWSER_TAB_REFERENCE_HELP).action(async (opts, cmd) => {
		...
		try {
			...
			result = await callBrowserAct({
				...
				profile,
				body,
				timeoutMs: resolveBrowserActExecutionBudgetMs(body)
			});
		} catch (err) {
