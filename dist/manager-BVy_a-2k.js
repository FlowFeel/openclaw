...
import { r as formatErrorMessage, s as readErrorName } from "./errors-Cg_yT1Sv.js";
import { i as resolveGlobalSingleton } from "./global-singleton-Dc_stLtU.js";
import { c as resolveAgentDir, o as resolveAgentConfig, u as resolveAgentWorkspaceDir } from "./agent-scope-config-Dusa8eSA.js";
...
import { F as ensureMemoryIndexSchema, G as ensureMemoryPathFtsTriggers, H as MEMORY_INDEX_PATHS_FTS_TABLE, I as MEMORY_EMBEDDING_CACHE_TABLE, J as MEMORY_INDEX_CHUNK_PROVENANCE_TABLE, K as MEMORY_INDEX_CHUNK_RECALL_METADATA_TABLE, L as MEMORY_INDEX_META_TABLE, V as MEMORY_INDEX_FTS_TABLE, W as dropMemoryPathFtsTriggers, Y as ensureMemoryChunkProvenance, q as ensureMemoryRecallMetadataSchema, x as ensureOpenClawAgentDatabaseSchema, z as MEMORY_INDEX_VECTOR_TABLE } from "./openclaw-agent-db-DemdumbZ.js";
import { n as onInternalSessionTranscriptUpdate } from "./transcript-events-BG9Ai61T.js";
import { _ as stripMemoryAnnotationCarriers, a as chunkMarkdown, c as extractProjectKeysFromCuratedEntry, d as normalizeExtraMemoryPaths, h as runMemoryHostTasksWithConcurrency, i as buildMultimodalChunkForIndexing, m as remapChunkLines, o as cosineSimilarity, p as parseEmbedding, r as buildFileEntry, s as ensureMemoryHostDir, t as INVALID_PROJECT_ANNOTATION_KEY, u as listMemoryFiles, v as hasNonTextEmbeddingParts } from "./internal-ClJ0MPdp.js";
import { c as resolveMemorySearchConfig, f as classifyMemoryMultimodalPath } from "./config-utils-CIfwi7ve.js";
import { r as listRegisteredMemoryEmbeddingProviderAdapters } from "./memory-embedding-provider-runtime-LDa5o1VF.js";
...
import { n as readMemoryFile } from "./read-file-AKvEQ6_3.js";
import { t as loadSqliteVecExtension } from "./sqlite-vec-U_Koo0d5.js";
import { a as readCuratedProjectMemoryCandidates, i as readCuratedMemoryTriggerCandidates, n as configureMemorySqliteWalMaintenance, o as readMemoryRecallMetadata, t as closeMemorySqliteWalMaintenance } from "./engine-storage-Bj1FSteu.js";
...
import { A as isEmbeddingBatchUnavailableError, u as enforceEmbeddingMaxInputTokens } from "./memory-core-host-engine-embeddings-DD6bd85Y.js";
import "./memory-core-host-engine-foundation-C9wj4OCo.js";
import { a as buildSessionEntry, d as statSessionEntrySync, f as listSessionTranscriptCorpusEntriesForAgent, l as sessionPathForFile, u as sessionPathForSessionIdentity } from "./engine-qmd-Ci26YE_-.js";
import "./memory-core-host-engine-qmd-2OgG7BxA.js";
import "./memory-core-host-engine-storage-Bxl8QSwD.js";
import { x as readMemoryCoreWorkspaceEntry } from "./dreaming-state-CswYxS-h.js";
import { r as tokenize, t as jaccardSimilarity } from "./tokenize-eIBzdr_p.js";
import { i as resolveEmbeddingProviderFallbackModel, n as resolveEmbeddingProviderAdapterId, o as resolveEmbeddingProviderIndexIdentity, r as resolveEmbeddingProviderAdapterTransport, t as createEmbeddingProvider } from "./embeddings-ssmg81f8.js";
import { a as resolveMemoryFallbackProviderRequest, i as resolveFallbackCurrentProviderId, n as createDegradedMemoryProviderLifecycle, o as resolveMemoryPrimaryProviderRequest, r as createPendingMemoryProviderLifecycle, s as resolveMemoryProviderState, t as applyMemoryFallbackProviderState } from "./manager-provider-state-BtzgkWaM.js";
...
function mmrRerank(items, config = {}) {
	...
	while (remaining.size > 0) {
		...
		if (bestItem) {
			...
			for (const candidate of remaining) {
				const similarity = jaccardSimilarity(tokenCache.get(candidate.id) ?? tokenize(candidate.content), selectedTokens);
...
async function mergeHybridResults(params) {
	...
	const rankable = applyImportanceMultiplier(await applyTemporalDecayToHybridResults({
...
function resolveMemoryIndexIdentityState(params) {
	...
	if (meta.chunkingVersion !== 2) return {
		status: "mismatched",
...
function requiresMemoryVectorRebuild(params) {
	...
	if (!vectorTableExists(params.db, params.vectorTable)) return Boolean(params.metaVectorDims && params.hasSemanticChunks);
...
var MemoryManagerSyncBase = class {
	...
	dropVectorTable() {
		try {
			...
			const message = formatErrorMessage(err);
			log$7.debug(`Failed to drop ${VECTOR_TABLE$2}: ${message}`);
			return false;
		}
	}
...
var MemoryManagerWatchOps = class extends MemoryManagerSyncBase {
	...
	warnIfMemoryWatchPressure(count, unit) {
		warnIfMemoryWatchPressureHigh(this.memoryWatchPressureWarning, count, unit, "Large memory folders or extraPaths can make OpenClaw run out of file watchers or open files.", "Remove large extraPaths, or set memory.search.sync.watch to false and refresh memory manually.", (message) => log$6.warn(message));
	}
...
var MemoryManagerSessionSyncOps = class extends MemoryManagerWatchOps {
	...
	async runSessionStartupCatchup() {
		...
		if (dirtyFiles.length === 0 && !this.sessionsFullRetryDirty || this.closed) return dirtyFiles;
	...
	scheduleSessionDirty(target) {
		...
		this.sessionWatchTimer = setTimeout(() => {
			...
			this.processSessionUpdateBatch().catch((err) => {
				log$5.warn(`memory session update failed: ${String(err)}`);
			});
	...
	async processSessionUpdateBatch() {
		...
		if (pending.length > 0) {
			...
			this.sync({ reason: "session-delta" }).catch((err) => {
				log$5.warn(`memory sync failed (session update): ${String(err)}`);
			});
		}
	}
...
var MemoryManagerSyncOps = class extends MemoryManagerSourceSyncOps {
	...
	async runSync(params) {
		...
		const needsChunkingVersionReindex = meta !== null && meta.chunkingVersion !== 2 && !hasTargetArchiveFiles;
...
var MemoryManagerEmbeddingOps = class extends MemoryManagerSyncOps {
		...
		try {
			return await this.withProviderUse(provider, async () => await runMemoryEmbeddingBatchRetryWithSplit({
				...
				run: async (batchItems) => {
					...
					log$2.debug(`memory embeddings: ${label} start`, {
						provider: provider.id,
					...
					if (!structured) log$2.debug("memory embeddings: batch completed", {
						provider: provider.id,
						items: batchItems.length
					});
					return result;
				...
				onSplit: ({ itemCount, splitAt }) => {
					log$2.warn(`memory embeddings transport failed after retries; splitting ${label} of ${itemCount} into ${splitAt} + ${itemCount - splitAt}`);
				}
			}));
		} catch (err) {
			if (!structured) log$2.debug("memory embeddings: batch failed", {
				provider: provider.id,
	...
	*/
	...
	async prepareIndexEntry(entry, options, generation) {
		...
		const baseChunks = filterNonEmptyMemoryChunks(chunkMarkdown(options.source === "memory" ? stripMemoryAnnotationCarriers(content) : content, {
...
const INDEX_SCOPE_CLOSES = resolveGlobalSingleton(MEMORY_INDEX_MANAGER_SCOPE_CLOSES_KEY, () => /* @__PURE__ */ new Map());
const INDEX_GLOBAL_LIFECYCLE = resolveGlobalSingleton(MEMORY_INDEX_MANAGER_GLOBAL_LIFECYCLE_KEY, () => ({
	closePromise: null,
	closeFailed: false
}));
async function runMemoryIndexManagerGlobalClose(operation) {
	const closePromise = (INDEX_GLOBAL_LIFECYCLE.closePromise ?? Promise.resolve()).then(operation, operation);
	INDEX_GLOBAL_LIFECYCLE.closePromise = closePromise;
	await closePromise;
	if (INDEX_GLOBAL_LIFECYCLE.closePromise === closePromise) INDEX_GLOBAL_LIFECYCLE.closePromise = null;
}
async function closeAllMemoryIndexManagersUnlocked() {
	const scopedCloses = Array.from(INDEX_SCOPE_CLOSES.values());
	if (scopedCloses.length > 0) await Promise.allSettled(scopedCloses);
	...
	for (const [key, manager] of entries) try {
		await manager.close();
		if (INDEX_CACHE.get(key) === manager) INDEX_CACHE.delete(key);
	} catch (err) {
...
async function runMemoryIndexManagerScopeOperation(params, operation) {
	while (INDEX_GLOBAL_LIFECYCLE.closePromise) {
		const globalClose = INDEX_GLOBAL_LIFECYCLE.closePromise;
		try {
			await globalClose;
		} catch {
			if (INDEX_GLOBAL_LIFECYCLE.closePromise === globalClose) await closeAllMemoryIndexManagers();
		}
	}
	const scopeKey = resolveMemoryIndexManagerScopeKey(params);
	const result = (INDEX_SCOPE_CLOSES.get(scopeKey) ?? Promise.resolve()).then(operation, operation);
	const tail = result.then(() => void 0, () => void 0);
	INDEX_SCOPE_CLOSES.set(scopeKey, tail);
	try {
		return await result;
	} finally {
		if (INDEX_SCOPE_CLOSES.get(scopeKey) === tail) INDEX_SCOPE_CLOSES.delete(scopeKey);
	}
}
...
var MemoryIndexManager = class MemoryIndexManager extends MemoryManagerEmbeddingOps {
	...
	static async get(params) {
		...
		return await runMemoryIndexManagerScopeOperation({
			agentId,
			purpose: params.purpose === "status" || params.purpose === "cli" ? params.purpose : "default"
		}, async () => {
			if (INDEX_GLOBAL_LIFECYCLE.closeFailed) try {
				await closeAllMemoryIndexManagersUnlocked();
				INDEX_GLOBAL_LIFECYCLE.closeFailed = false;
			} catch (err) {
				INDEX_GLOBAL_LIFECYCLE.closeFailed = true;
			...
			return await MemoryIndexManager.getWithinGlobalLifecycle({
				...params,
				agentId
			});
		});
	}
	static async getWithinGlobalLifecycle(params) {
		...
		if (transient) return await getOrCreate();
	...
	markLocalEmbeddingProviderDegraded(err) {
		...
		const workerFailure = isLocalEmbeddingWorkerFailure(err) ? err : err instanceof Error && isLocalEmbeddingWorkerFailure(err.cause) ? err.cause : null;
	...
	retireCurrentProvider() {
		...
		const retirement = this.providerRetirementPromise.catch(() => {}).then(async () => {
			...
			if (closeFailed) throw toLintErrorObject(firstError, "Embedding provider retirement failed");
		});
	...
	async search(query, opts) {
		...
		const ranked = applyProjectRanking(await this.searchUnranked(normalizedQuery, {
			...opts,
			maxResults: candidateMaxResults,
			minScore: candidateMinScore
		}), opts?.activeProjectKeys);
		return hasActiveProject ? ranked.filter((entry) => entry.score >= minScore).slice(0, maxResults) : ranked;
	}
	async searchUnranked(normalizedQuery, opts) {
		return await this.withManagerOperation(async () => {
			...
			if (!hasIndexedContent) {
				try {
					...
					} else log.warn(`memory sync failed (search-bootstrap): ${String(err)}`);
				}
			...
			if (!hybrid.enabled || !this.fts.enabled || !this.fts.available) return applyImportanceMultiplier(await applyTemporalDecayToHybridResults({
				results: vectorResults,
				temporalDecay: hybrid.temporalDecay,
				workspaceDir: this.workspaceDir
			})).toSorted((left, right) => right.score - left.score || left.path.localeCompare(right.path) || left.startLine - right.startLine || left.endLine - right.endLine).filter((entry) => entry.score >= minScore).slice(0, maxResults);
	...
	async finalizeKeywordOnlyResults(params) {
		...
		const ranked = this.rankKeywordOnlyResults(applyImportanceMultiplier(decayed), !appliesTemporalDecay);
	...
	async retryFailedClose() {
		...
		if (this.providersPendingRetirement.size > 0) throw toLintErrorObject(retirementErrors.at(-1), "Embedding provider retirement failed");
...
//#endregion
export { resolveMemoryCoreLocalServiceHostIdentity as a, formatMemoryVectorDegradedWriteReason as i, closeAllMemoryIndexManagers as n, closeMemoryIndexManagersForAgent as r, MemoryIndexManager as t };
