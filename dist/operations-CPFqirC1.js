...
import { a as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-Co_fOQeJ.js";
import { i as appendSystemAgentAuditEntry, r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-CDmyzGIA.js";
import { n as t } from "./i18n-BYpJa9f7.js";
import { t as createAgent } from "./agent-create-D_QgN2Cx.js";
import { r as isOpenClawTrustedPluginInstallSpec } from "./install-provenance-B5aa9rpU.js";
...
const loadOverviewModule$1 = async () => await import("./overview-xnwfin79.js");
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { requestSafeGatewayRestart } = await import("./restart-coordinator-BoguI5bJ.js");
		return requestSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-BHMyeOzI.js");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-CwPJJ-zX.js")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-DnMqkCc3.js")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-BRqpbkl-.js")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-DnMqkCc3.js");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-4O0UC6FC.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-4O0UC6FC.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-tFlXBKZs.js")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-Cb8CAJV5.js")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-tFlXBKZs.js");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-DyY3jhMN.js");
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			const runPluginInstall = ctx.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-D0oogcd3.js");
...
const loadOverviewModule = async () => await import("./overview-xnwfin79.js");
	...
	switch (operation.kind) {
		...
		case "plugin-list":
			await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
				const { runPluginsListCommand } = await import("./plugins-list-command-CyIR_C4P.js");
				await runPluginsListCommand({}, pluginRuntime);
			}))(runtime);
			return { applied: false };
		case "plugin-search":
			await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
				const { runPluginsSearchCommand } = await import("./plugins-search-command-WMZGEg6R.js");
		...
		case "config-schema": {
			const { buildConfigSchema, lookupConfigSchema } = await import("./schema-C1BvuKWE.js");
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-C2cHrmFP.js");
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-Coz98aPj.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-BqXweVRg.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-CL7s_fVu.js")).runTui)({
				local: true,
				session,
				deliver: false,
				historyLimit: 200,
