...
import { n as getCachedPluginModuleLoader, s as clearNativeRequireJavaScriptModuleCache, t as createPluginModuleLoaderCache } from "./plugin-module-loader-cache-17a12qxQ.js";
...
import { a as resolveDiscoveredProviderPluginIds } from "./providers-CqF04KXu.js";
...
function resolveManifestModelCatalogProviders(pluginRecords, config) {
	const providers = [];
...
function resolveRuntimeManifestCatalogPluginIds(pluginRecords, config) {
	...
	for (const plugin of pluginRecords) {
		...
		if (!plugin.modelCatalog?.providers) continue;
		if (planEffectiveModelCatalogRows({
			registry: { plugins: [plugin] },
			config
		}).entries.some((entry) => entry.discovery === "runtime" || entry.discovery === "refreshable")) pluginIds.add(plugin.id);
	}
	return pluginIds;
}
function resolveProviderDiscoveryEntryPlugins(params) {
	...
	const runtimeManifestCatalogPluginIds = resolveRuntimeManifestCatalogPluginIds(pluginRecords, config);
	...
	const manifestProviders = params.includeManifestModelCatalogProviders === false ? [] : resolveManifestModelCatalogProviders(pluginRecords, config);
...
function resolvePluginDiscoveryProvidersRuntime(params) {
	...
	const entryProviders = entryResult.providers.filter((provider) => hasProviderCatalogHook(provider) || params.includeSyntheticAuthProviders === true && typeof provider.resolveSyntheticAuth === "function");
