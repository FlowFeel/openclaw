...
import { N as validateConfigObjectWithPlugins, f as readConfigFileSnapshotWithPluginMetadata, l as readConfigFileSnapshot, tt as resolveConfigSnapshotHash } from "./io-BsQc3Kgy.js";
...
import { a as sameDefaultInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-Co_fOQeJ.js";
...
async function applySystemAgentSetup(params, hooks) {
	...
	const [{ readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults }, onboardHelpers, { applyLocalSetupWorkspaceConfig, resolveOnboardingWorkspaceConflict }, { transformConfigWithPendingPluginInstalls }] = await Promise.all([
		...
		import("./install-record-commit-CqVY1ClZ.js")
	]);
	...
	const guardModules = expectedAgentId || expectedAgentDir || expectedModelRef ? await Promise.all([import("./agent-scope-Jqp3H3Rj.js"), import("./model-selection-Cmjka01x.js")]) : void 0;
	...
	await runCommittedFollowUp(async () => {
		const { updateExecApprovals } = await import("./exec-approvals-k5vaGEp6.js");
	...
	if (surface === "cli") await runCommittedFollowUp(async () => {
		const { ensureGatewayServiceForOnboarding } = await import("./setup.finalize-DUvH4s--.js");
		...
		if (gateway.status === "failed") lines.push(`Gateway service: ${gateway.error}`);
			...
			const probe = await onboardHelpers.waitForGatewayReachable({
				...
				password: settings.authMode === "password" ? await (await import("./setup.secret-input-CZ1VKMMl.js")).resolveSetupSecretInputString({
					config: nextConfig,
