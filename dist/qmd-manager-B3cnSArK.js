...
import { i as asOptionalRecord, n as asNullableRecord } from "./record-coerce-DHZ4bFlT.js";
...
import { o as statRegularFile } from "./regular-file-jv7y-frB.js";
...
import { _ as isSessionArchiveArtifactName } from "./paths-DSnYpBD3.js";
import { t as PluginStateLeaseError } from "./plugin-state-lease.types-C0g0-ID5.js";
import { l as resolveMemorySearchSyncConfig } from "./config-utils-bVWMjALM.js";
import { t as isFileMissingError } from "./fs-utils-DF79bccZ.js";
import { a as buildMemoryReadResult, o as buildMemoryReadResultFromSlice } from "./read-file-DH_MTOy_.js";
...
import "./session-transcript-hit-B4G2oMcG.js";
import "./memory-core-host-engine-foundation-DCzOSZiP.js";
import { a as buildSessionEntry, f as listSessionTranscriptCorpusEntriesForAgent, i as runCliCommand, n as resolveCliSpawnInvocation } from "./engine-qmd-B5ZbsK-2.js";
import { i as parseQmdQueryJson, n as deriveQmdScopeChatType, r as isQmdScopeAllowed, t as deriveQmdScopeChannel } from "./memory-core-host-engine-qmd-CbVN1h9Q.js";
import "./memory-core-host-engine-storage-PRsCOUxC.js";
import { g as memoryCoreWorkspaceEntryKey, y as openMemoryCoreStateStore } from "./dreaming-state-CswYxS-h.js";
import "./dreaming-shared-8l3nTpoP.js";
import { a as replaceQmdSessionArtifactMappings, i as refreshQmdSessionArtifactDocIds, n as copyQmdSessionArtifactHit, o as resolveQmdSessionArtifactIdentity, t as attachQmdSessionArtifactHit } from "./qmd-session-artifacts-B7KCYtXT.js";
import { i as warnIfMemoryWatchPressureHigh, n as settleMemoryWatchEventPaths, r as countChokidarWatchedEntries, t as recordMemoryWatchEventPath } from "./watch-settle-b89mVCCb.js";
...
function parseFailedQmdSearchJson(err, command) {
	...
	try {
		...
		log$4.warn(`qmd ${command} exited non-zero after producing valid JSON; using captured search results (${formatQmdSearchExit(err)})`);
		return parsed;
	} catch {
		return null;
	}
...
const QMD_RUNTIME_CACHE_MAX_ENTRIES = 1e3;
...
function buildCollectionConfigHash(collections) {
	return hashText(collections.map((collection) => ({
...
function resolveQmdRuntimeCache(workspaceDir, namespace, keyHash) {
	return {
		key: memoryCoreWorkspaceEntryKey(workspaceDir, `${namespace}:${keyHash}`),
		store: openMemoryCoreStateStore({
			namespace,
			maxEntries: QMD_RUNTIME_CACHE_MAX_ENTRIES
		})
	};
}
...
function normalizeCacheEntryEnvelope(value, nowMs, expectedKeyHash) {
	...
	if (!record || record.version !== QMD_RUNTIME_CACHE_ENTRY_VERSION) return;
...
function normalizeCollectionValidationEntry(value, nowMs, expectedKeyHash) {
	...
	if (!validation || validation.ok !== true || typeof validation.collectionConfigHash !== "string" || typeof validation.collectionCount !== "number") return;
	return {
...
function normalizeMultiCollectionProbeEntry(value, nowMs, expectedKeyHash) {
	...
	if (!probe || typeof probe.supported !== "boolean") return;
	return {
		version: QMD_RUNTIME_CACHE_ENTRY_VERSION,
		createdAtMs,
		expiresAtMs,
		keyHash,
		multiCollectionProbe: { supported: probe.supported }
	};
}
async function readQmdRuntimeCache(params) {
	try {
		const { store, key } = resolveQmdRuntimeCache(params.workspaceDir, params.namespace, params.keyHash);
...
async function writeQmdRuntimeCache(params) {
	try {
		const { store, key } = resolveQmdRuntimeCache(params.workspaceDir, params.namespace, params.keyHash);
...
async function writeQmdCollectionValidationCache(params, nowMs = Date.now()) {
	return await writeQmdRuntimeCache({
		workspaceDir: params.workspaceDir,
		namespace: QMD_RUNTIME_CACHE_COLLECTION_VALIDATION_NAMESPACE,
		keyHash: buildQmdCollectionValidationCacheContextHash(params),
		nowMs,
...
async function clearQmdMultiCollectionProbeCache(params) {
	try {
		const { store, key } = resolveQmdRuntimeCache(params.workspaceDir, QMD_RUNTIME_CACHE_MULTI_COLLECTION_PROBE_NAMESPACE, buildQmdMultiCollectionProbeCacheContextHash(params));
...
var QmdCollectionController = class {
		...
		for (const collection of this.qmd.collections) {
			...
			if (listed) try {
				...
				if (!isQmdCollectionMissingError(message)) {
					...
					log$3.warn(`qmd collection remove failed for ${collection.name}: ${message}`);
				}
			}
			try {
				...
				if (isQmdCollectionAlreadyExistsError(message)) {
					if (await this.tryRebindSameNameCollection({
						...
						validationComplete = false;
						log$3.warn(`qmd collection add skipped for ${collection.name}: ${message}`);
					}
					continue;
				}
				validationComplete = false;
				log$3.warn(`qmd collection add failed for ${collection.name}: ${message}`);
			}
		}
		throwIfAborted(signal);
		assertLeaseActive(lease);
	...
	async tryRepairNullByteCollections(err, reason, lease) {
		...
		log$3.warn(`qmd update failed with suspected null-byte collection metadata (${reason}); rebuilding managed collections and retrying once`);
	...
	async tryRepairDuplicateDocumentConstraint(err, reason, lease) {
		...
		log$3.warn(`qmd update failed with duplicate document constraint (${reason}); rebuilding managed collections and retrying once`);
	...
	async ensureCollectionPathsBestEffort(lease) {
		...
		for (const collection of this.qmd.collections) try {
			...
			assertLeaseActive(lease);
			log$3.warn(`qmd collection path prepare failed for ${collection.name}: ${formatErrorMessage(err)}`);
		}
	}
	async tryRebindSameNameCollection(params) {
		...
		log$3.warn(`qmd collection add conflict for ${collection.name}: collection name already exists; recreating managed collection`);
		try {
			...
			if (!isQmdCollectionMissingError(removeMessage)) {
				log$3.warn(`qmd collection remove failed for ${collection.name}: ${removeMessage}`);
				return false;
			}
		}
		try {
			...
			log$3.warn(`qmd collection add failed for ${collection.name} after recreating same-name collection: ${retryMessage} (initial: ${addErrorMessage})`);
			return false;
		}
	}
	...
	async tryRebindConflictingCollection(params) {
		...
		log$3.warn(`qmd collection add conflict for ${collection.name}: path+pattern already bound by ${conflictName}; rebinding`);
		try {
			...
			assertLeaseActive(params.lease);
			const removeMessage = formatErrorMessage(removeErr);
			if (!isQmdCollectionMissingError(removeMessage)) log$3.warn(`qmd collection remove failed for ${conflictName}: ${removeMessage}`);
			return false;
		}
		try {
			await this.addCollection(collection.path, collection.name, collection.pattern, params.lease);
			...
			assertLeaseActive(params.lease);
			const retryMessage = formatErrorMessage(retryErr);
			log$3.warn(`qmd collection add failed for ${collection.name} after rebinding ${conflictName}: ${retryMessage} (initial: ${addErrorMessage})`);
			return false;
		}
	}
	async migrateLegacyUnscopedCollections(existing, lease) {
		for (const collection of this.qmd.collections) {
			...
			if (!canMigrateLegacyQmdCollection({
				...
				log$3.debug(`qmd legacy collection migration skipped for ${legacyName} (path/pattern mismatch)`);
				continue;
			}
			try {
				...
				assertLeaseActive(lease);
				const message = formatErrorMessage(err);
				if (!isQmdCollectionMissingError(message)) log$3.warn(`qmd collection remove failed for ${legacyName}: ${message}`);
			}
		}
	}
	...
	async addCollection(pathArg, name, pattern, lease) {
		...
		for (const flag of candidateFlags) try {
			...
			log$3.warn(`qmd collection add rejected ${flag}; retrying with legacy compatibility flag`);
		}
	...
	async rebuildManagedCollectionsForRepair(reason, lease) {
		...
		try {
			...
			assertLeaseActive(lease);
			log$3.warn(`qmd managed collection index refresh failed for update repair (${reason}): ${formatErrorMessage(configErr)}`);
		}
		for (const collection of this.qmd.collections) {
			try {
				await this.removeCollection(collection.name, lease);
				...
				assertLeaseActive(lease);
				const removeMessage = formatErrorMessage(removeErr);
				if (!isQmdCollectionMissingError(removeMessage)) log$3.warn(`qmd collection remove failed for ${collection.name}: ${removeMessage}`);
			}
			try {
				await this.addCollection(collection.path, collection.name, collection.pattern, lease);
			} catch (addErr) {
				...
				if (!isQmdCollectionAlreadyExistsError(addMessage)) log$3.warn(`qmd collection add failed for ${collection.name}: ${addMessage}`);
			}
		}
		throwIfAborted(signal);
		assertLeaseActive(lease);
		log$3.warn(`qmd managed collections rebuilt for update repair (${reason})`);
	}
};
//#endregion
...
var QmdCommandClient = class {
	...
	markQmdV1Fallback() {
		if (this.qmdMcpToolVersion !== "v1") {
			...
			log$2.warn("QMD MCP server does not expose the v2 'query' tool; falling back to v1 tool names (search/vector_search/deep_search).");
		}
	}
	async ensureMcporterDaemonStarted(mcporter) {
		...
		if (!mcporter.startDaemon) {
			if (!state.coldStartWarned) {
				...
				log$2.warn("mcporter qmd bridge enabled but startDaemon=false; each query may cold-start QMD MCP. Consider setting memory.qmd.mcporter.startDaemon=true to keep it warm.");
			}
			return;
		}
		if (!state.daemonStart) state.daemonStart = (async () => {
			try {
				...
				log$2.warn(`mcporter daemon start failed: ${String(err)}`);
...
var QmdDocumentResolver = class {
	...
	async resolveDocLocation(docid, hints) {
		...
		try {
			...
			rows = db.prepare("SELECT collection, path, modified_at FROM documents WHERE hash = ? AND active = 1").all(normalized);
			if (rows.length === 0) rows = db.prepare("SELECT collection, path, modified_at FROM documents WHERE hash LIKE ? AND active = 1").all(`${normalized}%`);
		} catch (err) {
			if (isSqliteBusyError(err)) {
				log$1.debug(`qmd index is busy while resolving doc path: ${String(err)}`);
	...
	resolveIndexedDocLocationFromHint(collection, preferredFile) {
		...
		try {
			const db = this.ensureDb();
			const exactRows = db.prepare("SELECT path, modified_at FROM documents WHERE collection = ? AND path = ? AND active = 1").all(trimmedCollection, exactPath);
			...
			rows = db.prepare("SELECT path, modified_at FROM documents WHERE collection = ? AND active = 1").all(trimmedCollection);
		} catch (err) {
			if (isSqliteBusyError(err)) {
				log$1.debug(`qmd index is busy while resolving hinted path: ${String(err)}`);
				throw createQmdBusyError(err);
			}
			log$1.debug(`qmd index hint lookup skipped: ${String(err)}`);
			return null;
		}
	...
	toDocLocation(collection, collectionRelativePath, modifiedAt) {
	...
	isIndexedWorkspaceReadPath(absPath) {
		...
		for (const [collection, rootValue] of this.collectionRoots.entries()) {
			...
			try {
				...
				if (isSqliteBusyError(err)) {
					log$1.debug(`qmd index is busy while checking read path: ${String(err)}`);
					throw createQmdBusyError(err);
				}
				log$1.debug(`qmd indexed read-path lookup skipped: ${String(err)}`);
			}
		}
		return false;
	}
};
function parseQmdModifiedAt(value) {
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-helpers.ts
...
function isEmbedBackoffActive(embedBackoffUntil) {
	...
	async exportSessions(lease) {
		...
		for (const corpusEntry of corpusEntries) {
			...
			if (revisionToken && state?.revisionToken === revisionToken && state.targetRevision !== null && targetRevision === state.targetRevision) {
				...
				artifactMappings.push(this.buildSessionArtifactMapping(sessionFile, targetName, target, corpusEntry));
				keep.add(target);
				continue;
			}
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-base.ts
const qmdManagerLog = createSubsystemLogger("memory");
var QmdManagerBase = class {
	constructor(params) {
		...
		this.sessionExporter = sessionExporterConfig ? new QmdSessionExporter(sessionExporterConfig, this.agentId, this.workspaceDir, this.indexPath, (collection, collectionRelativePath, workspaceRelativePath, absolutePath) => this.documentResolver.buildSearchPath(collection, collectionRelativePath, workspaceRelativePath, absolutePath)) : null;
	...
	async initialize(mode) {
		...
		if (mode === "cli") {
			...
			qmdManagerLog.info(`qmd manager initialized for agent "${this.agentId}" mode=cli collections=${this.qmd.collections.length} durationMs=${Date.now() - startTime}`);
			return;
		}
		this.ensureWatcher();
		qmdManagerLog.info(`qmd manager initialized for agent "${this.agentId}" mode=full collections=${this.qmd.collections.length} durationMs=${Date.now() - startTime}`);
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-sync.ts
var QmdManagerSync = class extends QmdManagerBase {
	async sync(params) {
		if (params?.sessions?.some((session) => session.sessionId.trim().length > 0) || params?.archiveFiles?.some((sessionFile) => sessionFile.trim().length > 0)) qmdManagerLog.debug("qmd sync ignoring targeted session hint; running regular update");
	...
	async runUpdate(reason, force, opts) {
		...
		const run = async () => {
			...
			qmdManagerLog.debug(`qmd sync started for agent "${this.agentId}" reason=${reason} force=${force === true}`);
			try {
			...
			qmdManagerLog.info(`qmd sync completed for agent "${this.agentId}" reason=${reason} durationMs=${Date.now() - startTime}`);
		};
	...
	async runQmdUpdateWithRetry(reason, lease) {
		...
		await retryAsync(() => this.runQmdUpdateOnce(reason, lease), {
			attempts: reason === "boot" || reason.startsWith("boot:") ? 3 : 1,
			minDelayMs: 500,
			maxDelayMs: 1e3,
			shouldRetry: (err) => {
				if (!this.isRetryableUpdateError(err)) return false;
				this.throwIfAborted(signal);
				return true;
			},
			onRetry: ({ attempt, maxAttempts, err }) => {
				qmdManagerLog.warn(`qmd update retry ${attempt}/${maxAttempts - 1} after failure (${reason}): ${String(err)}`);
			},
			sleep: async (delayMs) => {
				try {
	...
	isRetryableUpdateError(err) {
		if (isSqliteBusyError(err)) return true;
	...
	shouldRunEmbed(force) {
		...
		const now = Date.now();
		if (isEmbedBackoffActive(this.embedBackoffUntil)) return false;
	...
	noteEmbedFailure(reason, err) {
		...
		qmdManagerLog.warn(`qmd embed failed (${reason}): ${String(err)}; backing off for ${Math.ceil(delayMs / 1e3)}s`);
	}
	...
	shouldSkipUpdate(force) {
		...
		if (debounceMs <= 0 || !this.lastUpdateAt) return false;
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-lifecycle.ts
var QmdManagerLifecycle = class extends QmdManagerSync {
	constructor(..._args) {
		super(..._args);
	...
	ensureWatcher() {
		...
		const startTime = Date.now();
		qmdManagerLog.info(`qmd watcher starting for agent "${this.agentId}" paths=${watchPathList.length}`);
		...
		watcher.once("ready", () => {
			...
			qmdManagerLog.info(`qmd watcher ready for agent "${this.agentId}" paths=${watchPathList.length} durationMs=${Date.now() - startTime}`);
		});
	}
	warnIfWatchPressure(count) {
		warnIfMemoryWatchPressureHigh(this.watchPressureWarning, count, "paths", "Large QMD collections can make OpenClaw run out of file watchers or open files.", "Remove large collections, or set memory.search.sync.watch to false and refresh memory manually.", (message) => qmdManagerLog.warn(message));
	}
	...
	async maybeWarmSession(sessionKey) {
		if (this.mode === "cli" || !this.syncSettings?.onSessionStart) return;
	...
	async maybeSyncDirtySearchState() {
		if (this.mode === "cli" || !this.syncSettings?.onSearch || !this.dirty) return;
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-search-support.ts
const SNIPPET_HEADER_RE = /@@\s*-([0-9]+),([0-9]+)/;
var QmdManagerSearchSupport = class extends QmdManagerLifecycle {
	...
	async tryRepairMissingCollectionSearch(err, debugContext, parentSignal) {
		if (!isMissingCollectionSearchError(err)) return false;
		qmdManagerLog.warn("qmd search failed because a managed collection is missing; repairing collections and retrying once");
		await this.ensureCollections({
			force: true,
			debugContext,
			parentSignal
	...
	logScopeDenied(sessionKey) {
		...
		qmdManagerLog.warn(`qmd search denied by scope (channel=${channel}, chatType=${chatType}, session=${key})`);
	}
	...
	diversifyResultsBySource(results, limit) {
		...
		if (!bySource.has("sessions") || !bySource.has("memory")) return results.slice(0, target);
	...
	async runQueryAcrossCollectionGroups(query, limit, collectionGroups, command, signal, reportCommandPhase) {
		qmdManagerLog.debug(`qmd ${command} multi-source collection grouping active (${collectionGroups.length} groups)`);
	...
	buildQmdResultKey(entry) {
		...
		if (!hints.preferredCollection || !hints.preferredFile) return null;
		const collectionRelativePath = this.toCollectionRelativePath(hints.preferredCollection, hints.preferredFile);
		return collectionRelativePath ? `file:${hints.preferredCollection}:${collectionRelativePath}` : null;
	}
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-search.ts
function resolveQmdSearchProvenance(resultPath, source, observedAt) {
	const normalizedPath = resultPath.replaceAll("\\", "/").toLowerCase();
	const isSystemArtifact = normalizedPath === "dreams.md" || normalizedPath.startsWith("memory/dreaming/") || normalizedPath.startsWith("memory/.dreams/");
	return {
		originClass: source === "sessions" ? "untrusted" : isSystemArtifact ? "system" : normalizedPath === "memory.md" ? "agent" : "untrusted",
		sessionKind: "unknown",
		observedAt
	};
}
var QmdManagerSearch = class extends QmdManagerSearchSupport {
	async search(query, opts) {
		...
		const runSearchAttempt = async (allowMissingCollectionRepair) => {
			...
			try {
				...
				if (allowMissingCollectionRepair && isMissingCollectionSearchError(err)) throw err;
				if (!mcporterEnabled && qmdSearchCommand !== "query" && this.isUnsupportedQmdOptionError(err)) {
					...
					qmdManagerLog.warn(`qmd ${qmdSearchCommand} does not support configured flags; retrying search with qmd query`);
					try {
						const collectionGroups = await this.resolveCollectionSearchGroups(collectionNames, searchSignal, debugContext);
						this.recordSearchPlanDebug({
							debugContext,
...
//#endregion
//#region extensions/memory-core/src/memory/qmd-manager-io.ts
var QmdManagerIo = class extends QmdManagerSearch {
	async readFile(params) {
		...
		if (params.from !== void 0 || params.lines !== void 0) {
			...
			const requestedCount = normalizePositiveInteger(params.lines ?? 120, 120);
	...
	async probeVectorAvailability() {
		...
		try {
			const result = await this.runQmd(["status"], { timeoutMs: this.qmd.limits.timeoutMs });
...
var QmdMemoryManager = class QmdMemoryManager extends QmdManagerIo {
