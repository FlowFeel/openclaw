...
import { S as requireActivePluginRegistry, _ as getPluginRegistrationContext, s as getActivePluginGatewayCommandRegistry } from "./runtime-yJAYArQt.js";
...
const getCommandRegistry = () => getActivePluginGatewayCommandRegistry() ?? requireActivePluginRegistry();
const getPluginCommandMap = () => new Map(getCommandRegistry().commands.map((entry) => [`/${normalizeOptionalLowercaseString(entry.command.name) ?? ""}`, {
	...entry.command,
...
function registerPluginCommand(pluginId, command, opts) {
	...
	return registerPluginCommandInRegistry(context?.registry ?? getActivePluginGatewayCommandRegistry() ?? requireActivePluginRegistry(), context?.pluginId ?? pluginId, command, opts);
}
function registerPluginCommandInRegistry(registry, pluginId, command, opts) {
	if (registry.commandRegistryLocked) return {
		ok: false,
...
//#endregion
export { registerPluginCommandInRegistry as a, isTrustedReservedCommandOwner as c, setPluginCommandRegistryLocked as d, isOpenClawMainPromptSurface as f, registerPluginCommand as i, listRegisteredPluginAgentPromptGuidance as l, listPluginInvocationKeys as n, canExposeSenderIsOwner as o, pluginCommandSupportsChannel as r, clearPluginCommands as s, isReservedCommandName as t, pluginCommands as u };
