...
import { f as stripInternalRuntimeContext, t as INTERNAL_RUNTIME_CONTEXT_BEGIN } from "./internal-runtime-context-s8_TwHnQ.js";
...
function noteSessionActivityEvent(state, event, noteMaxChars = DEFAULT_NOTE_MAX_CHARS) {
	...
	switch (event.stream) {
		...
		case "command_output": {
			...
			addActivityNote(state, `${title}: ${readString(data.status) ?? (exitCode === 0 ? "completed" : "failed")}${exitCode === void 0 ? "" : ` (exit ${exitCode})`}`, noteMaxChars);
			return;
		}
		case "item": {
		...
		case "approval":
			...
			addActivityNote(state, `Waiting for approval: ${readString(data.title) ?? "user action"}`, noteMaxChars);
			break;
		default: break;
	}
}
...
//#endregion
export { terminalHealthFor as a, readFiniteNumber as i, flushSessionActivityAssistantNote as n, noteSessionActivityEvent as r, createSessionActivityNoteState as t };
