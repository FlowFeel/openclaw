...
import { i as getPluginModuleLoaderStats } from "./plugin-module-loader-cache--sRKvOp1.js";
import { t as STARTUP_UNAVAILABLE_GATEWAY_METHODS } from "./core-descriptors-Bngx0C2m.js";
import { i as getActiveGatewayRootWorkCount, m as runWithGatewayIndependentRootWorkAdmission } from "./gateway-work-admission-BaRJo64l.js";
import { v as sweepSessionStateWatchNotices } from "./session-state-events-COmXZrBv.js";
import { s as hasRestartSentinel } from "./restart-sentinel-C6_-wnpH.js";
import { t as scheduleGatewayIdleTask } from "./server-idle-task-BfjT2Jhx.js";
import { n as measureStartup } from "./server-startup-trace-1bubQfP_.js";
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-CrZXFXYx.js";
import { t as hasConfiguredInternalHooks } from "./configured-zsl7sWPM.js";
...
function scheduleContextCachePrewarm(params) {
	...
	const warm = async () => {
		if (stopped) return;
		const { ensureContextWindowCacheLoaded } = await import("./context-sS_05YkG.js");
...
async function prewarmGatewaySessionListData(cfg, agentId) {
	const [{ loadCombinedSessionStoreForGateway }, { listSessionsFromStoreAsync }] = await Promise.all([import("./combined-store-gateway-DYe_D072.js"), import("./session-utils-list-rnJ4bVRP.js")]);
...
function dashboardDataPrewarmItems(cfg, log) {
	...
	const shouldPrewarmSessionData = async () => {
		...
		const { canPrewarmCombinedSessionStoresForGateway } = await import("./combined-store-gateway-DYe_D072.js");
	...
	return [
		...
		})),
		{
			name: "plugins",
			load: async () => {
				const { listManagedPlugins } = await import("./management-service-Dm9rmdZW.js");
		...
		...agentIds.map((agentId) => ({
			...
			load: async () => {
				if (!await shouldPrewarmSessionData()) return;
				const { prewarmSessionCatalogList } = await import("./session-catalog-BNB8db38.js");
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-DMkyfztr.js"));
const loadMainSessionRestartRecoveryMarkingModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-marking-DtRHzWAq.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RjT9WtG0.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-Cmjka01x.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-kUoZlj2L.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-Bc6wMe2f.js"));
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { startGatewayMemoryBackend } = await import("./server-startup-memory-kmXq0E39.js");
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	runWithGatewayIndependentRootWorkAdmission(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-doy790HC.js");
		...
		idleTask = scheduleGatewayIdleTask({
			...
			run: async () => {
				await measureStartup(params.startupTrace, "post-ready.agent-runtime-plugins", async () => {
					...
					const { loadAgentRuntimePluginRegistryHandle } = await import("./runtime-plugins-zSs3_w_Q.js");
					const cfg = params.getConfig();
					if (isStopped()) return;
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-UvRNHfnu.js");
			if (isStopped()) return;
...
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-DYh047Jv.js");
...
async function publishConfiguredModelRuntimeSnapshots(params) {
	const { refreshPreparedModelRuntimeSnapshots } = await import("./prepared-model-runtime-BV4Woxz5.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-7QzVpkfQ.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-BDw2p4U8.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-CWlcMbNP.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadPreparedModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				...
				import("./prepared-model-catalog-CDZ2A_-7.js"),
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-Dc1XxOW0.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-Bfork5QQ.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	warmSystemCa: warmMacOSSystemCaOffMainThread,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-CjZaNdlR.js")).startGatewayTailscaleExposure(...args)
};
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	const controlUiAssetsSidecar = !params.minimalTestGateway && controlUiRootLifecycle?.state?.kind === "preparing" ? schedulePostReadySidecarTask({
	...
	const sidecarsPromise = params.minimalTestGateway ? Promise.resolve({
		...
		try {
			const { scheduleSubagentRegistrySweep } = await import("./subagent-registry-BH3v6WwW.js");
		...
		gatewayLifetimeSidecars.push(scheduleAgentRuntimePluginPrewarm({
			getConfig: params.providerAuthPrewarm?.getConfig ?? (() => params.gatewayPluginConfigAtStart),
			workspaceDir: params.defaultWorkspaceDir,
			startupTrace: params.startupTrace,
			log: params.log,
			waitForPostReadyWork: params.waitForPostReadyWork
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-BLFZBh42.js");
			runWithGatewayIndependentRootWorkAdmission(async () => {
