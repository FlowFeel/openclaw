...
async function ensureSelectedAgentHarnessPlugin(params) {
	...
	if (!params.pluginRegistry?.agentHarnesses.some((entry) => entry.harness.id === runtime)) throw new Error(`Agent harness runtime "${runtime}" is not present in the prepared registry.`);
}
//#endregion
