...
import { o as isLegacyCodexProviderId } from "./codex-route-model-ref-BDwklqCa.js";
import { E as parseSqliteSessionEntryRecord, _ as runOpenClawAgentWriteTransaction, f as isOpenClawAgentDatabaseOpen, h as openOpenClawAgentDatabase, i as closeOpenClawAgentDatabaseByPath } from "./openclaw-agent-db-DemdumbZ.js";
import { n as deliveryContextFromSession, u as sessionDeliveryChannel } from "./delivery-context.shared-DR6KpKlV.js";
import { r as resolveAgentMainSessionKey } from "./main-session-Bjm_i_Af.js";
import { i as resolveStoredSessionKeyForAgentStore, n as resolveSessionStoreAgentId } from "./session-store-key-DmGCpash.js";
...
import { J as listSqliteSessionGenerationIdsForCanonicalRepair, K as copySqliteSessionOwnedStateForCanonicalRepair, L as applySqliteSessionEntryLifecycleMutation, Tt as serializeJsonlLines, X as rehomeSqliteSessionDeliveryReferencesForCanonicalRepairBatch, Y as rehomeSqliteSessionDeliveryReferencesForCanonicalRepair, kt as writeSqliteTranscriptArchive, q as listSqliteSessionEntriesForCanonicalRepair } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { J as collectSqliteSessionStateIdsForEntry, W as setCanonicalSqliteSessionMainKey, X as deleteSessionMembersForRepair, Y as copySessionNodeArtifactsForRepair, et as publishSqliteSessionEntryCacheInvalidation, o as resolveAllAgentSessionStoreCandidateTargetsSync, s as resolveAllAgentSessionStoreTargetsSync } from "./targets-Bz-meSET.js";
import { o as resolveDeliveryProvenCanonicalSessionKey } from "./store-entry-DWPp52Lz.js";
import { l as resolveSqliteTranscriptArchiveDirectory } from "./session-accessor.sqlite-scope-G-CS03gJ.js";
import { C as selectSessionTranscriptTreePathNodes, _ as mergeSessionTranscriptTreePaths, b as scanSessionTranscriptTree, h as isSessionTranscriptLeafControl, v as mergeSessionTranscriptVisiblePathWithOpaqueAppendPath } from "./session-transcript-index-NEbbnqMc.js";
...
import { n as normalizeLegacySessionEntryDelivery } from "./state-migrations.legacy-session-store-Ca_XBZjo.js";
import { i as withDoctorSqliteMaintenanceLock, t as DoctorSqliteMaintenanceLockUnavailableError } from "./doctor-sqlite-maintenance-lock-D6TRnK8m.js";
import { f as resolveTargetSqlitePath } from "./doctor-session-sqlite-readers-x8j_5ZDj.js";
import { t as runDoctorAgentDatabaseOperation } from "./doctor-agent-database-operation-COH0BMs8.js";
...
function collectCanonicalSessionCandidates(params, stores) {
	const inventory = stores.flatMap((target) => listSqliteSessionEntriesForCanonicalRepair({
		agentId: target.agentId,
		clone: false,
		storePath: target.storePath
	...
	const addCanonicalMapping = (mappingKey, canonicalKey) => {
...
function selectCanonicalSessionCandidate(candidates, params) {
	...
	const selected = mergeCanonicalSessionEntryCandidates(candidates.toSorted((left, right) => Buffer.compare(Buffer.from(`${left.sqlitePath}\0${left.sessionKey}`, "utf8"), Buffer.from(`${right.sqlitePath}\0${right.sessionKey}`, "utf8"))).map((candidate) => ({
...
async function repairCanonicalSessionGroup(candidates, params) {
	...
	if (winner.sqlitePath !== destination.sqlitePath) {
		...
		for (const sessionId of generationIds) {
			...
			const [destinationEvents, sourceEvents] = await Promise.all([loadSqliteTranscriptEvents({
	...
	const winnerResult = await applySqliteSessionEntryLifecycleMutation({
		agentId: destination.agentId,
		allowCanonicalRepair: true,
...
async function noteSessionSqliteMigrationHealth(params) {
	const { runDoctorSessionSqlite } = await import("./doctor-session-sqlite-ZOctcDBK.js");
