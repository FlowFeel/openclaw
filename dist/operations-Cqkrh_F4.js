...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { requestSafeGatewayRestart } = await import("./restart-coordinator-C1G-gAzW.js");
		return requestSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-HUm6nmtj.js");
	if (operation === "start") {
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-DlL7H2wV.js")).applySystemAgentSetup;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-DlL7H2wV.js");
...
async function executeSystemAgentOperation(operation, runtime, opts = {}) {
	switch (operation.kind) {
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-DT36SNxD.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-BCS9DP_n.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-CeteviMv.js")).runTui)({
				local: true,
				session,
				deliver: false,
				historyLimit: 200,
