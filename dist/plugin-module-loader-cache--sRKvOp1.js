...
import { i as listWorkspacePackageExportAliasEntries, n as buildPluginLoaderJitiOptions, o as resolvePluginLoaderModuleConfig, r as createPluginLoaderModuleCacheKey, t as buildPluginLoaderAliasMap } from "./sdk-alias-BiwifeHS.js";
...
function isSourceTransformFallbackError(error, modulePath) {
	...
	return code === "ERR_REQUIRE_ESM" || code === "ERR_REQUIRE_ASYNC_MODULE" || code === "ERR_REQUIRE_ESM_RACE_CONDITION" || isMissingTargetModuleError(candidate, modulePath);
}
...
const pluginSdkNativeAliasesByMap = /* @__PURE__ */ new WeakMap();
const internalCorePackageHostRoots = new PluginLruCache(128);
const registeredInternalCorePackageHosts = new PluginLruCache(128);
let installed = false;
...
function listPluginSdkNativeAliases(options) {
	const aliasMap = buildPluginLoaderAliasMap(options.pluginModulePath ?? resolveLoaderModulePath(options), options.argv1 ?? process.argv[1], options.moduleUrl, "dist", options.devSourceRoot);
...
function registerInternalCorePackageNativeAliases(options) {
	...
	if (registeredInternalCorePackageHosts.get(packageRoot)) return;
