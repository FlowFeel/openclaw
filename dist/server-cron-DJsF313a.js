...
import { j as testRegexWithBoundedInput, k as compileSafeRegex, u as redactToolPayloadText } from "./redact-BGqyyKN9.js";
...
import { f as resolveAgentIdFromSessionKey, p as resolveEventSessionKey, y as toAgentStoreSessionKey } from "./session-key-DtTE9-Tg.js";
...
import { i as listKnownSessionStoreAgentIds, r as listConfiguredSessionStoreAgentIds } from "./targets-Bz-meSET.js";
...
import { o as enqueueSystemEventEntry, t as consumeSelectedSystemEventEntries } from "./system-events-BNZxjP0P.js";
...
import { a as cronStreamScheduleKey, c as resolveCronStreamBatching, l as truncateCronStreamBatch, o as markCronStreamBatchTruncated } from "./pacing-BxtLSQjv.js";
import { t as abortAndDrainEmbeddedAgentRun } from "./runs-Bdk5tY9J.js";
import "./sessions-BqBqRT1f.js";
import { d as claimSessionAutomationEpoch, f as registerSessionAutomationSource, h as resolveCronJobBoundSessionKeys, p as unregisterSessionAutomationSource, u as bumpSessionAutomationVersion } from "./session-utils-row-Br8x7LNG.js";
...
import { s as waitForActiveCronTaskRuns, t as abortActiveCronTaskRuns } from "./active-run-cancellation-DDKkkTKE.js";
...
import { g as createToolSearchCatalogRef, v as registerHeadlessToolSearchCatalog } from "./tool-search-BhU6HOAE.js";
import { r as resolveMainScopedEventSessionKey } from "./event-session-routing-BY6OxTFh.js";
import { t as createOpenClawCodingTools } from "./agent-tools-Df1_Fel3.js";
import { t as resolveToolLoopDetectionConfig } from "./tool-loop-detection-config-B_SwfAQE.js";
import { n as resolveCronStoredDeliveryContext } from "./delivery-context-DP6bIzIF.js";
import { t as normalizeHttpWebhookUrl } from "./webhook-url-DqjLJiFy.js";
import { t as sendDurableMessageBatch } from "./runtime-B-q2SkyY.js";
import { n as listConfiguredMessageChannels } from "./channel-selection-9UF9FMip.js";
import { n as resolveCronTriggerMinIntervalMs } from "./cron-limits-txevLFpr.js";
import { r as resolveEmbeddedAttemptToolConstructionPlan, t as applyEmbeddedAttemptToolsAllow } from "./attempt-tool-construction-plan-D4Oh4XYT.js";
...
import { a as runCodeModeScriptHeadless, o as CodeModeHeadlessAbortError, s as CodeModeHeadlessTimeoutError } from "./tool-surface-plan-D4N5PzfK.js";
import { n as resolveSandboxContext } from "./context-BSQua5VL.js";
import "./sandbox-C8V7Nism.js";
import "./embedded-agent-Cr03LQ3-.js";
import { t as createOutboundSendDeps } from "./outbound-send-deps-CtctZGaJ.js";
...
import { t as resolveDeliveryTarget } from "./delivery-target-DAbGZLUZ.js";
import { t as resolveScheduledToolPolicyContext } from "./scheduled-tool-policy-BTepG0go.js";
import { n as buildCronAgentDefaultsConfig, r as resolveCronActiveRuntimeConfig, t as runCronIsolatedAgentTurn } from "./isolated-agent-BwIFxQZ3.js";
import { t as toPublicCronJob } from "./public-job-K3v4W-Kg.js";
import { l as errorBackoffMs, n as applyJobPatch } from "./jobs-DscVQvMI.js";
import { t as CronService } from "./service-DFnY1hkL.js";
import { i as runHeartbeatOnce } from "./heartbeat-runner-ByanrMPP.js";
import { n as resolveHeartbeatMonitorSpecs, t as heartbeatMonitorAgentId } from "./heartbeat-monitor-DKYj-jhz.js";
import { g as retryTransientDirectCronDelivery } from "./delivery-dispatch-policy-DYlwnRgY.js";
import { t as buildGatewaySessionEventFields } from "./session-event-payload-Dsg0-I17.js";
...
async function sendFailureNotificationAnnounce(deps, cfg, agentId, jobId, target, message) {
	...
	try {
		...
		})(), FAILURE_NOTIFICATION_TIMEOUT_MS, { createError: () => {
			const error = /* @__PURE__ */ new Error("cron: failure destination announcement timed out");
...
function createCronExitWatchers(params) {
	...
	const cancel = (jobId) => {
		...
		try {
			params.getProcessSupervisor().cancelScope(scopeKey$1(jobId), "manual-cancel");
		} catch (err) {
			params.logger.warn({
				err: String(err),
				jobId
...
//#endregion
//#region src/gateway/server-cron-heartbeat-jobs.ts
/**
...
function dispatchGatewayCronFinishedNotifications(params) {
	...
	if (completionSummary || params.evt.status === "error") for (const webhookTarget of webhookTargets) {
...
function buildGatewayCronService(params) {
	...
	const reconcileStreamWatchers = async () => {
		...
		try {
			...
			for (let attempt = 0; attempt < 5; attempt += 1) {
				...
				const jobs = Array.isArray(result) ? result : result.jobs;
				await watchers.reconcile(jobs, cronEnabled && params.cfg.cron?.triggers?.enabled === true, params.cfg.cron?.triggers?.enabled === true);
				return;
			}
	...
	const routeStreamWatcherMutation = async (jobId, job, action) => {
		...
		try {
			...
			if (job?.schedule.kind === "stream" && job.enabled && !job.state.streamRestartExhausted && cronEnabled && params.cfg.cron?.triggers?.enabled === true) {
	...
	const cron = new CronService({
		...
		requestHeartbeat: (opts) => {
			...
			requestHeartbeat({
				...
				...opts?.scheduledAnchorMs !== void 0 ? { scheduledAnchorMs: opts.scheduledAnchorMs } : {},
		...
		sendCronFailureAlert: async ({ job, text, runAtMs, channel, to, mode, accountId, threadId }) => await sendGatewayCronFailureAlert({
			deps: params.deps,
			logger: cronLogger,
			resolveCronAgent,
			webhookToken: params.cfg.cron?.webhookToken,
	...
	cron.stopAndDrain = async () => {
		...
		const [activeRunDrain, streamWatchersResult] = await Promise.all([waitForActiveCronTaskRuns(CRON_ACTIVE_RUN_SHUTDOWN_DRAIN_MS), streamWatchersStop]);
	...
	let heartbeatReconcileTail = Promise.resolve();
	...
	const reconcileHeartbeatJobs = (cfgOverride) => {
		...
		const pass = async () => {
			if (epoch !== heartbeatReconcileEpoch) return;
	...
	cron.start = async () => {
		const generation = streamWatcherGeneration;
		await startCron();
		if (generation !== streamWatcherGeneration) return;
