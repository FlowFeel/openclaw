...
import { c as normalizeConfiguredProviderCatalogModelId } from "./plugin-metadata-lifecycle-NcA0EWhA.js";
import { _ as computeModelPolicyAllowlist, g as MODEL_POLICY_ALLOWLIST_MIGRATION_MARKER, y as hasModelPolicyAllowlistMigrationMarker } from "./agent-scope-config-Dusa8eSA.js";
import { t as planManifestModelCatalogRows } from "./manifest-planner-BAbScYx0.js";
import { v as normalizeOptionalAgentRuntimeId } from "./openai-routing-G4z6ipSe.js";
import { n as normalizeAgentModelRefForConfig } from "./model-input-BofPWz0k.js";
import { t as splitTrailingAuthProfile } from "./model-ref-profile-BIKs-96s.js";
import { t as isSafeExecutableValue } from "./exec-safety-DtLGRBJm.js";
import { r as isModelThinkingFormat } from "./types.models-BDWk1A38.js";
import { E as resolveUniqueCatalogModelRoute, w as modelTransportRoutesMatch } from "./model-selection-shared-BDTPW9Jk.js";
import { o as isLegacyCodexProviderId, u as legacyCodexProviderIdentityKey } from "./codex-route-model-ref-B7v0y8up.js";
import { t as listOpenClawPluginManifestMetadata } from "./manifest-metadata-scan-C_NSiHd_.js";
...
function applyRetiredModelTable(model, table, overrides) {
	const normalized = normalizeString(model);
	if (overrides && Object.hasOwn(overrides, normalized)) return overrides[normalized] ?? null;
...
function upgradeRetiredModelRef(value) {
	...
	const retiredOwnerModel = normalizedProvider === "groq" ? applyRetiredModelTable(model, RETIRED_GROQ_MODELS) : normalizedProvider === "xai" ? applyRetiredModelTable(model, RETIRED_XAI_MODELS) : normalizedProvider === "openai" || normalizedProvider === "openai-codex" || normalizedProvider === "github-copilot" ? applyRetiredModelTable(model, RETIRED_OPENAI_MODELS, normalizedProvider === "openai-codex" ? RETIRED_CODEX_MODEL_OVERRIDES : void 0) : void 0;
...
function normalizeKnownModelRef(value) {
	...
	const normalized = `${provider === "google" || provider === "google-gemini-cli" || provider === "google-vertex" || provider === "together" || normalizeString(modelId).startsWith("google/") ? normalizeAgentModelRefForConfig(split.model) : split.model}${split.profile ? `@${split.profile}` : ""}`;
...
function normalizeProviderCatalogModelId(provider, modelId) {
	...
	const normalizedProvider = normalizeString(provider);
	const normalized = normalizedProvider === "google" || normalizedProvider === "google-gemini-cli" || normalizedProvider === "google-vertex" || normalizedProvider === "together" || normalizeString(trimmed).startsWith("google/") ? normalizeConfiguredProviderCatalogModelId(provider, trimmed) : trimmed;
	...
	return slash > 0 && normalizeString(upgradedRef.slice(0, slash)) === normalizeString(provider) ? upgradedRef.slice(slash + 1) : normalized;
}
...
const LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS = [
	...
	defineLegacyConfigMigration({
		...
		legacyRules: [{
			path: ["models", "pricing"],
			message: "models.pricing is retired because pricing ships with the hosted catalog; run \"openclaw doctor --fix\" to remove it."
		}],
		apply: (raw, changes) => {
			const models = getRecord(raw.models);
