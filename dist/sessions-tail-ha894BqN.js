...
import { n as loadSqliteTrajectoryRuntimeEventRowsSync } from "./runtime-store.sqlite-CLuMwmB-.js";
...
async function sessionsTailCommand(opts, runtime) {
	...
	if (!targets) return;
	const selections = [];
	for (const target of targets) for (const { sessionKey, entry } of listSqliteSessionEntriesReadOnly({
