...
import { C as hashConfigIncludeRaw, T as resolveConfigIncludeWritePath, w as readConfigIncludeFileWithGuards } from "./redact-BGqyyKN9.js";
...
import { n as recordPluginInstallInRecords, o as withoutPluginInstallRecords, u as reconcileNpmPluginLoadPath } from "./installed-plugin-index-records-BkPV4HdU.js";
import { a as planPluginUninstall, n as applyPluginUninstallDirectoryRemoval } from "./uninstall-6WbfzFVM.js";
...
import { s as buildPluginSnapshotReport } from "./status-DBhSQc1q.js";
import { t as applySlotSelectionForPlugin } from "./slot-selection-C6Gkkfks.js";
...
async function persistPluginInstall(params) {
	...
	if (configEnablement.mode === "invalid") throw new Error(`Plugin "${params.pluginId}" has invalid configured settings: ${configEnablement.error}. Fix plugins.entries.${params.pluginId}.config, then rerun the install.`);
	const shouldEnable = params.enable !== false && configEnablement.mode === "ready";
	const configBase = params.enable === false || configEnablement.mode === "ready" ? reconciledConfig : prepareConfigForDisabledInstall(reconciledConfig, params.pluginId);
	const installConfig = params.enable === false ? configBase : removeInstalledPluginFromDenylist(addInstalledPluginToAllowlist(configBase, params.pluginId), params.pluginId);
	let next = shouldEnable ? enablePluginInConfig(installConfig, params.pluginId, { updateChannelConfig: false }).config : installConfig;
	const slotResult = shouldEnable ? await tracePluginLifecyclePhaseAsync("slot selection", async () => applySlotSelectionForPlugin(next, params.pluginId), {
		command: "install",
		pluginId: params.pluginId
	}) : {
		config: next,
	...
	const configWarning = params.enable !== false && configEnablement.mode === "missing" ? `Installed plugin "${params.pluginId}" without enabling it because it requires configuration first. Configure it, then run \`openclaw plugins enable ${params.pluginId}\`.` : void 0;
	const warningMessage = [params.warningMessage, configWarning].filter(Boolean).join("\n");
	if (warningMessage) runtime.log(theme.warn(warningMessage));
