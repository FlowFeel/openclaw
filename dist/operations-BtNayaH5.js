...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { requestSafeGatewayRestart } = await import("./restart-coordinator-BzuogWpJ.js");
		return requestSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-DTp-cveR.js");
	if (operation === "start") {
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-BkgGKs2I.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-BkgGKs2I.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-B9wOWPHc.js")).applySystemAgentSetup;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-B9wOWPHc.js");
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			const runPluginInstall = ctx.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-Di4-4c8K.js");
...
async function executeSystemAgentOperation(operation, runtime, opts = {}) {
	switch (operation.kind) {
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-BMu7kYem.js");
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-D1QuWvTH.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-BogyU2p3.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-BTZu2HoQ.js")).runTui)({
				local: true,
				session,
				deliver: false,
				historyLimit: 200,
