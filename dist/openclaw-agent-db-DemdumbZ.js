...
import { d as clearNodeSqliteKyselyCacheForDatabase, f as enableNodeSqliteKyselyStatementCache, h as getNodeSqliteKysely, l as runSqliteImmediateTransactionSync, m as executeSqliteQueryTakeFirstSync, p as executeSqliteQuerySync, t as openNodeSqliteDatabase } from "./node-sqlite-B_0DgpUE.js";
import { a as migrateSqliteSchemaToStrict, i as registerSqliteCacheExitClose, l as confirmSqliteFileIntegrity, n as configureSqlitePreSchemaPragmas, o as migrateSqliteSchemaToStrictInTransaction, s as assertSqliteIntegrity, t as configureSqliteConnectionPragmas, u as isTerminalSqliteIntegrityError } from "./sqlite-wal-CD8rJ7gZ.js";
import { G as readOpenClawDatabaseQuarantine, H as createOpenClawDatabaseVerificationError, S as ensureAgentDatabaseLeaseSchema, St as assertSqliteSchemaTablesPresent, W as clearOpenClawDatabaseQuarantine, bt as verifyAndRepairCanonicalSqliteIndexes, h as runOpenClawStateWriteTransaction, mt as createSqliteTerminalOpenLatch, v as detectOpenClawStateDatabaseSchemaMigrationsFromDatabase, wt as getCanonicalSqliteTableNames, xt as assertSqliteSchemaContains, yt as repairCanonicalSqliteIndexes, z as resolveSqliteDatabaseFilePaths } from "./openclaw-state-db-D9eH245j.js";
import { a as readSqliteUserVersion, c as OPENCLAW_SQLITE_BUSY_TIMEOUT_MS, n as resolveOpenClawStateSqlitePath, r as createNewerSqliteSchemaVersionError, t as resolveOpenClawStateSqliteDir } from "./openclaw-state-db.paths-2PkDmkRl.js";
import { n as withOpenClawStateDatabaseReadOnly } from "./openclaw-state-db-readonly-CdeeU5HX.js";
import { i as isPidDefinitelyDead, t as getFileLockProcessStartTime } from "./pid-alive-ClLrY9h9.js";
import { n as assertAgentDeletionPathFence, o as prepareAgentDeletionPathFence, t as assertAgentDeletionIdentityClaimAllowed } from "./agent-deletion-journal-CwmdlLOX.js";
import { n as OpenClawAgentDatabaseMediaMigrationRequiredError } from "./openclaw-agent-db-migration-required-BU5hc3wx.js";
...
function invalidateRegisteredAgentDatabasesMemo(options) {
	...
	if (registeredAgentDatabasesMemo?.pathname === pathname) registeredAgentDatabasesMemo = void 0;
}
...
function listOpenClawRegisteredAgentDatabases(options = {}) {
	...
	if (registeredAgentDatabasesMemo?.pathname === pathname) {
		...
		return options.includeIncompatibleSchemaVersions ? entries : entries.filter((entry) => entry.schemaVersion === 16);
	}
	if (!existsSync(pathname)) {
		if (hasUnavailableMissingSqlitePath(pathname)) throw new Error(`OpenClaw state database ${pathname} is unavailable.`);
		registeredAgentDatabasesMemo = {
			pathname,
			entries: []
		};
		return [];
	...
	return options.includeIncompatibleSchemaVersions ? cloned : cloned.filter((entry) => entry.schemaVersion === 16);
}
//#endregion
...
function registerOpenClawAgentDatabase(params) {
	...
	runOpenClawStateWriteTransaction((database) => {
		...
		executeSqliteQuerySync(database.db, db.insertInto("agent_databases").values({
			...
			path: params.path,
			schema_version: params.schemaVersion ?? 16,
...
function assertOpenClawAgentCurrentRuntimeSchema(database, options) {
	...
	if (metadata.schemaVersion !== 16) throw new Error(`OpenClaw agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 16; run openclaw doctor --fix before using it.`);
...
function assertSupportedAgentSchemaVersion(db, pathname) {
	...
	if (userVersion > 16) throw createNewerSqliteSchemaVersionError("OpenClaw agent database", pathname, userVersion, 16);
}
...
function assertCanonicalAgentMediaPersistenceVersion(db, pathname) {
	...
	if (userVersion < 16 && !isNewUnownedDatabase) throw new OpenClawAgentDatabaseMediaMigrationRequiredError(pathname, userVersion);
}
...
function hasValidSqliteSessionEntryIdentity(entry) {
...
function parseSqliteSessionEntryRecord(row) {
	try {
		...
		const record = parsed;
		if (!hasValidSqliteSessionEntryIdentity(record)) return null;
...
function migratedConversation(entry, persistedChatType, sessionKey) {
	...
	const delivery = migratedObject(canonicalDelivery ?? {}, "context") ?? migratedObject(entry, "deliveryContext");
	const origin = migratedObject(canonicalDelivery ?? {}, "origin") ?? migratedObject(entry, "origin");
...
function assertAgentDatabaseIntegrityBeforeMutation(database, agentId, pathname) {
	...
	if (userVersion === 0 && hasApplicationSchema || userVersion > 0 && userVersion < 16) agentDbLog$1.info("agent database schema migration pending; verifying integrity first", {
		fromVersion: userVersion,
		path: pathname,
		toVersion: 16
	});
	const hasPendingMemoryMigration = userVersion === 16 && hasPendingMemoryChunkMetadataMigration(database);
	const hasPendingSessionContractMigration = userVersion === 16 && hasPendingSessionKeyContractSchemaMigration(database);
	const hasPendingAdditiveMigration = hasPendingMemoryMigration || hasPendingSessionContractMigration;
	if (userVersion === 16 && !hasPendingAdditiveMigration) verifyAndRepairCanonicalSqliteIndexes(database, pathname, OPENCLAW_AGENT_SCHEMA_SQL, {
	...
	if (userVersion === 16 && !hasPendingAdditiveMigration) assertOpenClawAgentCurrentRuntimeSchema(database, {
		agentId,
		pathname
	});
}
...
function ensureAgentSchema(db, agentId, pathname, targetVersion = 16) {
...
function assertOpenClawAgentDatabaseForMaintenance(database, options) {
	...
	const userVersion = readSqliteUserVersion(database);
	if (userVersion > 16) throw createNewerSqliteSchemaVersionError("OpenClaw agent database", options.pathname, userVersion, 16);
	if (userVersion !== 16) throw new Error(`OpenClaw agent database ${options.pathname} uses schema version ${userVersion}; run openclaw doctor --fix before compacting it.`);
	if (metadata.schemaVersion !== 16) throw new Error(`OpenClaw agent database ${options.pathname} metadata schema version ${metadata.schemaVersion ?? "invalid"} does not match 16; run openclaw doctor --fix before compacting it.`);
	assertOpenClawAgentSchemaContains(database, options.pathname, OPENCLAW_AGENT_SCHEMA_SQL);
}
...
function migrateOpenClawAgentDatabaseForMaintenance(options) {
	...
	try {
		...
		const hasCurrentVersion = userVersion === 16 && metadataVersion === 16;
		if (!hasCurrentVersion && !(userVersion >= 1 && userVersion < 16 && metadataVersion !== null && metadataVersion === userVersion && metadataVersion >= 1 && metadataVersion < 16)) return;
		if (hasCurrentVersion) {
			const hadLegacyRecallMetadata = hasLegacyMemoryRecallMetadataColumns(database);
			const hadLegacyProvenanceTrigger = Boolean(database.prepare("SELECT 1 FROM sqlite_schema WHERE type = 'trigger' AND name = 'memory_index_chunk_provenance_after_insert'").get());
			...
			repairCanonicalSqliteIndexes(database, options.pathname, OPENCLAW_AGENT_SCHEMA_SQL, { validateAfterRepair: () => assertOpenClawAgentDatabaseForMaintenance(database, {
				agentId,
...
//#endregion
export { unregisterOpenClawAgentDatabase as $, assertSupportedAgentSchemaVersion as A, MEMORY_INDEX_CHUNKS_TABLE as B, buildConversationRef as C, assertCanonicalAgentMediaPersistenceVersion as D, parseSqliteSessionEntryRecord as E, ensureMemoryIndexSchema as F, ensureMemoryPathFtsTriggers as G, MEMORY_INDEX_PATHS_FTS_TABLE as H, MEMORY_EMBEDDING_CACHE_TABLE as I, MEMORY_INDEX_CHUNK_PROVENANCE_TABLE as J, MEMORY_INDEX_CHUNK_RECALL_METADATA_TABLE as K, MEMORY_INDEX_META_TABLE as L, ensureOpenClawAgentStandingIntentsSchema as M, ensureOpenClawAgentBoardSchemaInTransaction as N, assertExistingAgentSchemaOwner as O, OPENCLAW_AGENT_SCHEMA_SQL as P, registerOpenClawAgentDatabase as Q, MEMORY_INDEX_STATE_TABLE as R, migrateOpenClawAgentDatabaseToMediaPrerequisiteSchema as S, hasValidSqliteSessionEntryIdentity as T, MEMORY_INDEX_SOURCES_TABLE as U, MEMORY_INDEX_FTS_TABLE as V, dropMemoryPathFtsTriggers as W, isPersistentOpenClawAgentDatabasePath as X, ensureMemoryChunkProvenance as Y, isSameOpenClawAgentDatabasePath as Z, runOpenClawAgentWriteTransaction as _, closeOpenClawAgentDatabases as a, assertNoOpenClawAgentDatabaseLeases as at, migrateOpenClawAgentDatabaseForMaintenance as b, deferOpenClawAgentPostCommitPublication as c, inspectOpenClawAgentDatabaseOwner as d, listOpenClawRegisteredAgentDatabases as et, isOpenClawAgentDatabaseOpen as f, recordOpenClawAgentDatabaseOpenFailure as g, openOpenClawAgentDatabase as h, closeOpenClawAgentDatabaseByPath as i, resolveOpenClawAgentSqlitePath as it, readExistingAgentSchemaMeta as j, assertOpenClawAgentSchemaContains as k, disposeOpenClawAgentDatabaseByPath as l, listOpenIncognitoAgentDatabases as m, OPENCLAW_AGENT_DB_OPEN_HANDLE_CAP as n, isIncognitoOpenClawAgentSqlitePath as nt, closeOpenClawAgentDatabasesForTest as o, listOpenClawAgentDatabasesForTest as p, ensureMemoryRecallMetadataSchema as q, clearOpenClawAgentDatabaseOpenFailure as r, resolveIncognitoOpenClawAgentSqlitePath as rt, confirmOpenClawAgentDatabaseIntegrity as s, IncognitoAgentDatabasePathCollisionError as t, ensureOpenClawAgentDatabasePermissions as tt, getOpenClawAgentDatabaseIfOpen as u, assertOpenClawAgentDatabaseForMaintenance as v, normalizeConversationPeerId as w, ensureOpenClawAgentDatabaseSchema as x, assertOpenClawAgentDatabaseOwner as y, MEMORY_INDEX_VECTOR_TABLE as z };
