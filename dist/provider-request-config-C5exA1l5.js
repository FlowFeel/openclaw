...
import { o as assertSecretInputResolved } from "./types.secrets-BvApkFoj.js";
import { i as resolveProviderRequestPolicy, r as resolveProviderRequestCapabilities } from "./provider-attribution-DUC9Ms4a.js";
...
function attachModelProviderMetadataOwners(model, owners) {
	if (!owners) return model;
	const next = { ...model };
	next[MODEL_PROVIDER_METADATA_OWNERS_SYMBOL] = owners;
	return next;
}
/** Reads the plugin metadata generation attached to a prepared model. */
function getModelProviderMetadataOwners(model) {
	return model[MODEL_PROVIDER_METADATA_OWNERS_SYMBOL];
}
/** Carries request-policy ownership across provider/transport model projections. */
function inheritModelProviderMetadataOwners(source, target) {
	return attachModelProviderMetadataOwners(target, getModelProviderMetadataOwners(source));
}
//#endregion
export { getModelProviderMetadataOwners as a, mergeModelProviderRequestOverrides as c, resolveProviderRequestHeaders as d, resolveProviderRequestPolicyConfig as f, buildProviderRequestDispatcherPolicy as i, normalizeBaseUrl as l, sanitizeConfiguredProviderRequest as m, attachModelProviderMetadataOwners as n, getModelProviderRequestTransport as o, sanitizeConfiguredModelProviderRequest as p, attachModelProviderRequestTransport as r, inheritModelProviderMetadataOwners as s, applyPreparedRuntimeAuthToModel as t, resolveProviderRequestConfig as u };
