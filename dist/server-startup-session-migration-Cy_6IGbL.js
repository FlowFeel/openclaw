...
import { t as runSessionStartupMigration } from "./startup-migration-CdVUPMNH.js";
...
async function reconcileStartupSessionTranscriptIndexes(params) {
	...
	const reconcile = params.deps?.reconcileSessionTranscriptIndexes ?? (await import("./session-transcript-reconcile-C7cO0BDU.js")).reconcileSessionTranscriptIndexes;
...
async function runStartupSessionSqliteImport(params) {
	...
	const runDoctorSessionSqlite = params.deps?.runDoctorSessionSqlite ?? (await import("./doctor-session-sqlite-CAcdzSp5.js")).runDoctorSessionSqlite;
	let report;
	try {
...
async function restoreFailedStartupSessionSqliteRun(params, report, blockingIssues) {
	...
	if (!restoreSessionSqliteMigrationRun || !writeSessionSqliteMigrationFailureReports) {
		const doctorModule = await import("./doctor-session-sqlite-CAcdzSp5.js");
