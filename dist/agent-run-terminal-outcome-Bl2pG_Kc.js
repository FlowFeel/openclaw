...
import { i as AGENT_RUN_RESTART_ABORT_STOP_REASON, l as isAgentRunRestartAbortReason, s as isAbortedAgentStopReason, t as AGENT_RUN_ABORTED_ERROR, u as resolveAgentRunAbortLifecycleFields } from "./run-termination-BeCPtaNS.js";
...
function buildAgentRunTerminalOutcomeFromLifecycleEvent(input) {
	...
	const cancellationStatus = lifecycleStatus === "cancelled" || lifecycleStatus === "canceled" || lifecycleStatus === "aborted";
	...
	const normalizedStopReason = !timedOut && cancelled && !isAbortedAgentStopReason(stopReason) && !isCancellationStopReason(stopReason) && (stopReason === void 0 || cancellationStatus) ? aborted ? "aborted" : "stop" : stopReason;
...
function buildAgentRunTerminalOutcomeFromAttempt(input) {
	...
	const stopReason = projected.timedOut && timeoutPhase === void 0 && providerStarted !== true ? void 0 : abortFields.stopReason ?? (restartAborted ? "restart" : void 0) ?? (!timedOut && projected.aborted ? "aborted" : void 0) ?? (!timedOut ? assistantStopReason : void 0);
...
function mergeAgentRunTerminalOutcome(current, incoming) {
	...
	if (current.reason === "hard_timeout") {
		if (incoming.reason === "cancelled" && typeof incoming.endedAt === "number" && typeof current.endedAt === "number" && incoming.endedAt < current.endedAt) return incoming;
...
//#endregion
export { isBlockedLivenessState as _, buildAgentRunTerminalOutcomeFromWaitResult as a, mergeAgentRunAttemptTerminal as c, projectAgentRunAttemptTerminal as d, setAgentRunAttemptTerminalFailure as f, isAbandonedLivenessState as g, formatBlockedLivenessError as h, buildAgentRunTerminalOutcomeFromLifecycleEvent as i, mergeAgentRunTerminalOutcome as l, formatAbandonedLivenessError as m, buildAgentRunTerminalOutcome as n, classifyAgentRunTerminalOutcome as o, normalizeAgentRunTimeoutPhase as p, buildAgentRunTerminalOutcomeFromAttempt as r, isStickyAgentRunTerminalOutcome as s, AGENT_RUN_TERMINAL_RETRY_GRACE_MS as t, normalizeAgentRunAttemptTerminal as u, normalizeBlockedLivenessWaitStatus as v };
