...
import { n as unscopedPackageName } from "./install-safe-path-DL8AfrFO.js";
...
import { a as isPrereleaseResolutionAllowed, i as isOpenClawOrgNpmSpec, o as isPrereleaseSemverVersion, r as isExactSemverVersion, s as parseRegistryNpmSpec, t as compareOpenClawReleaseVersions } from "./npm-registry-spec-CCN1EPEy.js";
import { o as resolveCompatibilityHostVersion } from "./version-CeFj_iGk.js";
import { t as installPluginFromGitSpec } from "./git-install-sEb0FJKQ.js";
import { k as satisfiesPluginApiRange } from "./clawhub-DDwXNACq.js";
import { o as resolvePluginInstallDir, r as resolveDefaultPluginExtensionsDir } from "./install-paths-D18EkQi8.js";
import { t as resolvePackagePluginApiRange } from "./package-compat-DJjXEzu-.js";
import { r as validatePackageExtensionEntriesForInstall } from "./package-entry-resolution-3FNbzYup.js";
import { o as resetPluginSlotsToDefaults } from "./slots-pOTBtf7J.js";
...
import { n as createNpmMetadataEnv, o as resolveNpmSpecMetadata } from "./install-source-utils-YrpMqgeA.js";
import { c as buildNpmResolutionInstallFields, d as recordPluginInstall, f as resolveNpmInstallRecordSpec } from "./installed-plugin-index-records-BkPV4HdU.js";
import { r as withClawPackageLifecycleLease } from "./claw-package-lifecycle-lease-BpwN7iDf.js";
import { t as markClawPackageIndependentlyOwned } from "./claw-package-adoption-B8m5uMc8.js";
import { a as resolveBundledPluginSources } from "./bundled-sources-D7mf-n0T.js";
import { z as PLUGIN_INSTALL_ERROR_CODE } from "./install-managed-npm-state-DKGeDQRx.js";
import { a as reconcileRegisteredOpenClawHostLinks, t as auditDeclaredOpenClawHostDependency } from "./plugin-peer-link-DXF9ykyQ.js";
...
import { t as installPluginFromMarketplace } from "./marketplace-CyV7Lky0.js";
import { n as readInstalledPackageManifest, r as readInstalledPackageVersion, t as expectedIntegrityForUpdate } from "./package-update-utils-Dl9s-yCQ.js";
...
function resolveClawHubUpdateSpecs(params) {
	...
	return resolveClawHubInstallSpecsForUpdateChannel({
		spec: params.officialSpecOverride ?? params.record.spec ?? params.record.resolvedSpec ?? `clawhub:${clawhubPackage}`,
		updateChannel: params.updateChannel
	});
}
...
async function repairRegisteredOpenClawHostLink(params) {
	return (await reconcileRegisteredOpenClawHostLinks({
...
async function repairOpenClawPeerLinksForNpmInstalls(params) {
	return (await reconcileRegisteredOpenClawHostLinks({
		...
		onPackageReadError: (error, packageDir) => {
			params.logger.warn?.(`Could not repair openclaw peer link at ${packageDir}: ${String(error)}`);
		}
	})).repaired > 0;
}
//#endregion
...
function resolveClawHubRiskAcknowledgementOptions(params) {
	return {
		...params.acknowledgeClawHubRisk ? { acknowledgeClawHubRisk: true } : {},
		...!params.dryRun && params.onClawHubRisk ? { onClawHubRisk: params.onClawHubRisk } : {}
	};
}
function buildPluginUpdateVersionOutcome(params) {
	const currentLabel = params.currentVersion ?? "unknown";
	const nextLabel = params.nextVersion ?? "unknown";
	const unchanged = Boolean(params.currentVersion && params.nextVersion && params.currentVersion === params.nextVersion);
	return {
		pluginId: params.pluginId,
		status: unchanged ? "unchanged" : "updated",
		currentVersion: params.currentVersion,
		nextVersion: params.nextVersion,
		message: unchanged ? `${params.pluginId} already at ${currentLabel}.${params.channelFallbackSuffix}` : `Updated ${params.pluginId}: ${currentLabel} -> ${nextLabel}.${params.channelFallbackSuffix}`,
...
async function updateNpmInstalledPlugins(params) {
	...
	for (const pluginId of targets) {
		...
		if (!params.dryRun && record.source === "npm" && currentVersion) changed = await repairRegisteredOpenClawHostLink({
			pluginId,
			record,
			logger
		}) || changed;
		...
		if (!params.dryRun && record.source === "npm" && (currentVersion || params.syncOfficialPluginInstalls && trustedSourceLinkedOfficialInstall)) {
			...
			if (metadataResult.ok) {
				...
				if (currentVersion && !bypassTrustedOfficialUnchangedNpmCheck && isNpmMetadataCompatibleWithCurrentHost(metadataResult.metadata) && !await auditDeclaredOpenClawHostDependency({
		...
		const runAttempt = () => runPluginUpdateAttempt({
			pluginId,
			record,
			config: params.config,
		...
		changed = true;
		outcomes.push(buildPluginUpdateVersionOutcome({
			pluginId,
			currentVersion,
			nextVersion,
			channelFallbackSuffix,
