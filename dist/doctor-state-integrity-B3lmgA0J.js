...
import { C as resolveStateDir, S as resolveOAuthDir } from "./paths-CL43LNS6.js";
import { t as formatCliCommand } from "./command-format-C5kg4XY_.js";
import { d as resolveDefaultAgentDir, h as tryResolveDefaultAgentId, i as listAgentIds, n as listAgentEntries, o as resolveAgentConfig } from "./agent-scope-config-Dusa8eSA.js";
import { n as normalizeAgentId } from "./agent-id-DDgUze4y.js";
import { a as isSubagentSessionKey, c as parseAgentSessionKey } from "./session-key-utils-02xWdGSz.js";
import { f as resolveAgentIdFromSessionKey, r as LEGACY_IMPLICIT_AGENT_ID, y as toAgentStoreSessionKey } from "./session-key-DtTE9-Tg.js";
import { t as modelKey } from "./model-key-BaNhQShd.js";
import { r as resolveAgentModelFallbackValues } from "./model-input-BofPWz0k.js";
import { d as resolveAgentModelFallbacksOverride } from "./agent-scope-DyEposw2.js";
import { t as resolveAgentHarnessPolicy } from "./policy-BxKGFISr.js";
import { a as listConfiguredChannelIdsForReadOnlyScope } from "./channel-presence-policy-DhrCS5Fz.js";
...
import { a as resolveMainSessionKey, n as canonicalizeMainSessionAlias } from "./main-session-Bjm_i_Af.js";
import { a as resolveSessionFilePathOptions, c as resolveSessionTranscriptsDirForAgent, f as formatSessionArchiveTimestamp, h as isPrimarySessionTranscriptFileName, i as resolveSessionFilePath, l as resolveStorePath } from "./paths-DSnYpBD3.js";
import { R as applySqliteSessionEntryReplacements, ct as loadSqliteSessionEntryReadOnly, nt as listSqliteSessionEntriesReadOnly } from "./session-accessor.sqlite-B9iW7DOt.js";
...
import { n as formatSubagentRecoveryWedgedReason, r as isSubagentRecoveryWedgedEntry, t as clearWedgedSubagentRecoveryAbort } from "./subagent-recovery-state-CTVrxsMF.js";
import "./channel-plugin-ids-DnoOYgMp.js";
import { t as resolveHeartbeatDeliveryTarget } from "./targets-D8UdbAN9.js";
import { n as resolveHeartbeatIntervalMs } from "./heartbeat-summary-S-VZc34p.js";
import { n as isHeartbeatOkResponse, r as isHeartbeatUserMessage } from "./heartbeat-filter-DUXErom1.js";
import { t as resolveMemoryBackendConfig } from "./backend-config-Ca38C-dn.js";
import "./selection-kQ7hC4vz.js";
import { t as note } from "./note-uiKX_qfX.js";
import { i as updateLegacySessionStore, t as loadLegacySessionStore } from "./state-migrations.legacy-session-store-RXXUZyiq.js";
import { a as listPluginDoctorSessionRouteStateOwners } from "./doctor-contract-registry-CRvVJr_Z.js";
import "./engine-storage-PRsCOUxC.js";
import { n as clearTuiLastSessionPointers } from "./tui-last-session-DlXR_CtL.js";
...
function describeHeartbeatSessionTargetIssues(cfg) {
	...
	for (const agentId of listHeartbeatDoctorAgents(cfg)) {
		...
		if (loadSqliteSessionEntryReadOnly({
			agentId: storeAgentId,
			sessionKey: canonicalSession,
			storePath
		}) ?? (fs.existsSync(storePath) ? loadLegacySessionStore(storePath)[canonicalSession] : void 0)) continue;
		warnings.push([
...
async function noteMainSessionRecoveryIntegrity(params) {
	...
	const wedged = entries.flatMap(({ entry, sessionKey }) => {
		const tombstone = entry.mainRestartRecovery?.tombstone;
		return tombstone ? [{
			key: sessionKey,
			health: {
	...
	const repaired = await applySqliteSessionEntryReplacements({
...
function listOrphanAgentDirs(cfg, stateDir) {
	const configuredIds = /* @__PURE__ */ new Set([LEGACY_IMPLICIT_AGENT_ID]);
	const defaultAgentId = tryResolveDefaultAgentId(cfg);
	if (defaultAgentId) configuredIds.add(normalizeAgentId(defaultAgentId));
...
function detectStateIntegrityHealthIssues(cfg, params) {
	...
	const storePath = agentId ? resolveStorePath(cfg.session?.store, { agentId }) : void 0;
...
async function noteStateIntegrity(cfg, prompter, configPath, options) {
	...
	const storePath = agentId ? resolveStorePath(cfg.session?.store, { agentId }) : void 0;
	const storeDir = storePath ? path.dirname(storePath) : void 0;
	const absoluteStorePath = storePath ? path.resolve(storePath) : void 0;
	const displayStateDir = shortenHomePath(stateDir);
	const displayOauthDir = shortenHomePath(oauthDir);
	const displaySessionsDir = sessionsDir ? shortenHomePath(sessionsDir) : void 0;
	const displayStoreDir = storeDir ? shortenHomePath(storeDir) : void 0;
	const displayConfigPath = configPath ? shortenHomePath(configPath) : void 0;
	const requireOAuthDir = shouldRequireOAuthDir(cfg, env);
	const cloudSyncedStateDir = detectMacCloudSyncedStateDir(stateDir);
	const linuxSdBackedStateDir = detectLinuxSdBackedStateDir(stateDir);
	const linuxVolatileStateDir = detectLinuxVolatileStateDir(stateDir);
	const suppressOrphanTranscriptWarning = agentId ? shouldSuppressOrphanTranscriptWarning(cfg, agentId) : false;
	...
	if (!agentId || !sessionsDir || !storePath || !absoluteStorePath || !displaySessionsDir) {
		warnings.push("- Skipped default-agent session and transcript integrity checks because the agent roster does not have exactly one default.");
		if (warnings.length > 0) noteFn(warnings.join("\n"), "State integrity");
		if (changes.length > 0) noteFn(changes.join("\n"), "Doctor changes");
		return;
	}
	const sqliteEntries = listSqliteSessionEntriesReadOnly({
		agentId,
		storePath: absoluteStorePath
	});
	const sqliteSessionKeys = new Set(sqliteEntries.map(({ sessionKey }) => sessionKey));
	const store = { ...existsFile(absoluteStorePath) ? loadLegacySessionStore(absoluteStorePath) : {} };
	for (const { entry, sessionKey } of sqliteEntries) store[sessionKey] = entry;
	...
	if (entries.length > 0 || canonicalEntryCount > 0) {
		...
		const missing = recentTranscriptCandidates.filter(([key, entry]) => {
			if (sqliteSessionKeys.has(key)) return false;
			const sessionId = entry.sessionId;
			if (!sessionId) return false;
			const legacySessionFile = entry.sessionFile;
			if (parseSqliteSessionFileMarker(legacySessionFile)) return false;
		...
		if (wedgedSubagentSessions.length > 0) {
			...
			if (await prompter.confirmRuntimeRepair({
				...
				if (sqliteKeys.length > 0) repaired += await applySqliteSessionEntryReplacements({
