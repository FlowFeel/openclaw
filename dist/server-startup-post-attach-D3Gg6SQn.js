...
function scheduleContextCachePrewarm(params) {
	...
	const warm = async () => {
		if (stopped) return;
		const { ensureContextWindowCacheLoaded } = await import("./context-yzITBIn4.js");
...
async function prewarmGatewaySessionListData(cfg, agentId) {
	const [{ loadCombinedSessionStoreForGateway }, { listSessionsFromStoreAsync }] = await Promise.all([import("./combined-store-gateway-ebUxuEqi.js"), import("./session-utils-list-DodSIfea.js")]);
...
function dashboardDataPrewarmItems(cfg, log) {
	...
	return [
		...
		})),
		{
			name: "plugins",
			load: async () => {
				const { listManagedPlugins } = await import("./management-service-11nP17jW.js");
		...
		...agentIds.map((agentId) => ({
			...
			load: async () => {
				if (!await shouldPrewarmSessionData()) return;
				const { prewarmSessionCatalogList } = await import("./session-catalog-BR92k2Y7.js");
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-B_jlnGy5.js"));
const loadMainSessionRestartRecoveryMarkingModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-marking-Bw_whKXb.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RjT9WtG0.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-D3Bp_5RI.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-BMqilB60.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-jFjsNKIh.js"));
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { startGatewayMemoryBackend } = await import("./server-startup-memory-C7NcTxrG.js");
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	runWithGatewayIndependentRootWorkAdmission(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-B7KFGRAV.js");
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-dvVKxUkW.js");
			if (isStopped()) return;
...
async function publishConfiguredModelRuntimeSnapshots(params) {
	const { refreshPreparedModelRuntimeSnapshots } = await import("./prepared-model-runtime-BRjTAf26.js");
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadPreparedModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				...
				import("./prepared-model-catalog-BddDieHg.js"),
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-Db25BpxD.js")).scheduleGatewayUpdateCheck(...args),
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	const sidecarsPromise = params.minimalTestGateway ? Promise.resolve({
		...
		try {
			const { scheduleSubagentRegistrySweep } = await import("./subagent-registry-_565RV8I.js");
