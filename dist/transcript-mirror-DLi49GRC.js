...
import { a as publishSessionTranscriptUpdateByIdentity } from "./session-transcript-runtime-Cvc8VErx.js";
...
async function mirror(params) {
	...
	const { appendedUpdates, assistantMirrorIdentitiesOwned, messagesPresent, userMessagesPresent } = await withCodexSessionTranscriptMirrorWriteLock({
		...transcriptTarget,
		...
		for (const { dedupeIdentity, idempotencyKey, message, sourceFingerprint } of candidates) {
			...
			if (idempotencyKey && mirrorFacts.existingIdempotencyKeys.has(idempotencyKey)) {
				...
				if (persistedMessage && isMirroredAgentMessage(persistedMessage)) {
					...
					if (persistedMessage.role === "user") nextUserMessagesPresent.push(persistedMessage);
				}
			...
			if (message.role === "assistant") nextAssistantMirrorIdentitiesOwned.add(dedupeIdentity);
			if (appendedMessage.role === "user") nextUserMessagesPresent.push(appendedMessage);
			if (appended.appended) nextAppendedUpdates.push({
				messageId,
				message: appendedMessage,
				...messageSeq !== void 0 ? { messageSeq } : {}
			});
			if (idempotencyKey) mirrorFacts.existingIdempotencyKeys.add(idempotencyKey);
		}
		return {
...
//#endregion
export { projectBoundedCodexThreadHistory as a, fingerprintCodexMirrorSourceMessage as c, attachCodexMirrorIdentity as d, attachUpstreamUserText as f, mirrorPromptAtTurnStartBestEffort as i, readCodexMirrorSourceFingerprint as l, readUpstreamUserText as m, createCodexAppServerUserMessagePersistenceNotifier as n, buildCodexUserPromptMessage as o, readMirrorIdentity as p, importCodexThreadHistoryToTranscript as r, promptSnapshot as s, codexTranscriptMirrorRuntime as t, serializeCodexMirrorSourceEvidence as u };
