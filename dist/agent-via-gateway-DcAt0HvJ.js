...
import { b as waitForDiagnosticEventsDrained } from "./diagnostic-events-Dt41CZkD.js";
...
import { S as readGatewayDispatchConfigWithShellEnvFallback, _ as isGatewayTransportError, c as callGateway, g as isGatewayExplicitAuthRequiredError, h as isGatewayCredentialsRequiredError, v as randomIdempotencyKey, x as readGatewayDispatchConfig } from "./call-B_qeDbXh.js";
...
async function startOneShotDiagnosticsExporters(params) {
	...
	const [{ loadOpenClawPlugins }, { startPluginServices }] = await Promise.all([import("./plugins/loader.js"), import("./services-BDw2p4U8.js")]);
...
const embeddedAgentCommandLoader = createLazyPromiseLoader(() => import("./agent-Dzh1fzNt.js").then((module) => module.agentCommand), { cacheRejections: true });
const agentSessionModuleCache = createLazyPromiseLoader(() => agentSessionModuleLoader(), { cacheRejections: true });
const runtimeConfigModuleLoader = createLazyPromiseLoader(() => import("./io-Dzf83G4H.js"), { cacheRejections: true });
...
async function startEmbeddedRunDiagnosticsExporters(runtime, options) {
	try {
...
async function normalizeSessionKeyOptsForDispatch(opts) {
	...
	const cfg = isLegacySessionKey && (agentIdRaw || shouldScopeDefaultAgentKey) ? opts.local === true ? await loadRuntimeConfig() : readGatewayDispatchConfig() : void 0;
...
async function agentViaGatewayCommand(opts, runtime, signalBridge) {
	...
	const sessionKey = deferExplicitRecipientSession ? void 0 : classifySessionKeyShape(explicitSessionKey) === "agent" ? explicitSessionKey : explicitSessionKey || opts.to || opts.sessionId ? (await loadAgentSessionModule()).resolveSessionKeyForRequest({
		cfg,
		agentId,
		to: opts.to,
...
async function agentCliCommand(opts, runtime, deps) {
	...
	try {
		if (dispatchOpts.local === true) return returnAfterSignalExit(await runEmbeddedAgentCommand({
			...gatewayDispatchOpts,
		...
		}, runtime, deps, { suppressStdoutDiagnosticLogs: dispatchOpts.json === true }), signalBridge.getReceivedSignal(), runtime);
		try {
