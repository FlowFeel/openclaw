...
import { i as isPathInside, r as isNotFoundPathError } from "./path-D8zNGPJM.js";
import { t as sameFileIdentity } from "./file-identity-C0fBiekR.js";
import { E as createAsyncDirectoryGuard, r as getFsSafeTestHooks, w as assertAsyncDirectoryGuard } from "./pinned-write-CO4XA8tE.js";
import { c as resolveOpenedFileRealPathForHandle, i as root } from "./root-impl-DYBxk3hn.js";
import { t as getNativeBinding } from "./native-DEmD_dhE.js";
import { n as resolveSecureTempRoot, t as tempFile } from "./temp-target-CTNQantz.js";
import { t as writeSiblingTempFile } from "./sibling-temp-C8YFkxtG.js";
import { a as validateArchiveEntryPath, i as stripArchivePath, o as ArchiveFormatError, r as resolveArchiveOutputPath, s as ArchiveSecurityError } from "./archive-entry-C5HCSbsy.js";
import { d as createByteBudgetTracker, f as createExtractBudgetTransform, l as assertArchiveEntryCountWithinLimit, n as ARCHIVE_LIMIT_ERROR_CODE, p as resolveExtractLimits, r as ArchiveLimitError, t as loadZipArchiveWithPreflight, u as assertArchiveEntryPathComponentsWithinLimit } from "./archive-zip-preflight-Dnek-ZSK.js";
...
function shouldExtractArchiveEntry(params) {
	...
	if ((params.onFiltered ?? "reject-archive") === "reject-archive") throw new ArchiveSecurityError("entry-filtered", `archive entry rejected by filter: ${params.entry.path}`);
	return false;
}
//#endregion
...
function createTarEntryPreflightChecker(params) {
	...
	return (entry) => {
		...
		if (BLOCKED_TAR_ENTRY_TYPES.has(entry.type)) throw new ArchiveSecurityError("entry-link", `tar entry is a link: ${entry.path}`);
...
function throwMappedNativeError(error) {
	if (error instanceof Error) {
		...
		if (error.message.includes("archive-header-invalid")) throw new ArchiveFormatError(error.message, { cause: error });
	}
	throw error;
}
		...
		await withStagedArchiveDestination({
			...
			run: async (stagingDir) => {
				...
				for (const entry of manifest) {
					...
					if (entry.kind === "sparse") throw new ArchiveFormatError(`GNU sparse archive entry is not supported: ${entry.path}`);
					if (kind === "symlink") throw new ArchiveSecurityError("entry-link", `${params.kind === "zip" ? "zip" : "tar"} entry is a link: ${entry.path}`);
	...
	try {
		await writeSiblingTempFile({
			...
			writeTemp: async (tempPath) => {
				...
				try {
					await pipeline$1(readable, createExtractBudgetTransform({ onChunkBytes: params.budget.addBytes }), writable, { signal: params.deadline.signal });
				} catch (err) {
...
//#endregion
export { readTarEntryInfo as a, withStagedArchiveDestination as c, createTarEntryPreflightChecker as i, preflightTarMetadata as n, mergeExtractedTreeIntoDestination as o, importOptionalTar as r, prepareArchiveDestinationDir as s, extractArchive as t };
