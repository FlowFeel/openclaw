...
import { n as isTrustedSafeBinPath, o as listRiskyConfiguredSafeBins, s as normalizeSafeBinName, t as getTrustedSafeBinDirs } from "./exec-safe-bin-trust-DHudQEAW.js";
import { l as resolveCommandResolutionFromArgv } from "./exec-command-resolution-BcEGhdwN.js";
...
function collectExecSafeBinScopes(cfg) {
	...
	for (const { entry: agent, source } of listAgentEntriesWithSource(cfg)) {
		...
		scopes.push({
			scopePath: source.kind === "entries" ? `agents.entries.${source.key}.tools.exec` : `agents.list.${source.index}.tools.exec`,
			safeBins,
			exec: agentExec,
