...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-rawMgy1d.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-DlL7H2wV.js")).applySystemAgentSetup;
	...
	if (wantsDiscovery) {
		...
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-Dd-R2auq.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
