...
import { i as assertOkOrThrowProviderError, m as readProviderJsonResponse, n as asObject } from "./provider-http-errors-Cj7zfu8U.js";
...
function extractGoogleSpeechPcm(payload) {
	for (const candidate of payload.candidates ?? []) for (const part of candidate.content?.parts ?? []) {
		...
		if (!canonicalAudio) throw new Error("Google TTS response returned malformed base64 audio data");
...
function renderGoogleAudioProfilePrompt(params) {
	...
	if (label) sections.push(`# AUDIO PROFILE: ${label}`);
	const directorNotes = [];
