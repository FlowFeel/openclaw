...
import { C as resolveStateDir, i as isDefaultInstallIdentity } from "./paths-CL43LNS6.js";
...
import { n as hasAmbiguousGatewayAuthModeConfig } from "./auth-mode-policy-BeQbXCXc.js";
...
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, r as noteChromeMcpBrowserReadiness, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-CzNU6su5.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-CIbtW56M.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-CMiniG3c.js";
import { i as sessionLockToRepairEffect, r as sessionLockToHealthFinding, t as detectStaleSessionLocks } from "./doctor-session-locks-tEp-yL5c.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-_x6xBZRO.js";
import { i as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-DqpUS8hS.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-0iEPMKT5.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-gbLOFvF1.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-BSBL67OM.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-C9kWimJH.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-DoXgytFu.js");
...
const legacyStateCheck = {
	...
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-Dc88c8kL.js");
...
const bootstrapSizeCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { buildBootstrapInjectionStats, analyzeBootstrapBudget } = await import("./bootstrap-budget-B9sgprL7.js");
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-r8vxi8Il.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-C49E7Dm7.js");
...
const openAIOAuthTlsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { formatOpenAIOAuthTlsPreflightFix, runOpenAIOAuthTlsPreflight, shouldRunOpenAIOAuthTlsPrerequisites } = await import("./provider-openai-chatgpt-oauth-tls-DPOM8FXz.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	defaultEnabled: false,
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-33aKh_Rb.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-33aKh_Rb.js");
...
const telegramGeneralTopicConversationsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectTelegramGeneralTopicConversationRepairs } = await import("./doctor-telegram-general-topic-conversations-DhzF3T7f.js");
	...
	async repair(ctx) {
		const { repairTelegramGeneralTopicConversations } = await import("./doctor-telegram-general-topic-conversations-DhzF3T7f.js");
		const effect = {
			kind: "state",
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-B-BLa3Vg.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-B-BLa3Vg.js");
...
const gatewayPlatformNotesCheck = {
	...
	async detect(ctx) {
		...
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-CSj0MTF4.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-BsJWwqdG.js"), import("./mcp-config-BJPc4O9Z.js")]);
