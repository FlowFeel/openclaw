...
import { c as shouldUseEnvHttpProxyForUrl } from "./proxy-env-sKZhlk0j.js";
...
import "./runtime-snapshots-D6jlj0Te.js";
import { o as requireApiKey } from "./model-auth-runtime-shared-BVzqP6NP.js";
import { b as estimateUtf8Bytes, h as runMemoryHostTasksWithConcurrency, v as hasNonTextEmbeddingParts, x as splitTextToUtf8ByteLimit } from "./internal-C0br8ilW.js";
...
import "./embeddings-BrDLPCH_.js";
import { n as resolveMemorySecretInputString } from "./secret-input-C8FkUo1T.js";
...
const resolveApiKeyForProvider = async (...args) => {
	return (await import("./model-auth-CK2I_VyC.js")).resolveApiKeyForProvider(...args);
};
//#endregion
...
//#endregion
//#region packages/memory-host-sdk/src/host/embedding-error-classifier.ts
const UNREACHABLE_SOCKET_CODES = /* @__PURE__ */ new Set([
	"ECONNREFUSED",
	"ETIMEDOUT",
	"ENOTFOUND",
	"EHOSTUNREACH",
...
var EmbeddingProviderUnreachableError = class extends Error {
	constructor(message, code = "ECONNREFUSED") {
...
function classifyEmbeddingError(error) {
	...
	if (UNREACHABLE_SOCKET_CODES.has(code) || /ECONNREFUSED|ENOTFOUND|ETIMEDOUT|fetch failed/i.test(message)) return {
		kind: "TERMINAL_UNREACHABLE",
		isTerminal: true,
		code: code !== "UNKNOWN" ? code : "ECONNREFUSED",
		message
	};
	if (/rate limit|429|too many requests|overloaded/i.test(message)) return {
...
function readEmbeddingVector(value, errorPrefix) {
	if (!Array.isArray(value) || value.length === 0) throw malformedEmbeddingResponse(errorPrefix);
...
async function fetchRemoteEmbeddingVectors(params) {
	try {
		...
		if (classified.isTerminal) throw new EmbeddingProviderUnreachableError(classified.message, classified.code);
		throw error;
	}
}
//#endregion
