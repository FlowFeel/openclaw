...
function stringifyPreflightErrorValue(error) {
	try {
		return String(error);
	} catch {
...
function formatPreflightErrorDetail(error) {
	...
	const detail = name && message ? `${name}: ${message}` : message || name || (code ? `code=${code}` : stringifyPreflightErrorValue(error));
...
function formatPreflightError(error) {
	...
	const causeDetails = causeChain.map(formatPreflightErrorDetail).filter((detail, index, all) => detail && all.indexOf(detail) === index).join(" | ") || stringifyPreflightErrorValue(error);
	const redacted = redactSensitiveText(causeChain.some((candidate) => readErrorProperty(candidate, "name") === "TimeoutError") ? `Local provider preflight exceeded its configured ${PREFLIGHT_TIMEOUT_MS}ms deadline | ${causeDetails}` : causeDetails);
	return redacted.length <= MAX_PREFLIGHT_ERROR_CHARS ? redacted : `${truncateUtf16Safe(redacted, MAX_PREFLIGHT_ERROR_CHARS - 1)}…`;
}
