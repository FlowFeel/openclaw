...
import { $ as listTaskFlowRecords, B as isTerminalFlowStatus, O as listTasksForFlowId, Z as getTaskFlowById, rt as resolveTaskFlowForLookupToken } from "./task-registry-Bs8cA6Tm.js";
import { n as formatTaskStatusDetail } from "./task-status-Bo-Cq_rI.js";
import "./runtime-internal-grPJIfa5.js";
import { d as getFlowTaskSummary, n as cancelFlowById } from "./task-executor-verbdzU7.js";
...
function formatFlowListSummary(flows) {
	return `${flows.filter((flow) => flow.status === "queued" || flow.status === "running").length} active · ${flows.filter((flow) => flow.status === "blocked").length} blocked · ${flows.filter((flow) => flow.cancelRequestedAt != null && !isTerminalFlowStatus(flow.status)).length} cancel-requested · ${flows.length} total`;
}
...
async function flowsShowCommand(opts, runtime) {
	...
	for (const task of tasks) {
		...
		runtime.log(sanitizeTerminalText(`- ${task.taskId} ${task.status} ${safeFlowDisplayText(task.runId)} ${safeLabel}${safeDetail}`));
	}
}
