...
async function runRemoteGatewayInferenceOnboarding(target, runtime = defaultRuntime, deps = {}) {
	const callGateway = deps.callGateway ?? (await import("./call-ujGvHIZc.js")).callGatewayCli;
	const runGuidedOnboarding = deps.runGuidedOnboarding ?? (await import("./onboard-guided-BX9pHBfy.js")).runGuidedOnboarding;
	...
	await runGuidedOnboarding({}, runtime, {
		...
		runSystemAgentChat: async () => {
			...
			await (deps.runTui ?? (await import("./tui-CL7s_fVu.js")).runTui)({
				config: boundConfig,
				deliver: false,
