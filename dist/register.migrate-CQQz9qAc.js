...
import { i as migratePlanCommand, n as migrateDefaultCommand, r as migrateListCommand, t as migrateApplyCommand } from "./migrate-CYsKWwgH.js";
...
function readMigrationItems(value, command) {
	...
	const items = selected.filter((item) => typeof item === "string").map((item) => item.trim()).filter((item) => item.length > 0);
