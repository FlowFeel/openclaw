...
import { Ir as validateSessionsUsageParams } from "./src-BSn6va4B.js";
...
import { c as resolveExistingUsageSessionFile, i as loadSessionUsageTimeSeries, o as loadCostUsageSummaryFromCache, r as loadSessionLogs, s as loadSessionCostSummariesFromCache, t as discoverAllSessions } from "./session-cost-usage-BhR_rUA-.js";
import { r as loadUsageStatusStaleWhileRevalidate } from "./models-auth-status-usage-cache-Bb5hdknb.js";
...
const SESSIONS_USAGE_CACHE_TTL_MS = 3e4;
...
function resolveSessionUsageTarget(key, config, agentIdHint) {
	const { canonicalKey, entry, storePath } = loadSessionEntryReadOnly(key, agentIdHint ? { agentId: agentIdHint } : void 0);
	...
	const sessionFile = entry ? resolveExistingUsageSessionFile({
		...
		agentId,
		sessionId,
		sessionFile: resolveSessionFilePath(sessionId, void 0, resolveSessionFilePathOptions({
			storePath,
			agentId
		}))
	});
...
function findSessionsUsageCacheEvictionKey() {
	for (const [key, entry] of sessionsUsageCache) if (!entry.inFlight) return key;
	return sessionsUsageCache.keys().next().value;
}
function setSessionsUsageCache(cacheKey, entry) {
	if (!sessionsUsageCache.has(cacheKey) && sessionsUsageCache.size >= SESSIONS_USAGE_CACHE_MAX) {
		...
		if (evictKey !== void 0) sessionsUsageCache.delete(evictKey);
	}
...
async function loadSessionsUsageResultCached(params) {
	...
	const cached = candidate?.configRef === params.configRef ? candidate : void 0;
	if (cached?.result && cached.updatedAt && now - cached.updatedAt < SESSIONS_USAGE_CACHE_TTL_MS) return cached.result;
	if (cached?.inFlight) {
		if (cached.result && cached.updatedAt) return cached.result;
		return await cached.inFlight;
	}
	const entry = cached ?? { configRef: params.configRef };
	const inFlight = params.load().then((result) => {
		if (sessionsUsageCache.get(cacheKey) !== entry) return result;
		if (!result.cacheStatus || result.cacheStatus.status === "fresh") {
			entry.result = result;
	...
	if (entry.result && entry.updatedAt) return entry.result;
...
async function loadCostUsageSummaryCached(params) {
	...
	const agentId = allAgents ? void 0 : normalizeAgentId(params.agentId ?? resolveDefaultAgentId(params.config));
	...
	const entry = cached ?? {};
	const inFlight = (allAgents ? loadAllAgentCostUsageSummary({
		startMs: params.startMs,
		endMs: params.endMs,
...
async function loadAllAgentCostUsageSummary(params) {
	const summaries = await runUsageAgentTasks(listAgentIds(params.config).map((agentId) => normalizeAgentId(agentId)).map((agentId) => () => loadCostUsageSummaryFromCache({
		startMs: params.startMs,
		endMs: params.endMs,
		dayBucket: params.dayBucket,
		config: params.config,
	...
	"sessions.usage": async ({ respond, params, context }) => {
		...
		try {
			result = await loadSessionsUsageResultCached({
				...
				load: async () => {
					const { store } = loadCombinedSessionStoreForGateway(config, effectiveAgentId ? { agentId: effectiveAgentId } : {});
