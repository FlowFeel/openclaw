...
import { w as isSkillsMachineOutput } from "./argv-D6QuZiS5.js";
import { r as stripAnsi, t as sanitizeForLog } from "./ansi-DbP9Z_r_.js";
import { c as resolveUserPath } from "./home-dir-Cs7bTrwJ.js";
import { h as shortenHomePath, t as CONFIG_DIR } from "./utils-Bs67j6-3.js";
...
import { n as isImmutableGitCommitRef, r as parseGitPluginSpec } from "./git-install-sEb0FJKQ.js";
import { u as writeJson } from "./json-C-CW4mQo.js";
import "./json-files-v5WP3doI.js";
import { l as resolveAgentIdByWorkspacePath } from "./agent-scope-DyEposw2.js";
import { h as fetchClawHubSkillCard, t as CLAWHUB_SKILLS_SH_TRUST_LABEL, y as fetchClawHubSkillVerification } from "./clawhub-DDwXNACq.js";
...
import { s as withTempDir } from "./install-source-utils-YrpMqgeA.js";
...
import { t as parseFrontmatter } from "./frontmatter-CnS7mRCl.js";
import { n as decorativePrefix, t as decorativeEmoji } from "./decorative-emoji-CskzcmH9.js";
import { c as searchSkillsFromClawHub, f as readTrackedClawHubSkillSlugs, l as readVerifiedClawHubSkillSourceUrl, m as installExtractedSkillRoot, o as resolveClawHubSkillVerificationTarget, p as untrackClawHubSkill, r as updateSkillsFromClawHub, t as installSkillFromClawHub, v as validateRequestedSkillSlug } from "./clawhub-CxehPxLh.js";
import { t as CLAWHUB_TRUST_ERROR_CODE } from "./clawhub-install-trust-Dgg9MNPX.js";
import { n as resolveSkillStatusEntry } from "./status-CsptEi4i.js";
import { a as quarantineSkillProposal, d as inspectSkillProposal, f as listSkillProposals, h as readSkillProposalDraftFile, i as proposeUpdateSkill, m as readSkillProposalDraftDirectory, o as rejectSkillProposal, r as proposeCreateSkill, s as reviseSkillProposal, t as applySkillProposal } from "./service-CdkFj-Ch.js";
import { n as renderTable, t as getTerminalTableWidth } from "./table-xycldY5I.js";
import { t as resolveOptionFromCommand } from "./cli-utils-BWiGwzB-.js";
...
import { t as resolveClawHubRiskAcknowledgementCliOptions } from "./clawhub-risk-acknowledgement-6URLIbss.js";
...
async function loadSkillsStatusReport(options) {
	...
	const { buildWorkspaceSkillStatus } = await import("./status-2fBsHKki.js");
...
async function runSkillProposalApply(resolved, proposalId) {
	const { callGateway, isGatewayCredentialsRequiredError, isGatewayTransportError } = await import("./call-ujGvHIZc.js");
	try {
		await callGateway({
			config: resolved.config,
			method: "health",
		...
		const isOfflineCandidate = isGatewayCredentialsRequiredError(err) || isGatewayTransportError(err) && err.kind === "closed" && err.code === 1006;
		if (resolved.config.gateway?.mode === "remote" || !isOfflineCandidate) throw err;
		const { acquireGatewayLock } = await import("./gateway-lock-Db2v7iEo.js");
		let lock;
		try {
...
function registerSkillsCli(program) {
	...
	skills.command("search").description("Search ClawHub skills").argument("[query...]", "Optional search query").option("--limit <n>", "Max results", (value) => parseStrictPositiveIntOption(value, "--limit")).option("--json", "Output as JSON", false).action(async (queryParts, opts) => {
		try {
			...
			for (const entry of results) {
				...
				const skillsShInstallRef = installRef?.startsWith("skills-sh:") && entry.trustState === "not-scanned-by-clawhub" ? installRef : void 0;
				const skillRef = skillsShInstallRef ? formatClawHubSearchText(skillsShInstallRef) : ownerHandle ? `@${formatClawHubSearchText(ownerHandle)}/${slug}` : slug;
	...
	workshop.command("evaluate").description("Evaluate the exact current skill proposal through Gateway plugins").argument("<proposal-id>", "Skill proposal id").option("--correlation-id <id>", "External run or experiment correlation id").option("--json", "Output as JSON", false).action(async (proposalId, opts, command) => {
		try {
			const evaluated = await runSkillProposalEvaluate(resolveSkillsWorkspaceForCommand(command.parent, opts), proposalId, normalizeOptionalString(opts.correlationId));
			if (hasJsonOutput(opts)) {
	...
	workshop.command("apply").description("Apply a pending skill proposal").argument("<proposal-id>", "Skill proposal id").option("--json", "Output as JSON", false).action(async (proposalId, opts, command) => {
		try {
			const applied = await runSkillProposalApply(resolveSkillsWorkspaceForCommand(command.parent, opts), proposalId);
			if (hasJsonOutput(opts)) {
	...
	skills.command("info").description("Show detailed information about a skill").argument("<name>", "Skill name").option("--json", "Output as JSON", false).option("--agent <id>", "Target agent workspace (defaults to cwd-inferred, then default agent)").action(async (name, opts, command) => {
		await runSkillsAction((report) => formatSkillInfo(report, name, {
			...opts,
			json: hasJsonOutput(opts)
		}), { agentId: resolveAgentOption(command, opts) });
	});
