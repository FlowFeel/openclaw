...
import { t as measureCliCommandStartup } from "./command-startup-timing-Bx-rh4ao.js";
//#region src/cli/program/register.agent-turn.ts
async function loadAgentCliCommand() {
	return (await import("./agent-via-gateway-CgS4122P.js")).agentCliCommand;
}
async function loadAgentExecCommand() {
	return (await import("./agent-exec-ei8O1ZNx.js")).agentExecCommand;
}
function collectFallback(value, previous) {
	return [...previous, value];
}
async function loadDefaultRuntime() {
	return (await import("./runtime-BpncsYKr.js")).defaultRuntime;
}
async function loadRunCommandWithRuntime() {
	return (await import("./cli-utils-V6IH9hI4.js")).runCommandWithRuntime;
}
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openclaw.ai/cli/agent")}`).action(async (opts) => {
		...
		const [defaultRuntime, runCommandWithRuntime, setVerbose, agentCliCommand] = await measureCliCommandStartup("agent-action-imports", () => Promise.all([
			loadDefaultRuntime(),
		...
		["openclaw agent exec --message-file task.md --cwd ./repo", "Read a prompt file and set the workspace."],
		["openclaw agent exec \"Summarize this repo\" --model openai/gpt-5.6-sol --fallback anthropic/claude-sonnet-4-6 --json", "Use an explicit fallback chain and JSON output."],
		["openclaw agent exec \"Inspect this repo\" --model ollama/qwen3.5:9b --code-mode code --local-model-lean --json", "Force Code Mode with the lean local-model tool surface."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agent#agent-exec", "docs.openclaw.ai/cli/agent#agent-exec")}`).action(async (message, opts, command) => {
		...
		await runCommandWithRuntime(defaultRuntime, async () => {
			...
			if (result.exitCode !== 0) defaultRuntime.exit(result.exitCode, { resetStream: process.stderr });
		});
	});
}
//#endregion
