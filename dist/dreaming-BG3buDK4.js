...
import { C as resolveMemoryDreamingPluginConfig, T as resolveMemoryDreamingWorkspaces, _ as MANAGED_MEMORY_DREAMING_CRON_TAG, g as MANAGED_MEMORY_DREAMING_CRON_NAME, v as MEMORY_DREAMING_SYSTEM_EVENT_TEXT, x as resolveMemoryDeepDreamingConfig } from "./dreaming-CisvAHct.js";
import { l as peekSystemEventEntries } from "./system-events-BNZxjP0P.js";
...
import { n as normalizeTrimmedString, t as includesSystemEventToken } from "./dreaming-shared-QEKgks0k.js";
...
function isManagedDreamingJob(job) {
	if (normalizeTrimmedString(job.declarationKey) === MANAGED_DREAMING_DECLARATION_KEY) return true;
	if (normalizeTrimmedString(job.name) !== "Memory Dreaming Promotion") return false;
...
function resolveCronServiceFromCandidate(candidate) {
	...
	if (typeof cron.list !== "function" || typeof cron.add !== "function" || typeof cron.update !== "function" || typeof cron.remove !== "function" || typeof cron.removeStaleJobFamily !== "function") return null;
	return cron;
}
...
async function runShortTermDreamingPromotionIfTriggered(params) {
	...
	const [{ writeDeepDreamingReport }, { appendFallbackNarrativeEntry, runDreamNarrative }, { runDreamingSweepPhases }, { applyShortTermPromotions, repairShortTermPromotionArtifacts, rankShortTermPromotionCandidates }] = await Promise.all([
		import("./dreaming-markdown-BLxXkn1A.js"),
		import("./dreaming-narrative-BJHUjNxm.js"),
		import("./dreaming-phases-C3kxTael.js"),
		import("./short-term-promotion-BcSt0cw8.js")
	]);
...
function registerShortTermPromotionDreaming(api) {
	...
	const startDreamingSessionCleanup = async (config, generation, startupStartedAtMs) => {
		const { DREAMING_ORPHAN_MIN_AGE_MS, scrubDreamingNarrativeArtifacts } = await import("./dreaming-session-cleanup-D1PsrEcn.js");
