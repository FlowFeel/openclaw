...
import { a as normalizeLowercaseStringOrEmpty, o as normalizeNullableString } from "./string-coerce-DW4mBlAt.js";
...
import { r as formatErrorMessage$1, t as collectErrorGraphCandidates } from "./errors-Cg_yT1Sv.js";
import { r as fetchWithSsrFGuard } from "./fetch-guard-CE5RRhcq.js";
import { t as createDedupeCache } from "./dedupe-B4sBOca4.js";
import { g as readResponseTextLimited, m as readProviderJsonResponse } from "./provider-http-errors-Cj7zfu8U.js";
...
import { i as saveRemoteMedia, r as readRemoteMediaBuffer } from "./fetch-CNBCr8CY.js";
...
import { r as resolveChannelImplicitMentions, s as resolveStableChannelMessageIngress } from "./channel-ingress-runtime-Zqqr2Y7k.js";
...
var require_aura_cjs_production_min = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const e = BigInt("170141184475152167957503069145530368000");
	const r = BigInt("18446744073709551616");
	const a = BigInt("292277024400");
	function i(n) {
	...
	const h = (n) => y(4, 65535n, 65536n, 4294967295n, g, n);
	const y = (n, t, e, r, a, i) => {
		const o = b(n, t, e, a, i);
	...
	const x = (n) => w(4, 65535n, 65536n, 4294967295n, g, n);
	const w = (n, t, e, r, a, i) => {
		const o = B(n, t, e, a, i);
		return o < r ? o : B(n, t, e, a, o);
	};
	...
	const W = "0123456789abcdefghijklmnopqrstuv";
	function X(n, t, e) {
		let r = 0n;
		const a = BigInt(n);
...
var init_HttpProtocol = __esmMin((() => {
	...
	HttpProtocol = class extends SerdeContext {
		...
		async loadEventStreamCapability() {
			const { EventStreamSerde, eventStreamSerdeProvider } = await import("./event-streams-ShBRzt0o.js");
...
var import_dist_cjs$8, bindResolveAwsSdkSigV4Config;
...
var init_clock_skew_node_config = __esmMin((() => {
	...
	NODE_DISABLE_CLOCK_SKEW_CORRECTION_CONFIG_OPTIONS = {
		environmentVariableSelector: (env) => booleanSelector(env, ENV_DISABLE_CLOCK_SKEW_CORRECTION, SelectorType$1.ENV),
		configFileSelector: (profile) => booleanSelector(profile, CONFIG_DISABLE_CLOCK_SKEW_CORRECTION, SelectorType$1.CONFIG),
		default: false
	};
}));
//#endregion
...
Set AWS_CONTAINER_CREDENTIALS_FULL_URI or AWS_CONTAINER_CREDENTIALS_RELATIVE_URI.`, { logger: options.logger });
		...
		const provider = retryWrapper(async () => {
			...
			if (tokenFile) request.headers.Authorization = validateToken((await fs$1.readFile(tokenFile)).toString());
...
var import_dist_cjs$5, createEndpointRuleSetHttpAuthSchemeParametersProvider, _defaultSTSHttpAuthSchemeParametersProvider, defaultSTSHttpAuthSchemeParametersProvider, createEndpointRuleSetHttpAuthSchemeProvider, _defaultSTSHttpAuthSchemeProvider, defaultSTSHttpAuthSchemeProvider, resolveHttpAuthSchemeConfig$1;
		...
		const endpointRuleSetHttpAuthSchemeProvider = (authParameters) => {
			...
			for (const scheme of authSchemes) {
				...
				if (name === "sigv4a") {
					...
					if (import_dist_cjs$5.SignatureV4MultiRegion.sigv4aDependency() === "none" && sigv4Present) continue;
...
var import_dist_cjs$4, getRuntimeConfig$3;
var init_runtimeConfig_shared$1 = __esmMin((() => {
	...
	getRuntimeConfig$3 = (config) => {
		return {
			...
			sha256: config?.sha256 ?? Sha256Node,
			signerConstructor: config?.signerConstructor ?? import_dist_cjs$4.SignatureV4MultiRegion,
			urlParser: config?.urlParser ?? parseUrl,
			utf8Decoder: config?.utf8Decoder ?? fromUtf8,
			utf8Encoder: config?.utf8Encoder ?? toUtf8
		};
...
var import_dist_cjs$3, getRuntimeConfig$2;
var init_runtimeConfig$1 = __esmMin((() => {
	...
	init_retry();
	init_serde();
	import_dist_cjs$3 = require_dist_cjs$10();
	...
	getRuntimeConfig$2 = (config) => {
		...
		return {
			...
			requestHandler: import_dist_cjs$3.NodeHttpHandler.create(config?.requestHandler ?? defaultConfigProvider),
			retryMode: config?.retryMode ?? loadConfig({
				...NODE_RETRY_MODE_CONFIG_OPTIONS,
				default: async () => (await defaultConfigProvider()).retryMode || DEFAULT_RETRY_MODE
			}, config),
			sigv4aSigningRegionSet: config?.sigv4aSigningRegionSet ?? loadConfig(NODE_SIGV4A_CONFIG_OPTIONS, loaderConfig),
			streamCollector: config?.streamCollector ?? import_dist_cjs$3.streamCollector,
			useDualstackEndpoint: config?.useDualstackEndpoint ?? loadConfig(NODE_USE_DUALSTACK_ENDPOINT_CONFIG_OPTIONS, loaderConfig),
			useFipsEndpoint: config?.useFipsEndpoint ?? loadConfig(NODE_USE_FIPS_ENDPOINT_CONFIG_OPTIONS, loaderConfig),
			userAgentAppId: config?.userAgentAppId ?? loadConfig(NODE_APP_ID_CONFIG_OPTIONS, loaderConfig)
		};
...
var require_dist_cjs$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const ax = "argv";
	const ay = "backend";
	const az = "authSchemes";
	const aA = "disableDoubleEncoding";
	const aB = "signingName";
	const aC = "signingRegion";
	const aD = "signingRegionSet";
	const a = -1;
	const b = true;
	const c = false;
	const d = "isSet";
	const e = "booleanEquals";
	const f = "stringEquals";
	const g = "coalesce";
	const h = "substring";
	const i = "";
	const j = "aws.partition";
	const k = "partitionResult";
	const l = "accessPointSuffix";
	const m = "regionPrefix";
	const n = (n) => "outpostId_ssa_" + n + i;
	const o = "hardwareType";
	const p = "ite";
	const q = "isValidHostLabel";
	const s = "sigv4";
	const t = "aws.isVirtualHostableS3Bucket";
	const u = "url";
	const v = "getAttr";
	const w = "bucketArn";
	const x = "--";
	const y = "arnType";
	const z = "accesspoint";
	const A = (n) => "accessPointName_ssa_" + n + i;
	const B = "s3-object-lambda";
	const C = "s3-outposts";
	const D = "bucketPartition";
	const E = "us-east-1";
	const F = "outpostType";
	const G = "name";
	const H = "s3";
	const I = "{url#scheme}://{Bucket}.{url#authority}{url#path}";
	const J = "{url#scheme}://{url#authority}{url#path}";
	const K = "{url#scheme}://{url#authority}{url#normalizedPath}{Bucket}";
	const L = "https://{Bucket}.s3-accelerate.{partitionResult#dnsSuffix}";
	const M = "https://{Bucket}.s3.{partitionResult#dnsSuffix}";
	const N = (n) => "{url#scheme}://{accessPointName_ssa_" + n + "}-{bucketArn#accountId}.{url#authority}{url#path}";
	const O = (n) => "Invalid ARN: The access point name may only contain a-z, A-Z, 0-9 and `-`. Found: `{accessPointName_ssa_" + n + "}`";
	const P = "sigv4a";
	const Q = "{url#scheme}://{url#authority}{url#normalizedPath}{uri_encoded_bucket}";
	const R = "https://s3.{partitionResult#dnsSuffix}/{uri_encoded_bucket}";
	const S = "https://s3.{partitionResult#dnsSuffix}";
	const T = { [aw]: "UseFIPS" };
	const U = { [aw]: "UseDualStack" };
	const V = { [aw]: "Bucket" };
	const W = {
		"fn": v,
		[ax]: [{ [aw]: k }, G]
	...
	const Z = { [aw]: w };
	const aa = { [aw]: y };
	const ab = { [aw]: "accessPointName_ssa_1" };
	const ac = {
		"fn": v,
		[ax]: [Z, "region"]
	};
	...
	var packageInfo = { version: "3.1094.0" };
