...
import { Z as isSameOpenClawAgentDatabasePath, d as inspectOpenClawAgentDatabaseOwner, et as listOpenClawRegisteredAgentDatabases, nt as isIncognitoOpenClawAgentSqlitePath } from "./openclaw-agent-db-DemdumbZ.js";
import { lstatSync, readdirSync } from "node:fs";
import path from "node:path";
//#region src/config/sessions/session-sqlite-target.ts
function resolveRegisteredOwners(pathname, registeredDatabases) {
	return [...new Set(registeredDatabases.filter((entry) => isSameOpenClawAgentDatabasePath(entry.path, pathname)).map((entry) => normalizeAgentId(entry.agentId)))];
}
...
function resolveCustomStoreSqlitePath(params) {
	...
	const resolveSuffixedTarget = (ownerAgentId) => {
		...
		for (const registered of registeredDatabases) {
			if (!isSameOpenClawAgentDatabasePath(path.dirname(registered.path), sessionsDir)) continue;
