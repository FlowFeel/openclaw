...
import { I as stripEnvelope, L as stripMessageIdHints } from "./openclaw-state-db-D9eH245j.js";
...
import { it as resolveOpenClawAgentSqlitePath } from "./openclaw-agent-db-DemdumbZ.js";
import { E as materializeSessionArchiveForRead, S as parseUsageCountedSessionIdFromFileName, _ as isSessionArchiveArtifactName, b as isUsageCountedSessionTranscriptFileName, c as resolveSessionTranscriptsDirForAgent, h as isPrimarySessionTranscriptFileName, i as resolveSessionFilePath, r as resolveDefaultSessionStorePath, x as parseSessionArchiveTimestamp } from "./paths-DSnYpBD3.js";
import { Et as streamSessionTranscriptLines, P as normalizeUsage, st as loadSqliteSessionEntry } from "./session-accessor.sqlite-CtCo5VZ6.js";
...
import { b as scanSessionTranscriptTree, h as isSessionTranscriptLeafControl, m as isCanonicalSessionTranscriptEntry, p as selectVisibleTranscriptEvents } from "./session-transcript-index-NEbbnqMc.js";
import { D as readSqliteTranscriptStatsSync, v as loadSqliteTranscriptEventRowsAfterSeqSync, w as readSqliteTranscriptEventAtSeqSync, x as loadSqliteTranscriptEventsSync } from "./session-accessor.sqlite-transcript-store-BCfOcQgE.js";
...
import { a as resolveModelCostConfigFingerprint, i as resolveModelCostConfig, t as estimateUsageCost } from "./usage-format--pf0zzCz.js";
...
import { a as writeSessionCostUsageRollup, i as readSessionCostUsageRollupRows, n as deleteSessionCostUsageRollupsExcept, r as isSessionCostUsageRefreshRunning, t as acquireSessionCostUsageRefreshLock } from "./session-cost-usage-cache.sqlite-6Fy8xFzq.js";
...
function listUsageCountedSqliteTranscriptStats(agentId, params) {
	...
	for (const instance of listSessionTranscriptInstances({
		...
		const stats = readSqliteTranscriptStatsSync({
...
function loadSqliteUsageTranscriptEvents(marker) {
	return selectVisibleTranscriptEvents(loadSqliteTranscriptEventsSync({
		agentId: marker.agentId,
		sessionId: marker.sessionId,
		storePath: marker.storePath
	})).filter(isRecord);
...
function resolveExistingUsageSessionFile(params) {
	...
	if (target && completeTarget) {
		...
		const targetKeyEntry = loadSqliteSessionEntry({
	...
	const targetKeyAgentId = parseAgentSessionKey(target?.sessionKey)?.agentId;
	const targetKeyEntry = target?.sessionKey && sqliteMarker && !completeTarget ? loadSqliteSessionEntry({
...
async function scanSqliteUsageRollup(params) {
	...
	const snapshotLastRow = maxSeq > 0 ? readSqliteTranscriptEventAtSeqSync(scope, maxSeq) : void 0;
	...
	const previousAnchor = previousCheckpoint?.maxSeq ? readSqliteTranscriptEventAtSeqSync(scope, previousCheckpoint.maxSeq) : void 0;
	...
	const rows = loadSqliteTranscriptEventRowsAfterSeqSync(scope, appendCandidate ? previousCheckpoint?.maxSeq ?? 0 : 0, maxSeq);
	...
	const allRows = appendOnly ? rows : loadSqliteTranscriptEventRowsAfterSeqSync(scope, 0, maxSeq);
	...
	const currentLastRow = maxSeq > 0 ? readSqliteTranscriptEventAtSeqSync(scope, maxSeq) : void 0;
...
//#endregion
export { loadCostUsageSummary as a, resolveExistingUsageSessionFile as c, loadSessionUsageTimeSeries as i, loadSessionCostSummary as n, loadCostUsageSummaryFromCache as o, loadSessionLogs as r, loadSessionCostSummariesFromCache as s, discoverAllSessions as t };
