...
import { c as isGatewayWorkAdmissionClosed, l as markGatewayRestartDraining, s as isGatewaySubordinateWorkAdmissionClosed, t as GatewayDrainingError, u as resetGatewayWorkAdmission } from "./gateway-work-admission-D_DdbtmL.js";
import { i as logLaneEnqueue, r as logLaneDequeue, t as diagnosticLogger } from "./diagnostic-runtime-wzkpciZD.js";
...
const DEFAULT_STARVATION_CEILING_MS = 15e3;
function resolveBasePriorityRank(priority) {
	if (typeof priority === "number") {
		if (!Number.isFinite(priority)) return 1;
		return Math.min(3, Math.max(0, Math.floor(priority)));
	}
	switch (priority) {
		case "steering": return 3;
...
*/
function calculateEffectivePriority(entry, currentTimeMs, defaultCeilingMs = DEFAULT_STARVATION_CEILING_MS) {
	const basePriority = typeof entry.priority === "number" && Number.isFinite(entry.priority) ? Math.min(3, Math.max(0, Math.floor(entry.priority))) : 1;
	const ceilingMs = entry.starvationCeilingMs ?? defaultCeilingMs;
	if (!Number.isFinite(ceilingMs) || ceilingMs <= 0) return basePriority;
	const waitMs = Math.max(0, currentTimeMs - entry.enqueuedAt);
	const promotions = Math.floor(waitMs / ceilingMs);
	return Math.min(3, basePriority + promotions);
}
/**
* Pure comparator for queue entries.
* Returns negative if `a` should execute before `b`, positive if `b` before `a`, or 0 if equal.
*/
function compareQueueEntries(a, b, currentTimeMs, defaultCeilingMs = DEFAULT_STARVATION_CEILING_MS) {
...
*/
function pickNextQueueEntry(queue, currentTimeMs, defaultCeilingMs = DEFAULT_STARVATION_CEILING_MS) {
	if (queue.length === 0) return;
	if (queue.length === 1) return queue.shift();
	let bestIndex = 0;
	for (let i = 1; i < queue.length; i++) if (compareQueueEntries(queue[i], queue[bestIndex], currentTimeMs, defaultCeilingMs) < 0) bestIndex = i;
...
function enqueueLaneEntry(state, entry) {
	const now = Date.now();
	const insertAt = state.queue.findIndex((queued) => compareQueueEntries(entry, queued, now) < 0);
	...
	const pump = () => {
		try {
			while (state.activeTaskIds.size < state.maxConcurrent && state.queue.length > 0 && canAdmitInGroup(lane)) {
				const entry = pickNextQueueEntry(state.queue, Date.now());
				if (!entry) break;
