...
import { a as redactSensitiveFieldValue, u as redactToolPayloadText } from "./redact-BGqyyKN9.js";
import { o as resolveRequiredHomeDir } from "./home-dir-Cs7bTrwJ.js";
...
import { i as GPT5_HEARTBEAT_PROMPT_OVERLAY } from "./gpt5-prompt-overlay-BxdgIpPx.js";
...
import { c as isActiveHarnessContextEngine } from "./agent-end-side-effects--ABMNPXa.js";
import { s as isHostScopedAgentToolActive } from "./local-model-lean-CeG7_aMD.js";
import { t as log } from "./logger-PoK3sCV7.js";
import { i as buildSkillWorkshopPromptSection, r as SKILL_WORKSHOP_TOOL_NAME } from "./watched-sessions-prompt-BpQzHAn1.js";
import "./string-coerce-runtime-CLK2YdzD.js";
import { n as buildCodexUserMcpServersThreadConfigPatchForRuntime } from "./bundle-mcp-codex-CGkNi8LN.js";
import "./codex-mcp-projection-DwJ3zhC6.js";
...
import { F as resolveCodexPluginsPolicy, R as CODEX_PLUGINS_MARKETPLACE_NAME, b as codexSandboxPolicyForTurn, c as hashCodexAppServerBindingFingerprint, l as isCodexAppServerNativeAuthProfile, m as reclaimCurrentCodexSessionGeneration, o as createCodexSessionGenerationSupersededError, r as assertCodexBindingMayBeReplaced, u as normalizeCodexAppServerBindingModelProvider, v as sessionBindingIdentity, z as CODEX_PLUGINS_WORKSPACE_MARKETPLACE_NAME } from "./session-binding-Boew56yn.js";
import { G as getCodexAppServerClientInstanceId, J as isCodexAppServerConnectionClosedError, W as CodexAppServerRpcError, at as isJsonObject, b as consumeCodexAppServerLiveThread, et as resolveCodexAppServerClientInstanceId, it as flattenCodexDynamicToolFunctions, l as isCodexAppServerStartSelectionChangedError } from "./shared-client-CDGOYFn5.js";
import { a as closeCodexStartupClientBestEffort, c as isCodexAppServerUnsafeSubscriptionError, d as unsubscribeCodexThreadBestEffort, i as CodexAppServerUnsafeSubscriptionError, r as CODEX_APP_SERVER_UNSUBSCRIBE_TIMEOUT_MS, t as resumeCodexAppServerThread } from "./thread-resume-DATryQ9G.js";
import { a as assertCodexThreadStartResponse, r as assertCodexThreadForkResponse } from "./protocol-validators-Cf6MSlge.js";
import { a as serializeCodexAppInventoryError, i as defaultCodexAppInventoryCache, t as buildCodexAppServerConnectionFingerprint } from "./plugin-app-cache-key-B8whbfSH.js";
import "./incognito-session-DsfNYMyp.js";
import { a as projectBoundedCodexThreadHistory } from "./transcript-mirror-CcsTV4P4.js";
...
async function readCodexPluginInventory(params) {
	...
	for (const pluginPolicy of policy.pluginPolicies) {
		...
		let resolvedPlugin = pluginPolicy.marketplaceName === "workspace-directory" ? findWorkspaceMarketplacePlugin(listed, pluginPolicy.pluginName) : findOpenAiCuratedMarketplacePlugin(listed, pluginPolicy.pluginName);
		if (!resolvedPlugin && pluginPolicy.enabled && pluginPolicy.marketplaceName === "openai-curated") {
		...
		if (!(pluginPolicy.marketplaceName === "workspace-directory" ? listed.marketplaces.some((entry) => entry.name === "workspace-directory") : listed.marketplaces.some(isOpenAiCuratedMarketplace))) {
			diagnostics.push({
...
async function listCodexPluginMetadata(params) {
	const requestParams = {};
...
async function readInstalledCodexPluginMetadata(params) {
	...
	return (await params.metadataCache.load({
		...
		cacheable: (response) => params.policy.pluginPolicies.every((pluginPolicy) => {
			...
			return pluginPolicy.marketplaceName === "workspace-directory" ? findWorkspaceMarketplacePlugin(response, pluginPolicy.pluginName) !== void 0 : findOpenAiCuratedMarketplacePlugin(response, pluginPolicy.pluginName) !== void 0;
		})
	})).response;
}
function isSettledMissingPluginPolicy(params) {
	...
	const requestParams = queryKind === "installed" && params.configCwd ? { cwds: [params.configCwd] } : {};
	const listed = params.metadataCache.read(params.appCacheKey, queryKind, requestParams)?.response;
	if (!listed) return false;
	if (queryKind === "installed") return !findWorkspaceMarketplacePlugin(listed, params.pluginPolicy.pluginName);
...
function isOpenAiCuratedMarketplace(marketplace) {
	return marketplace.name === "openai-curated" || marketplace.name === CODEX_PLUGINS_REMOTE_MARKETPLACE_NAME || marketplace.name === CODEX_PLUGINS_API_MARKETPLACE_NAME;
}
//#endregion
...
async function listCuratedCodexPluginMetadata(params, options = {}) {
	const requestParams = options.forceRefetch ? { forceRefetch: true } : {};
	if (!params.metadataCache || !params.appCacheKey) return await params.request("plugin/list", requestParams);
	return (await params.metadataCache.load({
		appCacheKey: params.appCacheKey,
		queryKind: "curated-global",
...
async function buildCodexPluginThreadConfig(params) {
	...
	const unresolvedDisabledPluginOwnership = policy.allowAllPlugins ? policy.pluginPolicies.find((pluginPolicy) => !pluginPolicy.enabled && !inventory.records.some((record) => record.policy.configKey === pluginPolicy.configKey && record.detail)) : void 0;
...
function fingerprintCodexThreadConfig(request, authProfileId, dynamicToolsFingerprint) {
	return hashCodexAppServerBindingFingerprint(fingerprintJsonObject({
		...
		requestedModel: request.requestedModel === void 0 ? request.model ?? null : request.requestedModel,
		...
		serviceTier: request.serviceTier === void 0 ? "<omitted>" : request.serviceTier,
...
function buildDeveloperInstructions(params, options = {}) {
	...
	return [
		...
		deferredToolNames.size > 0 ? `Deferred searchable OpenClaw dynamic tools available: ${[...deferredToolNames].toSorted((left, right) => left.localeCompare(right)).join(", ")}. Use \`tool_search\` to load exact callable specs before use.` : void 0,
		hasSkillWorkshop ? buildSkillWorkshopPromptSection().join("\n") : void 0,
		nativeDelegationAvailable ? `Use Codex native \`spawn_agent\` for Codex subagents. \`spawn_agent\` and the other native collaboration tools may be deferred: when \`spawn_agent\` is not directly listed, load it with \`tool_search\` before spawning.${hasSessionsSpawn ? " Use OpenClaw `sessions_spawn` only for OpenClaw or ACP delegation, never as a substitute for `spawn_agent`." : ""}` : void 0,
...
function buildCodexRuntimeThreadConfigForRun(params, config, options = {}) {
	...
	if (restrictedToolSurface && configMcpServers !== void 0 && !isJsonObject(configMcpServers)) throw new Error("Codex ring-zero received invalid thread mcp_servers config");
	...
	const runtimeConfig = mergeCodexThreadConfigs(baseConfig, options.appServer?.networkProxy?.configPatch, shouldDisableCodexToolSearchForModel(params.modelId) ? CODEX_TOOL_SEARCH_UNSUPPORTED_THREAD_CONFIG : void 0, params.delegationCapability === "report_only" ? CODEX_DELEGATION_DISABLED_THREAD_CONFIG : void 0, messageOnlySourceReply ? buildCodexRestrictedToolThreadConfigPatch(ringZeroMcpServerNames) : buildCodexRingZeroThreadConfigPatch(params, options.hostSystemAgentActive, ringZeroMcpServerNames)) ?? baseConfig;
...
async function resumeExistingCodexThread(params, context) {
	...
	try {
		...
		if (ringZeroActive || isMessageOnlyCodexSourceReply(params.params)) try {
...
async function startFreshCodexThread(params, context) {
	...
	const { bindingIdentity, startModelSelection, startModelProvider, userMcpServersConfigPatch, dynamicToolsFingerprint, dynamicToolsContainDeferred, webSearchThreadConfigFingerprint, nativeSkillIsolationFingerprint, userMcpServersFingerprint, ringZeroConfigFingerprint, ringZeroClientInstanceId, networkProxyConfigFingerprint, contextEngineBinding, environmentSelectionFingerprint, hostSystemAgentActive, ringZeroActive, ringZeroInheritedMcpServerNames, nativeSkillIsolation, lifecycleTiming, normalizeBindingModelProvider, throwIfAborted, prebuiltPluginThreadConfig, preserveExistingBinding, rotatedContextEngineBinding } = context;
...
async function prepareCodexThreadLifecyclePreflight(params) {
	...
	const ringZeroInheritedMcpServerNames = restrictedToolSurface ? await lifecycleTiming.measure("ring-zero-mcp-config-read", () => readCodexInheritedMcpServerNames(params.client, params.cwd, params.signal)) : [];
	if (restrictedToolSurface) await lifecycleTiming.measure("ring-zero-config-requirements-read", () => assertCodexRingZeroHasNoManagedHooks(params.client, params.signal));
...
async function tryReuseCodexLiveThread(options) {
	const { params, binding, bindingIdentity, clientId, contextEngineBinding, dynamicToolsFingerprint, hostSystemAgentActive, lifecycleTiming, nativeSkillIsolation, releaseConsumedThread, ringZeroActive, ringZeroInheritedMcpServerNames, startModelProvider, startModelSelection, throwIfAborted, userMcpServersConfigPatch } = options;
	if (!binding.clientId || binding.clientId !== clientId || binding.preserveNativeModel === true || binding.connectionScope === "supervision" || ringZeroActive || contextEngineBinding) return {};
	...
	if (!await consumeCodexAppServerLiveThread(params.client, binding.threadId, liveThreadConfigFingerprint)) return { prebuiltFinalConfigPatch };
	try {
...
async function startOrResumeThread(params) {
	...
	return await params.bindingStore.withLease(bindingIdentity, async () => {
		...
		const releaseRetainedThread = (threadId) => releaseCodexRetainedLiveThread({
			client: params.client,
		...
		if (initialBoundThreadId) await releaseRetainedThread(initialBoundThreadId);
...
function buildTurnScopedCollaborationInstructions(params, options = {}) {
	...
	if (params.trigger === "heartbeat" && params.bootstrapContextRunKind !== "commitment-only") return joinPresentSections(buildHeartbeatCollaborationInstructions(), contextInstructions);
...
//#endregion
export { resolveCodexDynamicToolsLoadingForRuntime as A, buildCodexPluginAppsConfigPatchFromPolicyContext as B, readCodexSupportedReasoningEfforts as C, isSystemAgentOnlyCodexDynamicToolAllowlist as D, isForcedPrivateQaCodexRuntime as E, isContextEngineBindingCompatible as F, shouldBuildCodexPluginThreadConfig as G, buildCodexPluginThreadConfigInputFingerprint as H, fitCodexProjectedContextForTurnStart as I, pluginReadParams as J, ensureCodexPluginActivation as K, projectContextEngineAssemblyForCodex as L, codexDynamicToolsFingerprint as M, codexLegacyDynamicToolsFingerprint as N, normalizeCodexDynamicToolName as O, buildContextEngineBinding as P, resolveCodexContextEngineProjectionMaxChars as R, resolveReasoningEffort as S, filterCodexDynamicToolsWithOpenClawShell as T, buildCodexPluginThreadConfigTimeoutFallback as U, buildCodexPluginThreadConfig as V, mergeCodexThreadConfigs as W, resolveRecoverableCodexPluginConfigKeys as Y, CODEX_NATIVE_PERSONALITY_NONE as _, sanitizeCodexHistoryImagePayloads as a, resolveCodexAppServerThreadModelSelection as b, isCodexAppServerProfilerEnabled as c, buildCodexRingZeroThreadConfigPatch as d, buildCodexRuntimeThreadConfig as f, buildDeveloperInstructions as g, resolveCodexWebSearchPlan as h, invalidInlineImageText as i, areCodexDynamicToolFingerprintsCompatible as j, resolveCodexDynamicToolsLoading as k, assertCodexRingZeroHasNoManagedHooks as l, buildCodexNativeWebSearchThreadConfig as m, buildTurnStartParams as n, sanitizeInlineImageDataUrl as o, readCodexInheritedMcpServerNames as p, isOpenAiCuratedMarketplace as q, buildCodexUserInput as r, startOrResumeThread as s, buildTurnCollaborationMode as t, attestCodexRingZeroThreadHasNoMcpServers as u, resolveCodexAppServerModelProvider as v, filterCodexDynamicTools as w, resolveCodexBindingModelProviderFallback as x, resolveCodexAppServerRequestModelSelection as y, resolveCodexContextEngineProjectionReserveTokens as z };
