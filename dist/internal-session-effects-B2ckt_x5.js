...
import { Ct as buildSessionCreationStamp, L as applySqliteSessionEntryLifecycleMutation, Nt as resolveInternalSessionEffectsIdentity, at as loadExactSqliteSessionEntry, c as replaceSqliteTranscriptEvents, vt as upsertSqliteSessionEntry } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { p as createSessionTranscriptHeader } from "./session-accessor.sqlite-transcript-store-BCfOcQgE.js";
import { V as forkSessionFromParentTranscript } from "./session-accessor-D5Or7WgI.js";
...
async function prepareInternalSessionEffectsSession(params) {
	...
	const existing = loadExactSqliteSessionEntry(scope)?.entry;
	...
	}) : void 0)?.status !== "created") await replaceSqliteTranscriptEvents(scope, [createSessionTranscriptHeader({
		cwd: params.cwd,
		sessionId: scope.sessionId
	})]);
	const now = Date.now();
	const entry = await upsertSqliteSessionEntry(scope, {
