...
import { a as getLogger } from "./logger-Cv9z4NYi.js";
...
import { B as purgeSqliteDeletedAgentSessionEntries, L as applySqliteSessionEntryLifecycleMutation, bt as enforceSqliteSessionHistoryDiskBudget, xt as inspectSqliteSessionHistoryDiskBudget } from "./session-accessor.sqlite-CtCo5VZ6.js";
...
import { B as resolveSessionArtifactCanonicalPathsForEntry, G as pruneStaleEntries, H as capEntryCount, K as pruneStaleModelRunEntries, V as resolveMaintenanceConfig, X as shouldRunModelRunPrune, Y as shouldPreserveMaintenanceEntry, ot as collectSessionMaintenancePreserveKeysForStore, z as pruneUnreferencedSessionArtifacts } from "./session-entry-slot-keys-DPRQmSpa.js";
...
function sqliteTranscriptHasMessageRecords(params) {
	try {
		return loadSqliteTranscriptEventsSync(params).some(isTranscriptMessageRecord);
	} catch {
		return false;
	}
}
...
async function purgeAgentSessionStoreEntries(cfg, agentId) {
	try {
		...
		await purgeSqliteDeletedAgentSessionEntries({
			cfg,
