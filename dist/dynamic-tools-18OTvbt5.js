...
import { B as finalizeToolTerminalPresentation, J as consumePreExecutionBlockedToolCall, h as runBeforeToolCallHook, q as consumeAdjustedParamsForToolCall, r as getBeforeToolCallFailureDisposition, u as wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call-QIXc-Jm8.js";
import { c as resolveToolExecutionErrorKind, l as resolveToolResultFailureKind, n as isToolResultError, t as formatToolExecutionErrorMessage } from "./tool-result-error-D4i-Z2FR.js";
import { s as getMediaDir, u as saveMediaBuffer } from "./store-BDR50q7S.js";
import { o as resolveModelAuthMode } from "./model-auth-D32HIbZ7.js";
import { n as isToolAllowed } from "./tool-policy-CnoAKsDO.js";
...
import { b as isToolWrappedWithBeforeToolCallHook, m as getChannelAgentToolMeta, t as callGatewayTool, x as setBeforeToolCallDiagnosticsEnabled } from "./gateway-CRcKH8Wu.js";
import { r as isReplaySafeToolCall } from "./tool-mutation-BmnNoynG.js";
...
import { a as isMessagingToolSendAction, r as isMessagingTool } from "./embedded-agent-messaging-BHU1Makt.js";
import { a as extractMessagingToolSend, f as filterToolResultMediaUrls, h as sanitizeToolResult, o as extractMessagingToolSendResult, u as extractToolResultMediaArtifact } from "./embedded-agent-subscribe.tools-D7tyhgah.js";
import { n as isDeliveredMessageToolOnlySourceReplyResult, r as isDeliveredMessagingToolResult } from "./embedded-agent-message-tool-source-reply-DOQB4HRp.js";
import { a as supportsModelTools } from "./openclaw-tools-dPoXkP5n.js";
import { t as runAgentHarnessAfterToolCallHook } from "./hook-helpers-BQ43hdap.js";
import { n as normalizeAgentRuntimeTools } from "./tools-CnfMpH7Q.js";
...
import { r as resolveAttemptSpawnWorkspaceDir } from "./attempt.thread-helpers-CM0JLs3S.js";
import { i as buildEmbeddedAttemptToolRunContext } from "./settled-turn-finalization-result-CnDUUCjr.js";
...
import { f as createCodexAppServerToolResultExtensionRunner } from "./agent-harness-runtime-C6lLw_1r.js";
import { a as resolveNativeHookRelayDeferredToolApproval, n as invokeNativeHookRelay, r as registerNativeHookRelay, t as hasNativeHookRelayInvocation } from "./native-hook-relay-C_lKX44y.js";
...
import { P as readCodexPluginConfig } from "./session-binding-Dl6kaOMx.js";
import { L as resolveCodexGatewayTimeoutWithGraceMs, at as isJsonObject, d as releaseLeasedSharedCodexAppServerClient, rt as CODEX_OPENCLAW_DIRECT_DYNAMIC_TOOL_NAMESPACE } from "./shared-client-D2F1gOgp.js";
import { E as itemStatus, F as readItem, H as readString$2, _ as readCodexNotificationTurnId, b as auditNativeToolUnfinishedStatus, g as readCodexNotificationThreadId, v as auditNativeToolName, y as auditNativeToolTerminalStatus } from "./thread-resume-BTrJDDVj.js";
import { l as readCodexTurn } from "./protocol-validators-Cf6MSlge.js";
import { D as isSystemAgentOnlyCodexDynamicToolAllowlist, E as isForcedPrivateQaCodexRuntime, O as normalizeCodexDynamicToolName, T as filterCodexDynamicToolsWithOpenClawShell, h as resolveCodexWebSearchPlan, i as invalidInlineImageText, o as sanitizeInlineImageDataUrl, w as filterCodexDynamicTools } from "./thread-lifecycle-nCTQ_rzK.js";
import { r as formatCodexDisplayText } from "./command-formatters-ByZjvAJJ.js";
import { a as resolveCodexNodeExecToolOverrides, i as resolveCodexNativeExecutionPolicy } from "./sandbox-guard-gphn8P0A.js";
...
var CodexNativeToolLifecycleProjector = class {
	...
	recordRawWebSearchResult(item) {
		if (readString$2(item, "type") !== "web_search_call") return;
		const toolCallId = readString$2(item, "id");
		if (!toolCallId || this.completedItemIds.has(toolCallId)) return;
		const toolName = "web_search";
		this.recordStarted(toolCallId, toolName, "unknown");
		const rawStatus = readString$2(item, "status");
...
async function waitForPluginApprovalDecision(params) {
	const waitPromise = callGatewayTool("plugin.approval.waitDecision", { timeoutMs: resolveCodexGatewayTimeoutWithGraceMs(DEFAULT_CODEX_APPROVAL_TIMEOUT_MS) }, { id: params.approvalId }).catch((error) => {
		if (isApprovalNotFoundError(error)) return null;
		throw error;
	});
	const bindDecision = (result) => result === null ? null : result?.id === params.approvalId ? result.decision : void 0;
...
async function handleCodexAppServerApprovalRequest(params) {
	...
	try {
		...
		if (canAutoApproveConcreteToolCall && params.autoApproveOpenClawToolPolicy === true && policyOutcome?.outcome === "allowed") {
			emitApprovalEvent(params.paramsForRun, {
				phase: "resolved",
				kind: context.kind,
				status: "approved",
...
async function runOpenClawToolPolicyForApprovalRequest(params) {
	...
	const outcome = await runBeforeToolCallHook({
		...
		ctx: {
			...
			turnSourceChannel: params.paramsForRun.messageChannel ?? params.paramsForRun.messageProvider,
			turnSourceTo: params.paramsForRun.currentMessagingTarget ?? params.paramsForRun.currentChannelId,
			turnSourceAccountId: params.paramsForRun.agentAccountId,
			turnSourceThreadId: params.paramsForRun.currentThreadTs
		}
	});
...
//#endregion
export { createCodexNativeHookRelay as A, resolveTerminalDynamicToolBatchAction as B, filterToolsForVisionInputs as C, CODEX_NATIVE_HOOK_RELAY_TTL_GRACE_MS as D, CODEX_NATIVE_HOOK_RELAY_EVENTS as E, handleDynamicToolCallWithTimeout as F, resolveCodexToolAbortTerminalReason as G, shouldReleaseTurnAfterTerminalDynamicTool as H, hasPendingDynamicToolTerminalDiagnostic as I, isDynamicToolTerminalDiagnosticEvent as L, resolveCodexNativeHookRelayEvents as M, resolveCodexNativeHookRelayTtlMs as N, buildCodexNativeHookRelayConfig as O, scheduleCodexNativeHookRelayUnregister as P, isMatchingDynamicToolTerminalDiagnostic as R, shouldWarnCodexDynamicToolBuildStageSummary as S, CodexNativeToolLifecycleProjector as T, toCodexDynamicToolProgressResponse as U, shouldBlockTerminalReleaseForNonTerminalDynamicToolResult as V, toCodexDynamicToolProtocolResponse as W, resolveCodexExternalSandboxPolicyForOpenClawSandbox as _, emitDynamicToolStartedDiagnostic as a, shouldEnableCodexAppServerNativeToolSurface as b, resolveCodexProviderWebSearchSupportForClient as c, buildDynamicTools as d, createCodexDynamicToolBuildStageTracker as f, resolveCodexAppServerHookChannelId as g, resolveCodexAppServerExecutionCwd as h, emitDynamicToolErrorDiagnostic as i, emitCodexNativePreToolUseFailureDiagnostic as j, buildCodexNativeHookRelayDisabledConfig as k, buildCodexHookRequester as l, formatCodexDynamicToolBuildStageSummary as m, handleCodexAppServerApprovalRequest as n, emitDynamicToolTerminalDiagnostic as o, disableCodexPluginThreadConfig as p, handleCodexAppServerElicitationRequest as r, resolveCodexProviderWebSearchSupport as s, createCodexDynamicToolBridge as t, settleCodexSourceReplyFinality as u, resolveCodexMessageToolProvider as v, readBoundedCodexRemoteWorkspaceFile as w, shouldRequireCodexSandboxExecServerEnvironment as x, resolveCodexSandboxEnvironmentSelection as y, resolveDynamicToolCallTimeoutMs as z };
