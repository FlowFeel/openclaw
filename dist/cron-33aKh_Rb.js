...
import { c as resolveCronJobsStorePath, m as loadCronQuarantinedJobs } from "./store-BwY4XtES.js";
import { t as listReadOnlyChannelPluginsForConfig } from "./read-only-CG09Owk1.js";
import { n as resolveCronDeliveryPlan } from "./delivery-plan-CCXNSnwf.js";
import { t as note } from "./note-uiKX_qfX.js";
import { r as normalizeStoredCronJobs } from "./store-migration-DtxGNOYP.js";
import { c as formatUnresolvedCommandPromptAdvisory, l as formatUnresolvedShellPromptAdvisory, o as formatLegacyIssuePreview, r as loadLegacyCronRepairState, s as formatScheduledToolPolicyAdvisory, t as applyLegacyCronStoreRepair, u as countStaleDreamingJobs } from "./legacy-repair-B5vYy2zQ.js";
...
async function collectLegacyCronStoreHealthFindings(params) {
	...
	if (!state) return [];
	const findings = [];
	const { storePath, legacyStoreDetected, legacyRunLogDetected, legacyQuarantine, legacyImportCount, sqliteProjectionBackfillCount, rawJobs } = state;
...
async function maybeRepairLegacyCronStore(params) {
	...
	if (!state) return;
	const { storePath, legacyStoreDetected, legacyRunLogDetected, legacyQuarantine, legacyImportCount, sqliteProjectionBackfillCount, invalidConfigRows, rawJobs } = state;
	const sqliteStorePath = resolveOpenClawStateSqlitePath();
	try {
		const quarantine = loadCronQuarantinedJobs(storePath);
