...
function registerClawsCli(program) {
	...
	claws.command("export").description("Export portable state for one installed Claw agent").argument("<agent>", "Final id of the installed Claw agent").requiredOption("--out <path>", "New package directory to create").option("--json", "Print JSON", false).action(async (agent, opts) => {
