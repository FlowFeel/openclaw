...
import { n as extractErrorCode, t as collectErrorGraphCandidates } from "./errors-Cg_yT1Sv.js";
...
import { g as readResponseTextLimited } from "./provider-http-errors-Cj7zfu8U.js";
import "./error-runtime-Dbl9_3mW.js";
import { n as withTrustedEnvProxyGuardedFetchMode } from "./fetch-runtime-C6dvgZVF.js";
...
import { t as trimNonEmptyString } from "./openai-chatgpt-shared-BPBV8jKe.js";
import { t as resolveCodexAccessTokenExpiry } from "./openai-chatgpt-auth-identity-DtFnVVxz.js";
...
async function readOpenAICodexDeviceBody(response, timeoutMs) {
	return await readResponseTextLimited(response, response.ok ? OPENAI_CODEX_DEVICE_JSON_BODY_LIMIT_BYTES : OPENAI_CODEX_DEVICE_ERROR_BODY_LIMIT_BYTES, { chunkTimeoutMs: timeoutMs });
}
...
async function pollOpenAICodexDeviceCode(params) {
	...
	while (Date.now() < deadline) {
		...
		try {
			...
			if (!collectErrorGraphCandidates(error, (candidate) => [candidate.cause]).some((candidate) => classifyTransientNetworkErrorCode(extractErrorCode(candidate)) !== void 0)) throw error;
