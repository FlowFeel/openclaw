...
import { c as buildSummaryChunks, f as projectCompactionMessagesForPlanning, l as computeAdaptiveChunkRatio, m as readCompactionPlanningOmittedChars, o as buildOversizedFallbackPlan, p as sanitizeCompactionMessages, s as buildStageSplitPlan } from "./compaction-planning-CEkjkWXf.js";
...
function estimateMessagesBytes(messages) {
	...
	for (const msg of messages) {
		const rawContent = "content" in msg ? msg.content : void 0;
		if (typeof rawContent === "string") total += rawContent.length;
		else if (Array.isArray(rawContent)) {
			for (const block of rawContent) if (block && typeof block === "object") {
				const record = block;
