...
function resolveDiagnosticLivenessRecordLevel(event) {
	...
	return event.degradedSinceMs !== void 0 || hasBlockingWork || event.active > 0 && hasSustainedEventLoopDelay ? "warning" : "info";
}
...
function listRecords() {
	...
	if (state.count < state.capacity) return state.records.slice(0, state.count).filter((record) => record !== void 0);
	return [...state.records.slice(state.nextIndex), ...state.records.slice(0, state.nextIndex)].filter((record) => record !== void 0);
}
