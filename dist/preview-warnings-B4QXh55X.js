...
import { n as listAgentEntries, o as resolveAgentConfig } from "./agent-scope-config-Dusa8eSA.js";
...
import { n as isToolAllowedByPolicies, r as isToolAllowedByPolicyName } from "./tool-policy-match-BJgxicXr.js";
...
import { t as resolveDoctorPrimaryModelRef } from "./primary-model-ref-DL0gOHrp.js";
...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-ByaFIyYL.js"));
function loadChannelDoctorModule() {
	return channelDoctorModuleLoader.load();
}
function listAgentRecords(cfg) {
	return listAgentEntries(cfg).filter(isRecord);
}
...
async function resolveDoctorChannelPreviewConfig(params) {
	const [{ resolveCommandSecretRefsViaGateway }, { getConfiguredChannelsCommandSecretTargetIds }] = await Promise.all([import("./command-secret-gateway-DTPTBPhr.js"), import("./command-secret-targets-C8JL7Zgq.js")]);
...
async function collectDoctorPreviewNotes(params) {
	...
	warnings.push(...await collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env
	}));
	const channelPluginRuntime = await import("./channel-plugin-blockers-DuYdZ2MA.js");
	...
	if (hasChannelConfig) {
		...
		const { collectOpenPolicyAllowFromWarnings, maybeRepairOpenPolicyAllowFrom } = await import("./open-policy-allowfrom-D0sbVEMG.js");
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-DylXXlsK.js");
	...
	const { collectCodexRouteWarnings } = await import("./codex-route-warnings-C6noilE7.js");
	...
	if (hasPluginConfig) {
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-C04cPOzI.js");
	...
	if (hasSubagentAllowlistConfig(params.cfg)) {
		const { collectStaleSubagentAllowlistWarnings, scanStaleSubagentAllowlistReferences } = await import("./stale-subagent-allowlist-BSpsky_g.js");
	...
	const { collectCodexNativeAssetInfoNotes } = await import("./codex-native-assets-D3uBFlTA.js");
	infoNotes.push(...await collectCodexNativeAssetInfoNotes({
		cfg: params.cfg,
		env
	}));
	if (hasPluginLoadPaths(params.cfg)) {
		const { collectBundledPluginLoadPathWarnings, scanBundledPluginLoadPathMigrations } = await import("./bundled-plugin-load-paths-DyyGIars.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-B1JzJfqw.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-_JQeCTL_.js");
	...
	if (hasConfiguredSafeBins(params.cfg)) {
		const { collectExecSafeBinCoverageWarnings, collectExecSafeBinTrustedDirHintWarnings, scanExecSafeBinCoverage, scanExecSafeBinTrustedDirHints } = await import("./exec-safe-bins-BKoZkyMq.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-D3Q9vk1q.js");
	...
	const { collectStaleConfiguredAuthOrderWarnings } = await import("./stale-auth-order-D8DZqIuL.js");
