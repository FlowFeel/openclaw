...
import { i as formatLegacyAgentMediaMigrationRequiredMessage, t as GATEWAY_AGENT_MEDIA_MIGRATION_REQUIRED_REASON } from "./openclaw-agent-db-migration-required-BU5hc3wx.js";
...
function repairGatewayAgentMediaMigrationStartupFailures(params) {
	...
	try {
		return runOpenClawStateWriteTransaction(({ db }) => {
			const kysely = getNodeSqliteKysely(db);
			const legacyMessages = [...new Set(params.databasePaths.flatMap((pathname) => Array.from({ length: 16 }, (_, schemaVersion) => {
