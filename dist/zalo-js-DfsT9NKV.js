...
import { i as loadStoredZaloCredentials, n as clearStoredZaloCredentials, s as refreshStoredZaloCredentials, u as saveStoredZaloCredentials } from "./session-state-CDzDLiIs.js";
...
const loadZcaJsRuntime = createLazyRuntimeModule(() => import("./dist-DT6Cj1-9.js").then((mod) => mod));
	...
	if (!activeQrLogins.has(profile)) {
		...
		login.waitPromise = (async () => {
			...
			try {
				...
				const current = activeQrLogins.get(profile);
				if (current && current.id === login.id) current.error = formatErrorMessage(error);
			}
		})();
