...
import { A as resolvePositiveTimerTimeoutMs, a as addTimerTimeoutGraceMs, y as parseStrictNonNegativeInteger } from "./number-coercion-Crk_c9KW.js";
import { a as asRecord, i as asOptionalRecord } from "./record-coerce-DHZ4bFlT.js";
import { _ as uniqueStrings, a as normalizeOptionalTrimmedStringList } from "./string-normalization-CRyoFBPt.js";
import { r as getRootOptionAwareCommandPath } from "./cli-root-options-tXk2lfs9.js";
...
import { n as getRealtimeTranscriptionProvider, r as listRealtimeTranscriptionProviders } from "./provider-registry-DQUClwRz.js";
import { C as resolveRealtimeVoiceAgentConsultToolPolicy, H as REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ, T as resolveRealtimeVoiceAgentConsultToolsAllow, V as REALTIME_VOICE_AUDIO_FORMAT_G711_ULAW_8KHZ, f as consultRealtimeVoiceAgent, t as createRealtimeVoiceSessionHarness, w as resolveRealtimeVoiceAgentConsultTools, y as buildRealtimeVoiceAgentConsultWorkingResponse } from "./realtime-session-harness-bu55PsqP.js";
import { n as resolveConfiguredRealtimeVoiceProvider } from "./provider-resolver-Cdi3A0NB.js";
import { a as mulawToPcm, i as convertPcmToMulaw8k, n as createSpeechThresholdGate, r as readPcm16AudioStats, s as resamplePcm } from "./audio-energy-o8vC-5VK.js";
...
async function startMeetingRealtimeEngine(params) {
	...
	try {
		bridge = harness.createBridge({
			...
			onEvent: (event) => {
				if (event.direction === "server" && event.type === "session.created") continuityResetActive = false;
				...
				if (event.type === "input_audio_buffer.speech_started") harness.ensureTurn();
					...
					if (outputOwner.terminal(event.responseId)) {
						outputGenerationActive = false;
						harness.finishOutputAudio(event.type);
						if (event.type === "response.done") harness.endTurn(event.type);
					}
...
function createMeetingChromeTransport(options) {
	...
	async function assertAudioDeviceAvailable(params) {
		if (process.platform !== "darwin") throw new Error(`${options.meetingLabel} talk-back with BlackHole 2ch is macOS-only`);
		const result = await params.runtime.system.runCommandWithTimeout([options.systemProfilerCommand, "SPAudioDataType"], { timeoutMs: params.timeoutMs });
	...
	async function launchInChrome(params) {
		if (options.isTalkBackMode(params.mode)) await assertAudioDeviceAvailable({
			runtime: params.runtime,
		...
		if (!options.isRealtimeRouteReady(params.mode, result.browser)) return result;
		try {
			return {
				...result,
	...
	function parseNodeStartResult(raw) {
		...
		if (!value || typeof value !== "object") throw new Error(`${options.meetingLabel} node returned an invalid start result.`);
		return value;
	}
...
function createMeetingNodeHost(options) {
	...
	const listSessions = (params) => {
		const urlKey = options.normalizeMeetingKey(readString$1(params.url));
		const mode = readString$1(params.mode);
...
function readSetupCommand(params, name) {
...
function createMeetingConfiguredNodeHost(options) {
	...
	const assertAudioAvailable = (timeoutMs, commands = [options.defaultAudioInputCommand, options.defaultAudioOutputCommand]) => {
		if (process.platform !== "darwin") throw new Error(`${options.meetingLabel} talk-back with BlackHole 2ch is macOS-only`);
		const deadline = Date.now() + timeoutMs;
		const commandTimeout = () => options.sharePrerequisiteDeadline ? Math.max(1, deadline - Date.now()) : timeoutMs;
		...
		const output = `${result.stdout ?? ""}\n${stderr}`;
		if ((typeof result.status === "number" ? result.status : result.error ? 1 : 0) !== 0 || !options.outputMentionsAudioDevice(output)) throw new Error("BlackHole 2ch audio device not found on the node.");
		const commandNames = /* @__PURE__ */ new Set();
		for (const argv of commands) {
			const command = argv[0];
			if (!command) throw new Error(`Configured audio command not found on the node: ${command || "<empty>"}`);
			commandNames.add(command);
		}
		for (const command of commandNames) {
			if (options.sharePrerequisiteDeadline && Date.now() >= deadline) throw new Error(`${options.meetingLabel} audio prerequisite check timed out on the node.`);
	...
	return async (paramsJSON) => {
		if (paramsJSON) {
			...
			if (params.action === "setup") {
				const commands = [readSetupCommand(params, "audioInputCommand"), readSetupCommand(params, "audioOutputCommand")];
				if (params.bargeInInputCommand !== void 0) commands.push(readSetupCommand(params, "bargeInInputCommand"));
				assertAudioAvailable(1e4, commands);
...
function createMeetingPluginConfigSchema(options) {
	const buildSoxCommands = (format, bufferBytes) => buildMeetingSoxAudioCommands({
		bufferBytes,
		...
		async handle(ctx) {
			...
			if (action === "setup" && options.useConfiguredSetupCommands) {
				...
				for (const key of [
					...
					const command = copyCommand(options.start[key]);
					if (command) setupParams[key] = command;
				}
