...
import { a as validateSafeBinArgv, c as DEFAULT_SAFE_BINS, l as SAFE_BIN_PROFILES, n as isTrustedSafeBinPath } from "./exec-safe-bin-trust-DHudQEAW.js";
import { _ as resolvePolicyTargetTrustPath, d as resolveExecutionTargetCandidatePath, f as resolveExecutionTargetResolution, g as resolvePolicyTargetResolution, h as resolvePolicyTargetCandidatePath, l as resolveCommandResolutionFromArgv, p as resolveExecutionTargetTrustPath, r as matchAllowlist, t as buildHashedArgPatternFromArgv, u as resolveExecutableTrustPath, v as resolveExecWrapperTrustPlan } from "./exec-command-resolution-BcEGhdwN.js";
import { E as unwrapDispatchWrappersForResolution, N as normalizeExecutableToken, S as resolvePowerShellInlineCommandMatch, T as resolveDispatchWrapperTrustPlan, _ as hasPosixLoginStartupBeforeInlineCommand, g as hasPosixInteractiveStartupBeforeInlineCommand, h as advancePosixInlineOptionScan, i as extractBindableShellWrapperInlineCommand, l as isShellWrapperExecutable, m as POSIX_INLINE_COMMAND_FLAGS, n as POSIX_SHELL_WRAPPERS, r as POWERSHELL_WRAPPERS, t as POSIX_PARSEABLE_SHELL_WRAPPERS, v as isDirectShellPositionalCarrierCommand, w as isDispatchWrapperExecutable, x as resolveInlineCommandMatch, y as isPowerShellInlineFileCommandFlag } from "./shell-wrapper-resolution-4V1pkTCa.js";
import { a as isWindowsPlatform, i as analyzeWindowsShellCommand } from "./exec-approvals-analysis-XAN4qhrR.js";
import "./exec-wrapper-resolution-3NAqZbp9.js";
import { a as detectInlineEvalArgv, l as isInterpreterLikeAllowlistPattern } from "./risks-DVvbIVRa.js";
import { t as explainShellCommand } from "./extract-C3xLttuq.js";
...
function findFirstNonOptionIndex(argv, startIdx, params) {
	let idx = startIdx;
...
function hasLeadingContextOption(argv, startIdx, params) {
	let idx = startIdx;
	while (idx < argv.length) {
		const token = argv[idx]?.trim() ?? "";
		if (!token) {
...
function hasContextOptionBeforeTerminator(argv, startIdx, params) {
	let idx = startIdx;
	while (idx < argv.length) {
		const token = argv[idx]?.trim() ?? "";
		if (!token) {
...
function hasKnownPackageManagerExecContextOptions(argv) {
	switch (normalizePackageManagerExecToken(argv[0] ?? "")) {
		case "npm": {
			...
			if (hasLeadingContextOption(argv, 1, {
				...leadingOptions,
			...
			return subcommandIdx !== null && NPM_EXEC_SUBCOMMANDS.has(argv[subcommandIdx] ?? "") ? hasContextOptionBeforeTerminator(argv, subcommandIdx + 1, {
				optionsWithValue: NPM_EXEC_OPTIONS_WITH_VALUE,
				caseSensitiveOptionsWithValue: /* @__PURE__ */ new Set(["-C"]),
				flagOptions: NPM_EXEC_FLAG_OPTIONS,
				contextOptionsWithValue: NPM_EXEC_CONTEXT_OPTIONS_WITH_VALUE,
		...
		case "npx":
		case "bunx": return hasLeadingContextOption(argv, 1, {
			optionsWithValue: NPM_EXEC_OPTIONS_WITH_VALUE,
			caseSensitiveOptionsWithValue: /* @__PURE__ */ new Set(["-C"]),
			flagOptions: NPM_EXEC_FLAG_OPTIONS,
			contextOptionsWithValue: NPM_EXEC_CONTEXT_OPTIONS_WITH_VALUE,
		...
		case "pnpm": {
			...
			const subcommandIdx = findFirstNonOptionIndex(argv, 1, leadingOptions);
			return argv[subcommandIdx ?? -1] === "dlx" ? hasLeadingContextOption(argv, (subcommandIdx ?? 0) + 1, {
				...leadingOptions,
				contextOptionsWithValue: PNPM_EXEC_CONTEXT_OPTIONS_WITH_VALUE,
				contextCaseSensitiveOptionsWithValue: /* @__PURE__ */ new Set(["-C"]),
				contextFlagOptions: /* @__PURE__ */ new Set([
		...
		case "yarn": {
			...
			if (hasLeadingContextOption(argv, 1, {
				...leadingOptions,
				contextOptionsWithValue: /* @__PURE__ */ new Set(["--cwd"])
			})) return true;
			const subcommandIdx = findFirstNonOptionIndex(argv, 1, leadingOptions);
			return argv[subcommandIdx ?? -1] === "dlx" ? hasLeadingContextOption(argv, (subcommandIdx ?? 0) + 1, {
				optionsWithValue: YARN_DLX_OPTIONS_WITH_VALUE,
				flagOptions: YARN_DLX_FLAG_OPTIONS,
				contextOptionsWithValue: /* @__PURE__ */ new Set(["--package", "-p"])
			}) : false;
		}
...
function resolveSegmentAllowlistMatch(params) {
	...
	}, shellPositionalArgvCandidate.argv, params.context.platform) : null;
...
function resolveShellWrapperPositionalArgvCandidate(params) {
	...
	const candidatePath = resolveExecutionTargetCandidatePath(resolveCommandResolutionFromArgv([carriedExecutable], params.cwd, params.env, params.platform ?? void 0), params.cwd);
...
function buildScriptArgPatternFromArgv(argv, scriptPath, cwd, platform) {
	...
	if (!isWindowsPlatform(platform ?? process.platform)) return buildHashedArgPatternFromArgv([scriptPath, ...scriptArgs]);
	const normalized = scriptArgs.map((a) => a.replace(/\//g, "\\"));
	if (normalized.length === 0) return "^\0\0$";
	return `^${normalized.map(escapeRegExp).join("\0")}\x00$`;
}
function buildArgPatternFromArgv(argv, platform) {
	const args = argv.slice(1);
	if (!isWindowsPlatform(platform ?? process.platform)) return buildHashedArgPatternFromArgv(argv);
	const normalized = args.map((a) => a.replace(/\//g, "\\"));
	if (normalized.length === 0) return "^\0\0$";
	return `^${escapeRegExp(normalized.join("\0"))}\x00$`;
}
