...
import { a as writeRuntimeJson, o as writeRuntimeStdout } from "./runtime-DOr96aVu.js";
...
async function resolveExecBaseConfig(opts) {
	...
	if (opts.isolated || opts.authEnvOnly === true) return {};
	const { createConfigIO, getRuntimeConfig } = await import("./io-KBO9NxB7.js");
...
async function agentExecCommand(positionalMessage, opts, runtime, deps = {}) {
	...
	try {
		...
		configIo = await import("./io-KBO9NxB7.js");
		...
		const pluginInstallContext = opts.isolated !== true && opts.authEnvOnly !== true ? await import("./install-root-context-BCUAUnpl.js") : void 0;
		...
		const { resolveDefaultAgentDir } = await import("./agent-scope-config-BziOImKW.js");
		const storedAuthAgentDir = resolveDefaultAgentDir(baseConfig);
		restoreEnvironment = setAgentExecEnvironment({
			stateDir,
			cwd
		});
		runtimePaths = await import("./paths-DRqu1KUT.js");
		...
		const [{ withAuthProfileStoreAgentDir, withEnvOnlyAuthProfileStore }, { withHostExecInheritedEnvOmitted }, { listKnownProviderAuthEnvVarNames }, runAgent] = await Promise.all([
			import("./auth-profiles-DAMYArzq.js"),
			import("./host-env-security-zkNYlcbL.js"),
			import("./provider-env-vars-ClyOBYed.js"),
			deps.runAgent ? Promise.resolve(deps.runAgent) : import("./agent-COlyptaf.js").then((module) => module.agentCommand)
		]);
		...
		const runWithAuthScope = () => opts.authEnvOnly === true ? withEnvOnlyAuthProfileStore(runWithPluginInstallRoots) : withAuthProfileStoreAgentDir(storedAuthAgentDir, runWithPluginInstallRoots);
