...
import { c as retainSafeHeadersForCrossOriginRedirect, r as fetchWithSsrFGuard } from "./fetch-guard-CPWMHcWe.js";
import { c as isNonSecretApiKeyMarker } from "./model-auth-markers-Co0rjfKm.js";
import "./ssrf-runtime-B8V5-MiN.js";
import { d as buildSingleProviderApiKeyCatalog, o as getCachedLiveCatalogValue } from "./provider-catalog-shared-BFBHaHNZ.js";
...
function readLiveModelString(record, keys) {
	for (const key of keys) {
		const value = record?.[key];
		if (typeof value === "string" && value.trim()) return value.trim();
	}
}
function readLiveModelBoolean(record, keys) {
	for (const key of keys) {
		const value = record?.[key];
		if (typeof value === "boolean") return value;
	}
}
function readLiveModelPositiveInteger(records, keys) {
...
function buildOpenAICompatibleLiveModel(row, fallback, acceptUnknownModel) {
	...
	const id = readLiveModelString(record, [
		"id",
		"model",
		"model_name",
		"modelName"
	...
	if (readLiveModelBoolean(record, [
		"active",
		"enabled",
		"available"
	]) === false) return;
	if (readLiveModelBoolean(record, ["archived", "deprecated"]) === true) return;
	...
	const maxTokens = readLiveModelPositiveInteger([
		record,
		topProvider,
		capabilities,
		modelInfo
	...
	return {
		id,
		name: readLiveModelString(record, [
			"display_name",
			"displayName",
			"name"
		]) ?? id,
...
function readLiveModelCatalogCursor(body) {
	...
	const lastId = readLiveModelCatalogString(record.last_id) ?? readLiveModelCatalogString(record.lastId);
	if (lastId) return {
		name: "after_id",
		value: lastId
	};
...
function bodyAdvertisesMoreLiveModelCatalogPages(body) {
	...
	return Boolean(record.has_more === true || readLiveModelCatalogNextUrl(body) || readLiveModelCatalogString(record.next_cursor) || readLiveModelCatalogString(record.nextPageToken) || readLiveModelCatalogString(record.next_page_token));
}
...
//#endregion
export { buildOpenAICompatibleProviderFamilyCatalog as a, getCachedLiveProviderModelRows as c, buildOpenAICompatibleProviderCatalog as i, buildLiveModelProviderConfig as n, fetchLiveProviderModelIds as o, buildOpenAICompatibleLiveModelProviderConfig as r, fetchLiveProviderModelRows as s, LiveModelCatalogHttpError as t };
