...
import { ci as validateSystemAgentSetupActivateParams, di as validateSystemAgentSetupVerifyParams, li as validateSystemAgentSetupAuthStartParams, m_ as buildSystemAgentSessionInvalidatedErrorDetails, oi as validateSystemAgentChatHistoryParams, p_ as buildSystemAgentInferenceUnavailableErrorDetails, si as validateSystemAgentChatParams, ui as validateSystemAgentSetupDetectParams } from "./src-BSn6va4B.js";
import { a as errorShape } from "./error-codes-P4fBo0lR.js";
import { r as formatSystemAgentStartupMessage } from "./overview-8mY9XHbo.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-C05XXupU.js";
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-BbrEP4i9.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-9m6TOUCP.js";
import { n as isSystemAgentInferenceUnavailableError } from "./inference-error-ZQMpxTxR.js";
...
import { i as buildRequestedApprovalEvent, l as listVisiblePendingApprovalRequests, s as handlePendingApprovalRequest } from "./approval-shared-Cs2GHmag.js";
import { t as getUpdateAvailable } from "./update-startup-Dv-QoRTw.js";
import { n as getHealthCache } from "./health-state-n2ZsBJgJ.js";
import { t as WizardSession } from "./session-BeEA_WOJ.js";
import { n as SystemAgentChatEngine, r as SystemAgentWizardAnswerError } from "./chat-engine-JH6BtxRR.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.detect": async ({ params, respond }) => {
		...
		const { detectSetupInferenceIsolated } = await import("./setup-inference-detection-DMa2ZEP8.js");
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = new WizardSession(async (prompter, signal) => {
			await runExclusiveSystemAgentSetupActivation(async () => runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-BHrfdh0i.js"), import("./setup.shared-BQDwhL8U.js")]);
				...
				if (applied.agentModelOverride) session.setPreparedModelRef(applied.agentModelOverride);
			}));
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (params.wizardAnswer !== void 0 && !session) {
					respond(false, void 0, errorShape(ErrorCodes.INVALID_REQUEST, "No active OpenClaw chat session is awaiting that wizard answer.", { details: buildSystemAgentSessionInvalidatedErrorDetails() }));
					return;
				}
				let greetingAuditSequence;
				const welcomeOnly = params.wizardAnswer === void 0 && (params.message === void 0 || !params.message.trim());
				if (!session) {
					const inference = params.delegation ? await import("./inference-fallback-z8rdv5w1.js").then(({ verifySystemAgentInferenceWithFallback }) => verifySystemAgentInferenceWithFallback({
				...
				if (params.wizardAnswer === void 0 && (params.message === void 0 || !params.message.trim())) {
					respond(true, {
						sessionId,
						reply: session.welcome,
						action: "none",
