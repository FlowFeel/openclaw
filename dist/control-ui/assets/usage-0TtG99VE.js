import{n as e}from"./rolldown-runtime-DaJ6WEGw.js";import{rr as t,tr as n}from"./control-ui-core-CrKLOOVi.js";import{K as r,W as i,Y as a}from"./lit-runtime-D5xZwgO1.js";import{o,t as s}from"./control-ui-core-DkYXaYTI.js";function c(e,t){let n=t.trim().toUpperCase();return[`USD`,`EUR`,`GBP`,`CNY`,`JPY`].includes(n)?new Intl.NumberFormat(void 0,{style:`currency`,currency:n,maximumFractionDigits:n===`JPY`?0:2}).format(e):`${new Intl.NumberFormat(void 0,{maximumFractionDigits:2}).format(e)} ${t}`}function l(e){return!e||!Number.isFinite(e)?null:new Intl.DateTimeFormat(void 0,{month:`short`,day:`numeric`,hour:`numeric`,minute:`2-digit`}).format(new Date(e))}function u(e){return(e.billing??[]).map(e=>a`
    ...
    `)}function d(e,t){let n=e.costHistory;if(!n)return 0;let r=new Date,i=Date.UTC(r.getUTCFullYear(),r.getUTCMonth(),r.getUTCDate()),a=i-(Math.max(1,t)-1)*864e5;return n.daily.reduce((e,t)=>{let n=Date.parse(`${t.date}T00:00:00Z`);return Number.isFinite(n)&&n>=a&&n<=i?e+t.amount:e},0)}function f(e){let t=e.costHistory;if(!t||t.daily.length===0)return r;let i=Math.max(...t.daily.map(e=>e.amount),0),s=t.daily.reduce((e,t)=>({requests:e.requests+(t.requests??0),input:e.input+t.inputTokens,cache:e.cache+t.cacheReadTokens+t.cacheWriteTokens,output:e.output+t.outputTokens}),{requests:0,input:0,cache:0,output:0});return a`
    <div class="provider-cost-history">
      <div class="provider-cost-windows">
        ${[[o(`usage.providerUsage.today`),d(e,1)],[o(`usage.providerUsage.last7Days`),d(e,7)],[o(`usage.providerUsage.lastDays`,{count:String(t.periodDays)}),t.daily.reduce((e,t)=>e+t.amount,0)]].map(([e,n])=>a`
            <div class="provider-cost-window">
              <span>${e}</span>
              <strong>${c(n,t.unit)}</strong>
            </div>
          `)}
      </div>
      <div class="provider-cost-chart" aria-label=${o(`usage.providerUsage.dailyCost`)}>
        ${t.daily.map(e=>a`<span
            ...
            title=${`${e.date}: ${c(e.amount,t.unit)}`}
            aria-label=${`${e.date}: ${c(e.amount,t.unit)}`}
          ></span>`)}
      </div>
      <div class="provider-cost-tokens">
        ${s.requests>0?a`<span
            ...
            >`:r}
        <span>${o(`usage.providerUsage.inputTokens`,{count:n(s.input)})}</span>
        <span>${o(`usage.providerUsage.cacheTokens`,{count:n(s.cache)})}</span>
        <span
          >${o(`usage.providerUsage.outputTokens`,{count:n(s.output)})}</span
        >
      </div>
              ...
              ${t.categories.length>0?a`
                    <div class="provider-cost-breakdown">
                      ...
                      ${t.categories.slice(0,3).map(e=>a`
                          <div>
                            <span>${e.name}</span>
                            <strong>${c(e.amount,t.unit)}</strong>
                          </div>
                        `)}
                    </div>
                  `:r}
  ...
  `}var m=e((()=>{i(),s(),t()})),h=e((()=>{}));export{m as n,p as r,h as t};
//# sourceMappingURL=usage-0TtG99VE.js.map