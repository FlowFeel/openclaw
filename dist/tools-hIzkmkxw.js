...
import { C as resolveMemoryDreamingPluginConfig, S as resolveMemoryDreamingConfig, x as resolveMemoryDeepDreamingConfig } from "./dreaming-CisvAHct.js";
...
import { c as listMemoryCorpusSupplements } from "./memory-state-CngllfdS.js";
import { r as optionalFiniteNumberSchema, s as stringEnum } from "./typebox-ktRHOCRA.js";
import { _ as stripMemoryAnnotationCarriers } from "./internal-C0br8ilW.js";
...
import { d as recordShortTermRecalls } from "./short-term-promotion-y5kKzrnP.js";
				...
				try {
					...
					if (shouldQueryMemory && memorySetup && memory && !("error" in memory)) {
						await runUnavailablePhase("memory", async () => {
							...
							if (rawResults.length === 0 && !runtimeDebug.some((entry) => entry.embeddingBootstrap) && activeMemory.manager.sync && (statusBeforeRetry.backend !== "qmd" || options.oneShotCliRun === true)) {
							...
							const decorated = decorateCitations(rawResults.map((result) => ({
								...result,
