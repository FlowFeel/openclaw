...
import { o as resolvePackageExtensionEntries } from "./manifest-BUOsMMgJ.js";
import { l as validateRegistryNpmSpec, s as parseRegistryNpmSpec } from "./npm-registry-spec-CCN1EPEy.js";
import { c as emitTrustedSecurityEvent } from "./diagnostic-events-Dt41CZkD.js";
import { n as readJson, r as readJsonIfExists, u as writeJson } from "./json-C-CW4mQo.js";
import "./json-files-v5WP3doI.js";
import { k as satisfiesPluginApiRange } from "./clawhub-Clykbwlp.js";
import { c as resolvePluginNpmGenerationProjectDirPrefix, r as resolveDefaultPluginExtensionsDir, s as resolvePluginNpmGenerationProjectDir, u as resolvePluginNpmProjectDir } from "./install-paths-D18EkQi8.js";
import { t as resolvePackagePluginApiRange } from "./package-compat-DJjXEzu-.js";
import { r as hasRetainedManagedNpmInstallMarker } from "./managed-npm-retention-BDxTqE3b.js";
...
import { o as relinkOpenClawPeerDependenciesInManagedNpmRoot } from "./plugin-peer-link-B_98bc_B.js";
...
async function readHostWorkspaceOverrides(packageRoot) {
	const workspace = parse(await fs$1.readFile(path.join(packageRoot, "pnpm-workspace.yaml"), "utf8"));
...
function filterUnsupportedManagedNpmRootOverrides(value, omissions) {
	const overrides = readOverrideRecord(value);
	const filtered = {};
	for (const [key, raw] of Object.entries(overrides)) {
		if (omissions.pnpmParentChildSelectors && isPnpmParentChildOverrideSelector(key) || omissions.npmAliases && isUnsupportedManagedNpmOverride(raw)) continue;
		if (isRecord(raw)) {
...
async function upsertManagedNpmRootDependency(params) {
	...
	const managedOverrides = params.overrideOmissions ? filterUnsupportedManagedNpmRootOverrides(params.managedOverrides, params.overrideOmissions) : readOverrideRecord(params.managedOverrides);
...
async function syncManagedNpmRootPeerDependencies(params) {
	...
	const { overrides, managedOverrideKeys } = applyManagedNpmRootOverrides({
		manifest,
		managedOverrides: params.overrideOmissions ? filterUnsupportedManagedNpmRootOverrides(params.managedOverrides, params.overrideOmissions) : readOverrideRecord(params.managedOverrides),
		dependencies: nextDependencies,
...
const pluginInstallRuntimeLoader = createLazyImportLoader(() => import("./install2.runtime.js"));
...
function classifyNpmManagedOverrideCompatibilityError(result) {
	const output = `${result.stderr}\n${result.stdout}`;
	const selectorError = output.includes("EINVALIDTAGNAME") || output.includes("EINVALIDPACKAGENAME") || output.includes("Override without name:");
	const selectorFragments = [...output.matchAll(/"([^"]+)"/gu), ...output.matchAll(/Override without name: ([^\r\n]+)/gu)].flatMap((match) => match.slice(1));
	const compatibility = {
		npmAliases: output.includes("Invalid comparator: npm:"),
		pnpmParentChildSelectors: selectorError && selectorFragments.some((fragment) => /[^ |@]>/u.test(fragment))
	};
	return compatibility.npmAliases || compatibility.pnpmParentChildSelectors ? compatibility : void 0;
}
...
//#endregion
export { resolveEffectiveInstallMode as A, listMissingRequiredPlatformPackages as B, ensureInstallTargetAvailableForMode as C, installPluginDirectoryIntoExtensions as D, hasPackageRuntimeDependencies as E, validateOpenClawPackageInstallCompatibility as F, resolveManagedNpmRootDependencySpec as G, readManagedNpmRootPeerDependencySnapshot as H, emitPluginAuditSecurityEvent as I, syncManagedNpmRootPeerDependencies as K, emitPluginInstallSecurityEvent as L, runInstallSourceScan as M, sourceFamilyForInstallPolicyKind as N, loadPluginInstallRuntime as O, sourceFamilyForInstallPolicySource as P, pluginAuditOutcomeForReason as R, emitSuccessfulPluginInstallSecurityEvent as S, formatUnresolvedOpenClawPeerLinkError as T, readOpenClawManagedNpmRootOverrides as U, readManagedNpmRootInstalledDependency as V, repairManagedNpmRootOpenClawPeer as W, resolveRequiredPlatformPackageNames as _, formatManagedNpmProjectQuarantineArtifacts as a, buildDirectoryInstallResult as b, listManagedNpmRootPackageNames as c, removeEmptyDirectoryIfPresent as d, resolveManagedNpmGenerationUseForInstall as f, resolveManagedNpmRootPackageDir as g, resolveManagedNpmRootForInstall as h, createManagedNpmPluginInstallRollbackSnapshot as i, resolvePreparedDirectoryInstallTarget as j, readOptionalPackageManifest as k, listNewManagedNpmRootPackageDirs as l, resolveManagedNpmRootDependencySpecForInstall as m, cleanupManagedNpmPluginInstallRollbackSnapshot as n, formatNpmCommandFailureOutput as o, resolveManagedNpmInstallRoot as p, upsertManagedNpmRootDependency as q, cleanupManagedNpmRootPreparedDependency as r, isManagedNpmProjectCorruptionInstallFailure as s, classifyNpmManagedOverrideCompatibilityError as t, quarantineManagedNpmProjectRebuildArtifacts as u, rollbackManagedNpmPluginInstall as v, ensureOpenClawExtensions as w, defaultLogger as x, rollbackManagedNpmRootPreparedDependency as y, PLUGIN_INSTALL_ERROR_CODE as z };
