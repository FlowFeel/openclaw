...
import { n as CODEX_BUNDLE_MANIFEST_RELATIVE_PATH, o as mergeBundlePathLists, r as CURSOR_BUNDLE_MANIFEST_RELATIVE_PATH, s as normalizeBundlePathList, t as CLAUDE_BUNDLE_MANIFEST_RELATIVE_PATH } from "./bundle-manifest-BVamVGYB.js";
import { c as normalizePluginsConfig, u as resolveEffectivePluginActivationState } from "./config-state-DrPZVhOu.js";
...
function loadNativePluginMcpConfig(params) {
	const rootDir = normalizeBundlePath(params.rootDir);
	return {
		config: { mcpServers: Object.fromEntries(Object.entries(params.mcpServers).map(([serverName, server]) => [serverName, absolutizeBundleMcpServer({
			rootDir,
			baseDir: rootDir,
			server
		})])) },
