...
import { n as loadGatewayStartupPluginPlanWithMetadata } from "./gateway-startup-plugin-ids-C-hC0TmS.js";
...
import { o as resolveHookEntries } from "./config-CaQnSSBm.js";
import { a as buildPluginDiagnosticsReport } from "./status-DBhSQc1q.js";
...
import { t as runNativeHookRelayCli } from "./native-hook-relay-cli-Bsl-NwJo.js";
import { t as runPluginUpdateCommand } from "./plugins-update-command-DPNTYX5w.js";
...
async function loadHooksReport() {
...
function registerHooksCli(program) {
	const hooks = program.command("hooks").description("Manage internal agent hooks").option("--json", "Output as JSON", false).addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.openclaw.ai/cli/hooks")}\n`);
	...
	hooks.action(async (opts) => runOneShotHooksCliAction(async () => {
		const report = await loadHooksReport();
		const json = hasJsonOutput(opts);
		writeHooksOutput(formatHooksList(report, {
			...opts,
