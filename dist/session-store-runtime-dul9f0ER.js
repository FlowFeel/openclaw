...
import { i as resolveSessionFilePath$1, l as resolveStorePath$1 } from "./paths-DSnYpBD3.js";
import { H as deleteSqliteSessionEntryLifecycle, V as cleanupSqliteSessionLifecycleArtifacts, ct as loadSqliteSessionEntryReadOnly, dt as readSqliteSessionUpdatedAt, gt as resolveSqliteSessionKeyBySessionId, lt as patchSqliteSessionEntry, nt as listSqliteSessionEntriesReadOnly, z as applySqliteSessionStoreProjection } from "./session-accessor.sqlite-B9iW7DOt.js";
import "./store-entry-shape-DB6qjwcl.js";
import { s as resolveSessionStoreEntry$1 } from "./store-entry-0o94FtE8.js";
...
import { D as readSqliteTranscriptStatsSync, x as loadSqliteTranscriptEventsSync } from "./session-accessor.sqlite-transcript-store-Si6-bv-m.js";
import { W as normalizeResolvedMaintenanceConfigInput } from "./session-entry-slot-keys-DR5d2mKt.js";
...
import "./send-policy-BKJvZqJK.js";
import { t as readAmbientTranscriptWatermark$1 } from "./ambient-transcript-watermark-C1PEpxU7.js";
...
function materializeLegacyTranscriptFile(sessionFile, options) {
	...
	const stats = readSqliteTranscriptStatsSync(transcriptScope);
	const isOversized = stats.sizeBytes + (stats.eventCount > 0 ? 1 : 0) > LEGACY_TRANSCRIPT_INSPECTION_MAX_BYTES;
	replaceFileAtomicSync({
		filePath: transcriptPath,
		content: isOversized ? "" : (() => {
			const events = loadSqliteTranscriptEventsSync(transcriptScope);
...
async function updateSessionStore(storePath, mutator, options = {}) {
	const target = resolveLegacySessionStoreTarget(storePath);
	return await applySqliteSessionStoreProjection({
...
function getSessionEntry(params) {
	const entry = loadSqliteSessionEntryReadOnly(toSessionAccessScope(params));
...
function loadTranscriptEventsSync(params) {
	return loadSqliteTranscriptEventsSync(params);
}
/** Reads transcript freshness and byte size without materializing event rows. */
function readTranscriptStatsSync(params) {
	return readSqliteTranscriptStatsSync(params);
}
...
function resolveTranscriptSessionKeyBySessionId(params) {
	return resolveSqliteSessionKeyBySessionId(params);
}
/** Patches one session entry by agent/session identity. */
async function patchSessionEntry(params) {
	const entry = await patchSqliteSessionEntry(toSessionAccessScope(params), async (internalEntry, context) => {
...
function readSessionUpdatedAt(params) {
	return readSqliteSessionUpdatedAt(toSessionAccessScope(params));
}
function readAmbientTranscriptWatermark(params) {
	return readAmbientTranscriptWatermark$1(getSessionEntry(params), params.key, params.legacyKey);
}
...
async function upsertSessionEntry(params) {
	...
	await patchSqliteSessionEntry(toSessionAccessScope(params), (internalEntry) => {
...
async function deleteSessionEntry(params) {
	...
	return (await deleteSqliteSessionEntryLifecycle({
...
async function cleanupSessionLifecycleArtifacts(params) {
	return await cleanupSqliteSessionLifecycleArtifacts({
