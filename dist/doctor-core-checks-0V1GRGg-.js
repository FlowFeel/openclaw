...
import { n as maybeArchiveLegacyClawdBrowserProfileResidue, r as noteChromeMcpBrowserReadiness, t as detectLegacyClawdBrowserProfileResidue } from "./doctor-browser-Bmq1-Uub.js";
import { n as hasConfiguredCommandOwners } from "./doctor-command-owner-CIbtW56M.js";
import { a as shellCompletionStatusToRepairEffects, i as shellCompletionStatusToHealthFindings, t as checkShellCompletionStatus } from "./doctor-completion-B94REOG2.js";
import { i as sessionLockToRepairEffect, r as sessionLockToHealthFinding, t as detectStaleSessionLocks } from "./doctor-session-locks-B3j7RnzH.js";
import { n as disableUnavailableSkillsInConfig, r as formatMissingSkillSummary } from "./doctor-skills-core-DZQVCoNO.js";
import { i as uiProtocolFreshnessIssueToRepairEffects, r as uiProtocolFreshnessIssueToHealthFinding, t as detectUiProtocolFreshnessIssues } from "./doctor-ui-DZAeer0Q.js";
import { i as collectDisabledCodexPluginRouteIssues } from "./codex-route-warnings-DjZor_9F.js";
import { t as getSkippedExecRefStaticError } from "./exec-resolution-policy-0iEPMKT5.js";
import { t as detectSkillWorkshopToolPolicyDiagnostic } from "./tool-policy-diagnostic-IsE5Fe-8.js";
...
const loadDoctorWorkspaceModule = async () => await import("./doctor-workspace-Bc2wqDee.js");
...
async function collectSecurityWarningsWithRuntime(cfg) {
	const { collectSecurityWarnings } = await import("./doctor-security-DFL3cRBa.js");
	return collectSecurityWarnings(cfg);
}
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-BnZUtM-v.js");
...
const legacyStateCheck = {
	...
	async detect(ctx) {
		const { detectLegacyStateMigrations } = await import("./doctor-state-migrations-NUAm1xeS.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapContextForRun } = await import("./bootstrap-files-C6fmhr2b.js");
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-DkLz3NBv.js");
...
const legacyWhatsAppCrontabCheck = {
	...
	defaultEnabled: false,
	async detect() {
		const { collectLegacyWhatsAppCrontabHealthWarning } = await import("./cron-D65moqK_.js");
...
const legacyCronStoreCheck = {
	...
	defaultEnabled: false,
	async detect(ctx) {
		const { collectLegacyCronStoreHealthFindings } = await import("./cron-D65moqK_.js");
...
const telegramGeneralTopicConversationsCheck = {
	...
	source: "doctor",
	async detect(ctx) {
		const { detectTelegramGeneralTopicConversationRepairs } = await import("./doctor-telegram-general-topic-conversations--2uYl3gt.js");
	...
	async repair(ctx) {
		const { repairTelegramGeneralTopicConversations } = await import("./doctor-telegram-general-topic-conversations--2uYl3gt.js");
		const effect = {
			kind: "state",
...
const gatewayServicesExtraCheck = {
	...
	async detect(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToHealthFinding } = await import("./doctor-gateway-services-Cwy-Ncvl.js");
		return (await detectExtraGatewayServiceIssues({ deep: coreCtx.deep === true })).map(extraGatewayServiceToHealthFinding);
	},
	async repair(ctx) {
		const coreCtx = ctx;
		const { detectExtraGatewayServiceIssues, extraGatewayServiceToRepairEffects } = await import("./doctor-gateway-services-Cwy-Ncvl.js");
...
const gatewayPlatformNotesCheck = {
	...
	async detect(ctx) {
		...
		const { collectMacGatewayPlatformWarnings } = await import("./doctor-platform-notes-DEZ6dEvd.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-CVEzdHyf.js"), import("./mcp-config-BH8ZK-o2.js")]);
