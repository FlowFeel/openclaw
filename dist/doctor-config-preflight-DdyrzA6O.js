...
import { f as resolveHomeDir } from "./utils-Bs67j6-3.js";
import { u as resolveCanonicalConfigPath } from "./paths-CL43LNS6.js";
import { t as ExitError } from "./runtime-DOr96aVu.js";
import { _ as recoverConfigFromLastKnownGood, a as preserveConfigSnapshotAsClobbered, et as parseConfigJson5, f as readConfigFileSnapshotWithPluginMetadata, g as recoverConfigFromJsonRootSuffix, l as readConfigFileSnapshot } from "./io-BsQc3Kgy.js";
import { c as normalizePluginsConfig, l as resolveEffectiveEnableState } from "./config-state-DrPZVhOu.js";
import { n as cloneEnvWithPlatformSemantics } from "./config-env-vars-Dh6msnxE.js";
...
import { r as noteIncludeConfinementWarning } from "./doctor-config-analysis-DBfZ7XGO.js";
import { t as findDoctorLegacyConfigIssues } from "./legacy-config-issues-teyp-7ex.js";
...
async function measureGatewayStartupPreflightStep(name, run) {
	...
	try {
		...
		const { formatConsoleDiagnosticLine } = await import("./json-console-line-BNlbvf_c.js");
...
const loadInstalledPluginIndexStore = createLazyRuntimeModule(() => import("./installed-plugin-index-store-B12fL2oN.js"));
...
async function planStartupPluginVerification(params) {
	const { planStartupPluginConvergence } = await measureDoctorConfigPreflightStep("plugin-plan-import", () => import("./startup-plugin-convergence-plan-CMsDyl2_.js"), params.measure);
...
async function runStartupUpgradeConvergence(params) {
	...
	const { runPostCorePluginConvergence } = await measureDoctorConfigPreflightStep("plugin-convergence-import", () => import("./post-core-plugin-convergence-DdS3Ee4X.js"), params.measure);
...
async function refreshStartupPluginQuarantine(params) {
	...
	const { runActivePluginPayloadSmokeCheck } = await measureDoctorConfigPreflightStep("plugin-payload-verification-import", () => import("./active-plugin-payload-validation-CNmrrdrD.js"), params.measure);
...
const loadDoctorStateMigrations = createLazyRuntimeModule(() => import("./doctor-state-migrations-Dc88c8kL.js"));
const loadLegacyCronRepair = createLazyRuntimeModule(() => import("./legacy-repair-DK91AJDd.js"));
...
async function runDoctorConfigPreflight(options = {}) {
	...
	const migrationCheckpoint = gatewayStartupCheckpointRequired || options.requireStateMigrationCheckpoint === true ? await measurePreflightStep("startup-checkpoint-import", () => import("./startup-migration-checkpoint-D8ObpJmt.js")) : void 0;
	let stateMigrations;
	...
	const ensureStartupMigrationLease = () => {
		...
		startupMigrationLease = migrationCheckpoint.acquireStartupMigrationLease({ env: startupMigrationEnv });
	...
	const readConfigSnapshotForPreflight = async (allowCurrentPluginMetadata = true) => await measurePreflightStep("config-snapshot", async () => {
		const sharedOptions = {
		...
		if (migrationCheckpoint && !shouldSkipPluginValidationForDoctorConfigPreflight()) {
			...
			return {
				...
				pluginMigrationFingerprint: result.pluginMetadataSnapshot?.configFingerprint?.trim() || null,
	...
	try {
		if (migrationCheckpoint && !skipPristineStartupStateMigrations) {
			const { planPristineStartupStateMigrations } = await measurePreflightStep("pristine-state-plan-import", () => import("./pristine-startup-state-XkG5AB5u.js"));
		...
		if (migrationCheckpoint) {
			...
			if (shouldRecordStateCheckpoint || shouldRecordStartupCheckpoint || shouldPersistRefreshedPluginIndex) ensureStartupMigrationLease();
		}
		...
		if (shouldPersistRefreshedPluginIndex) ensureStartupMigrationLease();
		...
		if (stateMigrations && stateMigrationsAllowed && freshConfigGuardAllowed) {
			...
			if (stateMigrationInput) {
				...
				if (skipPristineCoreStateMigrations && pluginDoctorOnlyConfig) noteStartupStateMigrationResult(await measurePreflightStep("plugin-doctor-migrations", () => autoMigrateLegacyPluginDoctorState({
					...
					const legacyStateResult = await measurePreflightStep("legacy-state-migrations", () => autoMigrateLegacyState({
						cfg: migrationConfig,
						...pluginDoctorConfig ? { pluginDoctorConfig } : {},
						env: process.env,
					...
					const pluginDoctorConfig = stateMigrationInput.pluginDoctorConfig;
					noteStartupStateMigrationResult(await measurePreflightStep("plugin-doctor-migrations", () => autoMigrateLegacyPluginDoctorState({
