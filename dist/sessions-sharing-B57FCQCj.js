...
import { l as runQueuedStoreWrite } from "./store-entry-DWPp52Lz.js";
import "./session-accessor-D5Or7WgI.js";
import { Ln as validateSessionMemberAddParams, Rn as validateSessionMemberRemoveParams, Wn as validateSessionVisibilitySetParams, zn as validateSessionMembersListParams } from "./src-BSn6va4B.js";
...
import { d as removeSessionMember, l as listSessionMembers, s as addSessionMember } from "./sessions-BqBqRT1f.js";
import { s as listProfiles } from "./user-profiles-DoQoc9os.js";
import { t as assertValidParams } from "./validation-DyQ6nhKI.js";
import { d as isSessionVisibilityAllowed, g as invalidateSessionSharingSnapshot, h as resolveSessionVisibility, m as resolveSessionSharingTarget, n as allowedSessionVisibilities, p as resolveSessionSharingRole, s as canManageSessionSharing, v as gatewayClientSessionCreator } from "./session-sharing-BuGIPH6F.js";
...
const sharingMutationQueues = /* @__PURE__ */ new Map();
...
function knownSessionIdentities(params) {
	...
	for (const entry of Object.values(loadCombinedSessionStoreForGateway(params.cfg).store)) remember(entry.createdActor ?? null);
...
const sessionSharingHandlers = {
	"session.visibility.set": async ({ params, respond, client, context }) => {
		...
		await runExclusiveSharingMutation(managed.target, async () => {
			...
			await patchSqliteSessionEntry(scope, (entry) => {
			...
			try {
				...
				await patchSqliteSessionEntry(scope, (entry) => entry.sessionId === current.entry.sessionId && resolveSessionVisibility(entry) === visibility ? { visibility: previous } : null);
				invalidateSessionSharingSnapshot(current.canonicalKey);
				throw error;
			}
