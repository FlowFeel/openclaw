...
const PENDING_PLUGIN_BINDING_REQUEST_TTL_MS = 30 * 6e4;
const MAX_PENDING_PLUGIN_BINDING_REQUESTS = 512;
const pendingRequests = resolveGlobalMap(Symbol.for("openclaw.pluginBindingPendingRequests"), (requests) => {
	for (const entry of requests.values()) clearTimeout(entry.timeoutId);
	requests.clear();
});
function takePendingPluginBindingRequest(approvalId, expected) {
...
async function addPersistentApproval(entry) {
	...
	const approvals = getApprovals().approvals.filter((existing) => buildApprovalScopeKey({
...
async function resolvePluginConversationBindingApproval(params) {
	...
	if (!entry || Date.now() >= entry.expiresAtMs) {
		if (entry) takePendingPluginBindingRequest(params.approvalId, entry);
