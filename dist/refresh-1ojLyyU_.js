...
import { a as clearSkillsSnapshotVersionForWorkspace, c as resetSkillsRefreshStateForTest, i as bumpSkillsSnapshotVersion, l as setSkillsChangeListenerErrorHandler, n as resolveAllowedSkillSymlinkTargetRealPaths, r as tryRealpath } from "./symlink-targets-BsIBLVmY.js";
...
function createSkillsPathWatcher(target) {
	...
	const scheduleRawSkillFile = (changedPath) => {
		waitForStableSkillFile(changedPath, SKILLS_WATCH_DEBOUNCE_MS).catch((err) => {
...
function subscribeWorkspaceToPath(workspaceDir, watchTarget) {
...
function ensureSkillsWatcher(params) {
	...
	const watcherDepthsCoverTargets = watchTargets.every((watchTarget) => (pathWatchers.get(watchTarget.key)?.depth ?? -1) >= watchTarget.depth);
	...
	for (const watchTarget of watchTargets) subscribeWorkspaceToPath(workspaceDir, watchTarget);
