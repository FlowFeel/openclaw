...
import { n as isGeneratedHashedArgPattern } from "./exec-command-resolution-BcEGhdwN.js";
import { _ as hasPosixLoginStartupBeforeInlineCommand, g as hasPosixInteractiveStartupBeforeInlineCommand, i as extractBindableShellWrapperInlineCommand, m as POSIX_INLINE_COMMAND_FLAGS, u as isShellWrapperInvocation } from "./shell-wrapper-resolution-4V1pkTCa.js";
import { r as withOpenClawStateLease, t as OpenClawStateLeaseError } from "./openclaw-state-lease-DML4RR4y.js";
import { a as isAgentDeletionBlocked, n as AgentDeletionCommitUncertainError, t as AgentDeletionAuthorityRollbackError } from "./agent-lifecycle-registry-BngfIWZE.js";
import { c as resolveExecApprovalsPath, i as generateToken, l as resolveExecApprovalsSocketPath, o as normalizeExecApprovalsInternal, r as createFailClosedExecApprovalsFallback, s as resolveExecApprovalsDisplayPath } from "./exec-approvals-config-DrfE-I9X.js";
import { a as readExecApprovalsConfigRow, c as writeExecApprovalsConfigRow, n as EXEC_APPROVALS_MUTATION_LEASE_SCOPE, o as serializeExecApprovals, r as deleteExecApprovalsConfigRow, s as snapshotFromExecApprovalsRow, t as EXEC_APPROVALS_MUTATION_LEASE_KEY } from "./exec-approvals-sqlite-Dw7Ztd0n.js";
import "./exec-approvals-analysis-XAN4qhrR.js";
import "./exec-wrapper-resolution-3NAqZbp9.js";
import { s as resolveAllowAlwaysPatternEntries } from "./exec-approvals-allowlist-DRhJBoRl.js";
...
const EXEC_APPROVALS_DELETION_LEASE_MS = 12e4;
const EXEC_APPROVALS_DELETION_LEASE_WAIT_MS = 1e4;
let lastWarnAt;
...
function assertNoDeletedAgentApprovalChanged(current, next, params) {
	...
	for (const agentId of agentIds) {
		...
		const allowedRemoval = agentId === params.allowDeletedAgentRemoval && currentPolicy !== void 0 && nextPolicy === void 0;
		const allowedRestore = agentId === params.allowDeletedAgentRestore && currentPolicy === void 0 && nextPolicy !== void 0;
		if (isAgentDeletionBlocked(agentId) && !allowedRemoval && !allowedRestore && !isDeepStrictEqual(currentPolicy, nextPolicy)) throw new Error(`Exec approvals are unavailable while agent ${agentId} is deleted.`);
	}
}
...
async function withAgentExecApprovalsRemoved(agentId, commit) {
	...
	try {
		...
		if (error instanceof OpenClawStateLeaseError && error.code === "OPENCLAW_STATE_LEASE_STORAGE_FAILED") throw new ExecApprovalsStoreUnavailableError(error);
		throw error;
	}
}
function restoreExecApprovalsSnapshotInTransaction(snapshot, leaseOwner) {
...
async function restoreExecApprovalsSnapshotLocked(snapshot, baseHash) {
	assertNoPendingLegacyExecApprovals();
	return runOpenClawStateWriteTransaction(({ db }) => {
		if (snapshotFromExecApprovalsRow({
			path: resolveExecApprovalsDisplayPath(),
			row: readExecApprovalsConfigRow(db)
		}).hash !== baseHash) return false;
...
//#endregion
export { withAgentExecApprovalsRemoved as $, resolveExecApprovalAllowedDecisions as A, requireValidExecTarget as B, OPTIONAL_EXEC_APPROVAL_DECISIONS as C, minSecurity as D, maxAsk as E, normalizeExecAsk as F, ensureExecApprovalsSnapshot as G, resolveExecModePolicy as H, normalizeExecHost as I, readExecApprovalsSnapshot as J, loadExecApprovals as K, normalizeExecMode as L, resolveExecApprovalUnavailableDecisions as M, DEFAULT_EXEC_APPROVAL_TIMEOUT_MS as N, normalizeExecApprovalUnavailableDecisions as O, EXEC_TARGET_VALUES as P, updateExecApprovals as Q, normalizeExecSecurity as R, DEFAULT_EXEC_APPROVAL_DECISIONS as S, isExecApprovalDecisionAllowed as T, resolveExecPolicyForMode as U, resolveExecModeFromPolicy as V, ensureExecApprovals as W, restoreExecApprovalsSnapshotLocked as X, restoreExecApprovalsSnapshot as Y, saveExecApprovals as Z, persistAllowAlwaysDecision as _, requestExecApprovalViaSocket as a, resolveAllowAlwaysPersistenceDecision as b, recordAllowlistMatchesUse as c, addDurableCommandApproval as d, createExecApprovalPolicySnapshot as f, isExecApprovalPolicySnapshotCurrent as g, hasNodeCommandAllowAlwaysMarker as h, resolveExecApprovalsLocked as i, resolveExecApprovalRequestAllowedDecisions as j, requiresExecApproval as k, recordAllowlistUse as l, hasExactCommandDurableExecApproval as m, resolveExecApprovals as n, requestJsonlSocket as o, hasDurableExecApproval as p, loadExecApprovalsAsync as q, resolveExecApprovalsFromFile as r, commitExecAuthorizationLocked as s, normalizeExecApprovals as t, addAllowlistEntry as u, persistAllowAlwaysPatterns as v, commandRequiresSecurityAuditSuppressionApproval as w, resolveDurableExecApprovalRequirement as x, resolveAllowAlwaysPatternCoverage as y, normalizeExecTarget as z };
