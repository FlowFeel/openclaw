...
import { a as describeFailoverError, c as isFailoverError } from "./failover-error-DBKQENP4.js";
import { a as resolveCapabilityModelCandidates, c as resolveClosestSize, l as resolveMediaProviderRequestTimeoutMs, n as buildNoCapabilityModelConfiguredMessage, o as resolveClosestAspectRatio, s as resolveClosestResolution, t as buildMediaGenerationNormalizationMetadata, u as throwCapabilityGenerationFailure } from "./runtime-shared--HS7nAei.js";
import { n as listImageGenerationProviders, r as parseImageGenerationModelRef, t as getImageGenerationProvider } from "./provider-registry-DB9mcMsr.js";
...
function resolveImageGenerationOverrides(params) {
	...
	if (aspectRatio && (modelGeometry.aspectRatios?.length ?? 0) > 0 && modeCaps.supportsAspectRatio) {
		...
		const derivedSize = modeCaps.supportsSize && !size ? resolveClosestSize({
			...
			requestedAspectRatio: aspectRatio,
			supportedSizes: modelGeometry.sizes?.length === 0 ? geometry?.sizes : modelGeometry.sizes
		}) : void 0;
		let translated = false;
		if (derivedSize) {
			size = derivedSize;
