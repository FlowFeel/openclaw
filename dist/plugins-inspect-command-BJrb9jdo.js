...
async function runPluginsInspectCommand(id, opts) {
	const { buildAllPluginInspectReports, buildPluginDiagnosticsReport, buildPluginInspectReport, buildPluginSnapshotReport, formatPluginCompatibilityNotice } = await import("./status-CoKWGsHn.js");
	const { loadInstalledPluginIndexInstallRecords } = await import("./installed-plugin-index-records-D0VI1KgI.js");
	...
	if (!targetPlugin) {
		if (id === "skill-workshop") {
			const { detectSkillWorkshopToolPolicyDiagnostic } = await import("./tool-policy-diagnostic-BIW1dVZM.js");
