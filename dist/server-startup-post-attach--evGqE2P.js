...
import { i as getActiveGatewayRootWorkCount, m as runWithGatewayIndependentRootWorkAdmission } from "./gateway-work-admission-D_DdbtmL.js";
import { v as sweepSessionStateWatchNotices } from "./session-state-events-MWtkoPhW.js";
import { s as hasRestartSentinel } from "./restart-sentinel-LeBKd1mT.js";
import { t as scheduleGatewayIdleTask } from "./server-idle-task-DjyFy2K9.js";
import { n as measureStartup } from "./server-startup-trace-07DW45RV.js";
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-CrZXFXYx.js";
import { t as hasConfiguredInternalHooks } from "./configured-B9DE7lqE.js";
...
function scheduleContextCachePrewarm(params) {
	...
	const warm = async () => {
		if (stopped) return;
		const { ensureContextWindowCacheLoaded } = await import("./context-CqiCNvJp.js");
...
async function prewarmGatewaySessionListData(cfg, agentId) {
	const [{ loadCombinedSessionStoreForGateway }, { listSessionsFromStoreAsync }] = await Promise.all([import("./combined-store-gateway-ebUxuEqi.js"), import("./session-utils-list-D2Pyd1w5.js")]);
...
function dashboardDataPrewarmItems(cfg, log) {
	...
	const shouldPrewarmSessionData = async () => {
		...
		const { canPrewarmCombinedSessionStoresForGateway } = await import("./combined-store-gateway-ebUxuEqi.js");
	...
	return [
		...
		})),
		{
			name: "plugins",
			load: async () => {
				const { listManagedPlugins } = await import("./management-service-BBqJtsmX.js");
		...
		...agentIds.map((agentId) => ({
			...
			load: async () => {
				if (!await shouldPrewarmSessionData()) return;
				const { prewarmSessionCatalogList } = await import("./session-catalog-CP_mqskv.js");
...
const loadMainSessionRestartRecoveryModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-BESfitEw.js"));
const loadMainSessionRestartRecoveryMarkingModule = createLazyRuntimeModule(() => import("./main-session-restart-recovery-marking-D-1PdFxm.js"));
const loadAgentDefaultsModule = createLazyRuntimeModule(() => import("./defaults-RjT9WtG0.js"));
const loadAgentModelSelectionModule = createLazyRuntimeModule(() => import("./model-selection-ChsC3QwE.js"));
const loadInternalHooksModule = createLazyRuntimeModule(() => import("./internal-hooks-BMqilB60.js"));
const loadGatewayRestartSentinelModule = createLazyRuntimeModule(() => import("./server-restart-sentinel-BVUREIKm.js"));
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		runWithGatewayIndependentRootWorkAdmission(async () => {
			const { startGatewayMemoryBackend } = await import("./server-startup-memory-BYXieAPt.js");
...
function scheduleProviderAuthStatePrewarm(params) {
	...
	runWithGatewayIndependentRootWorkAdmission(async () => {
		...
		const loadProviderAuthWarmModule = () => import("./model-provider-auth-mXkC3nB8.js");
		...
		idleTask = scheduleGatewayIdleTask({
			...
			run: async () => {
				await measureStartup(params.startupTrace, "post-ready.agent-runtime-plugins", async () => {
					...
					const { loadAgentRuntimePluginRegistryHandle } = await import("./runtime-plugins-DeslGTeX.js");
					const cfg = params.getConfig();
					if (isStopped()) return;
...
function scheduleTranscriptsAutoStartSidecar(params) {
	...
	return schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const { createTranscriptsAutoStartService } = await import("./transcripts-tool-CQSu4Rw1.js");
			if (isStopped()) return;
...
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-BZc5jbHb.js");
...
async function publishConfiguredModelRuntimeSnapshots(params) {
	const { refreshPreparedModelRuntimeSnapshots } = await import("./prepared-model-runtime-Pn7NEBqf.js");
...
async function startGatewaySidecars(params) {
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([loadInternalHooksModule(), import("./loader-BhsJ69s8.js")]);
	...
	let pluginServices = params.shouldStartPluginServices?.() === false ? null : await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-CDjhPJ6J.js");
	...
	if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped, signal) => {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-BvSiLESb.js");
			if (isStopped()) return;
	...
	if (params.cfg.hooks?.gmail?.model) postReadySidecars.push(schedulePostReadySidecarTask({
		...
		run: async (isStopped) => {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadPreparedModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				...
				import("./prepared-model-catalog-ClbAcHfK.js"),
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-CfYybPlf.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-BweZzk3I.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	warmSystemCa: warmMacOSSystemCaOffMainThread,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-D3QmtQmW.js")).startGatewayTailscaleExposure(...args)
};
...
async function startGatewayPostAttachRuntime(params, runtimeDeps = defaultGatewayPostAttachRuntimeDeps) {
	...
	const sidecarsPromise = params.minimalTestGateway ? Promise.resolve({
		...
		try {
			const { scheduleSubagentRegistrySweep } = await import("./subagent-registry-CYpnAYTA.js");
	...
	sidecarsPromise.then(async (sidecarsResult) => {
		...
		if (hookRunner?.hasHooks("gateway_start")) {
			const { withPluginHttpRouteRegistry } = await import("./http-registry-DLaGujFC.js");
			runWithGatewayIndependentRootWorkAdmission(async () => {
