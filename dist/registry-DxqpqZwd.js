...
import { S as requireActivePluginRegistry, d as getActivePluginRegistry } from "./runtime-yJAYArQt.js";
import { m as getPluginCompatRecord } from "./installed-plugin-index-CqNI0grC.js";
...
const GUARDED_CONTEXT_ENGINE_METHODS = new Set("bootstrap maintain ingest ingestBatch afterTurn assemble compact prepareSubagentSpawn onSubagentEnded".split(" "));
...
function wrapResolvedContextEngine(engine, metadata) {
	...
	const projectParams = (params) => {
		const useLegacyDefault = removeAfter !== void 0 && (/* @__PURE__ */ new Date()).toISOString().slice(0, 10) <= removeAfter;
		const currentAccepted = accepted ?? (useLegacyDefault ? [] : void 0);
		return currentAccepted ? Object.fromEntries(Object.entries(params).filter(([key]) => currentAccepted.includes(key) || !CONTEXT_ENGINE_HOST_PARAMS.has(key))) : params;
	};
	const wrapped = new Proxy(Object.create(engine, { info: { get: () => engine.info } }), { get(_target, property) {
		...
		if (!fallback || !getFallbackEngine) return (params) => method.call(engine, projectParams(params));
...
//#endregion
export { registerContextEngineForOwner as a, resolveContextEngineOwnerPluginId as c, listContextEngineQuarantines as i, createRuntimeHealthRecordEnvelope as l, activateContextEngineRegistrations as n, registerContextEngineInRegistry as o, getContextEngineRegistration as r, resolveContextEngine as s, CONTEXT_ENGINE_HOST_PARAMS as t, createRuntimeHealthStore as u };
