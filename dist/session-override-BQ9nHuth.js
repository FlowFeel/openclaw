...
import { a as isModelScopedCooldownReason, o as isProfileInCooldown, r as isActiveUnusableWindow } from "./usage-state-C6UiSWt7.js";
import { i as resolveAuthProfileOrder, n as isStoredCredentialCompatibleWithAuthProvider, t as isConfiguredAwsSdkAuthProfileForProvider } from "./order-DCEZOH-n.js";
...
import "./usage-DhtWp5Y_.js";
//#region src/agents/auth-profiles/session-override.ts
const sessionAccessorLoader = createLazyImportLoader(() => import("./session-accessor-DnNtcMuD.js"));
...
async function resolveSessionAuthProfileOverride(params) {
	...
	if ((source !== "user" || !current) && order.every((profileId) => isProfileGloballyInCooldown(store, profileId))) {
		if (current) {
			...
			const latestSource = latest?.authProfileOverrideSource ?? (typeof latest?.authProfileOverrideCompactionCount === "number" ? "auto" : latestProfileId ? "user" : void 0);
