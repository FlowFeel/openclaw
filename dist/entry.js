...
import { n as runCliWithExitFinalization, t as requestExitAfterOneShotOutput } from "./one-shot-exit-B87g-N4r.js";
...
function shouldSkipRespawnForArgv(argv, platform = process.platform) {
	const invocation = resolveCliArgvInvocation(argv);
	return invocation.hasHelpOrVersion || isInteractiveTtyCommandArgv(argv) || shouldKeepNativeHookRelayInProcess(argv, platform) || invocation.primary === "gateway" && isForegroundGatewayRunArgv(argv);
}
...
function shouldSkipStartupEnvironmentRespawnForArgv(argv, platform = process.platform) {
	const invocation = resolveCliArgvInvocation(argv);
	return invocation.hasHelpOrVersion || shouldKeepNativeHookRelayInProcess(argv, platform) || invocation.primary === "gateway" && isForegroundGatewayRunArgv(argv);
}
//#endregion
...
function buildOpenClawCompileCacheRespawnPlan(params) {
	const env = params.env ?? process$1.env;
	const argv = params.argv ?? process$1.argv;
	const platform = params.platform ?? process$1.platform;
	if (shouldKeepNativeHookRelayInProcess(argv, platform)) return;
	if (!(isSourceCheckoutInstallRoot(params.installRoot) || platform === "win32" && isNodeVersionAffectedByCompileCacheDeadlock(params.nodeVersion ?? process$1.versions.node))) return;
...
function buildCliRespawnPlan(params = {}) {
	...
	if (shouldSkipStartupEnvironmentRespawnForArgv(normalizedArgv, platform) || isTruthyEnvValue(env.OPENCLAW_NO_RESPAWN)) return null;
	const childEnv = { ...env };
	...
	if (!shouldSkipRespawnForArgv(argv, platform) && !isTruthyEnvValue(env[OPENCLAW_NODE_OPTIONS_READY]) && !hasExperimentalWarningSuppressed({
		env,
		execArgv
	})) {
...
function runCliRespawnPlan(plan, runtime, writeError = (message, error) => console.error(message, error)) {
	...
	return runRespawnChildWithSignalBridge({
		...
		onError: (error) => {
			resolvedRuntime.writeError("[openclaw] Failed to respawn CLI:", error instanceof Error ? error.stack ?? error.message : error);
		}
	});
}
//#endregion
...
function tryHandleRootVersionFastPath(argv, deps = {}) {
	...
	(deps.resolveVersion ?? (async () => {
		const [{ VERSION }, { resolveCommitHash }] = await Promise.all([import("./version-BmsGkjsI.js"), import("./git-commit-DCqqsU-t.js")]);
		return {
			VERSION,
			resolveCommitHash
		};
...
if (!isMainModule({
	...
	if (!await respawnWithoutOpenClawCompileCacheIfNeeded({
		...
		async function ensureCliRespawnReady() {
			...
			if (!plan) return false;
			runCliRespawnPlan(plan, void 0, await prepareCliDiagnosticBlockWriter());
			return true;
		}
		if (!await ensureCliRespawnReady()) {
			...
			const containerTargetName = resolveCliContainerTarget(process$1.argv);
			if (parsed.profile) {
...
async function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	const handleError = deps.onError ?? (async (error) => {
		await writeCliDiagnosticBlock(`[openclaw] Failed to display help: ${error instanceof Error ? error.stack ?? error.message : String(error)}`);
		process$1.exit(1);
	});
	try {
		...
		await (deps.outputRootHelp ?? (await import("./root-help-D-eqCVAT.js")).outputRootHelp)(liveRootHelpOptions ?? void 0);
		return true;
	} catch (error) {
	...
	await runCliWithExitFinalization({
		run: async () => {
			if (isNativeHookRelayArgv(argv) && !argv.includes("--help") && !argv.includes("-h")) {
				const { runNativeHookRelayCliFromArgv } = await import("./native-hook-relay-cli-BOTsu2ug.js");
