...
import { F as stripUserEnvelopeForDisplay, I as stripEnvelope, P as stripInternalMetadataForDisplay } from "./openclaw-state-db-BU55lNCH.js";
import { t as readFileWindowFully } from "./file-read-DtMn74uz.js";
import { t as extractInboundSenderLabel } from "./strip-inbound-meta-BOGiSpdi.js";
import { E as materializeSessionArchiveForRead } from "./paths-DSnYpBD3.js";
import { Et as streamSessionTranscriptLines, M as hasNonzeroUsage, P as normalizeUsage, j as deriveSessionTotalTokens } from "./session-accessor.sqlite-B9iW7DOt.js";
import { x as selectSessionTranscriptActiveEntries } from "./session-transcript-index-cy-aJty7.js";
import { n as extractAssistantVisibleText } from "./chat-message-content-DwX2R88T.js";
import { a as waitForSessionTranscriptProjection } from "./session-transcript-reconcile-DSlGBhqD.js";
import { _ as readSessionTranscriptMessageEvents, b as isSessionTranscriptProjectionUnavailableError, g as readSessionTranscriptMessageEventPage, h as readSessionTranscriptMessageEventCount, l as readRecentSessionTranscriptMessageEvents, m as readSessionTranscriptMessageEventById, n as resolveSessionTranscriptReadTarget, t as resolveConcreteSessionStorePath } from "./session-accessor-t3qUoTeV.js";
...
import { a as resolveSessionTranscriptCandidates, o as resolveSessionTranscriptResetArchiveCandidatesAsync } from "./session-transcript-files.fs-DUSeZC1w.js";
...
async function readSessionMessagesAsync(scope, opts) {
	...
	if (opts.mode === "recent") {
		...
		if (records.length === 0 && opts.allowResetArchiveFallback === true) return (await archivedTranscriptReader(target).read({
			...opts,
...
async function readRecentSessionMessagesWithStatsAsync(scope, opts) {
	const target = resolveTranscriptReadTarget(scope);
	const { activeLeafEntryId, records, transcriptEvents, totalMessages } = await readRecentSqliteMessageRecords(target, opts);
	if (totalMessages === 0 && records.length === 0 && opts.allowResetArchiveFallback === true) return await archivedTranscriptReader(target).readRecentWithStats({
		...opts,
		resetArchiveOnly: true
	});
	return {
...
async function readLatestSessionUsageFromTranscriptAsync(scope) {
	...
	if (!Boolean(targetAgentId && scope.sessionKey?.trim() && concreteStorePath) && artifactFile && path.isAbsolute(artifactFile) && artifactFile.endsWith(".jsonl")) return await readLatestSessionUsageFromTranscriptAsync$1(scope.sessionId, concreteStorePath, artifactFile, void 0);
...
//#endregion
export { attachOpenClawTranscriptMeta as _, readRecentSessionUsageFromTranscript as a, stripEnvelopeFromMessage as b, readSessionMessagesAsync as c, readSessionPreviewItemsFromTranscript as d, resolveTranscriptReadTarget as f, ArchivedTranscriptReader as g, visitSessionMessagesAsync as h, readRecentSessionMessagesWithStatsAsync as i, readSessionMessagesPageWithStatsAsync as l, toTranscriptReadScope as m, extractMessageText as n, readSessionMessageByIdAsync as o, sqliteMessageEventWithSeq as p, readLatestSessionUsageFromTranscriptAsync as r, readSessionMessageCountAsync as s, extractMessageRole as t, readSessionMessagesWithSourceAsync as u, capArrayByJsonBytes as v, stripEnvelopeFromMessages as x, resolveSessionHistoryTranscriptPathAsync as y };
