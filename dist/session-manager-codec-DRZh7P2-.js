...
import { D as buildSessionContext$1, I as uuidv7 } from "./agent-core-00z91UZG.js";
...
function isIndexedSessionEntry(entry) {
	if (!isJsonRecord(entry) || !isSessionEntryType(entry.type) || typeof entry.id !== "string" || entry.id.length === 0 || entry.parentId !== void 0 && entry.parentId !== null && typeof entry.parentId !== "string" || entry.timestamp !== void 0 && typeof entry.timestamp !== "string") return false;
	switch (entry.type) {
...
function isReadableContent(value) {
	return typeof value === "string" || Array.isArray(value) && value.every((part) => isJsonRecord(part) && typeof part.type === "string");
}
function isReadableMessage(value) {
	if (!isJsonRecord(value) || typeof value.role !== "string") return false;
	switch (value.role) {
		case "user":
...
function isReadableLegacySessionEntry(value) {
	const message = isJsonRecord(value) && value.type === "message" ? value.message : void 0;
	const readableLegacyMessage = isJsonRecord(message) && message.role === "hookMessage" ? isReadableContent(message.content) : isReadableMessage(message);
	return isJsonRecord(value) && isSessionEntryType(value.type) && (value.type !== "message" || readableLegacyMessage);
}
function normalizePersistedLegacyHookMessage(value) {
	if (!isJsonRecord(value) || value.type !== "message" || !isJsonRecord(value.message)) return value;
...
function partitionSessionFileEntries(entries) {
	...
	const acceptsLegacyEntries = (entries.find((entry) => isJsonRecord(entry) && entry.type === "session" && typeof entry.id === "string")?.version ?? 1) < 3;
	let hasHeader = false;
...
//#endregion
export { isSessionContextMetadataEntry as a, migrateToCurrentVersion as c, parseParentLinkedOpaqueEntry as d, parseSessionEntries as f, generateSessionEntryId as h, isJsonRecord as i, normalizeLoadedFileEntry as l, createSessionId as m, getLatestCompactionEntry as n, migrateSessionEntries as o, partitionSessionFileEntries as p, isIndexedSessionEntry as r, migrateSessionFileEntryToCurrentVersion as s, buildSessionContext as t, parseOpaqueLeafEntry as u };
