...
import { c as resolveMemorySearchConfig, u as globalMemoryDegradationManager } from "./config-utils-bVWMjALM.js";
...
function hasExplicitAgentMemorySearchConfig(cfg, agentId) {
	return listAgentEntries(cfg).some((entry) => normalizeAgentId(entry.id) === agentId && entry.memory?.search != null);
}
...
function shouldEagerlyStartAgentMemory(params) {
	...
	if (params.cfg.memory?.search?.enabled === true) return true;
...
async function startGatewayMemoryBackend(params) {
	...
	for (const agentId of agentIds) {
		...
		try {
			...
			if (resolveMemorySearchConfig(params.cfg, agentId)?.failOnMissingProvider) throw new Error(`Fatal: memory.search embedding provider unreachable for agent "${agentId}": ${String(err)}`);
			globalMemoryDegradationManager.markDegraded({
				agentId,
				reason: String(err),
				fallbackMode: "fts_bm25"
			});
			params.log.warn(`[memory/search] embeddings provider unreachable for agent "${agentId}"; gracefully falling back to BM25 FTS mode: ${String(err)}`);
			continue;
		} finally {
