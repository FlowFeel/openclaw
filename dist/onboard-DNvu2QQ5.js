...
import { C as resolveStateDir, a as isDefaultStateDir, g as resolveGatewayPort } from "./paths-CL43LNS6.js";
...
import { Q as ConfigMutationConflictError, l as readConfigFileSnapshot } from "./io-DCw4R0kD.js";
import { _ as resolveSecretInputRef, f as isValidEnvSecretRefId } from "./types.secrets-BvApkFoj.js";
...
import { n as resolveConfiguredSecretInputWithFallback } from "./resolve-configured-secret-input-string-DxIa9XNX.js";
import { t as resolveGatewayAuthToken } from "./auth-token-resolution-B1No1sDg.js";
import { t as randomToken } from "./random-token-B1woZa_H.js";
import { r as formatInvalidPortOption } from "./error-format-BZ5G9eas.js";
import { r as resolveLocalControlUiProbeLinks } from "./control-ui-links-opBk-Hn5.js";
import { r as resolveManifestProviderAuthChoices } from "./provider-auth-choices-t9Huvqjl.js";
import { g as waitForGatewayReachable, l as normalizeGatewayTokenInput, n as applyWizardMetadata, s as handleReset, t as DEFAULT_WORKSPACE } from "./onboard-helpers-BlDftc97.js";
import { n as ensureOnboardingAgentWorkspace, r as resolveOnboardingAgentTarget } from "./onboard-agent-target-B75RdAGh.js";
import { n as t } from "./i18n-BYpJa9f7.js";
import { i as resolveProviderMatch } from "./provider-auth-choice-helpers-BK2qlXIT.js";
import { n as resolveProviderInstallCatalogEntries } from "./provider-install-catalog-BNM2sDYU.js";
import { o as normalizeTokenProviderInput } from "./provider-auth-input-DNzGlXUB.js";
import { r as logConfigUpdated } from "./logging-DDOofdYg.js";
import { r as formatAuthChoiceChoicesForCli } from "./auth-choice-options-DntxAwYM.js";
import { i as resolveDeprecatedAuthChoiceReplacement, n as isDeprecatedAuthChoice, r as normalizeLegacyOnboardAuthChoice, t as formatDeprecatedNonInteractiveAuthChoiceError } from "./auth-choice-legacy-CfBx4PYl.js";
import { t as resolvePluginProviders } from "./provider-auth-choice.runtime-CBQj83JM.js";
import { c as parseNonInteractiveCustomApiFlags, d as resolveCustomProviderId, n as applyCustomApiConfig, t as CustomApiError } from "./onboard-custom-config-CfOEv2ny.js";
import { n as validateGatewayWebSocketUrl } from "./onboard-remote-BFppbWUD.js";
import { n as isOnboardFlow, t as isNodeManagerChoice } from "./onboard-types-Du2Y9b-2.js";
import { n as applySkipBootstrapConfig, r as resolveOnboardingWorkspaceConflict, t as applyLocalSetupWorkspaceConfig } from "./onboard-config-DSJZ-plP.js";
import { t as runGuidedOnboarding } from "./onboard-guided-Ab2Nk4Lt.js";
import { t as hasInteractiveOnboardingTty } from "./onboard-interactive-runner-DCjqfU2_.js";
import { t as enableDefaultOnboardingInternalHooks } from "./onboard-hooks-Ca9woCpp.js";
import { n as runInteractiveSetup } from "./onboard-interactive-C43tVVzh.js";
import { t as inferAuthChoiceFromFlags } from "./auth-choice-inference-RuNqC507.js";
import { n as resolveNonInteractiveApiKey, t as createNonInteractiveLoggingPrompter } from "./non-interactive-prompter-Dfr8khjE.js";
...
async function commitNonInteractiveOnboardConfig(params) {
	const { writeWizardConfigFile } = await import("./setup.shared-BQDwhL8U.js");
...
async function collectGatewayHealthFailureDiagnostics() {
	...
	try {
		const { readLastGatewayErrorLine } = await import("./diagnostics-DygeoVra.js");
...
async function runNonInteractiveLocalSetup(params) {
	...
	const inferredAuthChoice = opts.authChoice ? void 0 : (await import("./auth-choice-inference-DZqvqOyw.js")).inferAuthChoiceFromFlags(opts, {
		config: nextConfig,
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-YFG9-ccY.js");
	...
	const { ensureOnboardingAgent } = await import("./onboard-agent-C0hW7e2o.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-QhqRaD-j.js");
	...
	if (!opts.skipHealth) {
		const { healthCommand } = await import("./health-Bk21SCBX.js");
...
async function runNonInteractiveMigrationImport(params) {
	...
	const { detectSetupMigrationSources, runSetupMigrationImport } = await import("./setup.migration-import-BK1_YNN0.js");
	const detections = await detectSetupMigrationSources({
		config: params.baseConfig,
		runtime: params.runtime
	});
	await (await runSetupMigrationImport({
		opts: {
			...params.opts,
...
function validatePreflightOptions(opts, runtime) {
	...
	const remoteOnlyFlags = [opts.remoteUrl !== void 0 ? "--remote-url" : void 0, opts.remoteToken !== void 0 ? "--remote-token" : void 0].filter((flag) => flag !== void 0);
	if (opts.nonInteractive && (opts.mode ?? "local") === "local" && remoteOnlyFlags.length > 0) return rejectOption(runtime, `${remoteOnlyFlags.join(" and ")} ${remoteOnlyFlags.length === 1 ? "requires" : "require"} --mode remote in non-interactive setup.`);
	...
	if (opts.gatewayTokenRefEnv !== void 0) {
		...
		if (!isValidEnvSecretRefId(gatewayTokenRefEnv)) return rejectOption(runtime, "Invalid --gateway-token-ref-env. Use an environment variable name like OPENCLAW_GATEWAY_TOKEN.");
		if (opts.gatewayToken !== void 0) return rejectOption(runtime, "Use either --gateway-token or --gateway-token-ref-env, not both. Prefer --gateway-token-ref-env to avoid writing plaintext tokens.");
		if (!process.env[gatewayTokenRefEnv]?.trim()) return rejectOption(runtime, `Environment variable "${gatewayTokenRefEnv}" is missing or empty. Export it first, then rerun ${formatCliCommand("openclaw onboard")}.`);
	}
...
function wantsClassicInteractiveSetup(opts) {
	...
	if (opts.installDaemon !== void 0 || opts.tailscaleResetOnExit !== void 0 || opts.customImageInput !== void 0) return true;
...
async function setupWizardCommand(opts, runtime = defaultRuntime) {
	...
	if (normalizedOpts.tui && normalizedOpts.nonInteractive) {
		runtime.error("--tui cannot be combined with --non-interactive. Remove --tui for automation, or remove --non-interactive to open the terminal hatch.");
		runtime.exit(1);
		return;
	}
	...
	if (normalizedOpts.resetScope && !normalizedOpts.reset) {
		runtime.error(`--reset-scope requires --reset. Re-run with ${formatCliCommand(`openclaw onboard --reset --reset-scope ${normalizedOpts.resetScope}`)}.`);
		runtime.exit(1);
		return;
	}
