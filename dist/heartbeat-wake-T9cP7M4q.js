...
function mergePendingWakeReasons(previous, next) {
	...
	const bypassGuardRetry = (preferred.intent === "manual" || preferred.intent === "immediate") && preferred.guardRetry !== true && (previous.guardRetry === true || next.guardRetry === true);
	...
	const merged = {
		...
		readyAtMs,
		...!bypassGuardRetry && (previous.notBeforeMs !== void 0 || next.notBeforeMs !== void 0) ? {
	...
	if (!bypassGuardRetry && (previous.guardRetry || next.guardRetry)) merged.guardRetry = true;
	else delete merged.guardRetry;
	...
	for (const { targetKey, group } of readyGroups) {
		...
		if (group.task) {
			...
			if (group.event) wakes.push(...[taskWake, group.event].toSorted((left, right) => {
				if (left.guardRetry !== right.guardRetry) return left.guardRetry ? -1 : 1;
...
function retryPendingWake(pendingWake) {
...
async function dispatchPendingWakeGroup(params) {
	...
	try {
		for (const [wakeIndex, pendingWake] of wakes.entries()) {
			...
			if (handlerGeneration !== generation) {
				handOffPendingWakeBatch(wakes, wakeIndex + (result.status === "skipped" && (isRetryableHeartbeatBusySkipReason(result.reason) || RETRYABLE_GUARD_SKIP_REASONS.has(result.reason) && (pendingWake.tasks?.length || pendingWake.intent === "task" || pendingWake.intent === "event" || pendingWake.intent === "immediate")) ? 0 : 1));
				return;
			}
...
//#endregion
export { requestHeartbeat as a, getHeartbeatWakeAbortSignal as c, isRetryableHeartbeatBusySkipReason as i, HEARTBEAT_SKIP_REQUESTS_IN_FLIGHT as n, setHeartbeatWakeHandler as o, areHeartbeatsEnabled as r, setHeartbeatsEnabled as s, HEARTBEAT_SKIP_CRON_IN_PROGRESS as t };
