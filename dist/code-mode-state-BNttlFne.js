...
import { At as emitSessionLifecycleEvent } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { n as ToolInputError, r as asToolParamsRecord } from "./common-yW0U9cHP.js";
import { t as toCodeModeJsonSafe } from "./code-mode-json-CQ3LP5c6.js";
import { n as CODE_MODE_WAIT_TOOL_NAME, t as CODE_MODE_EXEC_TOOL_NAME } from "./code-mode-control-tools-Bts-Jy9S.js";
import "./tool-search-BhU6HOAE.js";
import { m as initSubagentRegistry, p as getSwarmRunByLaunchReplayKey } from "./subagent-registry-C4ALZQd0.js";
...
import { i as waitForCollectorCompletion, n as SWARM_CODE_MODE_REQUEST_FINGERPRINT, t as SWARM_CODE_MODE_IDEMPOTENCY_KEY } from "./swarm-code-mode-DiB2V1M4.js";
import { t as parseNodeList } from "./node-list-parse-BlhiSeiu.js";
import { n as resolveEligibleNodeFromList } from "./nodes-utils-DC2wdBCD.js";
...
function setCodeModeSwarmDepsForTest(overrides) {
...
const JAVASCRIPT_GLOBAL = /^(?:ALL_TOOLS|AggregateError|Array|ArrayBuffer|BigInt|BigInt64Array|BigUint64Array|Boolean|DataView|Date|Error|EvalError|Float32Array|Float64Array|Function|Infinity|Int16Array|Int32Array|Int8Array|Intl|JSON|Map|Math|NaN|Number|Object|Promise|Proxy|RangeError|ReferenceError|Reflect|RegExp|Set|String|Symbol|SyntaxError|TypeError|URIError|Uint16Array|Uint32Array|Uint8Array|Uint8ClampedArray|WeakMap|WeakSet|console|decodeURI|decodeURIComponent|encodeURI|encodeURIComponent|eval|globalThis|isFinite|isNaN|parseFloat|parseInt|tools)$/u;
...
const CODE_MODE_SHELL_SOURCE_ERROR = "code-mode exec runs JavaScript or TypeScript, not shell commands. Call shell, file, or other tools from guest JavaScript with tools.callValue and an exact tool id from ALL_TOOLS or tools.search. Do not retry the same shell command as code.";
//#endregion
...
function enforceResultLimit(params) {
	...
	if (serializedOutputBytes > params.config.maxOutputBytes) throw new CodeModeLimitError("output_limit_exceeded", "code mode output limit exceeded");
	const outputBytes = params.output.length > 0 ? serializedOutputBytes : 0;
	if (params.value !== void 0 && outputBytes + jsonByteLength(params.value) > params.config.maxOutputBytes) throw new CodeModeLimitError("output_limit_exceeded", "code mode output limit exceeded");
}
...
async function loadTypeScriptRuntime() {
	if (typescriptRuntimeForTest) return await typescriptRuntimeForTest;
...
function pendingBridgeRequestsReplaySafe(pending, runtime) {
	return pending.every((request) => {
		if (request.method === "search" || request.method === "describe" || request.method === "yield" || request.method === "agentSpawn" || request.method === "agentWait" || request.method === "skillsList" || request.method === "skillsRead") return true;
...
//#endregion
export { prepareSource as A, resolveCodeModeSkills as B, codeModeFailureMessage as C, enforceResultLimit as D, enforceOutputLimit as E, resolveCodeModeHeadlessConfig as F, toToolSearchConfig as I, codeModeReplayIdForToolCall as L, readPositiveInteger as M, readRunId as N, enforceSnapshotPayloadLimits as O, resolveCodeModeConfig as P, runBridgeRequest as R, codeModeFailureCode as S, createCodeModeApiFilesForRun as T, telemetry as _, disposeAllCodeModeRuns as a, DEFAULT_HEADLESS_WALL_CLOCK_MS as b, pendingBridgeStatesForSettlement as c, reserveActiveRunSlot as d, resumingRunIds as f, takeUndeliveredCodeModeRunOutput as g, storeSnapshotState as h, createPendingBridgeStates as i, readCode as j, isCodeModeEngagedForModel as k, pendingToolCalls as l, snapshotState as m, cancelPendingBridgeStates as n, disposeCodeModeRun as o, settledBridgeRequestsInCompletionOrder as p, codeModeWaitingReason as r, pendingBridgeRequestsReplaySafe as s, activeRuns as t, removeExpiredRuns as u, waitForPendingBridgeSettlement as v, codeModeRuntimeTesting as w, MAX_HEADLESS_WALL_CLOCK_MS as x, CODE_MODE_WORKER_WATCHDOG_GRACE_MS as y, setCodeModeSwarmDepsForTest as z };
