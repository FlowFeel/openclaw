...
function enforceWorkerOutputLimit(value, config, consumedBytes = 0) {
	const bytes = Buffer.byteLength(JSON.stringify(toCodeModeJsonSafe(value)) ?? "null", "utf8");
	if (consumedBytes + bytes > config.maxOutputBytes) throw new CodeModeWorkerFailure("output_limit_exceeded", "code mode output limit exceeded");
	return bytes;
}
function throwWorkerFailureWithOutput(params) {
	...
	if (params.error instanceof CodeModeWorkerFailure && params.error.code === "output_limit_exceeded") throw new CodeModeWorkerFailureWithOutput(params.error.code, params.error.message, [], { cause: params.error });
	try {
		enforceWorkerOutputLimit(failureOutput, params.config);
	} catch (error) {
		if (error instanceof CodeModeWorkerFailure) throw new CodeModeWorkerFailureWithOutput(error.code, error.message, [], { cause: error });
		throw error;
	}
...
async function main() {
	...
	try {
		...
		const code = timedOut ? "timeout" : error instanceof CodeModeWorkerFailure ? error.code : "internal_error";
		return {
			status: "failed",
