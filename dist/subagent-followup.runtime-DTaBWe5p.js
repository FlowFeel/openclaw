...
import { c as listDescendantRunsForRequester } from "./subagent-registry-read-DBYMH4dZ.js";
import { u as stripHeartbeatToken } from "./heartbeat-Cw1AzhxC.js";
import { r as readLatestAssistantReply, s as waitForAgentRunsToDrain } from "./run-wait-DPJzrI0H.js";
...
async function readDescendantSubagentFallbackReply(params) {
	const descendants = listDescendantRunsForRequester(params.sessionKey).filter((entry) => typeof entry.execution.endedAt === "number" && entry.execution.endedAt >= params.runStartedAt && entry.childSessionKey.trim().length > 0).toSorted((a, b) => (a.execution.endedAt ?? 0) - (b.execution.endedAt ?? 0));
	...
	for (const entry of descendants) {
		...
		if (!current || (entry.execution.endedAt ?? 0) >= (current.execution.endedAt ?? 0)) latestByChild.set(childKey, entry);
	}
	const replies = [];
	const latestRuns = [...latestByChild.values()].toSorted((a, b) => (a.execution.endedAt ?? 0) - (b.execution.endedAt ?? 0)).slice(-4);
...
async function waitForDescendantSubagentSummary(params) {
	...
	const getActiveRuns = () => listDescendantRunsForRequester(params.sessionKey).filter((entry) => typeof entry.execution.endedAt !== "number");
	...
	const resolveUsableLatestReply = async () => {
		...
		if (latest && latest.toUpperCase() !== "NO_REPLY".toUpperCase() && !stripHeartbeatToken(latest, {
			mode: "heartbeat",
			maxAckChars: 0
		}).shouldSkip && !isSilentReplyPayloadText(latest, "HEARTBEAT_OK") && (latest !== initialReply || !isLikelyInterimCronMessage(latest))) return latest;
	};
