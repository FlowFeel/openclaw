...
import { i as normalizeModelCatalogProviderId, t as buildModelCatalogMergeKey } from "./model-catalog-refs-C_8nyypZ.js";
import { u as normalizeModelCatalogProviderRows } from "./manifest-BUOsMMgJ.js";
	...
	return Object.entries(providers).flatMap(([provider, providerCatalog]) => {
		...
		return plannedProviders.flatMap((plannedProvider) => {
			...
			const remoteRows = remoteProvider ? normalizeModelCatalogProviderRows({
				...
				providerCatalog: {
					...providerDefaults,
					models: remoteProvider.models.map((model) => mergeRemoteModelWithTrustedTransport(model, manifestModelsById.get(model.id)))
				},
