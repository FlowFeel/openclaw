...
function updateChatRunProgressSnapshot(snapshot, event) {
	...
	if (isTool) {
		...
		if (!(typeof data.progressText === "string" ? data.progressText.trim() : "")) {
			if (preambleItemId) removeWhere((candidate) => {
				if (candidate.stream !== "item" || candidate.data?.kind !== "preamble") return false;
