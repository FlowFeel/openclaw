...
import { i as clampNumber, r as clampInt } from "./utils-Bs67j6-3.js";
import "./paths-CL43LNS6.js";
import { r as normalizeProviderId } from "./provider-id-BIcU_2-A.js";
import { o as resolveAgentConfig$1 } from "./agent-scope-config-Dusa8eSA.js";
import { n as normalizeAgentId } from "./agent-id-DDgUze4y.js";
import "./subsystem-Cr19cPPQ.js";
import "./sqlite-wal-CMGyl8qq.js";
import "./internal-runtime-context-s8_TwHnQ.js";
...
import { n as resolveProviderEndpoint } from "./provider-attribution-D6GRQEFS.js";
import "./mime-Ir6g3Vae.js";
...
import "./input-provenance-CYGbY89H.js";
import "./heartbeat-Cw1AzhxC.js";
import "./memory-state-CngllfdS.js";
import { t as runtimeMemorySecretOwnerId } from "./runtime-memory-secret-owner-CRIh0blb.js";
...
import { t as getMemoryEmbeddingProvider } from "./memory-embedding-provider-runtime-zFJ3Eme9.js";
import "./heartbeat-filter-DUXErom1.js";
import "./session-store-runtime-dul9f0ER.js";
import "./heartbeat-events-filter-sLs_6Z5x.js";
import "./config-schema-Cjz2ed1U.js";
...
//#endregion
//#region packages/memory-host-sdk/src/host/memory-search-degradation.ts
function normalizeKey(agentId) {
	if (!agentId || !agentId.trim()) return "default";
	const norm = agentId.trim().toLowerCase();
	return norm === "main" ? "default" : norm;
}
var MemorySearchDegradationManager = class {
	constructor() {
		this.degradedMap = /* @__PURE__ */ new Map();
	}
	getStatus(agentId) {
		const key = normalizeKey(agentId);
		return this.degradedMap.get(key) ?? (agentId ? this.degradedMap.get(agentId) : void 0) ?? {
			isDegraded: false,
			fallbackMode: "none"
		};
	}
	markDegraded(params) {
		...
		if (key !== params.agentId) this.degradedMap.set(params.agentId, status);
	}
	restoreHealthy(agentId) {
		const key = normalizeKey(agentId);
		this.degradedMap.delete(key);
		if (agentId) this.degradedMap.delete(agentId);
	}
	applyRuntimeDegradation(config, agentId) {
		if (!this.getStatus(agentId).isDegraded) return config;
		return {
			...config,
			store: {
				...config.store,
...
const globalMemoryDegradationManager = new MemorySearchDegradationManager();
//#endregion
...
function mergeConfig(cfg, defaults, overrides, agentId) {
	...
	return {
		...
		failOnMissingProvider: overrides?.failOnMissingProvider === true || defaults?.failOnMissingProvider === true,
		fallbackToFtsOnFailure: overrides?.fallbackToFtsOnFailure !== false && defaults?.fallbackToFtsOnFailure !== false,
		embeddingConnectTimeoutMs: 3e3,
		fallback,
		model,
		inputType,
		queryInputType,
...
function resolveMemorySearchConfig(cfg, agentId) {
	...
	const overrides = resolveAgentConfig$1(cfg, agentId)?.memory?.search;
...
function readTranscriptContentRevisionSync(params) {
	const stats = readSqliteTranscriptStatsSync(params);
	return [
		"sqlite",
		stats.maxSeq,
		stats.sizeBytes,
...
//#endregion
export { applyAgentCompactionSettingsFromConfig as _, resolveMemoryHostUserPath as a, resolveMemorySearchConfig as c, buildCaseInsensitiveExtensionGlob as d, buildMemoryMultimodalLabel as f, applyAgentAutoCompactionGuard as g, DEFAULT_AGENT_COMPACTION_RESERVE_TOKENS_FLOOR as h, resolveMemoryHostSearchPathConfig as i, resolveMemorySearchSyncConfig as l, getMemoryMultimodalExtensions as m, resolveMemoryHostAgentContextLimits as n, resolveRememberAcrossConversations as o, classifyMemoryMultimodalPath as p, resolveMemoryHostAgentWorkspaceDir as r, readTranscriptContentRevisionSync as s, MEMORY_HOST_ROOT_FILENAME as t, globalMemoryDegradationManager as u, isSilentOverflowProneModel as v, resolveEffectiveCompactionMode as y };
