...
function normalizeImportRunEntries(value) {
	...
	return value.flatMap((raw) => {
		...
		const contentHash = typeof entry.contentHash === "string" && entry.contentHash.trim() ? entry.contentHash.trim() : void 0;
...
function normalizeMemoryWikiImportRunRecord(raw) {
	...
	const rollbackStartedAt = typeof record.rollbackStartedAt === "string" && record.rollbackStartedAt.trim() ? record.rollbackStartedAt.trim() : void 0;
	const rollbackTargetsFinalizedAt = typeof record.rollbackTargetsFinalizedAt === "string" && record.rollbackTargetsFinalizedAt.trim() ? record.rollbackTargetsFinalizedAt.trim() : void 0;
	return {
		version: 1,
		runId,
		importType: "chatgpt",
...
function normalizePathRecord(raw) {
	...
	const contentHash = typeof record.contentHash === "string" && record.contentHash.trim() ? record.contentHash.trim() : void 0;
...
async function readLegacyMemoryWikiImportRunRecords(vaultRoot) {
	...
	return await pMap((await walkMemoryWikiDirectory(importRunsDir, "", {
		maxDepth: 1,
		...
		throw error;
	})).filter((entry) => entry.kind === "file"), async (entry) => {
