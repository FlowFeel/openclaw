import { ct as loadSqliteSessionEntryReadOnly, lt as patchSqliteSessionEntry } from "./session-accessor.sqlite-B9iW7DOt.js";
...
async function recordSessionSkillCaptureSignals(options) {
	...
	const result = await patchSqliteSessionEntry(options, (entry) => ({ skillCaptureSignalHashes: appendSignalHashes(entry, signalHashes) }), { preserveActivity: true });
...
async function claimSessionSkillCaptureSignals(options) {
	...
	return await patchSqliteSessionEntry(options, (entry) => {
...
async function releaseSessionSkillCaptureSignals(options) {
	...
	await patchSqliteSessionEntry(options, (entry) => ({ skillCaptureSignalHashes: entry.skillCaptureSignalHashes?.filter((hash) => !released.has(hash)) }), { preserveActivity: true });
}
...
async function consumeSessionSkillSuggestion(options) {
	let currentEntry;
	let suggestion;
	const entry = await patchSqliteSessionEntry(options, (entry) => {
		currentEntry = entry;
