...
import { p as executeSqliteQuerySync } from "./node-sqlite-B_0DgpUE.js";
...
import { Q as buildConversationIdentity, Z as upsertConversationIdentity, nt as parseSqliteSessionEntryJson } from "./targets-Bz-meSET.js";
import { i as getSessionKysely, o as resolveSqliteReadScope, p as toDatabaseOptions } from "./session-accessor.sqlite-scope-G-CS03gJ.js";
import { it as validateConversationTurnParams, nt as validateConversationSendParams, rt as validateConversationTurnCancelParams, tt as validateConversationListParams } from "./src-BSn6va4B.js";
...
import { c as ConversationDeliveryInputError, d as getConversationDeliveryOperation, f as markConversationDeliveryQueued, h as markConversationDeliverySuppressed, l as beginConversationDeliveryOperation, m as markConversationDeliverySent } from "./delivery-queue-reconciliation-D8stEE20.js";
import { r as resolveMessageReceiptPrimaryId } from "./receipt-DSE0BXtY.js";
import { r as resolveOutboundSessionRoute, t as bindOutboundSessionEntry } from "./outbound-session-BNtSdGwc.js";
import { n as runMessageAction } from "./message-action-runner-CByxJGJu.js";
import { r as registerPendingConversationTurn, t as cancelPendingConversationTurn } from "./conversation-turns-BOzEqcJN.js";
...
function mapConversationRow(row) {
	...
	const hasCurrentBinding = (row.current_entry_json ? parseSqliteSessionEntryJson({ entry_json: row.current_entry_json }) : null)?.sessionId === row.current_session_id;
	return {
...
function createConversationHandlers(overrides = {}) {
	...
	return {
		"conversations.list": async ({ params, respond, context }) => {
			if (!assertValidParams(params, validateConversationListParams, "conversations.list", respond)) return;
		...
		"conversations.send": async ({ params, respond, context, client }) => {
			if (!assertValidParams(params, validateConversationSendParams, "conversations.send", respond)) return;
			const request = params;
		...
		"conversations.turn.cancel": ({ params, respond }) => {
			if (!assertValidParams(params, validateConversationTurnCancelParams, "conversations.turn.cancel", respond)) return;
			const request = params;
		...
		"conversations.turn": async ({ params, respond, context, client }) => {
			if (!assertValidParams(params, validateConversationTurnParams, "conversations.turn", respond)) return;
			const request = params;
			const requestIdentity = bindConversationOperationIdentity(context, {
				method: "turn",
