...
import { c as callGateway, h as isGatewayCredentialsRequiredError, o as buildGatewayConnectionDetails, s as buildGatewayProbeConnectionDetails } from "./call-B_qeDbXh.js";
import { f as redactSecretDegradationReason } from "./runtime-degraded-state-V0p9ck6P.js";
...
async function checkGatewayHealth(params) {
  ...
  Retry: openclaw secrets reload`).join("\n"), "Secret runtime degradation");
		...
		try {
			...
			note([`Channel status probe failed: ${sanitizeTerminalText(formatErrorMessage(error))}`, `Retry: ${formatCliCommand("openclaw channels status --probe")}`].join("\n"), "Channel warnings");
		}
		return {
			healthOk,
			authenticated: true,
