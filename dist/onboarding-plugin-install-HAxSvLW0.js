...
import { i as isOpenClawOrgNpmSpec, s as parseRegistryNpmSpec } from "./npm-registry-spec-CCN1EPEy.js";
import { n as VERSION } from "./version-CeFj_iGk.js";
import { r as resolveDefaultPluginExtensionsDir } from "./install-paths-D18EkQi8.js";
import { l as clearLoadInstalledPluginIndexInstallRecordsCache } from "./installed-plugin-index-record-reader-DA9P0iII.js";
...
import { a as resolveBundledPluginSources, n as findBundledPluginSourceInMap } from "./bundled-sources-D7mf-n0T.js";
import { t as invalidatePluginRuntimeDiscoveryAfterConfigMutation } from "./registry-refresh-Cg60AEHC.js";
import { i as installPluginFromNpmPackArchive, r as installPluginFromNpmSpec } from "./install-BNVuEMPU.js";
import { t as resolveBundledInstallPlanForCatalogEntry } from "./plugin-install-plan-B0dOu6aB.js";
...
async function finishOnboardingPluginInstall(params) {
	...
	return await markOnboardingPluginInstalled({
		cfg: params.install ? recordPluginInstall(enableResult.config, params.install) : enableResult.config,
		pluginId: params.pluginId,
...
async function installLocalOnboardingPlugin(params) {
	...
	return await markOnboardingPluginInstalled({
		cfg: pathsReferToSameDirectory(params.localPath, params.bundledLocalPath) ? enableResult.config : await recordLocalPluginInstall({
...
async function runOnboardingPluginInstallWithProgress(params) {
	...
	try {
		const result = await withTimeout(params.install({
			info: updateProgress,
			warn: (message) => {
		...
		if (params.rethrowUnexpectedErrors) throw error;
		return {
			status: "completed",
			result: {
				ok: false,
...
async function installPluginFromNpmSpecWithProgress(params) {
	return await runOnboardingPluginInstallWithProgress({
		...params,
		install: (logger) => installPluginFromNpmSpec({
			spec: params.npmSpec,
			mode: "update",
			config: params.cfg,
...
async function installPluginFromNpmPackArchiveWithProgress(params) {
	return await runOnboardingPluginInstallWithProgress({
		...params,
		install: (logger) => installPluginFromNpmPackArchive({
...
async function installPluginFromClawHubSpecWithProgress(params) {
	...
	try {
		const { installPluginFromClawHub } = await import("./clawhub-CClo9uUA.js");
