...
import { r as isSensitiveUrlConfigPath, t as SENSITIVE_URL_HINT_TAG } from "./redact-sensitive-url-YLuImt1m.js";
import { n as asNullableRecord } from "./record-coerce-DHZ4bFlT.js";
import { t as pruneMapToMaxSize } from "./map-size-DAGm21RM.js";
import { K as ZodPipe, R as ZodNever, W as ZodObject, Z as ZodRecord, dt as ZodUnion, k as ZodIntersection, n as ZodArray, v as ZodDiscriminatedUnion } from "./schemas-CZ9Toj_c.js";
import { n as VERSION } from "./version-CeFj_iGk.js";
import { i as GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA, t as CHANNEL_IDS } from "./ids-DPg7j456.js";
...
import { a as CLOUD_WORKER_FIELD_LABELS, i as CLOUD_WORKER_FIELD_HELP, t as OpenClawSchema } from "./zod-schema-9ObLtj6p.js";
...
const AGENT_FIELD_HELP = {
	...
	"agents.defaults.compaction.compactAtRatio": "Proactive compaction ratio: fire when context usage reaches this fraction of the context window (default: 0.70). Lower values compact earlier with a smaller, more summarizable context; higher values defer compaction until closer to the limit. Set to 1.0 to disable proactive compaction and use only the reactive edge.",
...
const AUTOMATION_FIELD_HELP = {
	...
	"cron.sessionRetention": "Controls how long completed automation run sessions are kept before pruning (`24h`, `7d`, `1h30m`, or `false` to disable pruning; default: `24h`). Use shorter retention to reduce storage growth on high-frequency schedules.",
...
const CORE_FIELD_HELP = {
	...
	"agents.defaults.contextLimits.maxResultChars": "Override the auto-derived max characters per live tool result. When unset, caps auto-derive from the model context window (16K/32K/64K at 30% share). Set lower (e.g. 8000) for chat bots that mostly read small files and search results.",
	...
	"agents.entries.*.contextLimits.maxResultChars": "Per-agent override for the max characters per live tool result. Omit to inherit the default auto-derived cap.",
...
const MODEL_FIELD_HELP = {
	...
	"memory.search.extraPaths": "Adds extra directories or .md files to the memory index beyond default memory files. Use this when key reference docs live elsewhere in your repo; when multimodal memory is enabled, matching image/audio files under these paths are also eligible for indexing.",
	"memory.search.qmd": "Use this when one agent should query another agent's transcript collections; QMD-specific extra collections let you opt into cross-agent memory search without flattening everything into one shared namespace.",
	"memory.search.qmd.extraCollections": "Use this when you need directional transcript search across agents; add collections here to scope QMD recalls without creating a shared global transcript namespace.",
	"memory.search.qmd.extraCollections.path": "Use an absolute or workspace-relative filesystem path for the extra QMD collection; keep it pointed at the transcript directory or note folder you actually want this agent to search.",
	"memory.search.qmd.extraCollections.name": "Preserves the configured collection label only when the path points outside the agent workspace; paths inside the workspace stay agent-scoped even if a name is provided. Use this for shared cross-agent transcript roots that live outside the workspace.",
	"memory.search.qmd.extraCollections.pattern": "Use a glob pattern to restrict which files inside the collection are indexed; keep the default `**/*.md` unless you need a narrower subset.",
	...
	"memory.qmd.searchTool": "Overrides the exact QMD search command while preserving `searchMode` as the semantic retrieval mode. Keep it unset for the normal built-in tool mapping.",
...
const RUNTIME_FIELD_HELP = {
	...
	attachments: "Top-level attachment retention behavior shared across providers and tools that handle inbound files. Use ttlHours when persisted inbound media needs bounded cleanup.",
	"attachments.ttlHours": "Optional retention window in hours for persisted media cleanup across the full media tree. Leave unset to disable automatic cleanup (media writes never prune), or set values like 24 (1 day) or 168 (7 days) to periodically remove media older than the window.",
	...
	"skills.workshop.autonomous.mode": "Controls background learning: \"off\" keeps only the suggestion nudge, \"propose\" creates pending proposals, and \"auto\" applies captured proposals through the normal scanner-gated Workshop path. Default: \"auto\".",
...
const FIELD_LABELS = {
	...
	"agents.defaults.contextLimits.maxResultChars": "Default Tool Result Max Chars",
	...
	"agents.entries.*.contextLimits.maxResultChars": "Agent Tool Result Max Chars",
...
function computeBaseConfigSchemaStablePayload() {
	...
	const stablePayload = {
		schema: publicSchema,
		uiHints: applyDerivedTags(applyResolvedConfigTierHints(publicSchema, stripLegacyCompatHints(applyDerivedTags(applySensitiveUrlHints(baseHints, sensitiveUrlPaths))))),
		version: VERSION
	};
