...
import { n as describeSystemAgentPersistentOperation } from "./operations-Cqkrh_F4.js";
...
import { t as getUpdateAvailable } from "./update-startup-CXZzSFeb.js";
import { n as getHealthCache } from "./health-state-BUN2e4QL.js";
import { t as WizardSession } from "./session-BeEA_WOJ.js";
import { n as SystemAgentChatEngine, r as SystemAgentWizardAnswerError } from "./chat-engine-XkQUgePn.js";
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const inference = params.delegation ? await import("./inference-fallback-BarnaGoF.js").then(({ verifySystemAgentInferenceWithFallback }) => verifySystemAgentInferenceWithFallback({
