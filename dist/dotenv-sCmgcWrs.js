...
import { n as listKnownProviderAuthEnvVarNames } from "./provider-env-vars-5Fdeltvz.js";
import { n as readDotEnvFile, t as loadGlobalRuntimeDotEnvFiles } from "./dotenv-global-cQGDIZ_K.js";
...
const BLOCKED_WORKSPACE_DOTENV_KEYS = /* @__PURE__ */ new Set([
...
function hasBlockedWorkspaceDotEnvTokenSequence(key) {
	const tokens = key.split("_").filter(Boolean);
	return BLOCKED_WORKSPACE_DOTENV_TOKEN_SEQUENCES.some((sequence) => {
		for (let index = 0; index <= tokens.length - sequence.length; index += 1) if (sequence.every((token, offset) => tokens[index + offset] === token)) return true;
		return false;
	});
}
...
function shouldBlockWorkspaceDotEnvKey(key, getProviderAuthBlockedKeys) {
	...
	return shouldBlockWorkspaceRuntimeDotEnvKey(upper) || BLOCKED_WORKSPACE_DOTENV_KEYS.has(upper) || BLOCKED_WORKSPACE_DOTENV_PREFIXES.some((prefix) => upper.startsWith(prefix)) || BLOCKED_WORKSPACE_DOTENV_SUFFIXES.some((suffix) => upper.endsWith(suffix)) || hasBlockedWorkspaceDotEnvTokenSequence(upper) || getProviderAuthBlockedKeys().has(upper);
}
