...
function resolveSubagentTargetFromRuns(params) {
	...
	const isActive = params.isActive ?? ((entry) => !entry.execution.endedAt);
	const recentCutoff = Date.now() - params.recentWindowMinutes * 6e4;
	const numericOrder = [...deduped.filter((entry) => isActive(entry)), ...deduped.filter((entry) => !isActive(entry) && Boolean(entry.execution.endedAt) && (entry.execution.endedAt ?? 0) >= recentCutoff)];
	if (/^\d+$/.test(trimmed)) {
...
//#endregion
export { sortSubagentRuns as i, resolveSubagentLabel as n, resolveSubagentTargetFromRuns as r, formatRunLabel as t };
