...
function apnsAbortError(signal) {
	return signal.reason instanceof Error ? signal.reason : /* @__PURE__ */ new Error("APNs send invalidated");
}
...
async function openApnsTlsTunnel(params) {
	...
	const abortFromCaller = () => {
		if (params.signal) abortController.abort(apnsAbortError(params.signal));
	};
...
async function connectApnsHttp2Session(params) {
	...
	if (!proxyUrl) {
		if (params.signal?.aborted) throw apnsAbortError(params.signal);
