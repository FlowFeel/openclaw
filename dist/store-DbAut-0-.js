...
import { i as writeExternalFileWithinRoot, t as ensureAbsoluteDirectory } from "./fs-safe-DVaClkIX.js";
import { g as iterateSqliteQuerySync, h as getNodeSqliteKysely, m as executeSqliteQueryTakeFirstSync, p as executeSqliteQuerySync } from "./node-sqlite-B_0DgpUE.js";
...
import { r as withOpenClawStateLease } from "./openclaw-state-lease-DLXouRV4.js";
...
const TRANSCRIPT_EXPORT_FILE_NAMES$1 = /* @__PURE__ */ new Set([
	"metadata.json",
	"summary.json",
	"summary.md",
	"transcript.jsonl"
...
function hasExactMeetingTranscriptUtterance(params) {
	...
	const rows = executeSqliteQuerySync(params.database, db.selectFrom("meeting_transcript_utterances").selectAll().where("session_id", "=", params.sessionId).where("session_started_at", "=", params.sessionStartedAt).where("utterance_id", "=", params.utterance.id)).rows;
...
function appendMeetingTranscriptUtterance(params) {
	...
	const stored = executeSqliteQueryTakeFirstSync(database, db.selectFrom("meeting_transcript_sessions").select("next_utterance_seq").where("session_id", "=", session.sessionId).where("started_at", "=", session.startedAt));
...
async function writeTranscriptJsonlArtifact(params) {
	...
	const sequenceHead = executeSqliteQueryTakeFirstSync(database.db, meetingTranscriptDb(database.db).selectFrom("meeting_transcript_sessions").select("next_utterance_seq").where("session_id", "=", params.session.sessionId).where("started_at", "=", params.session.startedAt))?.next_utterance_seq;
		...
		write: async (tempPath) => {
			...
			try {
				...
				while (nextSequence < sequenceHead) {
					const rows = executeSqliteQuerySync(database.db, meetingTranscriptDb(database.db).selectFrom("meeting_transcript_utterances").selectAll().where("session_id", "=", params.session.sessionId).where("session_started_at", "=", params.session.startedAt).where("sequence", ">=", nextSequence).where("sequence", "<", sequenceHead).orderBy("sequence", "asc").limit(TRANSCRIPT_EXPORT_ROW_BATCH_SIZE)).rows;
...
async function hasAliasedCanonicalTranscriptExportPathOwner(params) {
	...
	if (!owner && !metadataArtifact) {
		...
		for (const candidate of owners) {
			const candidateManifest = JSON.parse(candidate.export_manifest_json);
			const candidatePending = new Set(JSON.parse(candidate.export_pending_json));
			let verified = 0;
			let matches = true;
			for (const { entry, canonicalName } of artifacts) {
				...
				if (stat.isSymbolicLink() || !stat.isFile() || candidatePending.has(canonicalName) || !expectedHash || await sha256File(artifactPath) !== expectedHash) {
					matches = false;
					break;
				}
				verified += 1;
			}
			if (matches && verified > 0) manifestMatches.push(candidate);
		}
	...
	for (const { entry, canonicalName } of artifacts) {
		...
		if (pending.has(canonicalName)) return false;
		const expectedHash = manifest[canonicalName];
		if (!expectedHash || await sha256File(artifactPath) !== expectedHash) return false;
...
var TranscriptsStore = class {
	...
	readExportOwnership(session) {
		...
		const row = executeSqliteQueryTakeFirstSync(database.db, meetingTranscriptDb(database.db).selectFrom("meeting_transcript_sessions").select(["export_manifest_json", "export_pending_json"]).where("session_id", "=", session.sessionId).where("started_at", "=", session.startedAt));
		return row ? {
	...
	readSessionByIdentity(session) {
		const database = this.database();
		const row = executeSqliteQueryTakeFirstSync(database.db, meetingTranscriptDb(database.db).selectFrom("meeting_transcript_sessions").selectAll().where("session_id", "=", session.sessionId).where("started_at", "=", session.startedAt));
	...
	transcriptRows(session) {
		...
		return {
			database,
			query: meetingTranscriptDb(database.db).selectFrom("meeting_transcript_utterances").selectAll().where("session_id", "=", session.sessionId).where("session_started_at", "=", session.startedAt).orderBy("sequence", "asc")
		};
	}
	...
	async expectedExportHashes(session) {
		...
		const hashes = { "metadata.json": sha256Hex(`${JSON.stringify(storedSession, null, 2)}\n`) };
	...
	updateExportManifest(session, exportedHashes, removedExports = /* @__PURE__ */ new Set()) {
		runOpenClawStateWriteTransaction(({ db: database }) => {
			const db = meetingTranscriptDb(database);
			const stored = executeSqliteQueryTakeFirstSync(database, db.selectFrom("meeting_transcript_sessions").select(["export_manifest_json", "export_pending_json"]).where("session_id", "=", session.sessionId).where("started_at", "=", session.startedAt));
		...
		}, this.databaseOptions, { operationLabel: "meeting-transcripts.export.record" });
	}
	markPendingExports(session, fileNames) {
		runOpenClawStateWriteTransaction(({ db: database }) => {
			const db = meetingTranscriptDb(database);
			const stored = executeSqliteQueryTakeFirstSync(database, db.selectFrom("meeting_transcript_sessions").select("export_pending_json").where("session_id", "=", session.sessionId).where("started_at", "=", session.startedAt));
			if (!stored) throw new Error(`transcripts session not found: ${session.sessionId}`);
			const pending = new Set(JSON.parse(stored.export_pending_json));
			for (const fileName of fileNames) pending.add(fileName);
			executeSqliteQuerySync(database, db.updateTable("meeting_transcript_sessions").set({ export_pending_json: JSON.stringify([...pending].toSorted()) }).where("session_id", "=", session.sessionId).where("started_at", "=", session.startedAt));
		}, this.databaseOptions, { operationLabel: "meeting-transcripts.export.pending" });
	}
	async assertExportDestinationOwned(session, sessionDir = this.sessionDir(session)) {
		...
		for (const entry of entries) {
			...
			if (!TRANSCRIPT_EXPORT_FILE_NAMES$1.has(canonicalName)) continue;
	...
	async writeSession(session) {
		...
		}, this.databaseOptions, { operationLabel: "meeting-transcripts.session.write" });
	}
	...
	async readSessionEntry(sessionSelector) {
		...
		const exactRows = qualified ? executeSqliteQuerySync(database.db, db.selectFrom("meeting_transcript_sessions").selectAll().where("selector", "=", sessionSelector)).rows : executeSqliteQuerySync(database.db, db.selectFrom("meeting_transcript_sessions").selectAll().where("session_id", "=", sessionSelector).orderBy("started_at", "desc").limit(2)).rows;
		const slugRows = qualified ? [] : executeSqliteQuerySync(database.db, db.selectFrom("meeting_transcript_sessions").selectAll().where("session_slug", "=", sessionSelector).orderBy("started_at", "desc").limit(2)).rows;
		...
		const row = rows[0];
		if (!row) return;
		const summaryKeys = this.hasSummary(database, row) ? /* @__PURE__ */ new Set([`${row.session_id}\0${row.started_at}`]) : /* @__PURE__ */ new Set();
	...
	async appendUtteranceForSession(session, utterance) {
		...
		const now = Date.now();
		ensureMeetingTranscriptsSchema(this.databaseOptions);
		runOpenClawStateWriteTransaction(({ db: database }) => appendMeetingTranscriptUtterance({
			database,
			metadataJson,
			now,
			session,
			utterance
		}), this.databaseOptions, { operationLabel: "meeting-transcripts.utterance.append" });
	}
	async readUtterancesForSession(session, options = {}) {
		...
		const query = meetingTranscriptDb(database.db).selectFrom("meeting_transcript_utterances").selectAll().where("session_id", "=", session.sessionId).where("session_started_at", "=", session.startedAt);
	...
	async writeSummary(summary, session) {
		...
		runOpenClawStateWriteTransaction(({ db: database }) => {
			...
			}).onConflict((conflict) => conflict.columns(["session_id", "session_started_at"]).doUpdateSet({
				generated_at: summary.generatedAt,
				summary_json: summaryJson,
				markdown,
				utterance_count: summary.utteranceCount
			})));
		}, this.databaseOptions, { operationLabel: "meeting-transcripts.summary.write" });
	...
	async materializeSessionArtifactsOwned(session, kind) {
		...
		if (Object.keys(exportedHashes).length > 0 || removedExports.size > 0) this.updateExportManifest(session, exportedHashes, removedExports);
		return {
			sessionDir,
			metadataPath,
			transcriptPath,
