...
type MeetingBrowserHealth<TManualReason extends string = string, TSpeechBlockedReason extends string = string> = {
  ...
  speechBlockedMessage?: string; /** Non-silent sink audio observed again on the meeting microphone capture path. */
  outputLoopbackSignalBytes?: number;
  lastOutputLoopbackAt?: string;
  lastOutputLoopbackCorrelation?: number;
  lastOutputLoopbackRms?: number;
...
declare function createMeetingPluginChromeTransport<Mode extends string, Health extends MeetingBrowserHealth, Transcript extends MeetingTranscriptSnapshot>(options: Omit<MeetingChromeTransportOptions<Mode, Health, Transcript>, "browserNodeAdapter" | "isRealtimeRouteReady" | "isTalkBackMode" | "nodeCommandName" | "outputMentionsAudioDevice" | "systemProfilerCommand">): {
  assertAudioDeviceAvailable: (params: {
    runtime: PluginRuntime;
    timeoutMs: number;
  }) => Promise<void>;
