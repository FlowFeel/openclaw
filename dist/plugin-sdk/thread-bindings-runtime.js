...
function createAccountScopedConversationBindingManager(params) {
	...
	const resolveActiveBinding = (record, now = Date.now()) => {
		...
		state.bindingsByAccountConversation.delete(resolveBindingKey({
			accountId,
	...
	const manager = {
		accountId,
		getByConversationId: (conversationId) => resolveActiveBinding(state.bindingsByAccountConversation.get(resolveBindingKey({
			accountId,
			conversationId
		}))),
