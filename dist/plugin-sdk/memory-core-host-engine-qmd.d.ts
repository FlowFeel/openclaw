import { hi as MemorySessionSyncTarget, mi as MemorySessionKind, si as MemoryEntryProvenance } from "../types-dPYYJRo2.js";
...
type SessionTranscriptCorpusOptions = {
  /** Include rotated SQLite transcript identities retained behind current logical sessions. */includeRetainedSqlite?: boolean;
};
...
type SessionFileEntry = {
  ...
  messageTimestampsMs: number[]; /** Provenance aligned one-for-one with exported content lines. */
  lineProvenance: MemoryEntryProvenance[]; /** True when this transcript belongs to an internal dreaming narrative run. */
  generatedByDreamingNarrative?: boolean; /** True when this transcript belongs to an isolated cron run session. */
  generatedByCronRun?: boolean;
  sessionKind: MemorySessionKind;
};
...
type BuildSessionEntryOptions = {
  ...
  generatedByCronRun?: boolean;
  sessionKind?: MemorySessionKind; /** Session key for identity-backed transcript readers. */
  sessionKey?: string; /** Direct SQLite identity for live runtime transcripts. */
  agentId?: string;
  sessionId?: string;
  storePath?: string; /** Activity timestamp for transcript sources that do not have filesystem stats. */
