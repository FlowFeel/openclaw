...
async function runIsolatedAgentRuntimeCompletion(params) {
	...
	try {
		const operation = (async () => {
			const { runIsolatedCompletion } = await import("./isolated-completion-CeLOIUz8.js");
...
function createRuntimeLlm(options = {}) {
	...
	return { complete: async (params) => {
		...
		const [{ prepareSimpleCompletionModelForAgent, completeWithPreparedSimpleCompletionModel, resolveSimpleCompletionSelectionForAgent }, cfg] = await Promise.all([import("./simple-completion-runtime-Byat_1tR.js"), Promise.resolve(resolveRuntimeConfig(options))]);
