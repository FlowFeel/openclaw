...
import { a as resolveDefaultPluginNpmDir, d as resolvePluginNpmProjectsDir, i as resolveDefaultPluginGitDir, o as resolvePluginInstallDir } from "./install-paths-D18EkQi8.js";
import { n as defaultSlotIdForKey, o as resetPluginSlotsToDefaults } from "./slots-pOTBtf7J.js";
...
import { K as syncManagedNpmRootPeerDependencies, U as readOpenClawManagedNpmRootOverrides, t as classifyNpmManagedOverrideCompatibilityError } from "./install-managed-npm-state-BYJD6JNG.js";
...
function prepareConfigForPendingPluginDirectoryRemoval(config, pluginId) {
	return {
		...config,
		plugins: {
			...config.plugins,
...
function loadPathMatchesInstallPath(loadPath, installPath) {
	if (loadPath === installPath) return true;
...
function removePluginFromConfig(cfg, pluginId, opts) {
	...
	const trackedInstallPaths = [installRecord?.installPath, installRecord?.source === "path" ? installRecord.sourcePath : void 0].filter((value) => Boolean(value));
	if (trackedInstallPaths.length > 0) {
		const loadPaths = load?.paths;
		if (Array.isArray(loadPaths) && loadPaths.some((p) => trackedInstallPaths.some((installPath) => loadPathMatchesInstallPath(p, installPath)))) {
			const nextLoadPaths = loadPaths.filter((p) => !trackedInstallPaths.some((installPath) => loadPathMatchesInstallPath(p, installPath)));
...
//#endregion
export { planPluginUninstall as a, removePluginFromConfig as c, formatUninstallSlotResetPreview as i, resolveUninstallChannelConfigKeys as l, applyPluginUninstallDirectoryRemoval as n, pluginUninstallTargetExists as o, formatUninstallActionLabels as r, prepareConfigForPendingPluginDirectoryRemoval as s, UNINSTALL_ACTION_LABELS as t };
