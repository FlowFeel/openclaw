...
import { i as DEFAULT_PROMPT, n as DEFAULT_MAX_BYTES, r as DEFAULT_MAX_CHARS_BY_CAPABILITY } from "./defaults.constants-BD7Nlx_t.js";
import { n as runtimeMediaModelSecretOwnerId, o as resolveEffectiveMediaEntryCapabilities } from "./runtime-media-secret-owner-C-EJ4rYa.js";
...
function resolveModelEntries(params) {
	...
	const entries = (cfg.tools?.media?.models ?? []).map((entry, index) => ({
		entry,
		source: "shared",
	...
	return entries.filter(({ entry, source }) => {
		...
		return preferredMediaModelRank(right.entry, preferred) - preferredMediaModelRank(left.entry, preferred);
	});
}
