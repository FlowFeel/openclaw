...
import { c as tokenizeHtmlTags, r as markdownToIR } from "./construct-fallbacks-BR_o1n_g.js";
import { t as MarkdownIt } from "./markdown-it-BKYBOU__.js";
...
import { a as resolveMatrixAccountConfig } from "./account-config-BFrkzWp4.js";
import { t as getMatrixRuntime } from "./runtime-Drg4hYqm.js";
import { r as normalizeMatrixResolvableTarget, t as isMatrixQualifiedUserId } from "./target-ids-CFN4hb1k.js";
import { i as isStrictDirectRoom } from "./direct-room-DGg6GySX.js";
import { r as buildMatrixReactionContent } from "./reaction-common-BWj4zGM5.js";
import { a as EventType, c as MsgType, l as RelationType, n as persistMatrixDirectRoomMapping, s as MSC4357_LIVE_KEY, t as inspectMatrixDirectRooms } from "./direct-management-Cnt3QosU.js";
import "./lib-93jNnitP.js";
import { a as fromBuffer, t as APEv2Parser } from "./APEv2Parser-oBGKGqc4.js";
...
import { l as toRatio, n as decodeString } from "./Util-CgoGn7tD.js";
import { c as TimestampFormat, i as LyricsContentType } from "./ID3v2Token-B8ttukB6.js";
import { r as hasID3v1Header } from "./ID3v1Parser-D9umzOQS.js";
import { createHash } from "node:crypto";
import { fileTypeFromBuffer } from "file-type";
//#region extensions/matrix/src/matrix/send/client.ts
const loadMatrixSendClientRuntime = createLazyRuntimeModule(() => import("./client-bootstrap-B3CJ9Pnl.js"));
...
const DELIVERY_PLAN_TTL_MS = 1440 * 60 * 1e3;
...
async function loadQueuePlans(queueId) {
	const store = createDeliveryPlanStore();
	const keys = (await store.entries()).filter((entry) => entry.key.startsWith(queuePrefix(queueId))).map((entry) => entry.key);
...
function createReconciledMatrixReceipt(params) {
	const uniqueResults = params.results.filter((result, index, results) => results.findIndex((entry) => entry.eventId === result.eventId) === index);
	...
	return {
		...
		parts: uniqueResults.map((result, index) => {
			...
			if (params.replyToId) part.replyToId = params.replyToId;
			if (params.threadId) part.threadId = params.threadId;
			return part;
		}),
	...
	try {
		...
		return await withResolvedMatrixSendClient({
			...
			for (const plan of orderedPlans) {
				...
				for (const event of plan.events) results.push({
					eventId: await client.sendMessage(roomId, event.content, event.transactionId, async (dispatch) => {
			...
			const replyToId = ctx.effectiveReplyToId !== void 0 ? ctx.effectiveReplyToId : ctx.replyToMode === "off" ? void 0 : ctx.replyToId;
...
async function cleanupMatrixDeliveryPlans(ctx) {
	const store = createDeliveryPlanStore();
	await store.deleteExpired();
	const keys = (await store.entries()).filter((entry) => entry.key.startsWith(queuePrefix(ctx.queueId))).map((entry) => entry.key);
...
function findMatrixMarkdownMetadataRanges(markdown) {
	const env = {};
...
function findMatrixSpoilerDelimiterOffsets(markdown) {
	...
	for (const token of tokens) {
		...
		for (let index = start; index < end - 1; index += 1) {
			if (projected[index] !== "|" || projected[index + 1] !== "|") continue;
			const excluded = excludedRanges.some((range) => index >= range.start && index < range.end);
			if (isMarkdownEscaped(projected, index) || excluded) continue;
...
function hasMatrixSpoilerMetadataCollision(markdown) {
	const projected = projectMatrixMarkdown(markdown);
	...
	for (let index = 0; index < projected.length - 1; index += 1) {
		if (projected[index] !== "|" || projected[index + 1] !== "|") continue;
		if (ordinary.has(index) || isMarkdownEscaped(projected, index)) continue;
		if (tables.some((range) => index >= range.start && index < range.end)) continue;
		return true;
	}
	return false;
}
...
function markdownToMatrixHtml(markdown, options = {}) {
	if (hasMatrixSpoilerMetadataCollision(markdown)) return renderMatrixFallbackHtml(markdown);
...
function protectMatrixSpoilerDelimiters(markdown) {
...
function parseMatrixMarkdown(markdown, tableMode) {
...
function markdownToMatrixBody(markdown) {
	...
	return renderMarkdownWithMarkers(markdownToIR(body, {
...
function renderMatrixFallbackHtml(markdown) {
	return `<p>${escapeHtml(markdownToMatrixBody(markdown)).replaceAll("\n", "<br>\n")}</p>`;
}
async function resolveMarkdownMentionState(params) {
	const tokens = parseMatrixMarkdown(maskEscapedMentions(projectMatrixMarkdown(params.markdown)), params.tableMode);
...
async function renderMarkdownToMatrixHtmlWithMentions(params) {
	...
	if (hasMatrixSpoilerMetadataCollision(params.markdown)) {
		...
		return {
			html: renderMatrixFallbackHtml(params.markdown),
...
function protectMatrixUnderlineTags(markdown) {
	...
	const tags = [...tokenizeHtmlTags(markdown)].filter((tag) => (tag.name === "u" || tag.name === "ins") && !tag.selfClosing && !isInsideCode(tag.start, codeRegions) && !isMarkdownEscaped(markdown, tag.start) && !metadataRanges.some((range) => tag.start >= range.start && tag.start < range.end));
...
function restoreMatrixStyleChunks(chunks, spoiler, underline) {
	...
	return chunks.map((chunk) => {
		...
		for (const character of chunk) {
			const opening = Object.keys(syntax).find((style) => character === syntax[style].markers?.open);
			const closing = Object.keys(syntax).find((style) => character === syntax[style].markers?.close);
			if (opening) {
				stack.push(opening);
...
function prepareMatrixSingleText(text, opts) {
	...
	const tableMode = opts.tableMode ?? getCore$2().channel.text.resolveMarkdownTableMode({
		cfg,
		channel: "matrix",
	...
	const singleEventLimit = normalizeMatrixEventLimit(Math.min(getCore$2().channel.text.resolveTextChunkLimit(cfg, "matrix", opts.accountId), MATRIX_FORMAT_PROFILE.chunk.limit));
	const eventTextLength = Math.max(convertedText.length, markdownToMatrixBody(convertedText).length);
	return {
		trimmedText,
		convertedText,
		singleEventLimit,
...
function chunkMatrixText(text, opts) {
	...
	const chunkMode = getCore$2().channel.text.resolveChunkMode(cfg, "matrix", opts.accountId);
	const collisionRedacted = hasMatrixSpoilerMetadataCollision(preparedText.convertedText) ? markdownToMatrixBody(preparedText.convertedText) : void 0;
	const chunkSegment = (segmentText) => {
		const protectedUnderline = protectMatrixUnderlineTags(hasMatrixSpoilerMetadataCollision(segmentText) ? markdownToMatrixBody(segmentText) : segmentText);
		...
		while (reserve < preparedText.singleEventLimit) {
			const protectedChunks = getCore$2().channel.text.chunkMarkdownTextWithMode(protectedSpoilers.markdown, preparedText.singleEventLimit - reserve, chunkMode);
...
function resolveMatrixMsgType(contentType, _fileName) {
	switch (getCore$1().media.mediaKindFromMime(contentType ?? "")) {
...
const mpegParserLoader = {
	...
	async load() {
		return (await import("./MpegParser-_tkh0Les.js")).MpegParser;
	}
};
//#endregion
...
const apeParserLoader = {
	...
	async load() {
		return (await import("./APEv2Parser-B47Hw3Vl.js")).APEv2Parser;
	}
};
//#endregion
...
const asfParserLoader = {
	...
	async load() {
		return (await import("./AsfParser-TSz6cXpg.js")).AsfParser;
	}
};
//#endregion
...
const dsdiffParserLoader = {
	...
	async load() {
		return (await import("./DsdiffParser-Bp2AJRRo.js")).DsdiffParser;
	}
};
//#endregion
...
const aiffParserLoader = {
	...
	async load() {
		return (await import("./AiffParser-MyDxal8I.js")).AIFFParser;
	}
};
//#endregion
...
const dsfParserLoader = {
	...
	mimeTypes: ["audio/dsf"],
	async load() {
		return (await import("./DsfParser-OLASuj9X.js")).DsfParser;
	}
};
//#endregion
...
const flacParserLoader = {
	...
	async load() {
		return (await import("./FlacParser-CjfkXvac.js")).FlacParser;
	}
};
//#endregion
...
const matroskaParserLoader = {
	...
	async load() {
		return (await import("./MatroskaParser-CA_Exdeq.js")).MatroskaParser;
	}
};
//#endregion
...
const mp4ParserLoader = {
	...
	async load() {
		return (await import("./MP4Parser-BUlr0Zhv.js")).MP4Parser;
	}
};
//#endregion
...
const musepackParserLoader = {
	...
	async load() {
		return (await import("./MusepackParser-BmLlmLRi.js")).MusepackParser;
	}
};
//#endregion
...
const oggParserLoader = {
	...
	async load() {
		return (await import("./OggParser-ByD4ZOQs.js")).OggParser;
	}
};
//#endregion
...
const wavpackParserLoader = {
	...
	async load() {
		return (await import("./WavPackParser-B2oNtCwR.js")).WavPackParser;
	}
};
//#endregion
...
const riffParserLoader = {
	...
	async load() {
		return (await import("./WaveParser-lQp4-xCE.js")).WaveParser;
	}
};
//#endregion
...
async function sendSingleTextMessageMatrix(roomId, text, opts) {
	const { trimmedText, convertedText, singleEventLimit, eventTextLength, fitsInSingleEvent, tableMode } = prepareMatrixSingleText(text.trimEnd(), {
		cfg: opts.cfg,
		accountId: opts.accountId,
		preserveWhitespace: true
	});
...
//#endregion
export { resolveMatrixRoomId as S, parsePollStart as _, sendReadReceiptMatrix as a, cleanupMatrixDeliveryPlans as b, resolveMatrixMentionsForBody as c, buildPollResponseContent as d, buildPollResultsSummary as f, isPollStartType as g, isPollEventType as h, sendPollMatrix as i, chunkMatrixText as l, formatPollResultsAsText as m, reactMatrixMessage as n, sendSingleTextMessageMatrix as o, formatPollAsText as p, sendMessageMatrix as r, sendTypingMatrix as s, editMessageMatrix as t, prepareMatrixSingleText as u, parsePollStartContent as v, reconcileMatrixUnknownSend as x, resolvePollReferenceEventId as y };
