...
import { t as resolveMissingRequestedScope } from "./operator-scope-compat-C7_b0yme.js";
import { t as resolveNodePairApprovalScopes } from "./node-pairing-authz-DJqaCvmL.js";
import { N as updatePairedDeviceNodeSurfaceInTransaction } from "./device-bootstrap-D36P889E.js";
import { O as withPairedDeviceRecords, b as resolveNodePairingGeneration, i as clearNodePairingGenerationBins, x as resolveNodePairingState } from "./device-pairing-pwb3Oe75.js";
	...
	return await withPairedDeviceRecords(baseDir, () => {
		return {
			value: updatePairedDeviceNodeSurfaceInTransaction(nodeId, baseDir, (device) => {
				...
				if (!device?.nodeSurface || expectedPairingGeneration.nodeId !== device.deviceId || currentPairingGeneration?.key !== expectedPairingGeneration.key) return {
					value: false,
					persist: false
				};
				return {
		...
		return {
			value: updatePairedDeviceNodeSurfaceInTransaction(nodeId, baseDir, (device) => {
				...
				if (expectedPairingGeneration) {
					const currentPairingGeneration = resolveNodePairingGeneration(device);
					if (expectedPairingGeneration.nodeId !== device.deviceId || currentPairingGeneration?.key !== expectedPairingGeneration.key) return {
						value: { recorded: false },
						persist: false
					};
				}
...
//#endregion
export { listNodePairing as a, rejectNodePairing as c, requestNodePairing as d, reusePendingNodePairingForReconnect as f, sameNodePermissionSurface as g, sameNodeApprovalSurfaceSet as h, getPendingNodePairing as i, releaseNodePairingCleanupClaim as l, normalizeNodeApprovalSurfaceList as m, beginNodePairingConnect as n, projectNodePairing as o, updatePairedNodeBins as p, finalizeNodePairingCleanupClaim as r, recordPairedNodeConnection as s, approveNodePairing as t, renamePairedNode as u };
