...
import { d as normalizeOptionalAgentId, f as resolveAgentIdFromSessionKey } from "./session-key-DtTE9-Tg.js";
...
import { h as runWithGatewayIndependentRootWorkContinuation, r as beginGatewayRootWorkAdmissionWhenOpen, t as GatewayDrainingError } from "./gateway-work-admission-BaRJo64l.js";
import { n as deliveryContextFromSession } from "./delivery-context.shared-DR6KpKlV.js";
import { L as applySqliteSessionEntryLifecycleMutation, ct as loadSqliteSessionEntryReadOnly } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { V as resolveMaintenanceConfig } from "./session-entry-slot-keys-DPRQmSpa.js";
import { $ as listSessionEntries } from "./session-accessor-D5Or7WgI.js";
import { i as isRetryableHeartbeatBusySkipReason } from "./heartbeat-wake-D9__uym3.js";
import { t as buildPendingGeneratedMediaSessionKeySet } from "./task-status-access-C-YIcrT5.js";
import { d as saveCronJobsStore, i as loadCronJobsStoreWithConfigJobs, s as removeStaleCronJobFamilyRows, t as getCronJobsStoreRevision } from "./store-BwY4XtES.js";
import { t as cronStoreKey } from "./key-BBZ40bDq.js";
import { d as normalizeCronJobIdentityFields, u as cronSchedulingInputsEqual } from "./row-codec-CUrTP2BZ.js";
import { n as normalizeCronJobInput } from "./normalize-CupkFhHE.js";
import { t as parseAbsoluteTimeMs } from "./parse-mvoz8PbH.js";
import "./normalize-BHdv6zWT.js";
import { r as isInvalidCronSessionTargetIdError } from "./session-target-DJsUULzX.js";
import { a as cronStreamScheduleKey, i as createCronStreamSourceIdentity, n as resolvePacedNextRunAtMs, r as appendCronPayloadText } from "./pacing-BxtLSQjv.js";
import { t as getInvalidPersistedCronJobReason } from "./persisted-shape-777Zhc1Q.js";
import { c as markCronJobActive, d as noteActiveCronJobTriggerMutation, l as noteActiveCronJobRemoval, n as clearCronJobActive, o as isCronActiveJobMarkerCurrent, s as isCronJobActive, u as noteActiveCronJobScheduleMutation } from "./active-jobs-BGi1uzPV.js";
import { i as registerActiveCronTaskRun, o as trackActiveCronTaskRunSettlement } from "./active-run-cancellation-DDKkkTKE.js";
import { r as enqueueCommandInLane } from "./command-queue-DxY-0HMA.js";
import { n as AgentDeletionCommitUncertainError, t as AgentDeletionAuthorityRollbackError } from "./agent-lifecycle-registry-BIgOsVbj.js";
import { a as preExecutionTimeoutErrorMessage, c as timeoutErrorMessage, i as normalizeCronRunErrorText, r as isSetupTimeoutErrorText, s as setupTimeoutErrorMessage, t as abortErrorMessage } from "./execution-errors-DdimX5Wd.js";
import { n as resolveCronTriggerMinIntervalMs } from "./cron-limits-txevLFpr.js";
import { n as resolveCronDeliveryPlan, r as resolveFailureDestination } from "./delivery-plan-CCXNSnwf.js";
import { a as normalizeCronRunDiagnostics, n as createCronRunDiagnosticsFromError, o as summarizeCronRunDiagnostics } from "./run-diagnostics-BSxc7EvH.js";
import { a as failureNotificationDeliveryFromJobState, o as maybeEmitFailureAlert, r as normalizeCronTaskRunJobId, s as resolveFailureAlert, t as resolveCronListSnapshotRevision } from "./list-snapshot-revision-S5usKMOy.js";
import { S as maybeAutoDisableCronJobAfterRunFailure, _ as recomputeNextRunsForMaintenance, a as cronPatchTouchesDeliveryResolution, b as resolveJobLastRunStatus, c as computeJobPreviousRunAtOrBeforeMs, d as hasActiveCronRun, f as hasScheduledNextRunAtMs, g as recomputeNextRuns, h as nextWakeAtMs, i as assertSupportedJobSpec, l as errorBackoffMs, m as isJobEnabled, n as applyJobPatch, o as DEFAULT_ERROR_BACKOFF_SCHEDULE_MS, p as isJobDue, r as createJob, s as computeJobNextRunAtMs, t as applyDeclarativeJobSpec, u as findJobOrThrow, v as recordScheduleComputeError, x as resolveJobPayloadTextForMain, y as resolveJobErrorBackoffUntilMs } from "./jobs-DscVQvMI.js";
import { t as computeNextRunAtMs } from "./schedule-DgfSgRvu.js";
import { n as isHeartbeatTaskCronJob } from "./heartbeat-task-CoWdAuow.js";
import { a as tryFindFinalizedCronTaskRun, c as tryUpdateCronTaskRunSession, i as tryFindCronTaskRunIdForRecovery, l as resolveCronRunErrorReason, n as resolveMainSessionCronRunSessionKey, o as tryFinishCronTaskRun, r as tryCreateCronTaskRun, s as tryFinishCronTaskRunWithoutHistory } from "./task-runs-CoqbyJme.js";
import { a as writeCronJobScratch, r as readCronJobScratchState, t as deleteCronJobScratch } from "./scratch-store-UY2vnsGh.js";
...
async function persist(state, opts) {
	...
	try {
		await saveCronJobsStore(state.deps.storePath, store, quarantine ? { quarantine } : stateOnly ? { stateOnly: true } : void 0);
	} catch (error) {
	...
	for (const notify of opts?.postPersistNotifications ?? []) notify();
	return true;
}
...
function resolveMainSessionCronDeliveryContext(state, job) {
	...
	if (!storePath) return;
	try {
		return deliveryContextFromSession(loadSqliteSessionEntryReadOnly({
			agentId,
...
function resolveDeliveryState(params) {
	...
	if (!primaryDeliveryRequested) {
		if (primaryDeliveryPlan.mode === "webhook") {
			...
			if (params.deliveryAttempted === true) return {
				delivered: false,
...
async function executeJobCore(state, job, abortSignal, options) {
	...
	if (effectiveJob.payload.kind === "script") {
		const result = await executeScriptCronJob(state, effectiveJob, abortSignal, options?.activeJobMarker, options?.streamBatch);
		return triggerEval ? {
			...result,
			triggerEval
		} : result;
	...
	if (effectiveJob.payload.kind === "heartbeat") {
		state.deps.requestHeartbeat({
			...
			scheduledAnchorMs: effectiveJob.schedule.kind === "every" ? effectiveJob.schedule.anchorMs : void 0
		});
		const result = {
			status: "ok",
			summary: "heartbeat wake requested"
		};
		return triggerEval ? {
			...result,
			triggerEval
	...
	if (isHeartbeatTaskCronJob(effectiveJob)) {
		...
		const result = {
			status: "ok",
			summary: "heartbeat task wake requested"
		};
		return triggerEval ? {
			...result,
			triggerEval
...
async function executeMainSessionCronJob(state, job, abortSignal, waitWithAbort, activeJobMarker, owningCronLaneTaskMarker) {
	...
	const cronRunSessionKey = resolveMainSessionCronRunSessionKey(job, typeof job.state.runningAtMs === "number" ? job.state.runningAtMs : state.deps.nowMs(), state.deps.resolveDefaultAgentId?.() ?? state.deps.defaultAgentId);
...
async function executeScriptCronJob(state, job, abortSignal, activeJobMarker, streamBatch) {
...
async function sweepCronRunSessions(params) {
	...
	try {
		...
		if (removals.length > 0) {
			...
			const result = await applySqliteSessionEntryLifecycleMutation({
				agentId: params.agentId,
				storePath,
				removals,
...
function createCronAgentWatchdog(params) {
	...
	const noteExecutionProgress = (info) => {
		...
		if (state === "executing" && previousPhase !== void 0 && CRON_AGENT_PHASE_WATCHDOG_STAGE[previousPhase] === "execution" && stage === "pre_execution") {
...
async function deliverPrimaryWebhook(state, job, result, abortSignal, progress, deadlineAtMs) {
	...
	const deadlineExceeded = () => deadlineAtMs !== void 0 && Date.now() >= deadlineAtMs;
	const interruptionError = () => {
		const reason = abortErrorMessage(abortSignal);
		return deadlineExceeded() || abortSignal.reason instanceof Error && abortSignal.reason.name === "TimeoutError" ? `cron webhook delivery timed out: ${reason}` : `cron webhook delivery cancelled: ${reason}`;
	};
	if (abortSignal.aborted || deadlineExceeded()) return withPrimaryWebhookTrace({
		job,
		result,
		delivered: false,
...
async function executeJobCoreWithTimeout(state, job, opts) {
	...
	try {
		...
		const runPromise = corePromise.then(async (result) => {
			progress.completedCoreResult = result;
			return await deliverPrimaryWebhook(state, job, result, runAbortController.signal, progress, watchdog.deadlineAtMs());
		});
		trackActiveCronTaskRunSettlement(runPromise, runAbortController.signal);
		runPromise.catch((err) => {
			if (runAbortController.signal.aborted) state.deps.log.warn({
...
function notifyIsolatedAgentSetupTimeout(state, job, error, timeoutMs, onAsyncFailure) {
...
function maybeNotifyIsolatedAgentSetupTimeoutWithRecovery(state, result, onAsyncFailure) {
	const signal = result.isolatedAgentSetupTimeout;
	if (!signal) return false;
	if (!notifyIsolatedAgentSetupTimeout(state, result.job, signal.error, signal.timeoutMs, onAsyncFailure)) return false;
	return true;
}
//#endregion
...
function applyJobResult(state, job, result, opts) {
	...
	const shouldDelete = ownsSchedule && isOneShotSchedule && !preserveOneShotSchedule && job.deleteAfterRun === true && result.status === "ok";
...
function applyTriggerNoFireResult(state, job, result, opts) {
	...
	if (opts?.scheduleMode === "force-preserve" || opts?.scheduleMode === "stale-preserve") {
		...
		job.state.forcePreservedNextRunAtMs = opts.scheduleMode === "force-preserve" ? previousNextRunAtMs : previousForcePreservedNextRunAtMs;
		return;
	}
	job.state.pacedNextRunAtMs = void 0;
	job.state.forcePreservedNextRunAtMs = void 0;
	...
	try {
		...
		await locked(state, async () => {
			...
			for (const outcome of finalizedOutcomes) {
				const removedJob = applyOutcomeToStoredJob(state, outcome, { deferredNotifications: postPersistNotifications });
				if (removedJob) removedJobs.push(removedJob);
			}
			recomputeNextRunsForMaintenance(state, opts?.repairFutureCronNextRunAtMs === false ? {
...
function isErrorBackoffPending(_state, job, nowMs) {
	if (job.schedule.kind === "at" || resolveJobLastRunStatus(job) !== "error") return false;
...
async function onAdmittedTimer(state) {
	...
	let sessionReaperDefaultAgentId;
	state.running = true;
	armRunningRecheckTimer(state);
	try {
		const hasSessionReaperStore = Boolean(state.deps.resolveSessionStorePath || state.deps.sessionStorePath);
		sessionReaperDefaultAgentId = hasSessionReaperStore ? (state.deps.resolveDefaultAgentId?.() ?? state.deps.defaultAgentId)?.trim() : void 0;
		if (hasSessionReaperStore && !sessionReaperDefaultAgentId) throw new Error("Cron session reaper requires the prepared configured default agent id.");
		...
		try {
			if (sessionReaperDefaultAgentId) {
				...
				const addStoreTarget = (agentId, storePath) => {
					storeTargets.set(`${agentId}\0${storePath}`, {
						agentId,
						storePath
					});
				};
				const resolveJobAgentId = (job) => typeof job.agentId === "string" && job.agentId.trim() ? normalizeAgentId(job.agentId) : resolveAgentIdFromSessionKey(job.sessionKey, defaultAgentId);
				...
				if (state.deps.resolveSessionStorePath) {
					...
					for (const agentId of configuredAgentIds) addStoreTarget(normalizeAgentId(agentId), state.deps.sessionStorePath);
					for (const job of state.store?.jobs ?? []) addStoreTarget(resolveJobAgentId(job), state.deps.sessionStorePath);
					addStoreTarget(defaultAgentId, state.deps.sessionStorePath);
				}
				if (storeTargets.size > 0) {
					const nowMs = state.deps.nowMs();
					for (const { agentId, storePath } of storeTargets.values()) try {
...
function deferPendingBackoffMissedCronSlots(state, nowMs, opts) {
	...
	for (const job of state.store.jobs) {
		...
		if (!hasMissedCronSlotSinceLastRun(job, nowMs)) continue;
...
function resolveEffectiveJobAgentId(job, defaultAgentId) {
	const agentId = normalizeOptionalAgentId(job.agentId) ?? normalizeOptionalAgentId(parseAgentSessionKey(job.sessionKey)?.agentId) ?? normalizeOptionalAgentId(defaultAgentId);
	if (!agentId) throw new Error("Cron job requires an agent id or prepared configured default.");
	return agentId;
}
...
async function ensureLoadedForRead(state) {
	...
	const rollbackSnapshot = snapshotStoreForRollback(state);
	const postPersistNotifications = [];
	if (recomputeNextRunsForMaintenance(state, { deferredNotifications: postPersistNotifications })) await persistOrRestore(state, rollbackSnapshot, { postPersistNotifications });
}
...
async function activatePreparedManualRun(state, prepared, mode) {
	return await locked(state, async () => {
		...
		const executionJob = structuredClone(job);
		if (mode === "force" && executionJob.trigger && !prepared.evaluateTrigger) delete executionJob.trigger;
...
function resolveInterruptedStartupFailureNotificationStatus(params) {
	...
	if (resolveFailureDestination(params.job, params.state.deps.cronConfig?.failureAlert)) return "unknown";
...
function markInterruptedStartupRun(params) {
	...
	if (maybeAutoDisableCronJobAfterRunFailure({
		state: params.state,
		job,
		atMs: nowMs,
...
function restoreFinalizedStartupRun(params) {
	...
	job.state.nextRunAtMs = job.state.autoDisabled ? void 0 : replacementAtMs ?? entry.nextRunAtMs;
...
async function start(state) {
	...
	await locked(state, async () => {
		...
		for (const job of jobs) {
			...
			if (typeof job.state.runningAtMs === "number") {
				...
				if (finalized) {
					...
					if (repaired.replacementAtMs === void 0) interruptedJobIds.add(job.id);
					if (repaired.shouldDelete) completedJobIdsToDelete.add(job.id);
					repairedAnyStartupRun = true;
					continue;
				}
				...
				if (interrupted.replacementAtMs === void 0) interruptedJobIds.add(job.id);
		...
		if (repairedAnyStartupRun || jobs.length > 0) await persist(state, {
	...
	await locked(state, async () => {
		await ensureLoaded(state, { skipRecompute: true });
		if (state.stopped) return;
		const postPersistMaintenanceNotifications = [];
		if (recomputeNextRunsForMaintenance(state, {
			recomputeExpired: true,
			deferredNotifications: postPersistMaintenanceNotifications
		})) await persist(state, { postPersistNotifications: postPersistMaintenanceNotifications });
...
async function add(state, input, opts) {
	return await locked(state, async () => {
		...
		if (input.payload?.kind === "heartbeat" && opts?.systemOwned !== true) throw new Error("heartbeat payloads are system-owned; jobs cannot be created with them");
		await ensureLoaded(state, { skipRecompute: true });
		...
		if (existing) {
			if (existing.payload.kind === "heartbeat" && opts?.systemOwned !== true) throw new Error("heartbeat monitor jobs are system-owned; edit agents.*.heartbeat config instead");
			const now = state.deps.nowMs();
...
async function updateLoadedJob(params) {
	...
	if (patch.payload?.kind === "heartbeat") throw new Error("heartbeat payloads are system-owned; jobs cannot be patched to them");
	await ensureLoaded(state, { skipRecompute: true });
	const snapshot = snapshotStoreForRollback(state);
	const job = findJobOrThrow(state, id);
	if (job.payload.kind === "heartbeat") throw new Error("heartbeat monitor jobs are system-owned; edit agents.*.heartbeat config instead");
	const now = state.deps.nowMs();
...
async function remove(state, id, opts) {
	return await locked(state, async () => {
		...
		if (removedJob?.payload.kind === "heartbeat" && opts?.systemOwned !== true) throw new Error("heartbeat monitor jobs are system-owned; edit agents.*.heartbeat config instead");
	...
	return await locked(state, async () => {
		...
		try {
			...
			if (error instanceof AgentDeletionCommitUncertainError) {
				for (const notify of postPersistNotifications) notify();
				armTimer(state);
...
async function updateExternalState(state, id, streamScheduleKey, streamSourceIdentity, statePatch) {
	return await locked(state, async () => {
		...
		if (!job || !ownsStreamSource(job, streamScheduleKey, streamSourceIdentity)) return false;
...
async function retireExternalStreamSource(state, id, streamScheduleKey, streamSourceIdentity) {
	return await locked(state, async () => {
		await ensureLoaded(state, { skipRecompute: true });
		const job = state.store?.jobs.find((entry) => entry.id === id);
		if (!job || !ownsStreamSource(job, streamScheduleKey, streamSourceIdentity)) return;
		const snapshot = snapshotStoreForRollback(state);
...
async function updateExternalCounters(state, id, counters) {
	await locked(state, async () => {
		await ensureLoaded(state, { skipRecompute: true });
		const job = state.store?.jobs.find((entry) => entry.id === id);
		if (!job || job.schedule.kind !== "stream") return;
		await updateLoadedJob({
			state,
			id,
			patch: { state: {
				streamDroppedBatches: Math.max(job.state.streamDroppedBatches ?? 0, counters.streamDroppedBatches ?? 0),
				streamCoalescedBatches: Math.max(job.state.streamCoalescedBatches ?? 0, counters.streamCoalescedBatches ?? 0)
			} }
		});
	});
}
...
async function finishPreparedManualRun(state, prepared, mode) {
	...
	try {
		...
		const emitMissingQueuedTerminal = () => {
			...
			}, tracker, taskRunId, { errorClassification: triggerSkipped ? void 0 : coreResult.errorClassification });
		};
		...
		await locked(state, async () => {
			...
			const scheduleMode = scheduleOwnership === "stale" ? "stale-preserve" : mode === "force" ? "force-preserve" : "advance";
			const postPersistNotifications = [];
