...
import { s as resolveExecApprovalsDisplayPath, t as DEFAULT_EXEC_APPROVAL_ASK_FALLBACK } from "./exec-approvals-config-DrfE-I9X.js";
import { A as resolveExecApprovalAllowedDecisions, D as minSecurity, E as maxAsk, H as resolveExecModePolicy, V as resolveExecModeFromPolicy, r as resolveExecApprovalsFromFile } from "./exec-approvals-sCmCk_jQ.js";
...
function collectExecPolicyScopeSnapshots(params) {
	...
	const configAgentIds = new Set(configuredAgents.filter((agent) => agent.id !== defaultAgentId || agent.tools?.exec !== void 0).map((agent) => agent.id));
	const approvalAgentIds = Object.keys(params.approvals.agents ?? {}).filter((agentId) => agentId !== "*" && agentId !== "default" && agentId !== defaultAgentId);
