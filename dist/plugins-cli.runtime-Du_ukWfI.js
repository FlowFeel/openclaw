import { c as tracePluginLifecyclePhaseAsync } from "./discovery-B47lmslh.js";
import { m as shortenHomeInString } from "./utils-Bs67j6-3.js";
import { r as defaultRuntime } from "./runtime-DOr96aVu.js";
import { l as readConfigFileSnapshot, r as getRuntimeConfig } from "./io-DCw4R0kD.js";
import { r as theme } from "./theme-vjDs9tao.js";
import { n as emitDiagnosticsTimelineEvent } from "./diagnostics-timeline--Yi1HEPC.js";
...
import { a as resolveConfiguredRuntimePluginInstallCandidate, i as collectConfiguredRuntimePluginIds } from "./configured-runtime-plugin-installs-BhHRpPZt.js";
//#region src/cli/plugins-cli.runtime.ts
function createModuleLoader(load) {
	let promise;
	return () => promise ??= load();
}
const loadPluginsConfigState = createModuleLoader(() => import("./config-state-ClZB2KMb.js"));
const loadPluginsStatus = createModuleLoader(() => import("./status-B2HU2CGu.js"));
const loadPluginSlotSelection = createModuleLoader(() => import("./slot-selection-DYAMupEm.js"));
const loadPluginsCommandHelpers = createModuleLoader(() => import("./plugins-command-helpers-Ve7OSXEx.js"));
const loadPluginsRegistryRefresh = createModuleLoader(() => import("./registry-refresh-BfT1DNuO.js"));
...
async function runPluginsEnableCommandUnlocked(idInput) {
	let id = idInput;
	assertConfigWriteAllowedInCurrentMode();
	const { enableExplicitlySelectedPluginInConfig } = await import("./enable-BT-fWOyt.js");
...
async function runPluginsDisableCommandUnlocked(idInput) {
	...
	const { normalizePluginId } = await loadPluginsConfigState();
	const { buildPluginRegistrySnapshotReport } = await loadPluginsStatus();
	const { setPluginEnabledInConfig } = await import("./plugins-config-B2T-tNLv.js");
	const { refreshPluginRegistryAfterConfigMutation } = await loadPluginsRegistryRefresh();
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-D34QoVM6.js");
...
async function runPluginMarketplaceEntriesCommand(opts) {
	const catalog = await import("./official-external-plugin-catalog-PNJZTqnf.js");
	const cfg = getRuntimeConfig();
...
async function runPluginMarketplaceRefreshCommand(opts) {
	const { loadConfiguredHostedOfficialExternalPluginCatalogEntries } = await import("./official-external-plugin-catalog-PNJZTqnf.js");
	const cfg = getRuntimeConfig();
	...
	const { clearManagedPluginOfficialCatalogCache } = await import("./management-service-i5CtblEf.js");
	clearManagedPluginOfficialCatalogCache();
	let gatewayRefreshed = true;
	if (result.source !== "bundled-fallback") {
		const { notifyGatewayPluginMetadataChanged } = await import("./plugins-update-gateway-signal-DLJolDRc.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-Q9sTqIHS.js");
