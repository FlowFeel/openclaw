...
import { x as runTui } from "./tui-DzSvpTtF.js";
import { t as SYSTEM_AGENT_ID } from "./agent-id-D7-xzIog.js";
import { i as loadSystemAgentOverview, r as formatSystemAgentStartupMessage } from "./overview-8mY9XHbo.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-C05XXupU.js";
import { a as resolveSystemAgentVerifiedInferenceRoute } from "./verified-inference-CMyC8uSR.js";
import { t as executeSystemAgentOperation } from "./operations-Cqkrh_F4.js";
...
import { i as assertLocalGatewaySetupMode, n as SystemAgentChatEngine, t as GATEWAY_SETUP_AFTER_WRITE } from "./chat-engine-XkQUgePn.js";
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	if (handoff.target === "gateway") {
		...
		const [{ resolveGatewayPort }, { createClackPrompter }, { configureGatewayForSetup }, { readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults, writeWizardConfigFile }] = await Promise.all([
			...
			import("./setup.shared-BQDwhL8U.js")
		]);
	...
	if (handoff.target === "search") {
		...
		const [{ runSearchSetupFlow }, { createClackPrompter }, { readSetupConfigFileSnapshot, writeWizardConfigFile }] = await Promise.all([
			import("./search-setup-_GwBF9uB.js"),
			import("./clack-prompter-DyhRpF9c.js"),
			import("./setup.shared-BQDwhL8U.js")
		]);
		const snapshot = await readSetupConfigFileSnapshot();
	...
	await (opts.runChannelsAdd ?? (await import("./add-DQArr2_I.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
		beforePersistentEffect
	});
}
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (route) {
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-ClbAcHfK.js"), import("./model-thinking-default-B14nlDbu.js")]);
