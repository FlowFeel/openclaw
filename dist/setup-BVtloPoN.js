...
import { a as runSetupModelAuthStep, i as offerLiveModelVerification, n as listSetupMigrationOptions, r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-BzlF-H0j.js";
...
async function runSetupWizardOnce(opts, runtimeInput, prompter) {
	...
	if (opts.skipChannels) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { setupChannels } = await import("./onboard-channels-CmMP81FJ.js");
	...
	if (!usedImportFlow) {
		const { runSetupMemoryImportStep } = await import("./setup.memory-import-Bq2Erzmp.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { runSearchSetupFlow } = await import("./search-setup-_GwBF9uB.js");
	...
	if (flow !== "quickstart") {
		...
		const { setupAppRecommendations } = await import("./setup.app-recommendations-BiG57LQr.js");
	...
	const { finalizeSetupWizard } = await import("./setup.finalize-CbbfENzB.js");
