...
import { _ as findStructuredAuthParamRanges, d as HTTP_AUTH_LEGACY_VALUE_WHITESPACE_PATTERN, f as HTTP_AUTH_OPAQUE_CREDENTIAL_PATTERN, g as HTTP_AUTH_SERIALIZED_QUOTE_PATTERN, h as HTTP_AUTH_SCHEME_PATTERN, m as HTTP_AUTH_REQUIRED_VALUE_WHITESPACE_PATTERN, p as HTTP_AUTH_OPTIONAL_VALUE_WHITESPACE_PATTERN, u as HTTP_AUTH_HEADER_BOUNDARY_PATTERN, v as redactStructuredAuthHeaders } from "./errors-C7_LR8fF.js";
import "./src-CB-FniGQ.js";
import { i as isSensitiveUrlQueryParamName } from "./redact-sensitive-url-YLuImt1m.js";
import { t as expectDefined } from "./expect-CyE8FADM.js";
import "./src-COWbwBfI.js";
import { o as isRecord } from "./record-coerce-DHZ4bFlT.js";
import { t as pruneMapToMaxSize } from "./map-size-DAGm21RM.js";
import { n as getCommandPathWithRootOptions } from "./argv-D6QuZiS5.js";
...
import { i as openRootFileSync, t as canUseRootFileOpen } from "./root-file-dEMp_-h5.js";
...
import { _ as resolveIncludeRoots, d as resolveConfigPath } from "./paths-CL43LNS6.js";
...
var IncludeProcessor = class IncludeProcessor {
		...
		if (Array.isArray(value)) {
			...
			for (const item of value) {
				...
				if (item.includes("*")) {
					...
					for (const subItem of expanded) resolvedEntries.push(this.loadFile(subItem, logicalPath));
				} else resolvedEntries.push(this.loadFile(item, logicalPath));
			}
			return {
	...
	expandGlobInclude(globPattern) {
		const configDir = path.dirname(this.basePath);
		const resolvedPatternPath = path.isAbsolute(globPattern) ? globPattern : path.resolve(configDir, globPattern);
		const dir = path.dirname(resolvedPatternPath);
		const pattern = path.basename(resolvedPatternPath);
		const regex = new RegExp("^" + pattern.replace(/\./g, "\\.").replace(/\*/g, ".*") + "$");
		if (!fs.existsSync(dir)) return [];
		return fs.readdirSync(dir).filter((f) => regex.test(f)).sort().map((f) => path.join(path.dirname(globPattern), f));
	}
...
function redactMatch(match, groups, pattern, context) {
	...
	const masked = isShellReferencePattern ? maskToken(token) : `${maskSecretValue(formAwareValue.secret, { hinted: true })}${formAwareValue.suffix}`;
...
//#endregion
export { compileSafeRegexDetailed as A, hashConfigIncludeRaw as C, extensionUsesSkippedScannerPath as D, resolveConfigIncludes as E, compileConfigRegexes as O, createConfigIncludeResolutionSession as S, resolveConfigIncludeWritePath as T, readLoggingConfig as _, redactSensitiveFieldValue as a, ConfigIncludeError as b, redactSensitiveText as c, redactToolPayloadTextWithConfig as d, resolveRedactOptions as f, registerSecretValueForRedaction as g, redactRegisteredSecretValues as h, redactSecrets as i, testRegexWithBoundedInput as j, compileSafeRegex as k, redactToolDetail as l, isSecretValueRegisteredForRedaction as m, getDefaultRedactPatterns as n, redactSensitiveFieldValueWithConfig as o, hasRegisteredSecretValuesForRedaction as p, isSensitiveFieldKey as r, redactSensitiveLines as s, computeSensitiveRedactionBitmap as t, redactToolPayloadText as u, shouldSkipMutatingLoggingConfigRead as v, readConfigIncludeFileWithGuards as w, INCLUDE_KEY as x, CircularIncludeError as y };
