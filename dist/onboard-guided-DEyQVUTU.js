...
import { a as requireRiskAcknowledgement } from "./setup.shared-D0iINifk.js";
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-Dr31QrzZ.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-B9wOWPHc.js")).applySystemAgentSetup;
	...
	if (wantsDiscovery) {
		...
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-MwF2Mhxm.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
		...
		if (recommendedConfig !== persistedConfig) {
			...
			const { mergeWizardConfigOntoLatest, writeWizardConfigFile } = await import("./setup.shared-BQDwhL8U.js");
