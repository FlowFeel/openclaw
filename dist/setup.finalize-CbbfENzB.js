...
import { l as isSystemdUserServiceAvailable } from "./systemd-CwfIwjqs.js";
...
import { i as formatExternalSupervisorActionRequired, o as isGatewayExternallySupervised } from "./gateway-supervision-D_xpxHux.js";
import { a as resolveGatewayService, n as formatGatewayServiceStartRepairIssues, o as startGatewayService, t as describeGatewayServiceRestart } from "./service-BUC2FMQ8.js";
...
import { n as DEFAULT_BOOTSTRAP_FILENAME } from "./workspace-CelKqYGr.js";
import { f as probeGatewayReachable, g as waitForGatewayReachable, r as buildOnboardingControlUiUrl } from "./onboard-helpers-BlDftc97.js";
...
import { i as hasAuthProfileForProvider } from "./model-config.helpers-AHr3tIx3.js";
import { r as listConfiguredWebSearchProviders } from "./runtime-B9u5Z5Ps.js";
import { a as installCompletion, i as formatCompletionReloadCommand, l as resolveCompletionProfileHint } from "./completion-runtime-ciaYhinV.js";
...
import { i as waitForControlUiDocument, r as resolveControlUiHandoffTarget } from "./control-ui-handoff-l5Vx1uCm.js";
...
async function setupWizardShellCompletion(params) {
	...
	if (completionStatus.usesSlowPattern) {
		if (await ensureCompletionCache()) await deps.installCompletion(completionStatus.shell, true, cliName);
		return;
	}
...
async function startSessionGatewayForOnboarding(params) {
	...
	try {
		const { startGatewayServer } = await import("./server-D4KONp2g.js");
...
const loadSearchSetupModule = createLazyRuntimeModule(() => import("./search-setup-_GwBF9uB.js"));
/**
...
async function finalizeSetupWizard(options) {
	...
	if (gateway.status === "failed") gatewayProbe = {
		ok: false,
	...
	try {
		if (!opts.skipHealth) {
			...
			if (gateway.status !== "failed") gatewayProbe = await waitForGatewayReachable({
		...
		if (gateway.status !== "failed" && (opts.skipHealth || !gatewayProbe.ok)) gatewayProbe = await probeGatewayReachable({
			url: probeLinks.wsUrl,
			token: settings.authMode === "token" ? settings.gatewayToken : void 0,
		...
		const [{ resolveDefaultModelAuthStatus, resolveDefaultModelCatalogFacts }, { loadPreparedModelCatalogSnapshot }] = await Promise.all([import("./auth-choice-AmtLPSbu.js"), import("./prepared-model-catalog-ClbAcHfK.js")]);
		...
		await prompter.note([
			...
			dashboardReady && !opts.skipUi && gatewayProbe.ok && settings.authMode === "token" && settings.gatewayToken && !suppressGatewayTokenOutput ? t("wizard.finalize.webUiWithTokenUrl", { url: authedUrl }) : void 0,
		...
		await prompter.outro(gatewayProbe.ok ? dashboardReady ? t("wizard.finalize.outroDashboardLink") : controlUiEnabled ? [t("wizard.guided.complete"), t("wizard.finalize.dashboardWhenReady", { command: formatCliCommand("openclaw dashboard") })].join(" ") : t("wizard.guided.complete") : [t("wizard.finalize.gatewayNotDetected"), t("wizard.finalize.startGatewayNow", { command: formatCliCommand("openclaw gateway run") })].join(" "));
		if (shouldLaunchTui) {
