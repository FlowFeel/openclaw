...
import { r as formatSystemAgentStartupMessage } from "./overview-CDBIuwEP.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-C6-wAHoi.js";
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-DxY-0HMA.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-CPFqirC1.js";
...
import { i as buildRequestedApprovalEvent, l as listVisiblePendingApprovalRequests, s as handlePendingApprovalRequest } from "./approval-shared-DXraVglW.js";
import { t as getUpdateAvailable } from "./update-startup-CDkQtPQc.js";
import { n as getHealthCache } from "./health-state-B7JO1VbQ.js";
import { t as WizardSession } from "./session-BeEA_WOJ.js";
import { n as SystemAgentChatEngine, r as SystemAgentWizardAnswerError } from "./chat-engine-CtIOPV6b.js";
...
const systemAgentHandlers = {
	...
	"openclaw.setup.detect": async ({ params, respond }) => {
		...
		const { detectSetupInferenceIsolated } = await import("./setup-inference-detection-Dmof9Rsx.js");
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = new WizardSession(async (prompter, signal) => {
			await runExclusiveSystemAgentSetupActivation(async () => runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-BJgRkwzS.js"), import("./setup.shared-B1YQd8f0.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const inference = params.delegation ? await import("./inference-fallback-BVUjWQ3d.js").then(({ verifySystemAgentInferenceWithFallback }) => verifySystemAgentInferenceWithFallback({
