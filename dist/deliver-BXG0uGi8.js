...
import { y as createReplyToDeliveryPolicy } from "./reply-payload-BE_j43tQ.js";
import { i as resolveOutboundMediaMaxBytes } from "./configured-max-bytes-DnjCdIxf.js";
...
import { n as deriveDurableFinalDeliveryRequirementsForBatch, r as payloadRequiresDurablePayloadTransport } from "./capabilities-CjTX8w_2.js";
import { c as toOutboundDeliveryError, d as throwIfAborted, f as createChannelHandler, h as createRenderedMessageBatchPlan, n as prepareOutboundPayloadBatch, p as resolveOutboundDurableFinalDeliverySupport, s as suppressedPayloadOutcome, t as OutboundPayloadPreparationError } from "./deliver-prepare-WWZ8N40u.js";
import { a as emitOutboundAuditTerminals, c as createMessageSentEmitter, d as persistQueuedPostSendState, f as persistQueuedPreSendState, g as resolveDeferredDeliveryAdmission, h as OUTBOUND_DELIVERY_LOG_SCOPE, i as completedOutboundAuditTerminals, l as createQueuedDeliveryOwner, m as renewDeliveryPlatformSendLease, o as failedOutboundAuditTerminals, p as claimReusableDeliveryPlatformSendAttempt, r as withActiveDeliveryClaim, s as uniformOutboundAuditTerminals, u as isDeliveryAbortError } from "./delivery-queue-B6yQeeCg.js";
import { $ as emitMessageDeliveryStarted, B as PLATFORM_SEND_OWNER_LEASE_MS, C as failDeliveryBeforePlatformSend, G as mapPreparedOutboundAcceptedPayloads, I as StableDeliveryPreparationLostError, J as buildPayloadSummary, K as preparedOutboundSuppressionOutcomes, L as withStableDeliveryPreparation, N as markDeliveryPlatformSendDispatched, O as loadPendingDelivery, Q as emitMessageDeliveryError, S as failDeliveryAfterPlatformSend, T as findDeliveryIntentOwner, U as acceptedPreparedOutboundEntries, X as deliveryKindForPayload, Y as collectPayloadMediaSources, Z as emitMessageDeliveryCompleted, at as renderPresentationForDelivery, b as enqueuePreparedDeliveryOnce, ct as stripInternalRuntimeScaffoldingFromPayload, dt as runOutboundDeliveryCommitHooks, et as hasDeliveryResultIdentity, nt as maybePinDeliveredMessage, o as rejectDurableDelivery, ot as resolveOutboundMediaAccessForSend, r as completeDurableDelivery, rt as normalizeEmptyPayloadForDelivery, s as suppressDurableDelivery, st as sessionKeyForDeliveryDiagnostics, tt as maybeNotifyAfterDeliveredPayload, v as enqueueDelivery, x as failDelivery, y as enqueueDeliveryOnce } from "./delivery-queue-reconciliation-EW7LAhB7.js";
import { c as cancelDeliveryQueueMediaStage } from "./delivery-queue-media-staging-BXDYd5bo.js";
import { i as stageQueuePayloadMedia, r as releaseSpoolArtifacts } from "./delivery-queue-media-spool-zRDf89Bg.js";
import { n as hasTrustedMessageAuditListeners } from "./message-audit-events-DZQxEKuQ.js";
import { c as resolveTextChunkLimit, i as chunkMarkdownTextWithMode, n as chunkByParagraph, s as resolveChunkMode } from "./chunk-BJdR4AYS.js";
...
async function mirrorDeliveredPayloads(params) {
	...
	const deliveredMirror = {
		text: params.payloads.map((payload) => payload.hookContent ?? payload.text).filter((text) => text.trim()).join("\n"),
	...
	try {
		...
		if (!mirrorResult.ok) log$2.warn(`failed to mirror outbound delivery into session transcript; channel send already succeeded: ${mirrorResult.reason}`, {
			channel: params.channel,
			to: params.to,
			sessionKey: mirror.sessionKey
		});
	} catch (err) {
		log$2.warn(`failed to mirror outbound delivery into session transcript; channel send already succeeded: ${formatErrorMessage(err)}`, {
			channel: params.channel,
			to: params.to,
			sessionKey: mirror.sessionKey
		});
...
async function deliverOutboundPayloadsCore(params) {
	...
	const recordDeliveredPayload = (payloadSummary, deliveredResults) => {
		...
		try {
			...
			log$1.warn("Outbound delivered-payload observer failed after platform send.", {
				channel,
				to,
	...
	for (const [deliveryPayloadIndex, preparedEntry] of acceptedEntries.entries()) {
		...
		try {
			...
			if (!deliveryHandler.supportsMedia) {
				log$1.warn("Plugin outbound adapter does not implement sendMedia; media URLs will be dropped and text fallback will be used", {
					channel,
					to,
...
async function deliverOutboundPayloadsWithQueueCleanup(params, queueId, auditStartedAt, producerClaimId, producerLeaseSignal) {
	...
	const stablePayloadOutcomes = producerClaimId ? [] : void 0;
	const queuePolicy = params.queuePolicy ?? "best_effort";
	...
	if (hasMessageSentHooks && params.session?.agentId && !sessionKeyForInternalHooks) log.warn(`${OUTBOUND_DELIVERY_LOG_SCOPE}: session.agentId present without session key; internal message:sent hook will be skipped`, {
		channel: params.channel,
		to: params.to,
	...
	const wrappedParams = {
		...
		...auditPayloadOutcomes || stablePayloadOutcomes ? { onPayloadDeliveryOutcome: (outcome) => {
	...
	try {
		...
		if (queueId) {
			if (queuedPreSendState === "acked") {
				...
				if (producerClaimId !== void 0 && (deliveredResults.length > 0 || queuedPreSendState === "marked" || queuedPostSendState === "marked" || stablePayloadOutcomes?.some((outcome) => outcome.status === "sent"))) {
				...
				if (!terminalRejectionHandled) await recordOwnedQueueFailure(isProvenDeliveryNotSentError(err) ? failDeliveryBeforePlatformSend : failDelivery, formatErrorMessage(err)).catch((failErr) => {
...
var StableDeliveryProducerLeaseLostError = class extends Error {
	constructor(..._args) {
		super(..._args);
		this.name = "StableDeliveryProducerLeaseLostError";
	}
};
function lostProducerLeaseError(id, cause) {
	return new StableDeliveryProducerLeaseLostError(`Stable delivery platform claim was lost: ${id}`, { cause });
}
...
async function startDeliveryProducerLease(params) {
	...
	try {
		...
		if (error instanceof StableDeliveryProducerLeaseLostError) throw error;
...
async function runOutboundDeliveryInternal(params) {
...
async function deliverWithProducerLease(params, queueId, auditStartedAt, producerClaimId) {
	...
	const platformQueueId = queueId ?? params.deliveryQueueId;
	if (!platformQueueId || !producerClaimId) throw new Error("Stable delivery producer lease requires an exact queue owner");
...
async function runOutboundDeliveryWithQueue(params, stableIntentClaimHeld, stablePreparationOwner, allowFreshPreparation = true) {
	...
	if (params.deliveryIntentId && !existingStableDelivery && !stablePreparationOwner) {
		...
		if (owner) throw new Error(owner.namespace === "legacy" ? `Stable delivery intent is awaiting queue migration: ${params.deliveryIntentId}` : `Stable delivery intent is already queued: ${params.deliveryIntentId}`);
	}
	...
	try {
		...
		emitPreQueueFailure();
		const failedPayload = error instanceof OutboundPayloadPreparationError ? error.payload : params.payloads[0];
		if (failedPayload) {
	...
	const queued = params.skipQueue || preparedPayloads.length === 0 && !shouldPersistSuppressedIntent ? null : await stageAndEnqueueOutboundDelivery(deliveryParams, preparedBatch, stablePreparationOwner ? { getStablePreparation: stablePreparationOwner.current } : void 0).catch((err) => {
	...
	const deliverClaimedIntent = async () => {
		const producerClaimId = params.reusePendingDeliveryIntent ? await claimReusableDeliveryPlatformSendAttempt(queueId) : void 0;
		if (params.reusePendingDeliveryIntent && !producerClaimId) throw new Error(`Stable delivery intent is already queued: ${queueId}`);
		let claimedDeliveryParams = params.reusePendingDeliveryIntent && queued?.created === true ? {
			...deliveryParams,
			deliveryProducerLeaseRequired: true
		} : deliveryParams;
		if (queued?.created !== true) {
			const queuedEntry = await loadPendingDelivery(queueId);
			if (!queuedEntry || queuedEntry.producerClaimId !== producerClaimId) throw new Error(`Stable delivery platform claim was lost: ${queueId}`);
			const restoredDeliveryParams = restoreQueuedDeliveryCustody(deliveryParams, queuedEntry);
			claimedDeliveryParams = queuedEntry.requiresProducerClaim === true ? {
	...
	if (params.reusePendingDeliveryIntent) throw new Error(`Stable delivery intent is already queued: ${queueId}`);
	return [];
}
//#endregion
