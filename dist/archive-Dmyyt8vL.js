...
import { a as validateArchiveEntryPath, n as normalizeArchiveEntryPath, o as ArchiveFormatError } from "./archive-entry-C5HCSbsy.js";
import { a as readTarEntryInfo, n as preflightTarMetadata, r as importOptionalTar } from "./archive-ZGdbnIpa.js";
import { a as DEFAULT_MAX_ENTRIES, c as DEFAULT_MAX_META_ENTRY_BYTES, i as DEFAULT_MAX_ARCHIVE_BYTES_ZIP, n as ARCHIVE_LIMIT_ERROR_CODE, r as ArchiveLimitError, t as loadZipArchiveWithPreflight } from "./archive-zip-preflight-Dnek-ZSK.js";
...
function normalizedRequestedEntry(entryPath) {
	...
	if (!normalized || normalized.endsWith("/")) throw new Error(`archive entry is not a file: ${entryPath}`);
	return normalized;
}
...
async function readZipEntry(buffer, entryPath, maxBytes) {
	...
	if (!entry || entry.dir) throw new Error(`archive entry not found: ${entryPath}`);
	if (typeof entry.unixPermissions === "number" && (entry.unixPermissions & ZIP_UNIX_FILE_TYPE_MASK) === ZIP_UNIX_SYMLINK_TYPE) throw new Error(`archive entry is a link: ${entryPath}`);
	return await readStreamBounded(typeof entry.nodeStream === "function" ? entry.nodeStream() : Readable.from(await entry.async("nodebuffer")), maxBytes);
}
async function readTarEntry(archivePath, entryPath, maxBytes) {
	...
	await tar.t({
		...
		onReadEntry(entry) {
			...
			if (normalizeArchiveEntryPath(info.path).replace(/^\.\//, "") !== entryPath) {
				entry.resume();
				return;
			}
			if (info.type !== "File" && info.type !== "OldFile" && info.type !== "ContiguousFile") {
				matched = Promise.reject(/* @__PURE__ */ new Error(`archive entry is not a file: ${entryPath}`));
				entry.resume();
				return;
			}
	...
	if (!matched) throw new Error(`archive entry not found: ${entryPath}`);
	...
	try {
		...
		if (native) try {
			...
			for (const entry of manifest) {
				...
				if (normalizeArchiveEntryPath(entry.path).replace(/^\.\//, "") === requestedEntry) {
					if (entry.kind !== "file") throw new Error(`archive entry is not a file: ${entryPath}`);
			...
			if (error instanceof Error && error.message.includes("archive-header-invalid")) throw new ArchiveFormatError(error.message, { cause: error });
			throw error;
		}
