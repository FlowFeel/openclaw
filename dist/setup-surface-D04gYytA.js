...
import { N as splitSetupEntries, S as promptAccountId, p as mergeAllowFromEntries } from "./setup-wizard-helpers-Br2IavKW.js";
...
import { n as updateMatrixAccountConfig } from "./config-update-DXmd3dgG.js";
import { r as moveSingleMatrixAccountConfigToNamedAccount, t as createMatrixSetupDmPolicy } from "./setup-dm-policy-DeAndn8p.js";
import { n as validateMatrixHomeserverUrl, t as resolveValidatedMatrixHomeserverUrl } from "./url-validation-Dp_2HZGk.js";
...
async function promptMatrixAllowFrom(params) {
	...
	while (true) {
		...
		if (pending.length > 0) {
			const { resolveMatrixTargets } = await import("./resolve-targets-MkiZPGJm.js");
	...
	if (accessConfig) if (accessConfig.policy !== "allowlist") next = updateMatrixAccountConfig(next, params.accountId, { groupPolicy: accessConfig.policy });
		...
		if (accessConfig.entries.length > 0) try {
			...
			for (const entry of accessConfig.entries) {
				...
				const { listMatrixDirectoryGroupsLive } = await import("./directory-live-BCLEzyg6.js");
...
const matrixOnboardingAdapter = {
	...
	afterConfigWritten: async ({ previousCfg, cfg, accountId, runtime }) => {
		const { runMatrixSetupBootstrapAfterConfigWrite } = await import("./setup-bootstrap-B7sVnof1.js");
