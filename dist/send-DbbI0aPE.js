...
import { i as formatUncaughtError, r as formatErrorMessage } from "./errors-Cg_yT1Sv.js";
...
import { c as kindFromMime, r as extensionForMime, s as isGifMedia } from "./mime-Ir6g3Vae.js";
import { c as getImageMetadata } from "./image-ops-BtojRCsZ.js";
import { r as probeVideoDimensions } from "./media-probe-BL-Ub3TA.js";
import { o as loadCronStore, p as saveCronStore, u as resolveCronStorePath } from "./store-BwY4XtES.js";
import { C as normalizeOutboundLocation, S as formatLocationText } from "./reply-payload-BE_j43tQ.js";
...
import { c as resolveTextChunkLimit } from "./chunk-dNvLNXTy.js";
import { n as normalizePollInput } from "./polls-C-v11_tu.js";
import { a as sliceMarkdownIR, c as tokenizeHtmlTags, i as markdownToIRWithMeta } from "./construct-fallbacks-BR_o1n_g.js";
import { m as resolveStorePath } from "./session-store-runtime-DND3iX5-.js";
...
import { o as normalizeTelegramApiRoot, r as resolveTelegramTransport } from "./fetch-Cn7NhJH6.js";
import { t as expectDefined } from "./expect-runtime--WgnKYXT.js";
import { t as responseWithRelease } from "./fetch-runtime-C6dvgZVF.js";
import "./number-runtime-C6TGSEc_.js";
import "./string-coerce-runtime-CLK2YdzD.js";
import { n as createChannelApiRetryRunner } from "./retry-policy-DS4M6SwT.js";
import "./retry-runtime-F3LByspE.js";
import { a as isTelegramBadRequestError, f as isTelegramRateLimitError, h as tagTelegramNetworkError, p as isTelegramServerError, r as isSafeToRetrySendError, t as isRecoverableTelegramNetworkError, u as isTelegramMisdirectedRequestError } from "./network-errors-DPGlV01O.js";
import { r as resolveTelegramRequestTimeoutMs } from "./request-timeouts-C2F_8uWi.js";
import { t as resolveTelegramBotUserIdFromToken } from "./token-DRscXQWB.js";
import { n as renderMarkdownWithMarkers } from "./tables-CPvf63q-.js";
...
import { n as isVoiceMessageCompatibleAudio } from "./audio-DwSpd5Nu.js";
import { u as asObjectRecord } from "./runtime-doctor-BHX5ardJ.js";
import { t as requireRuntimeConfig } from "./plugin-config-runtime-AmbPKaTK.js";
import "./config-mutation-lKBWBI19.js";
import "./cron-store-runtime-RhWQ6xCq.js";
...
import { r as FormatCapabilityProfile } from "./text-chunking-nhEIGrpB.js";
import "./agent-runtime--vhO0pxB.js";
import "./logging-core-Bcy-4rwe.js";
import "./markdown-table-runtime-pDwHOEuO.js";
import "./diagnostic-runtime-DJ-vR5cE.js";
...
import { o as resolveTelegramAccount } from "./accounts-CAnRRRPr.js";
import { a as parseTelegramTarget, n as normalizeTelegramChatId, r as normalizeTelegramLookupTarget } from "./targets-CJIsAOe0.js";
import { c as isTelegramMessageCacheSourceMessage, d as resolveTelegramMessageCacheScope, f as normalizeTelegramReplyToMessageId, i as recordSentMessage, o as TELEGRAM_MESSAGE_CACHE_PERSISTENT_MAX_MESSAGES, p as parseTelegramMessageThreadId, s as TELEGRAM_MESSAGE_CACHE_PERSISTENT_NAMESPACE, u as resolveTelegramMessageCachePersistentScopeKey } from "./sent-message-cache-BxR6Sn3u.js";
import { t as getOptionalTelegramRuntime } from "./runtime-D4cq5Nic.js";
import { F as buildSenderName, G as resolveTelegramPrimaryMedia, I as extractTelegramLocation, K as resolveTelegramRichMessageBody, L as getTelegramTextParts, U as normalizeForwardedContext, Y as countTelegramHtmlVisibleCharacters, Z as markdownToTelegramChunks, _ as buildTypingThreadParams, at as decodeTelegramHtmlEntities, c as buildInlineKeyboard, et as renderTelegramHtmlText, g as buildTelegramThreadParams, nt as splitTelegramHtmlChunks, r as parseTelegramPromptContextProjection, rt as telegramHtmlToPlainTextFallback, tt as resolveTelegramHtmlVisibleText } from "./prompt-context-projection-BI7UvPeR.js";
...
function createTelegramClientFetch(params) {
	...
	const wrappedFetch = async (input, init) => {
		...
		const runFetch = async () => {
			...
			try {
				return responseWithRelease(await callFetch(input, {
					...init,
...
function normalizeTelegramMessageThreadBinding(value, threadId) {
	...
	if (normalizedThreadId === void 0 || observedThreadId !== normalizedThreadId) return;
	return {
		kind: "provider-observed-v1",
		threadId: String(normalizedThreadId)
	};
}
function createTelegramMessageThreadBinding(threadId) {
	const normalizedThreadId = parseTelegramMessageThreadId(threadId);
	return normalizedThreadId === void 0 ? void 0 : {
		kind: "provider-observed-v1",
		threadId: String(normalizedThreadId)
	};
}
function hasProviderObservedTelegramThreadBinding(node, threadId) {
	return normalizeTelegramMessageThreadBinding(node?.threadBinding, threadId) !== void 0;
}
function resolveProviderObservedTelegramThreadId(node) {
	const threadId = parseTelegramMessageThreadId(node?.threadId);
	return hasProviderObservedTelegramThreadBinding(node, threadId) ? threadId : void 0;
}
function normalizeMessageNodes(msg, params) {
	const observations = [];
	const visited = /* @__PURE__ */ new Set();
	const nodeThreadId = (node) => parseTelegramMessageThreadId(node.threadId);
	const visit = (message, inheritedThreadId, mode, promptContextProjectionMarker, threadBinding) => {
		...
		if (replyMessage?.message_id != null) visit(replyMessage, nodeThreadId(node) ?? inheritedThreadId, "partial", void 0, node.threadBinding);
		observations.push({
			node,
			mode
		});
	};
	visit(msg, params.threadId, "authoritative", params.promptContextProjectionMarker, params.threadBinding);
	return observations;
}
...
function parsePersistedCacheValue(key, value) {
	...
	const threadBinding = value.version === 1 ? normalizeTelegramMessageThreadBinding(value.threadBinding, threadId) : void 0;
...
function mergeCachedMessageNode(existing, incoming, mode) {
	...
	const threadBinding = normalizeTelegramMessageThreadBinding(incoming.threadBinding, threadId) ?? normalizeTelegramMessageThreadBinding(existing.threadBinding, threadId);
...
function createTelegramMessageCache(params) {
	...
	return {
		record: async ({ accountId, botUserId, chatId, msg, promptContextProjection, providerObservedThreadId, threadId }) => {
			await hydrateMessageCacheBucket(bucket, maxMessages, scopeKey);
...
async function recordOutboundMessageForPromptContext(params) {
	try {
		const providerGeneralTopicId = params.message.message_thread_id === void 0 && params.message.chat?.type === "supergroup" && params.successfulSendThread?.scope === "forum" && params.successfulSendThread.id === 1 ? 1 : void 0;
		...
		await createTelegramMessageCache({ scope: resolveTelegramMessageCacheScope(resolveStorePath(params.cfg.session?.store, { agentId: params.cfg.agents ? resolveDefaultAgentId(params.cfg) : "main" })) }).record({
			accountId: params.account.accountId,
			chatId: params.chatId,
			msg: cacheMessage,
			...params.botUserId !== void 0 ? { botUserId: params.botUserId } : {},
			...params.promptContextProjection ? { promptContextProjection: params.promptContextProjection } : {},
			...providerObservedThreadId !== void 0 ? { providerObservedThreadId } : {},
...
function markdownToTelegramRichBlocks(markdown, options = {}) {
	...
	if (hasMarkdownLists && (countInputRichBlocks(blocks) > 500 || maxInputRichBlockNesting(blocks) > 16)) {
...
async function editMessageTelegramWithContext(chatIdInput, messageIdInput, text, opts, context) {
	...
	if (!linkPreviewEnabled) textEditParams.link_preview_options = { is_disabled: true };
	if (replyMarkup !== void 0) textEditParams.reply_markup = replyMarkup;
	const plainTextParams = {};
	if (!linkPreviewEnabled) plainTextParams.link_preview_options = { is_disabled: true };
...
async function sendLocationTelegramWithContext(to, input, opts, context) {
	...
	const delivery = await withTelegramNativeQuoteFallback({
		...
		request: (effectiveParams, retryLabel) => prepared.request(() => hasName ? api.sendVenue(prepared.chatId, location.latitude, location.longitude, location.name ?? "", location.address ?? "", effectiveParams) : api.sendLocation(prepared.chatId, location.latitude, location.longitude, {
			...effectiveParams,
...
function isTelegramPhotoLimitError(error) {
	const description = error && typeof error === "object" && "description" in error ? error.description : void 0;
	return TELEGRAM_PHOTO_LIMIT_ERROR_RE.test(typeof description === "string" ? description : formatErrorMessage(error));
}
//#endregion
...
async function sendTelegramCaptionedMediaWithFallback(params) {
	if (!params.plainCaption) return await params.send(params.requestParams, params.shouldLog);
	try {
		return await params.send(params.requestParams, (err) => !isTelegramHtmlParseError(err) && (params.shouldLog?.(err) ?? true));
	} catch (err) {
		if (!isTelegramHtmlParseError(err)) throw err;
...
function createTelegramChunkDeliveryTracker(params) {
	...
	const throwAfterAccepted = (error) => {
		if (acceptedCount === 0 || isChannelPartialDeliveryError(error)) throw error;
...
function createTelegramTextSender(config) {
	const { cfg, account, api, chatId, opts, replyMarkup, reportDelivery, recordDeliveredPromptContext, singleUseReplyTo, buildThreadParams, requestWithChatNotFound, textMode, tableMode, renderHtmlText, linkPreviewOptions, useRichMessages } = config;
	...
	const createTextDelivery = (context) => {
		let lastMessageId = "";
		...
		const record = async (params) => {
			...
			await recordDeliveredPromptContext({
				...
				...params.acceptedParams?.message_thread_id !== void 0 ? { messageThreadId: params.acceptedParams.message_thread_id } : {}
			}, params.finalPart);
		};
	...
	const sendTelegramTextChunks = async (chunks, context, options = {}) => {
		...
		for (let index = 0; index < chunks.length; index += 1) {
			...
			await tracker.attempt(() => sendTelegramTextChunk(chunk, buildTextParams(index, chunks.length, finalPart, options.replyToAlreadyUsed === true)), ({ result, acceptedParams }) => delivery.record({
				result,
				acceptedParams,
	...
	const sendTelegramRichTextChunks = async (chunks, context, options = {}) => {
		...
		const tracker = createTelegramChunkDeliveryTracker({
			invalidate: () => opts.promptContextProjectionPlan?.cursor.invalidate(),
			onRejected: (error) => logVerbose(`telegram ${context} rich chunk rejected; continuing: ${formatErrorMessage(error)}`),
			partialDeliveryResult: delivery.partialDeliveryResult
		});
		for (let index = 0; index < chunks.length; index += 1) {
			const chunk = chunks[index];
			...
			try {
				...
				for (let fallbackIndex = 0; fallbackIndex < fallbackChunks.length; fallbackIndex += 1) {
					...
					const finalPart = index === chunks.length - 1 && fallbackIndex === fallbackChunks.length - 1;
					await tracker.attempt(() => sendTelegramTextChunk({ plainText: fallbackText }, fallbackParams), ({ result: fallbackResult, acceptedParams: fallbackAcceptedParams }) => delivery.record({
			...
			const finalPart = index === chunks.length - 1;
			await tracker.recordAccepted(result, (acceptedResult) => delivery.record({
	...
	if (mediaUrl) {
		...
		const sendMedia = async (label, sender) => {
			return await sendTelegramCaptionedMediaWithFallback({
				...
				...label === "photo" ? { shouldLog: (error) => !isTelegramPhotoLimitError(error) } : {},
				send: (requestParams, shouldLog) => withTelegramNativeQuoteFallback({
					label,
					requestParams,
					request: (effectiveParams, effectiveLabel) => requestWithChatNotFound(() => sender(effectiveParams), effectiveLabel, shouldLog ? { shouldLog } : void 0)
				})
			});
		};
		let mediaDelivery;
		...
		if (needsSeparateText && followUpText) {
			const textResult = await sendChunkedText(followUpText, "text follow-up send", { replyToAlreadyUsed: singleUseReplyTo && mediaUsedReplyTo });
			...
			const receipt = createMessageReceiptFromOutboundResults({
				...
				kind: "text",
				...acceptedMediaParams?.message_thread_id !== void 0 ? { threadId: String(acceptedMediaParams.message_thread_id) } : {}
			});
...
//#endregion
export { selectTelegramGroupHistoryAfterLastSelf as $, createForumTopicTelegram as A, buildTelegramConversationContext as B, markdownToTelegramRichBlocks as C, inputRichBlocksToPlainText as D, codeRichText as E, reactMessageTelegram as F, resolveProviderObservedTelegramThreadId as G, createTelegramMessageCache as H, sendTypingTelegram as I, isTelegramHistoryEntryAfterAmbientWatermark as J, buildTelegramSelfSenderName as K, unpinMessageTelegram as L, renameForumTopicTelegram as M, deleteMessageTelegram as N, italicRichText as O, pinMessageTelegram as P, retainTelegramGroupHistoryPromptContext as Q, recordOutboundMessageForPromptContext as R, toTelegramRichMessageContextParams as S, boldRichText as T, hasProviderObservedTelegramThreadBinding as U, buildTelegramReplyChain as V, isTelegramMessageFromCurrentBot as W, mergeTelegramGroupHistoryPromptContext as X, isTelegramSelfSenderName as Y, recordTelegramGroupHistoryEntry as Z, buildTelegramRichMarkdownPlan as _, isTelegramHtmlParseError as _t, createTelegramChunkDeliveryTracker as a, isTelegramQuoteParamError as at, removeTelegramRichNativeQuoteParam as b, sendTelegramCaptionedMediaWithFallback as c, resolveTelegramClientTimeoutMinimumSeconds as ct, sendLocationTelegram as d, withTelegramApiErrorLogging as dt, resetTelegramClientOptionsCacheForTests as et, editMessageReplyMarkupTelegram as f, getOrCreateAccountThrottler as ft, buildTelegramRichMarkdown as g, buildTelegramPlainFallbackPlan as gt, buildTelegramRichBlocksPlan as h, sequentialize as ht, sendMessageTelegram as i, getTelegramNativeQuoteReplyMessageId as it, editForumTopicTelegram as j, paragraphBlock as k, sendTelegramOutboundMediaWithPhotoFallback as l, resolveTelegramClientTimeoutSeconds as lt, TELEGRAM_RICH_TEXT_LIMIT as m, apiThrottler as mt, sendPollTelegram as n, TELEGRAM_OUTBOUND_RETRY_AFTER_CAP_MS as nt, prepareTelegramOutboundMedia as o, asTelegramClientFetch as ot, editMessageTelegram as p, Bot$1 as pt, isTelegramChatWindowPromptContext as q, sendStickerTelegram as r, buildTelegramSendParams as rt, resolveTelegramOutboundMediaSenders as s, createTelegramClientFetch as st, splitTelegramPlainTextChunksForTests as t, withTelegramNativeQuoteFallback as tt, isTelegramPhotoLimitError as u, resolveTelegramOutboundClientTimeoutFloorSeconds as ut, getTelegramRichRawApi as v, splitTelegramPlainTextChunks as vt, splitTelegramRichBlocks as w, splitTelegramRichMessageTextChunks as x, isEmptyTelegramRichMessage as y, warnTelegramRichBlocksDegradations as yt, registerTelegramOutboundGroupHistoryRecorder as z };
