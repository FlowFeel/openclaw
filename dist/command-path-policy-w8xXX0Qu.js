...
import { c as hasFlag, j as resolveGatewayCatalogCommandPath, n as getCommandPathWithRootOptions, r as getCommandPositionalsWithRootOptions } from "./argv-D6QuZiS5.js";
import { n as resolveCliStartupCommandPath } from "./argv-invocation-DfZI3Nyr.js";
...
const UPDATE_VALUE_FLAGS = [
	"--channel",
	"--tag",
	"--timeout"
];
function isRootUpdateDryRun(argv, commandPath) {
	if (commandPath.length !== 1 || !hasFlag(argv, "--dry-run")) return false;
	return getCommandPositionalsWithRootOptions(argv, {
		commandPath: hasFlag(argv, "--update") ? [] : ["update"],
...
const cliCommandCatalog = [
	...
	},
	{
		commandPath: ["update"],
		policy: {
			configGuard: ({ argv, commandPath }) => isRootUpdateDryRun(argv, commandPath) ? "skip" : "run",
			hideBanner: true
		}
	},
	{
...
function resolveCliCatalogCommandPath(argv) {
	...
	const tokens = resolveGatewayCatalogCommandPath(argv) ?? (startupPath[0] === "agent" ? startupPath : getCommandPathWithRootOptions(argv, argv.length));
