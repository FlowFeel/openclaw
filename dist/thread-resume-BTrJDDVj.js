...
import { W as CodexAppServerRpcError, Z as isCodexAppServerPrewriteRequestCancellationError, at as isJsonObject, g as retireSharedCodexAppServerClientIfCurrent, l as isCodexAppServerStartSelectionChangedError } from "./shared-client-D2F1gOgp.js";
import { i as assertCodexThreadResumeResponse } from "./protocol-validators-Cf6MSlge.js";
...
function readCodexErrorNotificationMessage(record) {
	...
	return isJsonObject(error) ? readString$1(error, "message") : void 0;
}
function readHookOutputEntries(value) {
	if (!Array.isArray(value)) return [];
	return value.flatMap((entry) => {
		if (!isJsonObject(entry)) return [];
		const text = readString$1(entry, "text");
		if (!text) return [];
		const kind = readString$1(entry, "kind");
		return [{
			...kind ? { kind } : {},
			text
		}];
...
function extractRawAssistantText(item) {
	const parts = (Array.isArray(item.content) ? item.content : []).flatMap((entry) => {
		if (!isJsonObject(entry)) return [];
		const type = readString$1(entry, "type");
		if (type !== "output_text" && type !== "text") return [];
		const value = readString$1(entry, "text");
...
//#endregion
export { shouldSynthesizeToolProgressForItem as A, readNullableString as B, isSideEffectingNativeToolItem as C, itemTitle as D, itemStatus as E, readItem as F, readString$1 as H, readItemString as I, readNonEmptyString as L, normalizeNonEmptyString as M, readCodexErrorNotificationMessage as N, shouldClearTerminalPresentationForNativeItem as O, readHookOutputEntries as P, readNonEmptyStringArray as R, isNonSuccessItemStatus as S, itemName as T, splitPlanText as U, readNumber as V, readCodexNotificationTurnId as _, closeCodexStartupClientBestEffort as a, auditNativeToolUnfinishedStatus as b, isCodexAppServerUnsafeSubscriptionError as c, unsubscribeCodexThreadBestEffort as d, CODEX_APP_SERVER_NATIVE_TURN_WAIT_TIMEOUT_MS as f, readCodexNotificationThreadId as g, isCodexNotificationForTurn as h, CodexAppServerUnsafeSubscriptionError as i, extractRawAssistantText as j, shouldRecordNativeToolTranscript as k, retireCodexAppServerClientAfterTimedOutTurn as l, CodexProjectionDiagnostics as m, CODEX_APP_SERVER_INTERRUPT_TIMEOUT_MS as n, interruptCodexTurnAndWaitBestEffort as o, getCodexAppServerTurnRouter as p, CODEX_APP_SERVER_UNSUBSCRIBE_TIMEOUT_MS as r, isCodexAlreadyTerminalInterruptError as s, resumeCodexAppServerThread as t, retireUnsafeCodexTurnClientBestEffort as u, auditNativeToolName as v, itemKind as w, isMutatingNativeToolItem as x, auditNativeToolTerminalStatus as y, readNonNegativeInteger as z };
