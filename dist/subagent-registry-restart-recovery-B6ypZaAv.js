...
import { lt as patchSqliteSessionEntry, st as loadSqliteSessionEntry } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { St as cancelSessionWorkAdmissionHandoff, ut as beginSessionWorkAdmission } from "./session-entry-slot-keys-DPRQmSpa.js";
import "./session-accessor-D5Or7WgI.js";
import "./sessions-BqBqRT1f.js";
import { c as readSessionMessagesAsync, n as extractMessageText, t as extractMessageRole } from "./session-transcript-readers-O8pyQfzE.js";
import { i as isStaleUnendedSubagentRun, s as getSubagentSessionStartedAt } from "./subagent-run-liveness-CjxLH_UA.js";
import { n as formatSubagentRecoveryWedgedReason, r as isSubagentRecoveryWedgedEntry } from "./subagent-recovery-state-CTVrxsMF.js";
import { r as resolveInternalSessionEffectsTarget } from "./internal-session-effects-B2ckt_x5.js";
import { createHash } from "node:crypto";
//#region src/agents/subagent-registry-restart-recovery-helpers.ts
...
//#endregion
//#region src/agents/subagent-registry-restart-recovery-session.ts
...
//#endregion
//#region src/agents/subagent-registry-restart-recovery.ts
...
async function reconcileAcceptedRecovery(params) {
	...
	if (params.runId !== params.receipt.idempotencyKey) {
		let remapped = false;
		try {
			remapped = params.isCurrent() && params.replaceRun({
	...
	try {
		if (!await settleAcceptedRecoverySession({
			...
			childSessionKey: params.childSessionKey,
			isOwnerCurrent: () => params.getRun(owner.runId) === owner && owner.execution.restartRecovery === params.receipt && isRestartRecoveryLifecycleCurrent(params.receipt),
	...
	try {
		if (!params.clearAcceptedRecovery({
			runId: owner.runId,
			expected: owner,
			sessionId: params.receipt.sessionId,
	...
	if (!params.resumeAcceptedRecovery({
		runId: owner.runId,
		expected: owner
	})) {
...
async function recoverInterruptedSubagentRow(params) {
	...
	const acceptedRecoveryCurrent = initialRecoveryReceipt?.phase === "accepted" && params.isCurrent();
	const isRecoverySourceCurrent = () => params.isCurrent() && params.entry.pauseReason !== "sessions_yield" && params.entry.suppressAnnounceReason !== "steer-restart" && params.entry.killReconciliation === void 0 && params.entry.killIntent === void 0 && typeof params.entry.execution.endedAt !== "number";
	...
	try {
		...
		const sessionEntry = loadSqliteSessionEntry({
			storePath,
		...
		const lastHumanMessage = extractMessageText([...messages].toReversed().find((message) => extractMessageRole(message) === "user"));
		const configChanged = messages.some((message) => extractMessageRole(message) === "assistant" && /openclaw\.json|openclaw gateway restart|config\.patch/i.test(extractMessageText(message) ?? ""));
