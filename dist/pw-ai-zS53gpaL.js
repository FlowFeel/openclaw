...
import { i as writeExternalFileWithinRoot, l as sanitizeUntrustedFileName } from "./fs-safe-DVaClkIX.js";
import { r as formatErrorMessage } from "./errors-D-7D3ZtF.js";
import { n as resolvePreferredOpenClawTmpDir } from "./tmp-openclaw-dir-BBjU-hqW.js";
import { t as SsrFBlockedError } from "./ssrf-C889LYfv.js";
...
import { l as DEFAULT_BROWSER_DOWNLOAD_TIMEOUT_MS } from "./constants-BoWgRtg0.js";
import { n as ACT_MAX_VIEWPORT_DIMENSION, o as resolveActInteractionTimeoutMs, r as ACT_MAX_WAIT_TIME_MS, s as resolveActWaitTimeoutMs, t as ACT_MAX_CLICK_DELAY_MS } from "./act-policy-9AEfR1NQ.js";
import { o as DEFAULT_DOWNLOAD_DIR, s as DEFAULT_TRACE_DIR, u as resolveStrictExistingUploadPaths } from "./config-DrBFz_Sl.js";
import { B as PLAYWRIGHT_TARGET_INFO_TIMEOUT_MS, G as withManagedProxyForCdpUrl, K as withNoProxyForCdpUrl, O as BrowserTabNotFoundError, a as fetchJson, f as redactCdpErrorText, g as withCdpSocket, h as stripCdpUrlCredentials, l as isWebSocketUrl, m as scopeCdpPolicyToConfiguredEndpoint, n as assertCdpEndpointAllowed, s as getHeadersWithAuth, t as appendCdpPath, u as normalizeCdpHttpBaseForJsonEndpoints } from "./tmp-openclaw-dir-Ci2i7GZs.js";
import "./errors-ByWLK1t1.js";
import { A as assertBrowserNavigationAllowed, C as finalizeRoleSnapshot, F as withBrowserNavigationPolicy, I as readCdpMainFrameDocumentIdentity, M as assertBrowserNavigationResultAllowed, S as buildRoleSnapshotFromAriaSnapshot, T as parseRoleRef, g as formatAriaSnapshot, j as assertBrowserNavigationRedirectChainAllowed, k as InvalidBrowserNavigationUrlError, l as ensureOutputDirectory, n as getChromeWebSocketUrl, p as AX_REF_PATTERN, v as normalizeCdpWsUrl, x as buildRoleSnapshotFromAiSnapshot } from "./chrome-BncZf3mR.js";
import { i as planAnnotations, l as matchBrowserUrlPattern, n as buildOverlayClearScript, r as buildOverlayInjectionScript, t as appendSnapshotUrls, u as normalizeBrowserEvaluateFunctionSource } from "./snapshot-urls-XEHa-0UE.js";
		...
		for (let attempt = 0; attempt < 3; attempt += 1) {
			...
			try {
				...
				const connectEndpoint = async (target) => {
					...
					return await withManagedProxyForCdpUrl(connectionUrl, () => withNoProxyForCdpUrl(connectionUrl, () => chromium.connectOverCDP(connectionUrl, {
						timeout,
						headers
					})));
				};
...
//#endregion
//#region extensions/browser/src/browser/pw-tools-core.extract.ts
...
function capturePageHtmlForExtract(params) {
	...
	} else nodes = [document.documentElement];
	const ignoredNodes = /* @__PURE__ */ new Set();
	for (const ignoreSelector of params.ignoreSelectors) try {
	...
	const html = nodes.map((node) => {
		const cloneWithoutIgnoredNodes = (source) => {
			if (source instanceof Element && ignoredNodes.has(source)) return null;
		...
		return cloneWithoutIgnoredNodes(node)?.outerHTML ?? "";
	}).filter(Boolean).join("\n");
	if (!html) return {
		ok: false,
		error: "selector_not_found"
...
async function pageContentViaPlaywright(opts) {
	const page = await getPageForTargetId(opts);
	opts.signal?.throwIfAborted();
	const capture = opts.selector || opts.ignoreSelectors?.length ? page.evaluate(capturePageHtmlForExtract, {
	...
	if (!opts.signal) return await capture;
	let onAbort;
	const aborted = new Promise((_, reject) => {
		onAbort = () => {
			const reason = opts.signal?.reason;
			reject(reason instanceof Error ? reason : /* @__PURE__ */ new Error("browser page capture aborted"));
		};
