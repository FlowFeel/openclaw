...
import { F as getAgentToolExecutionContext } from "./agent-core-3hvRadZX.js";
import { n as ToolInputError } from "./common-yW0U9cHP.js";
import { t as toCodeModeJsonSafe } from "./code-mode-json-CQ3LP5c6.js";
import { n as CODE_MODE_WAIT_TOOL_NAME, o as isCodeModeControlTool, s as markCodeModeControlTool, t as CODE_MODE_EXEC_TOOL_NAME } from "./code-mode-control-tools-Bts-Jy9S.js";
import { o as getActiveAgentRingZeroTools, t as applyLocalModelLeanToolSearchDefaults } from "./local-model-lean-CeG7_aMD.js";
import { _ as isDirectVisibleCatalogTool, c as ToolSearchRuntime, d as addClientToolsToToolCatalog, f as applyToolCatalogCompaction, h as compactToolSearchCatalogEntry, i as applyToolSchemaDirectoryCatalog, l as formatToolSearchControlResult, n as applyToolSearchCatalog, s as resolveToolSearchConfig } from "./tool-search-BhU6HOAE.js";
import { o as optionalStringEnum } from "./typebox-H9BhzMVW.js";
...
import { A as prepareSource, C as codeModeFailureMessage, D as enforceResultLimit, E as enforceOutputLimit, F as resolveCodeModeHeadlessConfig, I as toToolSearchConfig, L as codeModeReplayIdForToolCall, M as readPositiveInteger, N as readRunId, O as enforceSnapshotPayloadLimits, P as resolveCodeModeConfig, R as runBridgeRequest, S as codeModeFailureCode, T as createCodeModeApiFilesForRun, _ as telemetry, b as DEFAULT_HEADLESS_WALL_CLOCK_MS, c as pendingBridgeStatesForSettlement, d as reserveActiveRunSlot, f as resumingRunIds, g as takeUndeliveredCodeModeRunOutput, h as storeSnapshotState, i as createPendingBridgeStates, j as readCode, k as isCodeModeEngagedForModel, l as pendingToolCalls, m as snapshotState, n as cancelPendingBridgeStates, o as disposeCodeModeRun, p as settledBridgeRequestsInCompletionOrder, r as codeModeWaitingReason, s as pendingBridgeRequestsReplaySafe, t as activeRuns, u as removeExpiredRuns, v as waitForPendingBridgeSettlement, w as codeModeRuntimeTesting, x as MAX_HEADLESS_WALL_CLOCK_MS, y as CODE_MODE_WORKER_WATCHDOG_GRACE_MS, z as setCodeModeSwarmDepsForTest } from "./code-mode-state-BNttlFne.js";
...
function createCodeModeTools(ctx) {
	return [markCodeModeControlTool({
		...
		parameters: Type.Object({
			...
			restartSafe: Type.Optional(Type.Boolean({ description: "Set true only when every catalog call is explicitly replay-safe and OpenClaw may reconstruct the work after a gateway restart. Leave unset for ordinary calls; true rejects unmarked, side-effecting, or namespace tool calls." }))
		}),
		execute: async (toolCallId, args, signal, onUpdate) => {
			const input = readCode(args);
			const executionContext = getAgentToolExecutionContext();
			let runtime;
			return formatToolSearchControlResult(normalizeCodeModeTimeoutResult(await runExec({
				toolCallId,
				ctx,
				code: input.code,
...
function resolveAgentToolSurfacePlan(params) {
	...
	const toolsAvailable = params.toolsEnabled && getActiveAgentRingZeroTools().length === 0 && params.disableTools !== true && !params.isRawModelRun && params.skillWorkshopProposalOnly !== true && params.toolsAllow?.length !== 0 && !(params.forceDirectMessageTool && params.toolsAllow?.length === 1 && normalizeToolName(params.toolsAllow[0] ?? "") === "message");
