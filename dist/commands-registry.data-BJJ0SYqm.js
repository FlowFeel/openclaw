...
import { o as getActivePluginChannelRegistryVersionFromState } from "./registry-lookup-DCU2N45P.js";
import { i as listLoadedChannelPlugins } from "./registry-loaded-BSz-9sMZ.js";
import { t as BASE_THINKING_LEVELS } from "./thinking.shared-k6K-6JHM.js";
import { a as listThinkingLevels } from "./thinking-CLPqbAwx.js";
import { l as resolveFastModeModelAutoOnSeconds, t as formatFastModeAutoLabel } from "./fast-mode-BORk623r.js";
...
function registerAlias(commands, key, ...aliases) {
	...
	for (const alias of aliases) {
		...
		if (!lowered || existing.has(lowered)) continue;
...
function defineBuiltinCommand(...definition) {
	return definition;
}
...
function defineBuiltinChatCommand([key, description, category, tier, options = {}]) {
	const { nativeName = key, textAliases, ...fields } = options;
	return defineChatCommand({
		key,
		nativeName: nativeName === false ? void 0 : nativeName,
		description,
		textAlias: textAliases ? void 0 : `/${key}`,
		textAliases,
		category,
		tier,
		...fields
...
function buildBuiltinChatCommands(params = {}) {
	...
	const commands = [
		...
		defineBuiltinCommand("skill", "Run a skill by name.", "tools", "standard", { args: [defineCommandArgument("name", "Skill name", { required: true }), defineCommandArgument("input", "Skill input", { captureRemaining: true })] }),
		...
		defineBuiltinCommand("loop", "Loop a prompt: /loop [interval] <prompt> | /loop status | /loop stop [name]", "tools", "standard", { args: [defineCommandArgument("spec", "[interval] prompt, or status/stop", {
			required: false,
			captureRemaining: true
		})] }),
		defineBuiltinCommand("status", "Show current status.", "status", "essential", { acceptsArgs: true }),
		defineBuiltinCommand("goal", "Show or control the current goal.", "status", "standard", { args: [defineCommandArgument("action", "status, start, edit, pause, resume, complete, block, clear", { choices: [
			"status",
			"start",
			"edit",
			"pause",
		...
		defineBuiltinCommand("diagnostics", "Explain Gateway diagnostics and Codex feedback upload options.", "status", "standard", { args: [defineCommandArgument("note", "Optional note for Codex feedback upload", { captureRemaining: true })] }),
		defineBuiltinCommand("login", "Pair Codex login.", "management", "standard", {
			...
			args: [defineCommandArgument("provider", "Provider to pair", { choices: ["codex", "openai"] })]
		}),
		defineBuiltinCommand("openclaw", "Run the OpenClaw setup and repair helper.", "management", "essential", {
			nativeName: false,
			acceptsArgs: true
		}),
		...
		defineBuiltinCommand("approve", "Approve or deny exec requests.", "management", "power", { acceptsArgs: true }),
		defineBuiltinCommand("context", "Explain how context is built and used.", "status", "standard", { acceptsArgs: true }),
		...
		defineBuiltinCommand("session", "Manage session-level settings (for example /session idle).", "session", "power", {
			args: [defineCommandArgument("action", "idle | max-age", { choices: ["idle", "max-age"] }), defineCommandArgument("value", "Duration (24h, 90m) or off", { captureRemaining: true })],
			argsMenu: "auto"
		}),
		...
		defineBuiltinCommand("focus", "Bind this thread (Discord) or topic/conversation (Telegram) to a session target.", "management", "power", { args: [defineCommandArgument("target", "Subagent label/index or session key/id/label", { captureRemaining: true })] }),
		defineBuiltinCommand("unfocus", "Remove the current thread (Discord) or topic/conversation (Telegram) binding.", "management", "power"),
		defineBuiltinCommand("agents", "List thread-bound agents for this session.", "management", "standard"),
		defineBuiltinCommand("steer", "Send guidance to the active run in this session.", "management", "standard", { args: [defineCommandArgument("message", "Steering message", { captureRemaining: true })] }),
		...
		defineBuiltinCommand("reset", "Reset the current session.", "session", "essential", { acceptsArgs: true }),
		defineBuiltinCommand("new", "Start a new session.", "session", "essential", { acceptsArgs: true }),
		defineBuiltinCommand("name", "Name or rename the current session.", "session", "standard", { args: [defineCommandArgument("title", "New session name (omit to see a suggestion)", { captureRemaining: true })] }),
		...
		defineBuiltinCommand("verbose", "Toggle verbose mode.", "options", "standard", { args: [defineCommandArgument("mode", "on, off, or full", { choices: [
			"on",
			"off",
			"full"
		] })] }),
		...
		defineBuiltinCommand("exec", "Set exec defaults for this session.", "options", "power", {
			args: [
				defineCommandArgument("host", "sandbox, gateway, or node", { choices: [
					"sandbox",
					"gateway",
					"node"
				] }),
		...
		defineBuiltinCommand("model", "Show or set the model.", "options", "essential", { args: [defineCommandArgument("model", "Model id (provider/model or id)")] }),
		defineBuiltinCommand("models", "List model providers/models.", "options", "standard", { acceptsArgs: true }),
