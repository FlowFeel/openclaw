...
import { f as readProviderJsonArrayFieldResponse, g as readResponseTextLimited, m as readProviderJsonResponse } from "./provider-http-errors-Cj7zfu8U.js";
import { c as hasConfiguredSecretInput } from "./types.secrets-BvApkFoj.js";
import { t as resolveConfiguredSecretInputString } from "./resolve-configured-secret-input-string-DJX-j6lE.js";
import { c as isNonSecretApiKeyMarker, s as isKnownEnvApiKeyMarker } from "./model-auth-markers-z4js65k2.js";
import { t as normalizeOptionalSecretInput } from "./normalize-secret-input-Df_qhWv_.js";
import { i as normalizeApiKeyConfig } from "./models-config.providers.secrets-CkzsKj4c.js";
import "./provider-auth-SalgjzRl.js";
import "./number-runtime-C6TGSEc_.js";
import "./string-coerce-runtime-CLK2YdzD.js";
import { c as SELF_HOSTED_DEFAULT_COST, l as SELF_HOSTED_DEFAULT_MAX_TOKENS } from "./provider-setup-D4V1uLy-.js";
...
import "./secret-input-runtime-c5a4e1R9.js";
...
function normalizeConfiguredCompatStringList(value) {
	if (!Array.isArray(value) || !value.every((entry) => typeof entry === "string" && entry.length > 0)) return;
	return [...value];
}
function isLmstudioConfiguredThinkingFormat(value) {
	return typeof value === "string" && LMSTUDIO_CONFIGURED_THINKING_FORMATS.some((format) => format === value);
}
function normalizeLmstudioConfiguredCompat(value) {
	...
	for (const key of LMSTUDIO_CONFIGURED_BOOLEAN_COMPAT_FIELDS) {
		...
		if (typeof configuredValue === "boolean") compat[key] = configuredValue;
	}
	const visibleReasoningDetailTypes = normalizeConfiguredCompatStringList(record.visibleReasoningDetailTypes);
	if (visibleReasoningDetailTypes) compat.visibleReasoningDetailTypes = visibleReasoningDetailTypes;
	const unsupportedToolSchemaKeywords = normalizeConfiguredCompatStringList(record.unsupportedToolSchemaKeywords);
	if (unsupportedToolSchemaKeywords) compat.unsupportedToolSchemaKeywords = unsupportedToolSchemaKeywords;
	if (record.maxTokensField === "max_completion_tokens" || record.maxTokensField === "max_tokens") compat.maxTokensField = record.maxTokensField;
	if (isLmstudioConfiguredThinkingFormat(record.thinkingFormat)) compat.thinkingFormat = record.thinkingFormat;
	if (typeof record.toolSchemaProfile === "string") compat.toolSchemaProfile = record.toolSchemaProfile;
	if (typeof record.toolCallArgumentsEncoding === "string") compat.toolCallArgumentsEncoding = record.toolCallArgumentsEncoding;
...
function mapLmstudioWireEntry(entry) {
	...
	const compat = trainedForToolUse === true ? {
		...reasoningCompat,
...
async function fetchLmstudioModels(params) {
	...
	try {
		...
		try {
			...
			const validModels = models.filter((model) => typeof model === "object" && model !== null && !Array.isArray(model));
			if (models.length > 0 && validModels.length === 0) throw new Error("LM Studio model list: malformed JSON response");
			return {
				reachable: true,
				status: response.status,
				models: validModels
...
//#endregion
export { LMSTUDIO_DEFAULT_LOAD_CONTEXT_LENGTH as A, resolveLmstudioReasoningCompat as C, LMSTUDIO_DEFAULT_BASE_URL as D, LMSTUDIO_DEFAULT_API_KEY_ENV_VAR as E, LMSTUDIO_MODEL_PLACEHOLDER as F, LMSTUDIO_PROVIDER_ID as I, LMSTUDIO_PROVIDER_LABEL as L, LMSTUDIO_DOCKER_HOST_BASE_URL as M, LMSTUDIO_DOCKER_HOST_INFERENCE_BASE_URL as N, LMSTUDIO_DEFAULT_EMBEDDING_MODEL as O, LMSTUDIO_LOCAL_API_KEY_PLACEHOLDER as P, resolveLmstudioReasoningCapability as S, resolveLoadedContextWindow as T, normalizeLmstudioConfiguredCatalogEntries as _, resolveLmstudioConfiguredApiKey as a, resolveLmstudioCanonicalModelKey as b, resolveLmstudioRequestContext as c, resolveLmstudioProviderAuthMode as d, shouldUseLmstudioApiKeyPlaceholder as f, mapLmstudioWireModelsToConfig as g, mapLmstudioWireEntry as h, buildLmstudioAuthHeaders as i, LMSTUDIO_DEFAULT_MODEL_ID as j, LMSTUDIO_DEFAULT_INFERENCE_BASE_URL as k, resolveLmstudioRuntimeApiKey as l, buildLmstudioModelName as m, ensureLmstudioModelLoaded as n, resolveLmstudioConfiguredApiKeyForProvider as o, shouldUseLmstudioSyntheticAuth as p, fetchLmstudioModels as r, resolveLmstudioProviderHeaders as s, discoverLmstudioModels as t, hasLmstudioAuthorizationHeader as u, normalizeLmstudioConfiguredCatalogEntry as v, resolveLmstudioServerBase as w, resolveLmstudioInferenceBase as x, normalizeLmstudioProviderConfig as y };
