...
import { n as GATEWAY_CLIENT_IDS } from "./client-info-Dlrmm4mP.js";
import "./node-commands-CeG5w7c2.js";
import { a as removeConnectedNodePluginTools, i as normalizeNodePluginToolDescriptors, o as replaceConnectedNodePluginTools, t as createRegisteredNodePluginToolDescriptorMap } from "./node-plugin-tool-snapshot-dArpNFrx.js";
...
var NodeRegistry = class {
	...
	async invoke(params) {
		...
		if (this.options.resolveCurrentPairingState && !expectedPairingGeneration) return {
			ok: false,
			error: {
				code: "PAIRING_CHANGED",
		...
		if (expectedPairingGeneration && this.options.resolveCurrentPairingState) {
			const resolution = await this.resolvePairingLease(this.capturePairingLease(node), { invalidateStale: false });
