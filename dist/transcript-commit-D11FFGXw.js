...
import { St as publishSqliteTranscriptUpdate, mt as replaceSqliteSessionEntrySync, p as withSqliteTranscriptWriteTransaction, st as loadSqliteSessionEntry } from "./session-accessor.sqlite-B9iW7DOt.js";
...
//#region src/agents/session-transcript-repair/validate.ts
/**
* Validates invariant pairing between tool invocations and results without disk or network I/O.
*/
function validateTurnInvariants(toolCallIds, toolResultIds) {
	...
	if (orphanCalls.length === 0 && orphanResults.length === 0) return {
		valid: true,
		pairedCount: toolCallIds.length
	};
	return {
		valid: false,
...
async function applyWorkerTranscriptCommit(params) {
	...
	const applied = await withSqliteTranscriptWriteTransaction(params.target, (transcriptTarget) => {
		const currentEntry = loadSqliteSessionEntry(params.target);
		...
		const fullMessages = [...prefixMessages, ...incomingMessages].filter((m) => m !== void 0);
		const allToolCalls = [];
		const allToolResults = [];
		for (const msg of fullMessages) if (msg.role === "assistant") {
			const calls = extractToolCallsFromAssistant(msg);
			for (const call of calls) allToolCalls.push(call.id);
		} else if (msg.role === "toolResult") {
			const resultId = extractToolResultId(msg);
			if (resultId) allToolResults.push(resultId);
		}
		const validation = validateTurnInvariants(allToolCalls, allToolResults);
		if (!validation.valid) {
			if (validation.orphanResultIds.length > 0) return {
				ok: false,
				reason: "invalid-batch"
			};
			if (validation.orphanCallIds.length > 0) {
				...
				for (let i = 0; i < fullMessages.length; i++) {
					...
					if (msg && msg.role === "assistant" && extractToolCallsFromAssistant(msg).some((c) => c.id === firstOrphanCallId)) {
						orphanIndex = i;
						break;
					}
				}
				if (orphanIndex !== -1) for (let i = orphanIndex + 1; i < fullMessages.length; i++) {
					const msg = fullMessages[i];
		...
		const freshEntry = loadSqliteSessionEntry(params.target);
		...
		replaceSqliteSessionEntrySync(params.target, nextEntry);
		return {
			ok: true,
			messages
		};
	...
	for (const message of applied.messages) {
		...
		await publishSqliteTranscriptUpdate(params.target, {
