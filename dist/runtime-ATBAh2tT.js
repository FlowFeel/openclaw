...
import { a as resolveAgentModelTimeoutMsValue } from "./model-input-BofPWz0k.js";
import "./provider-env-vars-5Fdeltvz.js";
import { a as resolveCapabilityModelCandidates, c as resolveClosestSize, i as recordCapabilityCandidateFailure, l as resolveMediaProviderRequestTimeoutMs, n as buildNoCapabilityModelConfiguredMessage, o as resolveClosestAspectRatio, s as resolveClosestResolution, t as buildMediaGenerationNormalizationMetadata, u as throwCapabilityGenerationFailure } from "./runtime-shared-BMD-W29y.js";
import { t as hasMediaNormalizationEntry } from "./normalization-CJ51nPmx.js";
import { n as listVideoGenerationProviders, r as parseVideoGenerationModelRef, t as getVideoGenerationProvider } from "./provider-registry-D0e_4HDU.js";
...
async function generateVideo(params, deps = {}) {
	...
	const requestedTimeoutMs = params.timeoutMs ?? resolveAgentModelTimeoutMsValue(params.cfg.agents?.defaults?.mediaModels?.video);
			...
			return {
				videos: result.videos.map((video, index) => {
					if (video.buffer?.byteLength === 0) {
						...
						throw new Error(`Video generation provider returned an empty video buffer at index ${index}.`);
					}
					if (!video.buffer && !video.url) throw new Error(`Video generation provider returned an undeliverable asset at index ${index}: neither buffer nor url is set.`);
					return video;
				}),
				provider: candidate.provider,
