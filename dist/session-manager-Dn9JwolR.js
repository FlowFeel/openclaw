import { a as appendSqliteTranscriptEventSync, l as replaceSqliteTranscriptEventsSync, mt as replaceSqliteSessionEntrySync, s as appendSqliteTranscriptMessageSync, st as loadSqliteSessionEntry } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { n as projectCanonicalSessionEntryShape } from "./store-entry-shape-CTwpu-C_.js";
import { g as isSessionTranscriptSideAppendEntry } from "./session-transcript-index-NEbbnqMc.js";
import { x as loadSqliteTranscriptEventsSync } from "./session-accessor.sqlite-transcript-store-BCfOcQgE.js";
import "./session-accessor-D5Or7WgI.js";
import { D as buildSessionContext } from "./agent-core-3hvRadZX.js";
import { a as isSessionContextMetadataEntry, c as migrateToCurrentVersion, d as parseParentLinkedOpaqueEntry, h as generateSessionEntryId, i as isJsonRecord, m as createSessionId, p as partitionSessionFileEntries, r as isIndexedSessionEntry, u as parseOpaqueLeafEntry } from "./session-manager-codec-MEinfCA8.js";
...
var SessionManagerCore = class {
	...
	setSessionTarget(target) {
		const entries = loadSqliteTranscriptEventsSync(target);
		...
		for (let index = 0; index <= this.fileEntries.length; index += 1) {
			while (this.opaqueFileEntries[opaqueIndex]?.index === index) {
				...
				if (leafEntry) {
					...
					this.promptReleasedSideBranchParentId = effectiveLeafState.appendMode === "side" ? effectiveLeafState.appendParentId : void 0;
					opaqueIndex += 1;
					continue;
				}
	...
	replacePersistedTranscript(options) {
		...
		replaceSqliteTranscriptEventsSync(this.persistenceTarget, this.getPersistedFileEntries(leafAppendParentId, options?.leafAppendMode ?? this.appendMode));
		this.persistenceHeaderPending = false;
	}
...
var SessionManagerPersistence = class extends SessionManagerCore {
	...
	persistSqliteRecord(entry, options) {
		...
		if (this.persistenceHeaderPending) {
			...
			if (!header || header.type !== "session" || !appendSqliteTranscriptEventSync(scope, header)) throw new Error("Session transcript header was not persisted");
			this.persistenceHeaderPending = false;
		}
		const leafEntry = parseOpaqueLeafEntry(entry);
		if (leafEntry) {
			if (!appendSqliteTranscriptEventSync(scope, entry)) throw new Error(`Session transcript leaf control was not persisted: ${leafEntry.id}`);
			return;
		}
		if (!isIndexedSessionEntry(entry)) return;
		if (entry.type !== "message") {
			if (!appendSqliteTranscriptEventSync(scope, entry, options?.appendIntent === "active-branch" ? { appendIntent: options.appendIntent } : void 0)) throw new Error(`Session transcript entry was not persisted: ${entry.id}`);
			return;
		}
		const result = appendSqliteTranscriptMessageSync(scope, {
			cwd: this.cwd,
			eventId: entry.id,
		...
		if (result.messageId !== entry.id) {
			if ((entry.message.role === "user" && "idempotencyKey" in entry.message && typeof entry.message.idempotencyKey === "string" && entry.message.idempotencyKey.length > 0 ? entry.message.idempotencyKey : void 0) && options?.idempotencyLookup !== "caller-checked") return { adoptedMessageId: result.messageId };
	...
	mergePromptReleasedSessionEntries(entries, options) {
		...
		if (options?.persistLeaf !== true || !this.persistenceTarget || !sawPersistedStateUpdate || persistedLeafId === this.leafId && persistedAppendParentId === sideBranchParentId && persistedAppendMode === "side") return;
...
var SessionManagerEntries = class extends SessionManagerPersistence {
	...
	appendLeafControl(params) {
		...
		this.promptReleasedSideBranchParentId = params.appendMode === "side" ? params.appendParentId : void 0;
		this.pendingDeliberateAppend = false;
		return entry;
	}
	getLeafEntry() {
...
var SessionManagerBranching = class extends SessionManagerEntries {
	...
	createBranchedSession(leafId) {
		...
		if (persistenceTarget) {
			...
			const previousEntry = loadSqliteSessionEntry({
				agentId: persistenceTarget.agentId,
				sessionKey: persistenceTarget.sessionKey,
				storePath: persistenceTarget.storePath
			});
			const canonicalPreviousEntry = previousEntry ? projectCanonicalSessionEntryShape(previousEntry) : { updatedAt };
