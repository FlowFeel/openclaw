...
import { a as assertProviderBinaryResponseContent, i as assertOkOrThrowProviderError, m as readProviderJsonResponse, n as asObject } from "./provider-http-errors-Dm9G78mz.js";
...
async function xaiTTS(params) {
	...
	try {
		...
		const audio = await readResponseWithLimit(response, maxBytes, { onOverflow: ({ maxBytes: maxBytesLocal }) => /* @__PURE__ */ new Error(`xAI TTS audio response exceeds ${maxBytesLocal} bytes`) });
		if (audio.byteLength === 0) throw new Error("xAI TTS API error: malformed audio response");
		return audio;
	} finally {
		await release();
	}
