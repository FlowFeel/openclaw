...
import { o as createProviderHttpError } from "./provider-http-errors-Dm9G78mz.js";
import { c as getEnvApiKey, r as calculateCost } from "./llm-DFGpShix.js";
...
import { n as transformTransportMessages } from "./ai-transport-runtime-host-DwMiqDOL.js";
import { i as isGoogleGemini3ProModel, l as stripInvalidGoogleThinkingBudget, n as isGoogleGemini25ThinkingBudgetModel, r as isGoogleGemini3FlashModel, s as resolveGoogleGemini3ThinkingLevel } from "./stream-payload-utils-BcOTedPh.js";
	...
	try {
		while (true) {
			...
			if (done) {
				...
				if (buffer.split(/\r\n|\n|\r/u).some((line) => line.startsWith("data:") && line.slice(5).trim().length > 0)) throw new Error("Google SSE stream ended with an incomplete frame");
				completed = true;
				break;
			}
			...
			while (boundary) {
				...
				const data = rawEvent.split(/\r\n|\n|\r/u).filter((line) => line.startsWith("data:")).map((line) => line.slice(5).trim()).join("\n");
				...
				if (isRecord(chunk) && isRecord(chunk.error)) {
					...
					throw Object.assign(new Error(normalizeOptionalString(providerError.message) ?? "Google stream failed"), {
						code: normalizeOptionalString(providerError.status) ?? "GOOGLE_STREAM_ERROR",
						status: typeof providerError.code === "number" ? providerError.code : void 0,
...
function updateUsage(output, model, chunk, knownUsage) {
	...
	const outputTokens = (knownUsage.candidatesTokenCount ?? 0) + (knownUsage.thoughtsTokenCount ?? 0);
	output.usage = {
			...
			try {
				...
				for await (const chunk of chunks) {
					...
					if (!candidate && promptFeedback) {
						const blockReason = normalizeOptionalString(promptFeedback.blockReason) ?? "PROMPT_BLOCKED";
					...
					if (candidate?.content?.parts) for (const part of candidate.content.parts) {
						...
						if (part.functionCall) {
							...
							const isDuplicate = (typeof providedId === "string" ? toolCallBlocksById.get(providedId) : void 0) !== void 0;
							const toolCall = {
								type: "toolCall",
				...
				if (currentBlockIndex >= 0) pushTextBlockEnd(stream, output, currentBlockIndex);
				if (terminalGenerationError && !options?.signal?.aborted) throw terminalGenerationError;
				if (!sawTerminalReason && !options?.signal?.aborted) throw Object.assign(/* @__PURE__ */ new Error("Google stream ended before a terminal finish reason"), {
