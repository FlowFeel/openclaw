...
import { r as findStaleOpenClawUpdateLaunchdJobs } from "./launchd-DT4INyGN.js";
...
async function collectMacLaunchctlGatewayEnvOverrideWarning(cfg, deps) {
	...
	return [
		...
		envToken && envTokenKey ? `- \`${envTokenKey}\` is set; explicit environment URL or node-host targets can use a different token than gateway.auth.token.` : void 0,
		envPassword ? `- \`${envPasswordKey ?? "OPENCLAW_GATEWAY_PASSWORD"}\` is set; explicit environment URL or node-host targets can use a different password than gateway.auth.password.` : void 0,
