...
import { a as LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES, m as resolveGatewaySystemdServiceName, p as resolveGatewayServiceDescription } from "./constants-Qf4OESGR.js";
import { C as resolveStateDir } from "./paths-CL43LNS6.js";
import { r as formatErrorMessage } from "./errors-Cg_yT1Sv.js";
import { a as normalizeEnvVarKey, n as isDangerousHostEnvOverrideVarName, r as isDangerousHostEnvVarName } from "./host-env-security-D4EXCIbD.js";
import "./regular-file-8uj3r-Aa.js";
import { t as createSubsystemLogger } from "./subsystem-Ess1Ww-N.js";
import { a as collectConfigServiceEnvVars } from "./config-env-vars-Dh6msnxE.js";
import { t as splitArgsPreservingQuotes } from "./arg-split-CR3xkHmb.js";
import { r as resolveHomeDir } from "./paths-DsnYGLDu.js";
import { n as runExec, r as runCommandWithTimeout } from "./exec-SGTX01mL.js";
import { t as execFileUtf8 } from "./exec-file-Bf07bZfW.js";
...
import { i as renderSystemdEnvAssignment, n as parseSystemdEnvAssignments, r as parseSystemdExecStart, t as buildSystemdUnit } from "./systemd-unit-DACqAHrA.js";
...
async function findMarkerOwnedSystemSystemdUnit() {
	const { findSystemGatewayServices } = await import("./inspect-D9Whg19p.js");
	let services;
	try {
...
async function uninstallSystemdService({ env, stdout }) {
	...
	const disabled = await execSystemctlUser(env, [
		"disable",
		"--now",
		unitName
	]);
	if (disabled.code !== 0) {
		...
		if (!new RegExp(`^(?:failed to (?:disable unit|stop\\s+${escapedUnitName}):\\s*)?(?:unit file\\s+${escapedUnitName}\\s+does not exist|unit\\s+${escapedUnitName}(?:\\s+is)?\\s+(?:inactive|not\\s+active|not\\s+loaded|not-found|could not be found))[.!]?$`, "u").test(normalizeLowercaseStringOrEmpty(detail))) throw new Error(`systemctl disable failed: ${detail || "unknown error"}`);
	}
	const unitPath = resolveSystemdUnitPath(env);
	let removed = false;
	try {
...
async function runSystemdServiceAction(params) {
	...
	if (installed?.scope === "system") {
		...
		if (params.action !== "stop") await execSystemctl(["reset-failed", unitName], env);
...
async function uninstallUserSystemdGatewayUnit({ env, stdout }) {
	...
	if (removed && disabled) await execSystemctlUser(env, ["daemon-reload"]);
	return {
		unitName,
		unitPath,
		removed,
...
//#endregion
export { writeManagedServiceEnvKeysToEnvironment as A, collectInlineManagedServiceEnvKeys as C, isEnvironmentFileOnlySource as D, hasInlineEnvironmentSource as E, collectDurableServiceEnvVars as M, readEnvironmentValueSource as O, readSystemdUserLingerStatus as S, hasEnvironmentFileSource as T, uninstallLegacySystemdUnits as _, isNonFatalSystemdInstallProbeError as a, classifySystemdUnavailableDetail as b, isSystemdUnitActive as c, readSystemdServiceRuntime as d, resolveSystemdUserUnitPath as f, stopSystemdService as g, startSystemdService as h, installSystemdService as i, collectDurableServiceEnvVarSources as j, readManagedServiceEnvKeysFromEnvironment as k, isSystemdUserServiceAvailable as l, stageSystemdService as m, findSystemdGatewayInstallation as n, isSystemUnitActiveAndEnabled as o, restartSystemdService as p, formatDuelingScopesWarning as r, isSystemdServiceEnabled as s, findInstalledSystemdGatewayScope as t, readSystemdServiceExecStart as u, uninstallSystemdService as v, formatManagedServiceEnvKeys as w, enableSystemdUserLinger as x, uninstallUserSystemdGatewayUnit as y };
