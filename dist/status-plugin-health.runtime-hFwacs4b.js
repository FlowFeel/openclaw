...
import { r as listLoadedRuntimePluginIds, t as getActiveRuntimePluginRegistry } from "./active-runtime-registry-3hUls5u9.js";
import { n as resolveReadOnlyChannelPluginsForConfig } from "./read-only-CG09Owk1.js";
import { n as listPersistedRuntimeToolSchemaQuarantines } from "./tool-schema-quarantine-health-iiox1BbR.js";
...
async function collectInstalledPluginHealthSnapshot(params) {
	const { buildPluginCompatibilityNotices, buildPluginSnapshotReport } = await import("./status-ZNqyvmhV.js");
...
async function resolveUnregisteredMemoryEmbeddingProviders(params) {
	...
	try {
		const { collectRegisteredEmbeddingProviderIds, collectUnregisteredConfiguredMemoryEmbeddingProviders } = await import("./gateway-startup-plugin-ids-DyiQ0MdF.js");
...
async function resolveShouldRunPluginIds(params) {
	...
	try {
		const { loadGatewayStartupPluginPlan } = await import("./gateway-startup-plugin-ids-DyiQ0MdF.js");
		const { resolvePluginActivationSourceConfig } = await import("./activation-source-config-DY8840o8.js");
		const { resolveGatewayStartupPluginActivationConfig } = await import("./plugin-activation-runtime-config-CFcyOwZU.js");
