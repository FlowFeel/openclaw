...
import { m as normalizeSecretInputString, p as normalizeResolvedSecretInputString } from "./types.secrets-BvApkFoj.js";
import { s as sleepWithAbort } from "./src-DKBD8PDy.js";
import { o as warn } from "./globals-Cw62Mq_M.js";
...
import { n as createDebugProxyWebSocketAgent, r as resolveDebugProxySettings } from "./env-CuNs3BeI.js";
import { i as isProviderAuthProfileConfigured, s as resolveProviderAuthProfileApiKey } from "./provider-auth-SalgjzRl.js";
import "./runtime-env-DEukRWMB.js";
import { n as captureWsEvent } from "./runtime-CgTPepwg.js";
...
import { H as REALTIME_VOICE_AUDIO_FORMAT_PCM16_24KHZ, V as REALTIME_VOICE_AUDIO_FORMAT_G711_ULAW_8KHZ } from "./realtime-session-harness-BwpuC40T.js";
import { t as RealtimeVoiceSessionLifecycle } from "./realtime-voice-Bx9kWxXN.js";
import { n as buildOpenAIQuicksilverInstructions } from "./realtime-quicksilver-instructions-CUVT6eIm.js";
import { n as isOpenAIGptLiveModel, r as isSupportedOpenAIGptLiveModel, t as OPENAI_GPT_LIVE_MODELS } from "./realtime-quicksilver-BdMyAyC5.js";
import { i as resolveOpenAIChatGptSubscriptionAuth, t as OPENAI_QUICKSILVER_CAPABILITIES } from "./realtime-quicksilver-session-RVAs5_eQ.js";
import { a as resolveOpenAIProviderConfigRecord, i as readRealtimeErrorDetail, n as createOpenAIRealtimeClientSecret, o as trimToUndefined, t as captureOpenAIRealtimeWsClose } from "./realtime-provider-shared-Cl_UFmyW.js";
import { t as OpenAIQuicksilverVoiceBridge } from "./realtime-quicksilver-bridge-CEcJ40TT.js";
import { t as OpenAIQuicksilverGatewayBridge } from "./realtime-quicksilver-gateway-bridge-Bd5yyb50.js";
...
var OpenAIRealtimeVoiceBridge = class OpenAIRealtimeVoiceBridge {
		...
		const openWebSocket = (resolvedConnection) => {
			...
			ws.on("message", (data) => {
				...
				try {
					...
					if (event.type === "error" && !attempt.ready) {
						rejectStartup(isDirectOpenAIRealtimeWebSocketUrl(url) && isOpenAIRealtimeStartupAuthFailure(event.error) ? /* @__PURE__ */ new Error(OPENAI_REALTIME_CONFIGURED_API_KEY_REJECTED) : new Error(readRealtimeErrorDetail(event.error)));
						return;
					}
			...
			ws.on("error", (error) => {
				...
				if (!attempt.ready) {
					...
					rejectStartup(isDirectOpenAIRealtimeWebSocketUrl(url) && isOpenAIRealtimeStartupAuthFailure(startupError) ? /* @__PURE__ */ new Error(OPENAI_REALTIME_CONFIGURED_API_KEY_REJECTED) : startupError);
					return;
				}
	...
	async attemptReconnect(reason, connection) {
		const retry = this.lifecycle.retry(connection, OpenAIRealtimeVoiceBridge.MAX_RECONNECT_ATTEMPTS);
		if (!retry) return;
		if (retry === "exhausted") {
			this.config.onEvent?.({
				direction: "client",
	...
	handleEvent(event, connection) {
		...
		switch (event.type) {
			...
			case "response.done":
				if (this.handleCompletedResponse(event, connection)) return;
				this.responseActive = false;
				this.responseCreateInFlight = false;
				this.manualResponseCreateEventId = null;
				this.responseCancelInFlight = false;
	...
	handleBargeIn(options) {
		...
		const shouldInterruptProvider = assistantItemId !== null && (responseStartTimestamp !== null && (this.oldestOutstandingMarkSequence !== null || options?.audioPlaybackActive === true) || force);
	...
	handleCompletedResponse(event, connection) {
		if (event.type !== "response.done" || event.response?.status !== "completed" || !Array.isArray(event.response.output) || !this.config.onToolCall) return false;
		for (const output of event.response.output) {
			if (!this.lifecycle.acceptsEvents(connection) || this.ws?.readyState !== WebSocket$1.OPEN) return true;
			...
			if (this.completedToolCallIds.size >= OpenAIRealtimeVoiceBridge.MAX_COMPLETED_TOOL_CALL_IDS) {
				const ws = this.ws;
				if (ws) this.failConnection(/* @__PURE__ */ new Error(`OpenAI realtime tool-call session limit exceeded (${OpenAIRealtimeVoiceBridge.MAX_COMPLETED_TOOL_CALL_IDS})`), ws, connection, {
					code: 1008,
...
const quicksilverBrokerBySession = /* @__PURE__ */ new WeakMap();
async function createOpenAIRealtimeBrowserSession(req, quicksilverBroker) {
	...
	if (isOpenAIGptLiveModel(model)) {
		...
		if (subscriptionAuth) {
			const session = await quicksilverBroker.createBrowserSession(quicksilverRequest, subscriptionAuth);
...
async function cancelOpenAIRealtimeBrowserSession(_req, session) {
...
function buildOpenAIRealtimeVoiceProvider(options) {
	const provider = {
		...
		createBrowserSession: (req) => createOpenAIRealtimeBrowserSession(req, options?.quicksilverBrowserSessionBroker)
	};
