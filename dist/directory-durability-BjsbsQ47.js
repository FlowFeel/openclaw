...
import { f as pinDirectory, p as syncDirectory, r as getFsSafeTestHooks } from "./pinned-write-CO4XA8tE.js";
import "./fs-safe-DVaClkIX.js";
import { n as requireNativeBinding, t as getNativeBinding } from "./native-DEmD_dhE.js";
import { d as syncNativeFileBestEffort } from "./sidecar-lock-DvH3ZuQw.js";
...
function readFlags() {
	return fs.constants.O_RDONLY | (process.platform !== "win32" && typeof fs.constants.O_NOFOLLOW === "number" ? fs.constants.O_NOFOLLOW : 0) | (process.platform !== "win32" && typeof fs.constants.O_NONBLOCK === "number" ? fs.constants.O_NONBLOCK : 0);
}
...
async function hashPath(filePath) {
	...
	let handle;
	try {
		handle = await fs$1.open(filePath, readFlags());
	} catch (error) {
	...
	try {
		...
		if (current.isSymbolicLink() || !current.isFile() || !sameFileIdentity(opened, current)) throw new FsSafeError("path-mismatch", "SHA-256 path changed while opening");
...
async function openNativeParent(binding, filePath) {
	...
	try {
		const pathname = await fs$1.lstat(parentPath);
...
async function assertPinnedSourceCurrent(params) {
	...
	const current = await fs$1.lstat(params.sourcePath);
...
async function copyPinnedSource(params) {
	if (params.native && params.targetNativeParent) for (const method of ["clone", "copy-file-range"]) {
		...
		try {
			...
			const identity = await fs$1.lstat(params.targetPath);
...
async function publishFileExclusive(params) {
	...
	try {
		...
		if (!sameFileIdentity(sourcePathStat, sourceIdentity) || params.expectedSourceIdentity && !sameFileIdentity(params.expectedSourceIdentity, sourceIdentity)) throw new FsSafeError("path-mismatch", "publication source identity did not match");
		...
		if (params.strategy === "rename-noreplace") {
			...
			const targetIdentity = await fs$1.lstat(targetPath);
			if (targetIdentity.isSymbolicLink() || !targetIdentity.isFile() || !sameFileIdentity(targetIdentity, sourceIdentity)) throw new FsSafeError("path-mismatch", "no-replace publication target changed");
			try {
		...
		try {
			...
			const targetIdentity = await fs$1.lstat(targetPath);
			if (targetIdentity.isSymbolicLink() || !targetIdentity.isFile() || !sameFileIdentity(targetIdentity, sourceIdentity)) throw new FsSafeError("path-mismatch", "hardlink publication target changed");
			await assertPinnedSourceCurrent({
				sourcePath,
				handle: source,
				identity: sourceIdentity
		...
		try {
			...
			if (targetPathStat.isSymbolicLink() || !sameFileIdentity(targetPathStat, targetIdentity) || copiedBack.bytes !== copied.bytes || copiedBack.digest !== copied.digest || sourceAfter.bytes !== copied.bytes || sourceAfter.digest !== copied.digest) throw new FsSafeError("path-mismatch", "exclusive publication copy failed content fencing");
			await assertPinnedSourceCurrent({
				sourcePath,
				handle: source,
				identity: sourceIdentity
		...
		throw publicationFailure(error, failure, failure.preserveTarget || preserveSyncFailure ? "preserved" : await removeCreatedTargetIfUnchanged(targetPath, failure.targetIdentity));
	} finally {
