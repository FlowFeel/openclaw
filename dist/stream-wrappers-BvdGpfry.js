...
import { d as resolveClaudeSonnet5ModelIdentity, f as supportsClaude1MContext, m as supportsClaudeFastMode, u as resolveClaudeOpus5ModelIdentity } from "./src-DwO28Ae0.js";
...
import { i as createAnthropicThinkingPrefillPayloadWrapper, r as composeProviderStreamWrappers, t as applyAnthropicPayloadPolicyToParams, y as resolveAnthropicPayloadPolicy } from "./provider-stream-shared-DSBkVQvj.js";
...
function createAnthropicFastModeWrapper(baseStreamFn, enabled) {
	...
	return (model, context, options) => {
		...
		if (supportsClaudeFastMode(model)) {
			...
			return streamWithPayloadPatch(underlying, applyAnthropicFastModePricing(model), context, {
				...options,
...
function createAnthropicServiceTierWrapper(baseStreamFn, serviceTier) {
	const underlying = baseStreamFn ?? streamSimple;
	return (model, context, options) => {
		if (isAnthropicOAuthApiKey(options?.apiKey) || resolveClaudeOpus5ModelIdentity(model) !== void 0 || resolveClaudeSonnet5ModelIdentity(model) !== void 0) return underlying(model, context, options);
...
function wrapAnthropicProviderStream(ctx) {
	...
	return composeProviderStreamWrappers(ctx.streamFn, needsAnthropicBetaWrapper ? (streamFn) => createAnthropicBetaHeadersWrapper(streamFn, anthropicBetas ?? []) : void 0, serviceTier ? (streamFn) => createAnthropicServiceTierWrapper(streamFn, serviceTier) : void 0, hasFastModeParam && serviceTier === void 0 ? (streamFn) => createAnthropicFastModeWrapper(streamFn, () => resolveAnthropicFastMode(ctx.extraParams)) : void 0, (streamFn) => createAnthropicThinkingPrefillWrapper(streamFn));
}
//#endregion
