...
import { t as addGatewayServiceCommands } from "./register-service-commands-BMc7x7KD.js";
import { r as callGatewayFromCliWithTransport } from "./gateway-rpc-DDZpjK7K.js";
...
import { t as addGatewayRunCommand } from "./run-command-DNIx7eoz.js";
...
function addGatewayRestartHandoffCommands(gateway) {
	...
	restartHandoff.command("consume").description("Atomically consume a gateway restart handoff").helpOption(false).allowUnknownOption().allowExcessArguments().option("--expected-pid [pid]", "PID of the exited gateway process", collectExpectedPid, []).option("--json", "Output JSON", false).action(async (opts, command) => {
		...
		try {
			const { consumeGatewayRestartHandoffSync } = await import("./restart-handoff-CVz4afyS.js");
...
const gatewayStatusModuleLoader = createLazyImportLoader(() => import("./gateway-status-BUX--TBo.js"));
const gatewayHealthModuleLoader = createLazyImportLoader(() => import("./health-Bk21SCBX.js"));
const bonjourDiscoveryModuleLoader = createLazyImportLoader(() => import("./bonjour-discovery-CotGhtdb.js"));
...
const usageFormatModuleLoader = createLazyImportLoader(() => import("./usage-format-BnFm8-SP.js"));
const stabilityBundleModuleLoader = createLazyImportLoader(() => import("./diagnostic-stability-bundle-BTA5OjHT.js"));
const supportExportModuleLoader = createLazyImportLoader(() => import("./diagnostic-support-export-Ci_8XQ3g.js"));
const daemonStatusGatherModuleLoader = createLazyImportLoader(() => import("./status.gather-Bg0rca7r.js"));
...
async function runGatewayCommand(action, label, opts) {
	try {
		...
		if (opts?.json) {
			const { formatGatewayAuthErrorJson, formatGatewayClientRequestErrorJson, formatGatewayTransportErrorJson } = await import("./call-DctZOBCQ.js");
...
function registerGatewayCli(program, deps = {}) {
	...
	gateway.command("auth-token").description("Reveal the configured shared Gateway token").option("--show", "Print the token to an interactive terminal", false).action(async (opts) => {
		await runGatewayCommand(async () => {
			...
			const { gatewayAuthTokenCommand } = await import("./gateway-auth-token-mQKkQSeA.js");
