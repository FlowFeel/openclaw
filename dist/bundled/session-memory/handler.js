...
import { p as selectVisibleTranscriptEvents } from "../../session-transcript-index-cy-aJty7.js";
...
import { f as readSessionTranscriptBoundedMessageTailPage } from "../../session-accessor-t3qUoTeV.js";
import { r as sanitizeModelSpecialTokens } from "../../external-content-NkkZExk2.js";
...
import { r as isSessionAutoResetReason } from "../../session-auto-reset-Bs72jkPR.js";
import { t as generateSlugViaLLM } from "../../llm-slug-generator-BRscoLGX.js";
...
function renderSessionMemoryLines(events) {
	const allMessages = [];
...
function getRecentSessionContentFromEvents(events, messageCount = 15) {
	...
	return renderSessionMemoryLines(events).slice(-limit).join("\n") || null;
}
//#endregion
...
async function getRecentSqliteSessionContent(scope, messageCount, capturedEvents) {
	try {
		const events = capturedEvents ?? await loadSqliteTranscriptEvents({ ...scope });
		...
		return getRecentSessionContentFromEvents(selectVisibleTranscriptEvents(latestResetIndex >= 0 ? events.slice(0, latestResetIndex) : events), messageCount);
	} catch {
		return null;
	}
}
...
async function saveSessionMemoryNow(event, capturedEvents) {
	try {
		...
		const agentId = typeof context.agentId === "string" && context.agentId.trim() ? context.agentId.trim() : resolveAgentIdFromSessionKey(event.sessionKey, resolveDefaultAgentId(cfg ?? {}));
...
const saveSessionToMemory = (event) => {
	...
	try {
		...
		if (sessionId) {
			const cfg = context.cfg;
			const agentId = typeof context.agentId === "string" && context.agentId.trim() ? context.agentId.trim() : resolveAgentIdFromSessionKey(event.sessionKey, resolveDefaultAgentId(cfg ?? {}));
			const storePath = typeof context.storePath === "string" && context.storePath.trim() ? context.storePath.trim() : resolveStorePath(cfg?.session?.store, { agentId });
			const hookConfig = resolveHookConfig(cfg, "session-memory");
			const messageCount = typeof hookConfig?.messages === "number" && hookConfig.messages > 0 ? hookConfig.messages : 15;
	...
	const writePromise = isAutoReset ? saveSessionMemoryNow(event, capturedEvents) : runWithGatewayIndependentRootWorkContinuation(() => saveSessionMemoryNow(event, capturedEvents));
