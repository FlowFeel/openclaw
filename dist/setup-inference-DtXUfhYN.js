...
import { i as createSystemAgentModelSelectionUpdater, r as createQuickstartNotePrompter, t as applySystemAgentModelSelection } from "./setup-apply-DTlZjgUO.js";
...
async function runSetupInferenceTest(params) {
	...
	try {
		...
		let result;
		if (plan.runner === "cli") result = await (deps.runCliAgent ?? (await import("./cli-runner-5ZVub1v0.js")).runCliAgent)({
			sessionId,
			sessionKey,
			sessionManager,
		...
		else result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-Bv58Yqvq.js")).runEmbeddedAgent)({
			sessionId,
			sessionKey,
			sessionManager,
			agentId: effectiveAgentId,
