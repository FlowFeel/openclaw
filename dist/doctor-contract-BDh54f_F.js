import { a as defineChannelAliasMigration, d as hasLegacyAccountStreamingAliases, l as stripRetiredChannelKeys, n as defineKeyMoveMigration, s as normalizeChannelAccounts, u as asObjectRecord } from "./runtime-doctor-9vOSwNaE.js";
import { s as isSupportedRealtimeVoiceActivationName, u as normalizeRealtimeVoiceActivationNamePrefix } from "./realtime-voice-D6uheH7o.js";
...
const legacyConfigRules = [
	...
	},
	{
		...
		match: (value) => hasLegacyAccountStreamingAliases(value, (accountValue) => {
	...
	},
	{
		...
		match: (value) => hasLegacyAccountStreamingAliases(value, hasUnsupportedDiscordRealtimeWakeNames)
	},
...
function normalizeCompatibilityConfig({ cfg }) {
	...
	const rawEntry = asObjectRecord(tuningKnobs.config.channels?.discord);
	if (!rawEntry) return {
		config: cfg,
		changes: []
	};
	...
	if (tuningKnobs.changed) changes.push("Removed retired Discord tuning knobs.");
	...
	const accounts = normalizeChannelAccounts({
		...
		normalizeAccount: ({ account, accountId, pathPrefix, changes: accountChanges }) => {
			...
			const wakeNames = normalizeUnsupportedRealtimeWakeNames(agentIds.entry, pathPrefix, accountChanges);
			return {
