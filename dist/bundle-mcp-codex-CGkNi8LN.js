...
import { i as partitionMcpServersByConnectionScope } from "./mcp-connection-resolver-WHjckvMq.js";
import { r as resolveMcpBearerBundleConfig, t as requiresMcpBearerProjection } from "./mcp-auth-profile-BllBNrDt.js";
import { i as normalizeCodexMcpServerConfig, n as buildCodexMcpServersConfig, t as applyCodexSessionMcpToolDenials } from "./codex-mcp-config-BJcBYgV0.js";
...
function buildCodexUserMcpServersThreadConfigPatch(cfg, options) {
	...
	for (const [name, server] of entries) {
		...
		if (serverOverride === false || serverOverride !== true && server.enabled === false) continue;
		if (!isCodexMcpServerAllowedForAgent(server, options)) continue;
		projected.push([name, normalizeCodexMcpServerConfig(name, applyCodexSessionMcpToolDenials(name, server, options?.toolOverrides))]);
	}
	const mcp_servers = Object.fromEntries(projected);
...
async function buildCodexUserMcpServersThreadConfigPatchForRuntime(cfg, options) {
	...
	const entries = Object.entries(staticServers);
	if (entries.length === 0) return;
	let allowedServers = Object.fromEntries(entries.filter(([name, server]) => {
		const serverOverride = readSessionMcpServerOverride(options, name);
		return serverOverride !== false && (serverOverride === true || server.enabled !== false) && isCodexMcpServerAllowedForAgent(server, options);
	}));
	...
	const mcp_servers = Object.fromEntries(Object.entries(resolvedConfig.config.mcpServers).map(([name, server]) => [name, normalizeCodexMcpServerConfig(name, applyCodexSessionMcpToolDenials(name, server, options?.toolOverrides))]));
