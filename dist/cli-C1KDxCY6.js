...
import { i as resolveMatrixAccount } from "./accounts-oeV3IBG_.js";
import { n as updateMatrixAccountConfig, t as resolveMatrixConfigPath } from "./config-update-DXmd3dgG.js";
import { n as matrixSetupAdapter } from "./setup-core-BxGPB4bE.js";
import { t as withResolvedActionClient } from "./client-BG2sMPZz.js";
import { r as resolveMatrixAuthContext } from "./config-DFyPAhPJ.js";
...
function printTimestamp(label, value) {
	...
	const formatted = Number.isFinite(parsed.getTime()) ? formatZonedTimestamp(parsed, { displaySeconds: true }) ?? value : value;
...
//#endregion
//#region extensions/matrix/src/cli-account.ts
async function addMatrixAccount(params) {
	...
	if (accountConfig.encryption === true) {
		const { maybeBootstrapNewEncryptedMatrixAccount } = await import("./setup-bootstrap-B7sVnof1.js");
...
function registerMatrixAccountCommands(root) {
...
//#endregion
//#region extensions/matrix/src/cli-devices.ts
...
function registerMatrixDeviceCommands(root) {
...
//#endregion
//#region extensions/matrix/src/cli-direct.ts
const loadMatrixActionClientModule = createLazyRuntimeModule(() => import("./client-BJL5yQUZ.js"));
const loadMatrixDirectManagementModule = createLazyRuntimeModule(() => import("./direct-management-D9ttw7Hn.js"));
...
function registerMatrixDirectCommands(root) {
	...
	direct.command("inspect").description("Inspect direct-room mappings for a Matrix user").requiredOption("--user-id <id>", "Peer Matrix user ID").option("--account <id>", "Account ID (for multi-account setups)").option("--verbose", "Show detailed diagnostics").option("--json", "Output as JSON").action(async (options) => {
		const accountId = resolveMatrixCliAccountContext(options.account).accountId;
		await runMatrixCliCommand({
			verbose: options.verbose === true,
			json: options.json === true,
...
//#endregion
//#region extensions/matrix/src/cli-encryption.ts
...
function registerMatrixEncryptionCommands(root) {
...
//#endregion
//#region extensions/matrix/src/cli-profile.ts
function registerMatrixProfileCommands(root) {
	root.command("profile").description("Manage Matrix bot profile").command("set").description("Update Matrix profile display name and/or avatar").option("--account <id>", "Account ID (for multi-account setups)").option("--name <name>", "Profile display name").option("--avatar-url <url>", "Profile avatar URL (mxc:// or http(s) URL)").option("--verbose", "Show detailed diagnostics").option("--json", "Output as JSON").action(async (options) => {
		await runMatrixCliCommand({
			verbose: options.verbose === true,
			json: options.json === true,
			run: async () => await applyMatrixProfileUpdate({
				account: options.account,
...
//#endregion
//#region extensions/matrix/src/cli-verification-backup.ts
function registerMatrixVerificationBackupCommands(verify) {
...
//#endregion
//#region extensions/matrix/src/cli-verification.ts
...
function registerMatrixVerificationCommands(root) {
