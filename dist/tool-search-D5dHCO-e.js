...
import { a as isPreExecutionBlockedToolResult, d as runWithToolExecutionValidation, l as rewrapToolWithBeforeToolCallHook, r as getBeforeToolCallFailureDisposition, u as wrapToolWithBeforeToolCallHook } from "./agent-tools.before-tool-call-Cp_0kD4x.js";
import { N as SESSION_TOOL_STDERR_TAIL_BYTES, P as appendBoundedTextTail } from "./sessions-Digi7ATr.js";
import { n as ToolInputError, r as asToolParamsRecord } from "./common-RkLs-2lL.js";
import { a as wrapExternalContent, i as truncateSanitizedExternalContent } from "./external-content-NkkZExk2.js";
import { i as protectNetworkToolExecutionError, l as resolveToolResultFailureKind, r as isTrustedToolExecutionPreflightError } from "./tool-result-error-D4i-Z2FR.js";
...
import { b as isToolWrappedWithBeforeToolCallHook, m as getChannelAgentToolMeta } from "./gateway-CRcKH8Wu.js";
import { t as isCoreCodingSurfaceToolName } from "./core-tool-factory-descriptors-BnrevR-u.js";
...
function readToolSearchCallArgs(args, catalog) {
	...
	return {
		id,
		input: Object.fromEntries(Object.entries(params).filter(([key]) => !wrapperKeys.has(key)))
	};
}
...
async function validateCatalogSchemaValue(entry, schemaName, value) {
	...
	try {
		schemaValidatorModulePromise ??= import("./schema-validator-B-qv2NmI.js");
...
function createToolSearchTools(ctx) {
	...
	return [
		...
		},
		{
			...
			label: "Tool Search",
			description: "Search the effective Tool Search catalog. Query in English: matching is lexical against tool names and descriptions, which are written in English, so a query in another language will usually match nothing. Pass an exact result id or name to tool_call; use tool_describe only when you need its input schema.",
			parameters: Type.Object({
				query: Type.String({ description: "Search query, in English. Describe the capability you need." }),
