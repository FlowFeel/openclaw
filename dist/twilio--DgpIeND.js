...
import { i as chunkAudio, n as mapProviderStatusToEndReason, r as normalizeProviderStatus, t as isProviderStatusTerminal } from "./call-status-BQOZpYgy.js";
import { n as getHeader } from "./proxy-ip-BGDYS7mz.js";
import { c as verifyTwilioWebhook, i as readVoiceCallProviderJsonResponse, n as cancelProviderResponseBody, r as readProviderErrorResponseSnippet, t as guardedJsonApiRequest } from "./guarded-json-api-DYJvbLcq.js";
...
async function twilioApiRequest(params) {
	...
	try {
		...
		return await readVoiceCallProviderJsonResponse(response, "Twilio API returned malformed JSON.");
	} finally {
		await release();
	}
}
...
var TwilioProvider = class TwilioProvider {
	...
	*/
	releaseCallState(providerCallId, callId) {
		const webhookUrl = this.callWebhookUrls.get(providerCallId);
		let resolvedCallId = callId;
		if (!resolvedCallId && webhookUrl) try {
			resolvedCallId = new URL(webhookUrl).searchParams.get("callId") || void 0;
		} catch {}
		if (resolvedCallId) this.deleteStoredTwiml(resolvedCallId);
