...
function resolveSystemAgentTargetAgentId(config, requestedAgentId) {
	const configuredAgentId = normalizeOptionalString(requestedAgentId) ?? normalizeOptionalString(config.agents?.defaults?.systemAgent?.agentId);
...
function projectSystemAgentExecutionConfig(config, routeAgentId) {
	...
	if (agents.length === 0) return config;
	...
	const projectedAgents = [...retainedAgents, ...hasProjectedSettings ? [{
...
async function resolveSystemAgentConfiguredRouteFromConfig(runConfig, requestedAgentId, deps = {}) {
	const [agentScope, modelSelection, modelRuntimeAliases, simpleCompletion, harnessPolicy] = await Promise.all([
		...
		import("./model-selection-Cmjka01x.js"),
		import("./model-runtime-aliases-DDe-vRfc.js"),
		import("./simple-completion-runtime-BnmXQj8E.js"),
		import("./policy-BMvwECKC.js")
	]);
...
async function projectInferenceRoute(config, requestedAgentId, deps = {}) {
	const { resolveProviderIdForAuth } = await import("./provider-auth-aliases-CtWRNFWC.js");
	...
	const executionAgent = listAgentEntries(route?.runConfig ?? {}).find((entry) => normalizeAgentId(entry.id) === SYSTEM_AGENT_EXECUTION_AGENT_ID);
...
//#endregion
export { sameDefaultInferenceRoute as a, resolveSystemAgentTargetAgentId as i, projectInferenceRoute as n, resolveSystemAgentConfiguredRouteFromConfig as r, projectDefaultInferenceRoute as t };
