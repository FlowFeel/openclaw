...
import { A as readRealtimeVoiceConsultQuestion, j as readSpeakableRealtimeVoiceToolResult, m as REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME, t as createRealtimeVoiceSessionHarness, y as buildRealtimeVoiceAgentConsultWorkingResponse } from "./realtime-session-harness-bu55PsqP.js";
import "./realtime-voice-DUe3NMvT.js";
import { n as createSpeechThresholdGate, t as calculateMulawRms } from "./audio-energy-o8vC-5VK.js";
...
var RealtimeAudioPacer = class {
	...
	pump() {
		...
		if (this.pendingQueueSize > 0) this.timer = setTimeout(() => this.pump(), delayMs);
	}
};
//#endregion
...
<Response>
  <Connect>
	...
	handleWebSocketUpgrade(request, socket, head) {
		...
		new WebSocketServer({
			...
			ws.on("close", (code) => {
				...
				const reason = this.serverClosingSockets.has(ws) || stopReceived || code === 1e3 || code === 1005 ? "completed" : "error";
	...
	handleCall(streamSid, callSid, ws, callerMeta, adapter) {
		...
		let providerHandlesInputAudioBargeIn = this.realtimeProvider.capabilities?.handlesInputAudioBargeIn === true;
		...
		const emitCallEnd = (reason) => {
			...
			Promise.resolve(this.provider.hangupCall({
				callId,
				providerCallId: callSid,
				reason
			})).catch((error) => {
		...
		const telephonyCloser = {
			owner: session,
			close: closeTelephony
		};
