...
import { g as isDefaultAgentRuntimeId, v as normalizeOptionalAgentRuntimeId } from "./openai-routing-Db2edxk0.js";
import { w as resolveDefaultModelForAgent } from "./codex-route-model-ref-BDwklqCa.js";
import "./config-BBVHtcXg.js";
import { M as hasNonzeroUsage, P as normalizeUsage, p as withSqliteTranscriptWriteTransaction, st as loadSqliteSessionEntry } from "./session-accessor.sqlite-CtCo5VZ6.js";
import { _t as interruptSessionWorkAdmissions, lt as SESSION_WORK_ADMISSION_DRAIN_TIMEOUT_MS, wt as runExclusiveSessionStoreWrite, xt as runExclusiveSessionLifecycleMutation } from "./session-entry-slot-keys-DPRQmSpa.js";
import "./session-accessor-D5Or7WgI.js";
import { n as WORKER_LAUNCH_V2_PROTOCOL_FEATURE } from "./worker-admission-cYaTLkLk.js";
import { t as WORKER_INFERENCE_MAX_CONTEXT_MESSAGES } from "./worker-inference-jzapa-pT.js";
import { N as convertToLlm } from "./agent-core-3hvRadZX.js";
...
import { n as projectConversationToolNames } from "./conversation-tool-policy-pipeline-Bo3TW3-_.js";
import { t as clearSessionQueues } from "./cleanup-CYBjc1T_.js";
import { n as mapThinkingLevelForProvider } from "./utils-CefVZRZM.js";
import { t as applyEmbeddedAttemptToolsAllow } from "./attempt-tool-construction-plan-D4Oh4XYT.js";
import { S as mergeUsageIntoAccumulator, a as buildUsageAgentMetaFields, b as createUsageAccumulator, v as resolveReportedModelRef } from "./helpers-BN65NIti.js";
import { n as installSessionPlacementResetGuard, t as installSessionPlacementAdmissionProvider } from "./session-placement-admission-BEJd445g.js";
...
import { n as recoverWorkerWorkspaceReconciliation } from "./workspace-reconcile-S25e948H.js";
import { a as deleteWorkerWorkspaceResultCleanupRefs, c as moveStagedWorkerWorkspaceResultToCleanup, d as workerWorkspaceResultRef, i as deleteStagedWorkerWorkspaceResult, l as preparedWorkerWorkspaceResultRef, n as applyStagedWorkerWorkspaceResult, o as hasWorkerWorkspaceResultRef, r as cleanupWorkerWorkspaceResultRef, s as isWorkerWorkspaceResultCleanupRef, t as WorkerTunnelOwnerDisconnectedError, u as restoreStagedWorkerWorkspaceResultFromCleanup } from "./tunnel-contract-LdhQDk2X.js";
import { n as workerEnvironmentIdForIdempotencyKey } from "./service-BOtEdzHa.js";
import { t as WORKER_LOCAL_TOOL_NAMES } from "./tool-authority-CJ-HJhS-.js";
import { a as toWorkerTranscriptMessage } from "./transcript-message-oguPQDoF.js";
import { n as parseWorkerLaunchDescriptor } from "./launch-descriptor--JhiDKGo.js";
import { t as ActiveTurnClaimError } from "./placement-turn-claims-BLyXMWZZ.js";
...
function supportsWorkerLaunchV2(environment) {
	return environment?.bootstrapReceipt?.protocolFeatures.includes(WORKER_LAUNCH_V2_PROTOCOL_FEATURE) === true;
}
function sameActiveEnvironment(placement, environment) {
	return Boolean(environment && environment.state === "attached" && placement.environmentId && environment.environmentId === placement.environmentId && placement.activeOwnerEpoch !== null && environment.ownerEpoch === placement.activeOwnerEpoch && placement.workerBundleHash && environment.bootstrapReceipt?.bundleHash === placement.workerBundleHash && supportsWorkerLaunchV2(environment) && environment.attachedSessionIds.length === 1 && environment.attachedSessionIds[0] === placement.sessionId);
}
...
function createPlacementRecoveryActions(deps) {
	...
	const resumeStarting = async (placement) => {
		...
		if (!(environment && expectedBundle && environment.bootstrapReceipt?.bundleHash === expectedBundle && supportsWorkerLaunchV2(environment) && hasSyncedWorkspace)) {
...
async function forceAbandonWorkerEnvironment(params) {
	...
	for (const pending of placements.listPendingWorkspaceResults()) if (pending.environmentId === environmentId) {
		const placement = placements.get(pending.sessionId);
		if ((placement?.state === "active" || placement?.state === "draining") && placement.environmentId === pending.environmentId && placement.activeOwnerEpoch === pending.ownerEpoch && placement.generation === pending.placementGeneration) {
...
function createWorkerPlacementDispatchService(options) {
	...
	const reclaim = async (request) => {
		...
		const operation = reclaimOnce(request).catch((error) => {
...
function resolveWorkerToolAuthority(params) {
	...
	const runtimeCappedTools = applyEmbeddedAttemptToolsAllow(WORKER_LOCAL_TOOL_NAMES.map((name) => ({ name })), turn.toolsAllow);
				...
				const finalized = await finalizeWorkspaceResultConflicts({
					...
					report: async (report) => {
						if ("cleared" in report) {
							SessionManager.open(transcriptTarget).appendCustomMessageEntry(WORKSPACE_CONFLICT_CLEARED_TRANSCRIPT_TYPE, "A later cloud workspace result superseded the previous conflict.", false);
							return;
						}
						workspaceConflict = {
							...report,
						...
						SessionManager.open(transcriptTarget).appendCustomMessageEntry(WORKSPACE_CONFLICT_TRANSCRIPT_TYPE, workspaceConflict.summary, true, {
...
function createWorkerWorkspaceConflictTranscriptHandlers(loadSessionRuntime) {
	return {
		resolveWorkspaceResultConflict: async (identity) => {
			...
			return await withSqliteTranscriptWriteTransaction({
		...
		reportWorkspaceResultConflict: async (conflict) => {
			...
			await withSqliteTranscriptWriteTransaction({
				agentId: target.agentId,
...
const loadWorkerPlacementSessionRuntimeModule = createLazyRuntimeModule(async () => {
	const [placementSessionRuntime, { managedWorktrees }, sessionUtils] = await Promise.all([
		import("./placement-session-runtime-C0o1Isco.js"),
		import("./service-109IUYPJ.js"),
...
function createGatewayWorkerPlacementRuntime(params) {
	...
	const resolveWorkspacePath = async ({ sessionId, sessionKey, agentId }) => {
		const { managedWorktrees, resolveCanonicalSessionEntryFromStoreKeys, resolveGatewaySessionStoreTargetWithStore } = await loadWorkerPlacementSessionRuntimeModule();
		const target = resolveGatewaySessionStoreTargetWithStore({
			cfg: getRuntimeConfig(),
			key: sessionKey,
			agentId,
	...
	const dispatchService = coordinateWorkerPlacementDispatch(createWorkerPlacementDispatchService({
		...
		runLocalBarrier: async ({ sessionId, sessionKey, agentId, startDispatch }) => {
			const { isWorkerPlacementSessionRuntimeSupported, managedWorktrees, resolveCanonicalSessionEntryFromStoreKeys, resolveGatewaySessionStoreTargetWithStore, resolveWorkerPlacementSessionRuntime } = await loadWorkerPlacementSessionRuntimeModule();
			const target = resolveGatewaySessionStoreTargetWithStore({
				cfg: getRuntimeConfig(),
				key: sessionKey,
				agentId,
			...
			await runExclusiveSessionLifecycleMutation({
				...
				prepare: async () => {
					...
					const currentEntry = resolveCanonicalSessionEntryFromStoreKeys(currentTarget.store, currentTarget.storeKeys);
