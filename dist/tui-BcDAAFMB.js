...
import { r as stripAnsi$1 } from "./ansi-DbP9Z_r_.js";
...
import { s as registerUncaughtExceptionHandler } from "./unhandled-rejections-CyRemXOR.js";
...
import { a as buildAgentMainSessionKey, i as agentSessionKeysMatchByRequestKey, u as normalizeMainKey, y as toAgentStoreSessionKey } from "./session-key-DtTE9-Tg.js";
import { r as getRuntimeConfig } from "./io-BsQc3Kgy.js";
import { s as setConsoleSubsystemFilter } from "./console-DRDfhKk-.js";
...
import { l as resolveAgentIdByWorkspacePath } from "./agent-scope-DyEposw2.js";
import { f as resolveResponseUsageMode, i as isSessionDefaultDirectiveValue, l as normalizeUsageDisplay, s as normalizeThinkLevel } from "./thinking.shared-k6K-6JHM.js";
import { r as listThinkingLevelLabels, t as formatThinkingLevels } from "./thinking-CTxMHCzD.js";
import { n as ok, t as err } from "./result-BQGgYouL.js";
import "./model-ref-shared-BEKlpylb.js";
import "./config-BBVHtcXg.js";
import { r as runCommandWithTimeout } from "./exec-SGTX01mL.js";
import { i as resolveTrustedWindowsCmdExe, n as isWindowsBatchCommand, t as buildWindowsCmdExeCommandLine } from "./windows-command-DkNKhCLn.js";
import { i as formatRawAssistantErrorForUi } from "./assistant-error-format-Dw1scAnL.js";
import { m as isAuthErrorMessage } from "./sanitize-user-facing-text-Ba4C3tZ5.js";
import { i as classifyFailoverReason } from "./errors-Bujrccyt.js";
import { n as shouldForwardModelCommandToServer } from "./commands-registry.data-CxG6KLOu.js";
import { n as listChatCommands, r as listChatCommandsForConfig } from "./commands-registry-list-BItk55Bi.js";
import { i as resolveTextCommand } from "./commands-registry-normalize-BaFSffsi.js";
import { t as formatTokenCount } from "./token-format-D942KbWN.js";
import "./usage-format--pf0zzCz.js";
import "./embedded-agent-helpers-dlCGQLQ2.js";
import { i as resolveToolDisplay, t as formatToolDetail } from "./tool-display-BfWMni_i.js";
...
import { o as isChatStopCommandText } from "./chat-abort-B1M0lO2h.js";
import { n as isApprovalStaleError } from "./approval-errors-BNzerOwp.js";
import { n as formatTimeAgo, t as formatRelativeTimestamp } from "./format-relative-Dd106CpA.js";
import { t as normalizeGroupActivation } from "./group-activation-MKTJBUwi.js";
import { a as writeTuiLastSessionKey, i as resolveRememberedTuiSessionKey, r as readTuiLastSessionKey, t as buildTuiLastSessionScopeKey } from "./tui-last-session--_zXRWKF.js";
import { _ as resolveFinalAssistantText, a as extractContentFromMessage, b as sanitizeRenderableText, c as extractTuiAbortedText, d as formatTuiErrorMessage, f as formatTuiFooter, g as isolateRtlRenderedLine, h as isTuiAssistantAttachmentBlock, i as extractAssistantAttachmentText, l as formatContextUsageLine, m as isTerminalSafeAutocompleteValue, n as asString, o as extractTextFromMessage, p as isCommandMessage, r as composeThinkingAndContent, s as extractThinkingFromMessage, t as resolveLocalRunShutdownGraceMs, u as formatTuiAbortDiagnostic, v as sanitizeMarkdownSource, y as sanitizeRenderableLine } from "./local-run-shutdown-DwCidCNe.js";
...
var BtwInlineMessage = class extends Container {
	...
	setResult(params) {
		...
		if (params.isError) this.addChild(new Text(theme.error(text), 1, 0));
...
var ToolExecutionComponent = class extends Container {
	constructor(toolName, args) {
		...
		this.output = new ToolOutputComponent("", 0, 0, markdownTheme, { color: (line) => theme.toolOutput(line) });
		this.addChild(new Spacer(1));
...
function createCommandHandlers(context) {
	...
	const openSessionSelector = async () => {
		...
		try {
			...
			if (!isCurrentSessionSelection(selection)) return;
			const selector = createFilterableSelectList(result.sessions.map((session) => {
	...
	const commandHandlers = {
		...
		reset: async () => {
			...
			try {
				...
				state.sessionInfo.totalTokens = null;
				tui.requestRender();
				const result = await client.resetSession(resetSelection.sessionKey, "reset", resetSelection.sessionKey === "global" ? { agentId: resetSelection.agentId } : void 0);
...
function createTuiRunLifecycle(context) {
	...
	const flushPendingHistoryRefreshIfIdle = () => {
		if (state.activeChatRunId || hasPendingSubmit(state) || runCoordinator.isSessionMessagePersistencePending) return;
		if (!runCoordinator.pendingHistoryRefresh && !runCoordinator.hasPendingSessionMessageRefresh) return;
	...
	const applyFallbackStepModelUpdate = (event) => {
		...
		if (event.stream !== "lifecycle" || asString(data.phase, "") !== "fallback_step") return false;
...
var TuiSessionRunCoordinator = class {
	constructor(context) {
		...
		this.sessionMessagePersistenceRunIds = /* @__PURE__ */ new Set();
	...
	noteFinalizedRun(runId, options) {
		...
		if (options?.displayedFinal) {
			...
			if (this.sessionMessageRefreshPending && !this.persistedTerminalRunIds.has(runId)) this.sessionMessagePersistenceRunIds.add(runId);
		}
		...
		for (const retainedRunId of this.sessionMessagePersistenceRunIds) if (!this.finalizedRunsWithDisplay.has(retainedRunId) && retainedRunId !== runId) this.sessionMessagePersistenceRunIds.delete(retainedRunId);
	}
	...
	bindRegisteredPendingRun(runId) {
		if (this.sessionMessageRefreshPending && this.context.state.pendingSubmit?.runId === runId && !this.persistedTerminalRunIds.has(runId)) this.sessionMessagePersistenceRunIds.add(runId);
	}
	deferSessionMessageRefresh() {
		this.sessionMessageRefreshPending = true;
		const activeRunId = this.context.state.activeChatRunId;
		if (activeRunId && !this.persistedTerminalRunIds.has(activeRunId)) this.sessionMessagePersistenceRunIds.add(activeRunId);
		for (const runId of this.finalizedRunsWithDisplay.keys()) if (!this.persistedTerminalRunIds.has(runId)) this.sessionMessagePersistenceRunIds.add(runId);
		return Boolean(activeRunId || hasPendingSubmit(this.context.state) || this.sessionMessagePersistenceRunIds.size);
	}
	get hasPendingSessionMessageRefresh() {
		return this.sessionMessageRefreshPending;
	}
	get isSessionMessagePersistencePending() {
		return this.sessionMessagePersistenceRunIds.size > 0;
	}
	consumeSessionMessageRefresh() {
		this.sessionMessageRefreshPending = false;
...
function createEventHandlers(context) {
	...
	const handleSessionMessageEvent = (payload) => {
		...
		if (isReplayableTuiSessionMessage(evt)) reduceTuiSessionProjection(state, {
			type: "messagePersisted",
			message: evt.message,
			envelope: evt,
			scope: readTuiSessionProjectionScope(state)
	...
	const handleAgentEvent = (payload) => {
		...
		if (evt.stream === "lifecycle" && asString(evt.data?.phase, "") === "start" && !finalizedRuns.has(evt.runId) && !(isLocalBtwRunId?.(evt.runId) ?? false) && matchesSelectedTuiSession(state, evt)) {
...
function createSessionActions(context) {
	const { client, chatLog, btw, tui, opts, state, agentNames, initialSessionInput, initialSessionAgentId, resolveSessionKey, updateHeader, updateFooter, updateAutocompleteProvider, setActivityStatus, invalidateRunOwnership, clearLocalRunIds, rememberSessionKey } = context;
	...
	const setSession = async (rawKey) => {
		...
		const selectionChanged = !(normalizeAgentId(parseAgentSessionKey(nextKey)?.agentId ?? previousSelection.agentId) === previousSelection.agentId && agentSessionKeysMatchByRequestKey(nextKey, previousSelection.sessionKey));
		if (selectionChanged) {
...
function resolveLocalAuthCliInvocation(params) {
	const hasDistEntry = params?.hasDistEntry ?? (existsSync(DIST_ENTRY_JS_PATH) || existsSync(DIST_ENTRY_MJS_PATH));
...
async function runTui(opts) {
	...
	if (opts.backend) client = opts.backend;
		...
		const { EmbeddedTuiBackend } = await import("./embedded-backend-D3RLwvhB.js");
		client = new EmbeddedTuiBackend();
	} else {
		const { GatewayChatClient } = await import("./gateway-chat-BCsRJT4Q.js");
	...
	const updateFooter = () => {
		...
		const agentLabel = formatAgentLabel(state.currentAgentId);
		footer.setText(theme.dim(formatTuiFooter({
			agentLabel,
			sessionLabel,
	...
	const handleBackendDisconnected = (reason) => {
		if (exitRequested) return;
		connectionGeneration += 1;
