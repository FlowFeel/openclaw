...
import { a as requireRiskAcknowledgement } from "./setup.shared-WvGMBt-N.js";
...
async function runManualStage(params) {
	...
	const [{ ensureAuthProfileStore }, { detectAvailableSetupProviderIds }, { promptAuthChoiceGrouped }] = await Promise.all([
		...
		import("./provider-setup-availability-9vTOFTsX.js"),
...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-Fx3-ZaLD.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	const prompter = await (deps.createPrompter?.() ?? import("./clack-prompter-DyhRpF9c.js").then(({ createClackPrompter }) => createClackPrompter()));
	...
	const localOnboarding = custodianMode ? await import("./local-onboarding-state-BDRTwv4h.js") : void 0;
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-CnQeusDS.js")).runSetupMemoryImportStep)({
	...
	const { resolveSetupWorkspaceSelection } = await import("./setup.workspace-BP-u6tdW.js");
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-B7OOZFgA.js")).applySystemAgentSetup;
			...
			const appliedSnapshot = localSetup?.status === "pending" ? await (await import("./setup-recovery-ChhhqXEu.js")).completeLocalSetupRecovery({
				owner: localSetup,
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-CnQeusDS.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-Cwq8rwFb.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
		...
		if (recommendedConfig !== persistedConfig) {
			...
			const { mergeWizardConfigOntoLatest, writeWizardConfigFile } = await import("./setup.shared-B20Bo9O_.js");
	...
	if (opts.tui !== true) {
		if ((await (deps.runBrowserHandoff ?? (await import("./onboard-browser-handoff-DBfaGjpN.js")).runBrowserHatchHandoff)({
			config: persistedConfig,
			prompter,
...
async function launchHatchTui(workspace) {
	const [{ launchTuiCli }, { DEFAULT_BOOTSTRAP_FILENAME }, { restoreTerminalState }, fs, path] = await Promise.all([
		import("./tui-launch-CwXRFyQM.js"),
		import("./workspace-fBzfEn2v.js"),
