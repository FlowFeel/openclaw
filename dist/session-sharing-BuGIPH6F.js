...
import { c as isSessionMember } from "./sessions-BqBqRT1f.js";
import { A as resolveGatewaySessionStoreTargetWithStore, D as resolveCanonicalSessionStoreMatchFromStoreKeys } from "./session-utils-row-Br8x7LNG.js";
...
function resolveSessionGroupMutationTargets(params) {
	...
	if (!groupName) return;
	return resolveAllAgentSessionStoreTargetsSync(params.getCfg()).flatMap((storeTarget) => listSessionEntries({
...
function resolveSessionMutationTargets(params) {
	...
	const directKey = field ? readStringParam(params.requestParams, field) : void 0;
...
function resolveSessionMutationAuthorization(params) {
	...
	for (const targetRef of targetRefs) {
		...
		const error = target ? authorizeSessionSharingTarget({
			client: params.client,
			target
		}) : null;
		if (error) return { error };
		...
		authorization: (() => {
			...
			return {
				...
				assertTargetCurrent: (targetRef) => {
					const expected = authorizedTargets.find((target) => target.sessionKey === targetRef.sessionKey && target.agentId === targetRef.agentId);
