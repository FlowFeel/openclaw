import { c as tracePluginLifecyclePhaseAsync, s as tracePluginLifecyclePhase } from "./discovery-BwgYKZ4p.js";
...
async function runPluginUninstallCommandUnlocked(id, opts, runtime, skipPreview = false) {
	...
	const { loadInstalledPluginIndexInstallRecords, removePluginInstallRecordFromRecords, withoutPluginInstallRecords, withPluginInstallRecords } = await import("./installed-plugin-index-records-BJ1P8R7e.js");
	const { buildPluginSnapshotReport } = await import("./status-ZNqyvmhV.js");
	const { applyPluginUninstallDirectoryRemoval, formatUninstallActionLabels, formatUninstallSlotResetPreview, planPluginUninstall, pluginUninstallTargetExists, prepareConfigForPendingPluginDirectoryRemoval, resolveUninstallChannelConfigKeys, UNINSTALL_ACTION_LABELS } = await import("./uninstall-UvJDi1Gc.js");
	const { commitPluginInstallRecordsWithConfig } = await import("./install-record-commit-CqVY1ClZ.js");
	const { selectInstallMutationWriteOptions } = await import("./install-persistence-ypHLxIMb.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./registry-refresh-BJY8rroB.js");
	const { resolvePluginUninstallId } = await import("./plugins-uninstall-selection-CNJmpU6d.js");
	const { PromptInputClosedError, promptYesNo } = await import("./prompt-BW-syFKG.js");
	...
	if (!skipPreview) {
		...
		const { collectClawPluginUninstallWarnings } = await import("./uninstall-claw-references-A7Mi-qz2.js");
