...
import { g as withOpenClawStateStartupMigrationCheckpointDatabase } from "./openclaw-state-db-D9eH245j.js";
...
function acquireStartupMigrationLease(params = {}) {
	...
	writeStartupMigrationCheckpointDatabase(env, (db) => {
		...
		if (existing && isStartupMigrationLeaseOwnerDefinitelyGone(existingOwner)) executeSqliteQuerySync(db, stateDb.deleteFrom("state_leases").where("scope", "=", STARTUP_MIGRATION_LEASE_SCOPE).where("lease_key", "=", STARTUP_MIGRATION_LEASE_KEY).where("owner", "=", existing.owner));
			...
			throw new Error(`OpenClaw startup migrations are already running for this state directory; retry after the other OpenClaw process finishes or after ${new Date(existing.expiresAt ?? expiresAt).toISOString()}.${ownerHint}`);
		}
...
//#endregion
export { needsStateMigrationCheckpoint as a, recordSuccessfulStateMigrations as c, needsStartupMigrationCheckpoint as i, acquireStartupMigrationLease as n, readStartupMigrationVersion as o, hasActiveStartupMigrationLease as r, recordSuccessfulStartupMigrations as s, STARTUP_MIGRATION_LEASE_TTL_MS as t };
