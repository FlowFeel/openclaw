...
import { n as deliveryContextFromSession, s as normalizeDeliveryContext } from "./delivery-context.shared-B-QSuGw_.js";
import { R as applySqliteSessionEntryReplacements, at as loadExactSqliteSessionEntry, st as loadSqliteSessionEntry } from "./session-accessor.sqlite-B9iW7DOt.js";
import { t as isDeliverableMessageChannel } from "./message-channel-normalize-Bmutiks_.js";
import "./message-channel-1n7hD5_u.js";
import { St as cancelSessionWorkAdmissionHandoff, l as resolveRestartRecoveryChannelAuthority, n as buildRestartRecoveryClaimCleanupPatch, o as hasRestartRecoveryTerminalRun, ut as beginSessionWorkAdmission } from "./session-entry-slot-keys-DR5d2mKt.js";
import { K as updateSessionEntry, S as persistSessionTranscriptTurn, st as listSessionEntriesByStatus } from "./session-accessor-t3qUoTeV.js";
import "./backoff-CCtTkmwj.js";
import { t as buildSessionContext } from "./session-manager-codec-DRZh7P2-.js";
import { t as repairOrphanToolCalls } from "./repair-h5r9ZjXr.js";
...
import { n as AGENT_RUN_RESTART_ABORT_ERROR, r as AGENT_RUN_RESTART_ABORT_ERROR_CODE } from "./run-termination-7xcnysfe.js";
import { f as listActiveEmbeddedRunSessionIds, p as listActiveEmbeddedRunSessionKeys } from "./run-state-ewY1D1VR.js";
...
import { a as hasInterSessionUserProvenance, n as MAIN_SESSION_RESTART_RECOVERY_SOURCE_TOOL, s as isCompletionReportInputProvenance } from "./input-provenance-CYGbY89H.js";
import "./session-utils-C8yYh4dv.js";
import { o as sanitizePendingFinalDeliveryText } from "./pending-final-delivery-BzZojDaN.js";
import { n as buildMainSessionRecoveryClearPatch } from "./main-session-recovery-clear-BngYLTap.js";
import { c as hasCurrentProcessOwner, d as normalizeStringSet, f as resolveRestartRecoveryStorePaths, l as log$1, n as isMainSessionRecoveryPending, o as UNRESUMABLE_SESSION_NOTICE, s as buildRestartRecoveryExpectedState, t as isMainRestartRecoveryCandidate, u as normalizeFiniteTimestamp } from "./main-session-recovery-state-CUJxZLgx.js";
import { n as markStartupOrphanedMainSessionsForRecovery } from "./main-session-restart-recovery-marking-CAtwjO0p.js";
import { a as scheduleMainSessionRecoveryMutation, i as retryMainSessionRecoveryMutation, r as repairMainSessionRecoveryMutation } from "./main-session-recovery-lifecycle-CrLAHQ2O.js";
import { n as commitMainSessionRecovery } from "./main-session-recovery-store-Dmn-Y9cG.js";
import { d as isTrustedMessageActionTurnIngress } from "./code-mode-control-tools-B1t6Lt9X.js";
import { n as findRestartRecoveryUnsafeReplyHook } from "./restart-recovery-hook-safety-D4F8bLYO.js";
...
import { i as buildRunUserTurnIdempotencyKey } from "./user-turn-transcript-Cd8sokau.js";
import { r as isAnnounceRunId } from "./announce-idempotency-D7LnUTJR.js";
import { a as isIntermediateAssistantTranscriptMessage, c as isTerminalSilentAssistantMessage, l as readTerminalSourceReplyDeliveryMirror, n as getTranscriptMessageRole, o as isMeaningfulTranscriptMessage } from "./message-visibility-Cv_6nt4F.js";
import { n as isAgentToolReplaySafe } from "./tool-replay-safety-CRqM1och.js";
...
//#endregion
//#region src/infra/sqlite-kv-store.ts
/**
...
var SqliteKvStore = class {
	...
	get(key) {
		const { db } = openOpenClawStateDatabase(this.env ? { env: this.env } : {});
		const row = db.prepare(`SELECT value FROM ${this.tableName} WHERE key = ?`).get(key);
		if (!row) return;
		try {
	...
	listKeys(prefix = "") {
		const { db } = openOpenClawStateDatabase(this.env ? { env: this.env } : {});
		if (prefix) return db.prepare(`SELECT key FROM ${this.tableName} WHERE key LIKE ? ORDER BY key ASC`).all(`${prefix}%`).map((r) => r.key);
		return db.prepare(`SELECT key FROM ${this.tableName} ORDER BY key ASC`).all().map((r) => r.key);
	}
	entries(prefix = "") {
		const { db } = openOpenClawStateDatabase(this.env ? { env: this.env } : {});
		let rows;
		if (prefix) rows = db.prepare(`SELECT key, value, updated_at FROM ${this.tableName} WHERE key LIKE ? ORDER BY key ASC`).all(`${prefix}%`);
		else rows = db.prepare(`SELECT key, value, updated_at FROM ${this.tableName} ORDER BY key ASC`).all();
		const results = [];
	...
	getAll() {
		const result = {};
		for (const entry of this.entries()) result[entry.key] = entry.value;
		return result;
	}
	clear() {
		runOpenClawStateWriteTransaction(({ db }) => {
	...
	*/
	migrateLegacyJsonFile(jsonPath) {
		if (!fs.existsSync(jsonPath)) return false;
		try {
			const data = tryReadJsonSync(jsonPath);
			if (data && typeof data === "object") if (Array.isArray(data)) data.forEach((item, index) => {
				const itemKey = item?.id || item?.key || String(index);
				this.set(String(itemKey), item);
			});
			else for (const [key, val] of Object.entries(data)) this.set(key, val);
...
//#endregion
//#region src/agents/working-state/working-state-types.ts
const MAX_WORKING_STATE_PLAN_CHARS = 4e3;
//#endregion
//#region src/agents/working-state/working-state-synthesizer.ts
/**
...
*/
function normalizeWorkingStateInput(input, nowMs = Date.now()) {
	const activeFiles = (input.activeFiles ?? []).slice(-50).filter((file) => typeof file === "string" && file.trim().length > 0);
	const recentSearchQueries = (input.recentSearchQueries ?? []).slice(-20).filter((q) => typeof q === "string" && q.trim().length > 0);
	const activePlanSummary = input.activePlanSummary ? input.activePlanSummary.trim().slice(0, MAX_WORKING_STATE_PLAN_CHARS) : void 0;
	const subGoalTree = input.subGoalTree ? input.subGoalTree.slice(-20) : void 0;
	return {
		sessionId: input.sessionId,
		turnIndex: Math.max(0, input.turnIndex),
		activeFiles,
		recentSearchQueries,
		activePlanSummary: activePlanSummary && activePlanSummary.length > 0 ? activePlanSummary : void 0,
		subGoalTree,
		updatedAt: nowMs
	};
}
...
*/
function formatWorkingStateRecoveryPrompt(state) {
	const sections = [`[RECONSTRUCTED WORKING CONTEXT — Turn ${state.turnIndex}]`];
	if (state.activePlanSummary) sections.push(`Active Plan:\n${state.activePlanSummary}`);
	if (state.activeFiles.length > 0) sections.push(`Active Working Files:\n- ${state.activeFiles.join("\n- ")}`);
	if (state.recentSearchQueries.length > 0) sections.push(`Recent Search Context:\n- ${state.recentSearchQueries.join("\n- ")}`);
	if (state.subGoalTree && state.subGoalTree.length > 0) {
		const goals = state.subGoalTree.map((g) => `[${g.status.toUpperCase()}] ${g.id}: ${g.description}`).join("\n");
		sections.push(`Task Graph:\n${goals}`);
	}
	return sections.join("\n\n");
}
//#endregion
//#region src/agents/working-state/working-state-store.ts
/**
...
*/
var WorkingStateStore = class {
	constructor(options = {}) {
		this.kvStore = new SqliteKvStore("session_working_state", options);
	}
	saveWorkingState(input, nowMs) {
...
let defaultWorkingStateStore;
function getGlobalWorkingStateStore() {
	if (!defaultWorkingStateStore) defaultWorkingStateStore = new WorkingStateStore();
...
function resolveRestartRecoveryResumeBlockReason(params) {
	...
	if (!pluginRegistry) return "pre-hook recovery runtime plugins could not be loaded";
	const unsafeHook = withPluginRuntimeRegistryScope(pluginRegistry, () => findRestartRecoveryUnsafeReplyHook({ trigger: "user" }));
	return unsafeHook ? `pre-hook recovery cannot bypass the active ${unsafeHook} hook` : void 0;
}
function buildResumeMessage(pendingFinalDeliveryText, sessionId) {
	...
	if (sanitizedPendingText) baseMessage = `${RESTART_RECOVERY_RESUME_MESSAGE}\n\nNote: The interrupted final reply was captured: "${sanitizedPendingText}"`;
	if (sessionId) {
...
async function settleRestartRecoveryDispatch(params) {
	await applySqliteSessionEntryReplacements({
...
async function resumeMainSession(params) {
	...
	try {
		...
		try {
			...
			const syntheticResults = repairOrphanToolCalls(buildSessionContext(manager.getEntries(), manager.getLeafId()).messages);
			if (syntheticResults.length > 0) {
				log.info(`Found ${syntheticResults.length} orphan tool calls for session ${params.sessionKey}; patching synthetic cancellation results.`);
				for (const res of syntheticResults) manager.appendMessage(res);
			}
		} catch (err) {
...
//#endregion
//#region src/agents/main-session-restart-recovery-resume-policy.ts
...
//#endregion
//#region src/agents/main-session-restart-recovery-checkpoint.ts
...
async function reconcileInterruptedCompletionReport(params) {
	...
	if (didReconcile) {
		log$1.info(`reconciled interrupted completion report to non-running: ${params.sessionKey}`);
...
async function markSessionCompletedAfterRecoveryCheckpoint(params) {
	...
	if (toolCallId && sourceTurnId && sourceTurnRange !== void 0 && messageToolCallIndex !== void 0 && recoveryToolResultIdempotencyKey && successfulToolResultIndex === void 0) {
		...
		if (completed) log$1.info(`reconciled delivered terminal reply after restart: ${params.sessionKey}`);
		return { outcome: completed ? "completed" : "changed" };
	}
	const marked = await applySqliteSessionEntryReplacements({
		sessionKeys: [params.sessionKey],
		storePath: params.storePath,
		update: (entries) => {
	...
	if (marked) log$1.info(params.reason === "delivered-terminal" || params.reason === "delivered-terminal-receipt" ? `reconciled delivered terminal reply after restart: ${params.sessionKey}` : `reconciled handled silent reply after restart: ${params.sessionKey}`);
	return { outcome: marked ? "completed" : "changed" };
}
//#endregion
//#region src/agents/main-session-restart-recovery-notice.ts
async function sendUnresumableSessionNotice(params) {
	...
	try {
		...
		log$1.warn(`failed to send interrupted main session recovery notice ${params.sessionKey}: ${String(err)}`);
	}
}
async function writeUnresumableSessionNotice(params) {
	const result = await appendAssistantMessageToSessionTranscript({
		...
		sessionKey: params.sessionKey,
		expectedSessionId: params.entry.sessionId,
		expectedSessionState: params.expectedSessionState ?? buildRestartRecoveryExpectedState(params.entry),
		sessionLifecyclePatch: params.sessionLifecyclePatch,
		storePath: params.storePath,
		text: params.text ?? "I was interrupted by a gateway restart and couldn't safely resume the previous turn. Please send that last request again and I'll pick it up cleanly.",
		idempotencyKey: buildUnresumableSessionNoticeIdempotencyKey(params.entry)
	}).catch((error) => ({
		ok: false,
		reason: String(error)
...
async function failUnresumableMainSession(params) {
	...
	if (!deliveryContext && await writeUnresumableSessionNotice({
		agentId: resolveAgentIdFromSessionKey(params.sessionKey, params.cfg ? resolveDefaultAgentId(params.cfg) : void 0),
		entry: params.entry,
		sessionKey: params.sessionKey,
		storePath: params.storePath
	}) !== "written") return "failed";
	...
	})).transition.kind !== "failed") return "skipped";
	log$1.warn(`marked interrupted main session failed: ${params.sessionKey} (${params.reason})`);
...
async function tombstoneMainRestartRecoveryWithNotice(params) {
	...
	if (!deliveryContext) {
		...
		for (let attempt = 0; attempt < 3; attempt += 1) {
			...
			if (!current || current.sessionId !== params.entry.sessionId || state?.cycleId !== params.observation.cycleId || state.tombstone || current.status !== "running" || current.abortedLastRun !== true || state.chargedAttempts < 3) return "skipped";
			entry = current;
			observation = {
...
//#endregion
//#region src/agents/main-session-restart-recovery-store.ts
...
function resolveRecoveryDispatchSessionKey(params) {
	...
	try {
		...
		log$1.warn(`failed to resolve recovery store for ${params.sessionKey}: ${String(err)}`);
		return;
	}
}
async function recoverStore(params) {
	...
	try {
		...
		log$1.warn(`failed to load session store ${params.storePath}: ${String(err)}`);
		result.failed++;
		return result;
	}
	for (const { sessionKey, entry: loadedEntry } of entries.toSorted((a, b) => a.sessionKey.localeCompare(b.sessionKey))) {
		...
		const agentId = resolveAgentIdFromSessionKey(sessionKey, params.cfg ? resolveDefaultAgentId(params.cfg) : void 0);
		...
		const failCurrent = async (reason) => {
		...
		if (requiresRestartRecoveryMessageActionAuthority(entry) && !hasRestartRecoveryMessageActionAuthority(entry)) {
			if (!await failCurrent("message-tool-only recovery authority is unavailable")) return result;
			continue;
		}
		...
		const resumeCurrent = async (options = {}) => {
			if (await failBlockedResume()) return;
		...
		try {
			...
			if (entry.pendingFinalDelivery?.kind === "replayable") {
				log$1.warn(`transcript unavailable for ${sessionKey}; resuming its durable pending final delivery`);
				await resumeCurrent({ pendingFinalDeliveryText: entry.pendingFinalDelivery.text });
				continue;
			}
			log$1.warn(`failed to read transcript for ${sessionKey}: ${String(err)}`);
			result.failed++;
			continue;
		}
		if (stopped()) return result;
		...
		if (resumePolicy.action === "complete") {
			...
			else if (!await failCurrent(completion.reason)) return result;
			continue;
		}
		if (resumePolicy.action === "fail") {
			if (!await failCurrent(resumePolicy.reason)) return result;
			continue;
		}
		await resumeCurrent({
...
//#endregion
//#region src/agents/main-session-restart-recovery-runtime.ts
...
async function recoverRestartAbortedMainSessions(params) {
	...
	if (result.recovered > 0 || result.failed > 0) log$1.info(`main-session restart recovery complete: recovered=${result.recovered} failed=${result.failed} skipped=${result.skipped}`);
	return result;
}
...
function scheduleRestartAbortedMainSessionRecoveryAfterOwnerRelease(params) {
	...
	runRecoveryRetries({
		...
		onError: (error, finalAttempt) => {
			if (finalAttempt) log$1.warn(`main-session owner-release recovery failed: ${String(error)}`);
		}
	});
}
function scheduleRestartAbortedMainSessionRecovery(params) {
	...
	const reconcileExhaustedTargets = async (targets) => {
		...
		for (const outcome of outcomes) if (outcome.status === "rejected") log$1.warn(`main-session exhaustion reconciliation failed: ${String(outcome.reason)}`);
	};
	...
	const run = Promise.resolve().then(async () => {
		...
		await runRecoveryRetries({
			...
			onError: async (err, finalAttempt) => {
				if (finalAttempt) {
					log$1.warn(`main-session restart recovery gave up: ${String(err)}`);
					await reconcileExhaustedTargets(exhaustedTargets.values());
				} else log$1.warn(`main-session restart recovery failed: ${String(err)}`);
			}
		});
	});
	return { stop: async () => {
...
//#endregion
export { scheduleRestartAbortedMainSessionRecoveryAfterOwnerRelease as i, retryRestartAbortedMainSessionRecovery as n, scheduleRestartAbortedMainSessionRecovery as r, recoverRestartAbortedMainSessions as t };
