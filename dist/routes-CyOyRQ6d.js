...
import { f as pathScope } from "./fs-safe-DVaClkIX.js";
...
import { c as getRuntimeConfigSourceSnapshot } from "./runtime-snapshot-Bzqj8IgJ.js";
...
import { a as ensureMediaDir, u as saveMediaBuffer } from "./store-QVlUiM0_.js";
import { t as expectDefined } from "./expect-runtime--WgnKYXT.js";
import "./number-runtime-C6TGSEc_.js";
import "./string-coerce-runtime-CLK2YdzD.js";
import { n as redactCdpUrl, t as parseBrowserHttpUrl } from "./browser-config-y8Xi4vs-.js";
import "./text-utility-runtime-Dwuhfjgs.js";
import { a as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, b as MIN_BROWSER_EXTRACT_TIMEOUT_MS, m as DEFAULT_BROWSER_SCREENSHOT_TIMEOUT_MS, n as BROWSER_EXTRACT_MAX_HTML_CHARS, o as DEFAULT_AI_SNAPSHOT_MAX_CHARS, y as MAX_BROWSER_EXTRACT_TIMEOUT_MS } from "./constants-BoWgRtg0.js";
import { a as normalizeActBoundedNonNegativeMs, n as ACT_MAX_VIEWPORT_DIMENSION, r as ACT_MAX_WAIT_TIME_MS, t as ACT_MAX_CLICK_DELAY_MS } from "./act-policy-9AEfR1NQ.js";
...
import { a as resolveProfile, i as resolveManagedBrowserHeadlessMode, l as resolveExistingUploadPaths, n as getOwnBrowserProfile, o as DEFAULT_DOWNLOAD_DIR, r as resolveBrowserConfig, s as DEFAULT_TRACE_DIR } from "./config-B2k8sZ1m.js";
import { A as BrowserValidationError, C as BrowserError, M as toBrowserErrorResponse, O as BrowserTabNotFoundError, S as BrowserConflictError, T as BrowserProfileUnavailableError, _ as normalizeBrowserTimerDelayMs, f as redactCdpErrorText, g as withCdpSocket, k as BrowserTargetAmbiguousError, n as assertCdpEndpointAllowed, w as BrowserProfileNotFoundError } from "./tmp-openclaw-dir-BK3eq-mf.js";
import "./config-CCqm3Ogf.js";
import "./errors-Cb3-8IJ1.js";
import { a as isValidProfileName, i as setDefaultBrowserProfile, n as deleteBrowserProfileConfig, t as createBrowserProfileConfig } from "./config-mutations-C47D-91i.js";
import { r as resolveBrowserExecutableForPlatform } from "./chrome.executables-B2udiA_j.js";
import { A as assertBrowserNavigationAllowed, C as finalizeRoleSnapshot, D as INTERACTIVE_ROLES, E as CONTENT_ROLES, F as withBrowserNavigationPolicy, M as assertBrowserNavigationResultAllowed, O as STRUCTURAL_ROLES, R as resolveBrowserNavigationProxyMode, _ as getMainFrameDocumentIdentityViaCdp, b as snapshotRoleViaCdp, l as ensureOutputDirectory, m as captureScreenshot, n as getChromeWebSocketUrl, s as resolveOpenClawUserDataDir, w as getRoleSnapshotIdentityKeys, y as snapshotAria } from "./chrome-CObhw6aD.js";
import { a as getOrCreateProfileRuntime, g as getPwAiModule$1, l as isProfileRestartRequiredError, o as getProfileLifecycle, r as beginProfileTransition } from "./server-context.lifecycle-DGUJZrvY.js";
...
import { n as resolveCdpControlPolicy, t as movePathToTrash } from "./trash-BJi77wmF.js";
import { S as withChromeMcpDocument, _ as resizeChromeMcpPage, b as takeChromeMcpSnapshot, d as fillChromeMcpElement, f as fillChromeMcpForm, g as pressChromeMcpKey, h as navigateChromeMcpPage, k as ChromeMcpDocumentUnavailableError, l as dragChromeMcpElement, m as hoverChromeMcpElement, o as clickChromeMcpCoords, r as getChromeMcpPid, s as clickChromeMcpElement, u as evaluateChromeMcpScript, x as uploadChromeMcpFile, y as takeChromeMcpScreenshot } from "./chrome-mcp-8Aev-65v.js";
import { n as normalizeString } from "./record-shared-BCecxvOL.js";
import { i as resolveTargetIdFromTabs, r as withProfileContextOperation } from "./server-context-CirUoj7g.js";
import { n as listSystemProfiles, t as importSystemProfileCookies } from "./system-profiles-DODBUT3N.js";
import "./sdk-setup-tools-BpwkGBkq.js";
import { a as scaleAnnotations, l as matchBrowserUrlPattern, s as normalizeBrowserFormField, t as appendSnapshotUrls, u as normalizeBrowserEvaluateFunctionSource } from "./snapshot-urls-XEHa-0UE.js";
...
//#endregion
//#region extensions/browser/src/browser/routes/agent.extract.ts
/** Browser page-content capture route for the extract action. */
function resolveExtractTimeoutMs(value) {
...
function formatExtractCaptureError(params) {
	if (params.error === "invalid_selector") return "One or more CSS selectors are invalid; check selector and ignoreSelectors and try again.";
	return params.selector ? `CSS selector ${JSON.stringify(params.selector)} matched no usable elements; check selector and ignoreSelectors, or omit selector to extract the whole page.` : "ignoreSelectors removed all captured page content; adjust ignoreSelectors and try again.";
}
/** Register the Playwright-only page-content capture endpoint. */
function registerBrowserExtractRoute(app, ctx) {
	app.post("/extract", async (req, res) => {
		const body = readBody(req);
		const targetId = toStringOrEmpty(body.targetId) || void 0;
		const selector = toStringOrEmpty(body.selector) || void 0;
		const ignoreSelectors = Array.isArray(body.ignoreSelectors) ? body.ignoreSelectors.filter((value) => typeof value === "string") : [];
		let timeoutMs;
		try {
		...
		if (!profileCtx) return;
		if (getBrowserProfileCapabilities(profileCtx.profile).usesChromeMcp) return jsonError(res, 501, EXISTING_SESSION_LIMITS.extract);
		...
		await withPlaywrightRouteContext({
			...
			run: async ({ cdpUrl, tab, signal, resolveTabUrl, pw }) => {
				...
				if (!captured.ok) return res.json({
					...captured,
					message: formatExtractCaptureError({
				...
				const { html } = captured;
				if (html.length > 2e6) return jsonError(res, 413, `page HTML exceeds the ${BROWSER_EXTRACT_MAX_HTML_CHARS} character extraction limit; use snapshot instead.`);
				res.json({
					ok: true,
					targetId: tab.targetId,
					url,
...
const cacheByContext = /* @__PURE__ */ new WeakMap();
		...
		try {
			await runProfileRouteOperation({
				...
				run: async (signal) => {
					...
					const readPlaywrightDocumentIdentity = pwModule?.getMainFrameDocumentIdentityViaPlaywright;
					...
					const readDocumentIdentity = async () => {
						if (!deltaFamily) return;
						const playwrightIdentity = readPlaywrightDocumentIdentity ? await readPlaywrightDocumentIdentity({
							cdpUrl: profileCtx.profile.cdpUrl,
							targetId: tab.targetId
						}).catch(() => void 0) : void 0;
						if (playwrightIdentity || !tab.wsUrl) return playwrightIdentity;
					...
					const assertDocumentIdentityUnchanged = async () => {
						...
						if (await readDocumentIdentity() !== initialDocumentIdentity) throw new Error("Frame changed while its browser snapshot was being captured; retry.");
					};
					if (plan.format === "ai") {
...
async function probeChromeMcpPageReady(profileCtx, timeoutMs, signal) {
...
async function buildBrowserStatus(ctx, profileCtx, signal) {
	...
	const [cdpHttp, cdpReady, pageReady] = capabilities.usesChromeMcp ? await (async () => {
		...
		return [
			transportReady,
			transportReady,
			await probeChromeMcpPageReady(profileCtx, remainingChromeMcpStatusTimeoutMs(statusStartedAtMs), signal)
		];
	})() : await (async () => {
