...
async function collectWorkspaceSuggestionNotesWithRuntime(workspaceDir) {
	const { collectWorkspaceBackupTip } = await import("./doctor-state-integrity-B3lmgA0J.js");
...
const bootstrapSizeCheck = {
	...
	async detect(ctx) {
		...
		const { resolveBootstrapMaxChars, resolveBootstrapTotalMaxChars } = await import("./embedded-agent-helpers-b5jBfa_R.js");
...
function createConvertedWorkflowChecks(deps) {
	return [
		...
		...isExperimentalClawsEnabled() ? [{
			...
			source: "doctor",
			async detect(ctx) {
				const [{ collectClawStateHealthFindings }, { listConfiguredMcpServers }] = await Promise.all([import("./doctor-Ca9rbYjl.js"), import("./mcp-config-BH8ZK-o2.js")]);
