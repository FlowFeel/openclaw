...
import { h as getNodeSqliteKysely, m as executeSqliteQueryTakeFirstSync, n as requireNodeSqlite, p as executeSqliteQuerySync } from "./node-sqlite-BJTPe7U8.js";
import { Y as normalizeSqliteNumber, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, l as isOpenClawStateDatabaseOpen, r as closeOpenClawStateDatabase } from "./openclaw-state-db-BU55lNCH.js";
import { n as resolveOpenClawStateSqlitePath } from "./openclaw-state-db.paths-2PkDmkRl.js";
import { t as withExistingOpenClawStateDatabaseReadOnly } from "./openclaw-state-db-readonly-DKc3VYcD.js";
...
function hasStateTablesBeyondStartupCheckpoint(db) {
		...
		return withExistingOpenClawStateDatabaseReadOnly(({ db, path }) => {
			...
			try {
				...
				if (isMissingPluginStateTableError(error)) {
					if (!hasStateTablesBeyondStartupCheckpoint(db)) return;
				}
				throw error;
			}
		}, options);
...
function pluginStateEntriesInKeyRange(params) {
	...
	try {
		return withPluginStateDatabaseReadOnly("entries", ({ db }) => selectPluginStateEntriesInKeyRange(db, {
			...
			now: Date.now()
		}).map((row) => rowToEntry(row, "entries")), envOptions(params.env)) ?? [];
	} catch (error) {
