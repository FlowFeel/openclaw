...
function resolveGeminiCliAmbientHome(ctx) {
	return normalizeString(ctx.baseEnv?.GEMINI_CLI_HOME) ?? normalizeString(process.env.GEMINI_CLI_HOME) ?? os.homedir();
}
function projectGeminiCliTrustedTransportEnv(ctx, ambientEnv) {
	return Object.fromEntries([...GEMINI_CLI_TRUSTED_TRANSPORT_ENV].map((name) => [name, normalizeString(ctx.baseEnv?.[name]) ?? normalizeString(process.env[name]) ?? normalizeString(ambientEnv.transport[name]) ?? ""]));
}
...
async function resolveGeminiCliAmbientAuth(ctx) {
	...
	const systemSelectedType = normalizeString(typeof systemAuth?.selectedType === "string" ? systemAuth.selectedType : void 0);
	const userSelectedType = normalizeString(typeof userAuth?.selectedType === "string" ? userAuth.selectedType : void 0);
	const selectedType = systemSelectedType ?? (preparedSelectorOwnsAuth ? void 0 : userSelectedType);
	const enforcedType = normalizeString(typeof systemAuth?.enforcedType === "string" ? systemAuth.enforcedType : void 0);
	...
	const envValue = (name) => {
		...
		return normalizeString(process.env[name]) ?? normalizeString(ambientEnv.auth[name]) ?? normalizeString(ambientEnv.unsafeAuth[name]);
	};
