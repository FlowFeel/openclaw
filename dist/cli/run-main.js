...
async function closeCliMcpLoopbackServer() {
	try {
		...
		const { closeMcpLoopbackServer } = await import("../mcp-http-BmAqPvWf.js");
...
async function runCliWithPreparedOutputMode(originalArgv, options) {
	...
	try {
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-CZVbKa20.js");
		...
		if (bareRootLaunchTarget) {
			if (bareRootLaunchTarget.kind === "remote-gateway-inference") {
				...
				const { runRemoteGatewayInferenceOnboarding } = await import("../onboard-remote-gateway-Dd6o_xgh.js");
			...
			if (bareRootLaunchTarget.kind === "onboarding") {
				...
				const { setupWizardCommand } = await import("../onboard-B7ORu_Xl.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-t2yMFPmL.js"));
		...
		try {
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-DmihPopd.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-BO1e6Lbd.js");
