...
import { h as CLAUDE_CLI_PROFILE_ID, s as overlayRuntimeExternalOAuthProfiles, t as areOAuthCredentialsEquivalent, v as MINIMAX_CLI_PROFILE_ID } from "./oauth-shared-B-oYylTU.js";
import { C as resolveExternalAuthProfilesWithPlugins } from "./provider-runtime-pe0BhJuO.js";
...
function syncPersistedExternalCliAuthProfiles(store, params) {
	...
	const persistedProfiles = (resolveExternalCliAuthProfiles?.(store, {
		...
		profileIds: explicitProfileIds
	}) ?? []).filter((profile) => profile.persistence === "persisted" && isExternalAuthProfileAllowed(profile, store, params?.config, explicitProfileIds, env));
