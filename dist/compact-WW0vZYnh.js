...
import { t as CODEX_APP_SERVER_BINDING_GUARDED_REQUEST_TIMEOUT_MS, v as sessionBindingIdentity } from "./session-binding-Dl6kaOMx.js";
import { W as CodexAppServerRpcError, at as isJsonObject, b as consumeCodexAppServerLiveThread, d as releaseLeasedSharedCodexAppServerClient, s as getLeasedSharedCodexAppServerClient } from "./shared-client-D2F1gOgp.js";
import { _ as readCodexNotificationTurnId, a as closeCodexStartupClientBestEffort, d as unsubscribeCodexThreadBestEffort, g as readCodexNotificationThreadId, i as CodexAppServerUnsafeSubscriptionError, s as isCodexAlreadyTerminalInterruptError, t as resumeCodexAppServerThread } from "./thread-resume-BTrJDDVj.js";
import { _ as readCodexNotificationItem } from "./attempt-notifications-ql8QFhNK.js";
		...
		return await runExclusiveCodexNativeCompaction(binding.threadId, params.abortSignal, async () => {
			...
			const acquireThreadSubscription = async (timeoutMs) => {
				if (!isIncognitoSessionKey(params.sessionKey)) {
					if (!await consumeCodexAppServerLiveThread(client, binding.threadId)) await resumeCodexAppServerThread({
						client,
			...
			try {
				if (options.allowNonManualNativeRequest) {
					const guardedResult = await options.bindingStore.withLease(bindingIdentity, async () => {
						...
						const guardedRequestTimeoutMs = Math.min(appServer.requestTimeoutMs, CODEX_APP_SERVER_BINDING_GUARDED_REQUEST_TIMEOUT_MS);
