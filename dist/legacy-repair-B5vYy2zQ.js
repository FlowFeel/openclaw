...
import { J as migrateLegacyCronRunLogsToTaskRuns, d as openOpenClawStateDatabase, h as runOpenClawStateWriteTransaction, nt as parseCronRunLogEntryObject } from "./openclaw-state-db-D9eH245j.js";
import { n as resolveOpenClawStateSqlitePath } from "./openclaw-state-db.paths-2PkDmkRl.js";
import { v as MEMORY_DREAMING_SYSTEM_EVENT_TEXT } from "./dreaming-CisvAHct.js";
import { a as loadCronJobsStoreWithConfigJobsReadOnly, c as resolveCronJobsStorePath, d as saveCronJobsStore, f as saveCronJobsStoreWithMetadata, h as saveCronQuarantinedJobs, i as loadCronJobsStoreWithConfigJobs } from "./store-BwY4XtES.js";
...
function formatScheduledToolPolicyAdvisory(params) {
	...
	lines.push("- These jobs continue through restrictive sender-policy resolution; doctor will not infer authority from delivery or current configuration.", "- Reauthorize with `openclaw cron edit <id> --tools <tool,...>`, or use `--clear-tools` to adopt the current default cap.");
	return lines.join("\n");
}
...
async function loadLegacyCronRepairState(params) {
	...
	const currentJobs = currentEntries.length > 0 ? currentEntries.map((entry) => entry.job) : loaded.store.jobs;
	const sqliteProjectionBackfillCount = currentEntries.filter((entry) => needsSqliteProjectionBackfill({
		configJob: entry.job,
...
async function applyLegacyCronStoreRepair(params) {
	...
	const storeChanged = state.legacyStoreDetected && !state.legacyMigrationAlreadyImported || state.sqliteProjectionBackfillCount > 0 || state.invalidConfigRows.length > 0 || normalized.mutated || notifyMigration.changed || dreamingMigration.changed;
...
//#endregion
export { repairLegacyCronStoreWithoutPrompt as a, formatUnresolvedCommandPromptAdvisory as c, repairCronCodexModelRefsAfterConfigWrite as i, formatUnresolvedShellPromptAdvisory as l, collectCronCodexRuntimePolicyTargetsReadOnly as n, formatLegacyIssuePreview as o, loadLegacyCronRepairState as r, formatScheduledToolPolicyAdvisory as s, applyLegacyCronStoreRepair as t, countStaleDreamingJobs as u };
