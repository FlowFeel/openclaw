...
import { a as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-CFCg6zZl.js";
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { requestSafeGatewayRestart } = await import("./restart-coordinator-D-klAPCa.js");
		return requestSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-DMr2gP0_.js");
	if (operation === "start") {
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-C1W93SAu.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-C1W93SAu.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-p4c0RhoJ.js")).applySystemAgentSetup;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-p4c0RhoJ.js");
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			const runPluginInstall = ctx.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-DPOxZ8p-.js");
...
async function executeSystemAgentOperation(operation, runtime, opts = {}) {
	switch (operation.kind) {
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-CoTQdC_r.js");
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-C2LnNbZc.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-SUPhxciR.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-BavjTmCc.js")).runTui)({
				local: true,
				session,
				deliver: false,
				historyLimit: 200,
