...
import { r as summarizeTranscripts, t as TranscriptsStore } from "./store-DXleh4bY.js";
...
function createMeetingDurableTranscriptBridge(params) {
	...
	const reportCaptureError = (sessionId, error) => {
		params.logger.debug?.(`[meeting-transcripts] capture ignored session=${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
	};
	const notifySubscriberStatus = (subscriber, status) => {
		...
		try {
			Promise.resolve(subscriber.onStatus(status)).catch((error) => {
				params.logger.warn(`[meeting-transcripts] subscriber status failed session=${status.sessionId ?? "unknown"}: ${error instanceof Error ? error.message : String(error)}`);
			});
		} catch (error) {
			params.logger.warn(`[meeting-transcripts] subscriber status failed session=${status.sessionId ?? "unknown"}: ${error instanceof Error ? error.message : String(error)}`);
		}
	...
	return {
				...
				const initialize = async () => {
					...
					try {
						...
						if (!active.initializationWarned) {
							params.logger.warn(`[meeting-transcripts] durable capture initialization pending session=${session.id}: ${error instanceof Error ? error.message : String(error)}`);
				...
				for (const line of lines) {
					...
					for (const [subscriberSessionId, subscriber] of subscribers) {
						...
						try {
							...
							params.logger.warn(`[meeting-transcripts] detached failing subscriber session=${subscriberSessionId}: ${error instanceof Error ? error.message : String(error)}`);
		...
		async stop(session, finalCapture) {
			...
			try {
				...
				params.logger.warn(`[meeting-transcripts] could not finalize durable capture session=${session.id}: ${error instanceof Error ? error.message : String(error)}`);
				throw error;
			}
