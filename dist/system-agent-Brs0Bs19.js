...
import { g as setCommandLaneConcurrency, r as enqueueCommandInLane } from "./command-queue-Cl58ne2E.js";
import { n as describeSystemAgentPersistentOperation } from "./operations-BtNayaH5.js";
...
import { t as getUpdateAvailable } from "./update-startup-D395Gvpn.js";
import { n as getHealthCache } from "./health-state-CVIlUfcm.js";
import { t as WizardSession } from "./session-BeEA_WOJ.js";
import { n as SystemAgentChatEngine, r as SystemAgentWizardAnswerError } from "./chat-engine-B_dpEtAo.js";
	...
	"openclaw.setup.prepare.start": async ({ params, respond, context }) => {
		...
		const session = new WizardSession(async (prompter, signal) => {
			await runExclusiveSystemAgentSetupActivation(async () => runSystemAgentGatewayTask(async () => {
				const [{ applyAuthChoiceLoadedPluginProvider }, setupShared] = await Promise.all([import("./provider-auth-choice-UzG0SthV.js"), import("./setup.shared-BQDwhL8U.js")]);
	...
	"openclaw.chat": async ({ params: rawParams, respond, client, context }) => {
		...
		await runSystemAgentGatewayTask(async () => {
			...
			await getSystemAgentSessionQueue(sessions).enqueue(sessionId, async () => {
				...
				if (!session) {
					const inference = params.delegation ? await import("./inference-fallback-SzdghaBZ.js").then(({ verifySystemAgentInferenceWithFallback }) => verifySystemAgentInferenceWithFallback({
