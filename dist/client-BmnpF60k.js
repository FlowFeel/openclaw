...
import { i as omitEnvKeysCaseInsensitive, n as listKnownProviderAuthEnvVarNames } from "./provider-env-vars-D-oDm8A0.js";
...
import { n as isMutatingToolCall } from "./tool-mutation-BmnNoynG.js";
...
const ACP_SERVER_KILL_GRACE_MS = 1e3;
const ACP_SERVER_FORCE_KILL_TIMEOUT_MS = 1e3;
...
async function waitForChildExit(child, timeoutMs) {
	...
	while (!hasChildExited(child) && Date.now() < deadline) await setTimeout$1(ACP_SERVER_EXIT_POLL_MS);
	return hasChildExited(child);
}
async function terminateAcpServer(child) {
	...
	if (await waitForChildExit(child, 2e3)) return;
	if (child.pid) signalProcessTree(child.pid, "SIGKILL", { detached: false });
