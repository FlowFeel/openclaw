...
async function openSystemAgentChat(deps, workspace, runtime, acceptRisk) {
	await (deps.runSystemAgentChat ?? (async (setupWorkspace, chatRuntime, riskAccepted) => {
		const { runConversationalOnboarding } = await import("./onboard-interactive-mOiRBnv4.js");
...
async function runGuidedOnboardingFlow(opts, runtime, deps) {
	...
	if (!custodianMode) {
		if (wantsDiscovery) await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-Bq2Erzmp.js")).runSetupMemoryImportStep)({
	...
	if (alreadyConfigured) {
		...
		try {
			...
			const applySetup = deps.applySetup ?? (await import("./setup-apply-p4c0RhoJ.js")).applySystemAgentSetup;
	...
	if (wantsDiscovery) {
		await (deps.runSetupMemoryImportStep ?? (await import("./setup.memory-import-Bq2Erzmp.js")).runSetupMemoryImportStep)({
			config: persistedConfig,
			prompter,
			runtime
		});
		const recommendationOutcome = await (deps.runAppRecommendations ?? (await import("./setup.app-recommendations-BiG57LQr.js")).setupAppRecommendations)({
			config: persistedConfig,
			prompter,
			runtime,
