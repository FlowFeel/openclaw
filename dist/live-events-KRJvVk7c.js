...
import { a as emitAgentEventForOwner, o as emitAgentEventIfCurrent, s as getAgentEventLifecycleGeneration } from "./agent-events-COCf-9-O.js";
import { a as getAgentRunContext, h as releaseAgentRunContext, n as claimAgentRunContext, o as getAgentRunContextOwnerStatus, p as registerAgentRunContext } from "./agent-run-registry-BluEqSPq.js";
import { h as normalizeToolName } from "./tool-policy-CrjVfI-s.js";
import { jt as onSessionIdentityMutation } from "./session-accessor.sqlite-CtCo5VZ6.js";
import "./session-accessor-D5Or7WgI.js";
import { h as sanitizeToolResult, m as sanitizeToolArgs, n as capLiveExecResult } from "./embedded-agent-subscribe.tools-agHp_6fR.js";
...
function createWorkerLiveEventReceiver(options) {
	...
	const claimRun = (window, runId, allowBufferedTerminalCapacity) => {
		...
		let emissionMode = "exclusive";
		let claimId = claimAgentRunContext(runId, {
		...
		if (!claimId && existingContext) {
			claimId = claimAgentRunContext(runId, {
				...window.target.agentId ? { agentId: window.target.agentId } : {},
				isControlUiVisible: controlUiVisible,
				lifecycleGeneration,
				projectSessionActive: true,
	...
	const publish = (window, request, allowBufferedTerminalCapacity) => {
		...
		} else emitAgentEventForOwner(event, owned.claimId);
