...
const loadModelsStatusCommands = createModuleLoader(() => import("./list.status-command-KAA4jikc.js"));
...
const loadModelsAuthCommands = createModuleLoader(() => import("./auth-BuXdH39D.js"));
...
function registerModelsCli(program) {
	...
	models.command("set").description("Set the default model").argument("<model>", "Model id or alias").action(async (model, _opts, command) => {
		...
		await runtime.runModelsCommand(async () => {
			const { modelsSetCommand } = await import("./set-BBG8Ro1n.js");
