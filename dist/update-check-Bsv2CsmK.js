...
import { t as compareOpenClawReleaseVersions } from "./npm-registry-spec-CCN1EPEy.js";
...
import { t as readPackageManagerSpec } from "./package-json-DkVfRSR1.js";
...
async function checkGitUpdateStatus(params) {
	...
	const upstreamCommitRes = upstream && sha ? await runCommandWithTimeout([
		"git",
		"-C",
		root,
		"rev-parse",
	...
	const upstreamCommit = upstreamCommitRes?.code === 0 ? upstreamCommitRes.stdout.trim() || null : null;
	const mergeBase = sha && upstreamCommit ? await runCommandWithTimeout([
		"git",
		"-C",
		root,
		"merge-base",
	...
	const counts = sha && upstreamCommit && mergeBase?.code === 0 && mergeBase.stdout.trim().length > 0 ? await runCommandWithTimeout([
		"git",
		"-C",
		root,
		"rev-list",
