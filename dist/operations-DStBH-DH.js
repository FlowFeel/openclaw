...
import { a as sameDefaultInferenceRoute, n as projectInferenceRoute, t as projectDefaultInferenceRoute } from "./inference-route-fxRKzhD7.js";
import { i as appendSystemAgentAuditEntry, r as SYSTEM_AGENT_AUDIT_STORE_LABEL } from "./audit-CYKiw6TQ.js";
import { n as t } from "./i18n-BYpJa9f7.js";
import { t as createAgent } from "./agent-create-BWS4wqkg.js";
import { r as isOpenClawTrustedPluginInstallSpec } from "./install-provenance-DZLEqpF9.js";
...
const loadOverviewModule$1 = async () => await import("./overview-D5AgZdDN.js");
...
async function runGatewayLifecycle(operation, surface) {
	if (operation === "restart" && surface === "gateway") {
		const { requestSafeGatewayRestart } = await import("./restart-coordinator-xZAlsWQp.js");
		return requestSafeGatewayRestart({
			reason: "gateway.restart.safe",
			delayMs: 0
		}).ok;
	}
	const lifecycle = await import("./lifecycle-e2LlrIFl.js");
	if (operation === "start") {
...
async function resolveChannelSetupState(deps) {
	const listPlugins = deps?.listChannelSetupPlugins ?? (await import("./setup-registry-CyOT7DZt.js")).listChannelSetupPlugins;
	const resolveEntries = deps?.resolveChannelSetupEntries ?? (await import("./discovery-CZtoPgzJ.js")).resolveChannelSetupEntries;
	const isConfigured = deps?.isChannelConfigured ?? (await import("./channel-configured-shared-CFXc1ltB.js")).isStaticallyChannelConfigured;
	const { shouldShowChannelInSetup } = await import("./discovery-CZtoPgzJ.js");
...
async function runConfigSetOperation(params) {
	...
	const runConfigSet = ctx.deps?.runConfigSet ?? (async (setOpts) => {
		const { runConfigSet: importedRunConfigSet } = await import("./config-cli-Cy1bhY-K.js");
...
async function assertConfigWriteDoesNotBypassInferenceVerification(operation) {
	const { parseConfigSetPath } = await import("./config-cli-Cy1bhY-K.js");
...
async function executeSetup(operation, runtime, opts) {
	...
	return await applyPersistentOperation({
		...
		opts,
		run: async (ctx) => {
			const applySetup = ctx.deps?.applySetup ?? (await import("./setup-apply-B7OOZFgA.js")).applySystemAgentSetup;
			const surface = ctx.deps?.setupSurface ?? "cli";
			const recovery = surface === "cli" ? await (await import("./setup-recovery-ChhhqXEu.js")).loadLocalSetupRecovery(operation.workspace) : void 0;
...
async function executeSetDefaultModel(operation, runtime, opts) {
	return await applyPersistentOperation({
		...
		run: async (ctx) => {
			...
			const { applySystemAgentModelSelection, createSystemAgentModelSelectionUpdater } = await import("./setup-apply-B7OOZFgA.js");
...
async function isPluginBackingDefaultInferenceRoute(pluginId) {
	...
	const { resolveModelRuntimePolicy } = await import("./model-runtime-policy-BSd6jAUF.js");
	...
	const { resolveOwningPluginIdsForProviderRef } = await import("./providers-DLHNoNpC.js");
	...
	const result = await applyPersistentOperation({
		...
		run: async (ctx) => {
			const runPluginInstall = ctx.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
				const { runPluginInstallCommand } = await import("./plugins-install-command-CREdN5aL.js");
...
const loadOverviewModule = async () => await import("./overview-D5AgZdDN.js");
	...
	switch (operation.kind) {
		...
		case "plugin-list":
			await (opts.deps?.runPluginsList ?? (async (pluginRuntime) => {
				const { runPluginsListCommand } = await import("./plugins-list-command-C_arX4gE.js");
				await runPluginsListCommand({}, pluginRuntime);
			}))(runtime);
			return { applied: false };
		case "plugin-search":
			await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
				const { runPluginsSearchCommand } = await import("./plugins-search-command-DQhYaai0.js");
		...
		case "config-schema": {
			const { buildConfigSchema, lookupConfigSchema } = await import("./schema-B_U3mdCM.js");
			...
			const result = await applyPersistentOperation({
				...
				run: async (ctx) => {
					const runPluginUninstall = ctx.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
						const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-Ch_vWo5V.js");
		...
		case "doctor":
			await (opts.deps?.runDoctor ?? (await import("./doctor-X25xgZ8C.js")).doctorCommand)(runtime, { nonInteractive: true });
			return { applied: false };
		case "doctor-fix":
			...
			return { applied: false };
		case "status": {
			const { statusCommand } = await import("./status.command-cVhlF6bs.js");
		...
		case "open-tui": {
			...
			const result = await (opts.deps?.runTui ?? (await import("./tui-Zv0pEDOV.js")).runTui)({
				local: true,
				session,
				deliver: false,
				historyLimit: 200,
