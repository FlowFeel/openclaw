...
import { a as resolveSystemAgentVerifiedInferenceRoute, i as resolveSystemAgentExpectedAgentHarnessRuntimeArtifact } from "./verified-inference-CMyC8uSR.js";
import { a as parseSystemAgentOperation, i as isPersistentSystemAgentOperation, n as describeSystemAgentPersistentOperation, t as executeSystemAgentOperation } from "./operations-Cqkrh_F4.js";
import { n as hashSystemAgentOperation } from "./system-agent-tool-C1-ILT2W.js";
...
import { t as approvalQuestion } from "./dialogue-BMhlXxwJ.js";
import { n as classifySystemAgentApprovalText } from "./approval-intent-DqI7rAip.js";
import { o as normalizeCliModel } from "./helpers-ZECrr_yU.js";
...
async function mirrorSystemAgentToolStateFromEvents(params) {
	const [{ onAgentEvent }, { extractToolResultText }, { resolveSystemAgentProposalTransition, resolveSystemAgentDirectiveTransition }] = await Promise.all([
		...
		import("./system-agent-tool-DAlXvWiF.js")
	]);
...
async function runSystemAgentTurnWithDeps(params, deps = {}) {
	...
	try {
		...
		if (plan.runner === "cli") {
			...
			const runCli = deps.runCliAgent ?? (await import("./cli-runner-5ZVub1v0.js")).runCliAgent;
			...
			result = await (deps.runEmbeddedAgent ?? (await import("./embedded-agent-Bv58Yqvq.js")).runEmbeddedAgent)({
				...shared,
				extraSystemPrompt: SYSTEM_AGENT_SYSTEM_PROMPT,
				toolsAllow: ["openclaw"],
				systemAgentTool,
...
async function runHostedConfigWizard(params) {
	const { readSetupConfigFileSnapshot, writeWizardConfigFile } = await import("./setup.shared-BQDwhL8U.js");
...
async function defaultChannelSetupWizardRunner(channel, prompter, beforePersistentApply) {
	const { createChannelOnboardingPostWriteHookCollector, runCollectedChannelOnboardingPostWriteHooks, setupChannels } = await import("./onboard-channels-CmMP81FJ.js");
...
var SystemAgentChatEngine = class {
	...
	*/
	...
	async classifyApprovalIntent(text) {
		...
		return await (this.opts.classifyApproval ?? (await import("./approval-intent-Cw48s2MR.js")).classifySystemAgentApprovalIntent)({
			message: text,
	...
	*/
	async resolveAssistantTurn(text, approvalArmed, uiContext) {
		...
		const planner = this.opts.planWithAssistant ?? (await import("./assistant-DyJ1T4be.js")).planSystemAgentCommand;
		let plannerFailure;
		let plan;
		try {
			plan = await planner({
	...
	async planGreeting(params) {
		...
		const plan = planner ? await planner(params) : await import("./assistant-DyJ1T4be.js").then(({ planSystemAgentGreetingWithConfiguredModel }) => planSystemAgentGreetingWithConfiguredModel({
			...params,
			verifiedInference: this.verifiedInference,
			deps: this.opts.deps
		}));
