...
import { t as createRealtimeTranscriptionWebSocketSession } from "./realtime-transcription-Ck1164z6.js";
import { n as DEFAULT_DEEPGRAM_AUDIO_MODEL, t as DEFAULT_DEEPGRAM_AUDIO_BASE_URL } from "./audio-jD83M0CH.js";
...
function createDeepgramRealtimeTranscriptionSession(config) {
	...
	const joinTranscript = (left, right) => collapseWhitespace(left && right ? `${left} ${right}` : left || right);
	const clearFinalizeFallback = () => {
		if (finalizeFallbackTimer) {
			clearTimeout(finalizeFallbackTimer);
			finalizeFallbackTimer = void 0;
		}
	};
	const clearTurn = () => {
	...
	const updateTurn = (nextFinalized, nextPartial, transport) => {
		if (Buffer.byteLength(nextFinalized, "utf8") + Buffer.byteLength(nextPartial, "utf8") > DEEPGRAM_REALTIME_MAX_RETAINED_TRANSCRIPT_BYTES) {
			clearTurn();
			config.onError?.(/* @__PURE__ */ new Error(`Deepgram realtime retained transcript exceeded ${DEEPGRAM_REALTIME_MAX_RETAINED_TRANSCRIPT_BYTES} bytes`));
			transport.closeNow();
			return false;
		}
	...
	const flushTurn = () => {
		...
		clearTurn();
		if (full) config.onTranscript?.(full);
	};
		...
		switch (event.type) {
			case "Results": {
				...
				if (event.speech_final || event.from_finalize) {
					const nextFinalized = text ? joinTranscript(finalizedTranscript, text) : finalizedTranscript;
					if (!updateTurn(nextFinalized, "", transport)) return;
					flushTurn();
					return;
				}
				if (!text) return;
				if (event.is_final) {
					...
					if (!updateTurn(finalizedTranscript, text, transport)) return;
	...
	return createRealtimeTranscriptionWebSocketSession({
		...
		onOpen: () => {
			if (openedOnce) flushFinalizedTurn();
			else {
				openedOnce = true;
				clearTurn();
			}
		...
		onClose: (transport) => {
			...
			if (finalizedTranscript) finalizeFallbackTimer = setTimeout(() => {
				finalizeFallbackTimer = void 0;
