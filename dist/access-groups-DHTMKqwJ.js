...
const normalizeAllowFrom = (list) => {
	const rawEntries = (list ?? []).map((value) => normalizeOptionalString(String(value)) ?? "").filter(Boolean);
	const entries = [];
	for (const entry of rawEntries) if (entry.includes(",")) entries.push(...entry.split(",").map((s) => s.trim()).filter(Boolean));
