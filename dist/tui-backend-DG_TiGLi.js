...
import { x as runTui } from "./tui-BcDAAFMB.js";
import { t as SYSTEM_AGENT_ID } from "./agent-id-D7-xzIog.js";
import { i as loadSystemAgentOverview, r as formatSystemAgentStartupMessage } from "./overview-CDBIuwEP.js";
import { t as buildOnboardingWelcome } from "./onboarding-welcome-C6-wAHoi.js";
import { a as resolveSystemAgentVerifiedInferenceRoute } from "./verified-inference-CRg3rEqZ.js";
import { t as executeSystemAgentOperation } from "./operations-CPFqirC1.js";
...
import { i as assertLocalGatewaySetupMode, n as SystemAgentChatEngine, t as GATEWAY_SETUP_AFTER_WRITE } from "./chat-engine-CtIOPV6b.js";
...
async function runSetupHandoff(handoff, opts, runtime) {
	...
	if (handoff.target === "gateway") {
		...
		const [{ resolveGatewayPort }, { createClackPrompter }, { configureGatewayForSetup }, { readSetupConfigFileSnapshot, resolveQuickstartGatewayDefaults, writeWizardConfigFile }] = await Promise.all([
			import("./config/config.js"),
			import("./clack-prompter-DyhRpF9c.js"),
			import("./setup.gateway-config-CWCcBRM9.js"),
			import("./setup.shared-B1YQd8f0.js")
		]);
		const snapshot = await readSetupConfigFileSnapshot();
		if (!snapshot.exists || !snapshot.valid || !snapshot.hash) throw new Error("Gateway setup requires a valid saved config snapshot. Run `openclaw doctor --fix`, then retry.");
	...
	if (handoff.target === "search") {
		...
		const [{ runSearchSetupFlow }, { createClackPrompter }, { readSetupConfigFileSnapshot, writeWizardConfigFile }] = await Promise.all([
			import("./search-setup-D9t5LPIq.js"),
			import("./clack-prompter-DyhRpF9c.js"),
			import("./setup.shared-B1YQd8f0.js")
		]);
		const snapshot = await readSetupConfigFileSnapshot();
		if (!snapshot.exists || !snapshot.valid || !snapshot.hash) throw new Error("Web search setup requires a valid saved config snapshot. Run `openclaw doctor --fix`, then retry.");
		const baseConfig = snapshot.sourceConfig ?? snapshot.config;
		const searchSetup = await runSearchSetupFlow(baseConfig, runtime, createClackPrompter(), {
			preserveDisabledSearchState: false,
			beforePersistentEffect
		});
		if (searchSetup.outcome !== "completed") return;
		await beforePersistentEffect();
	...
	await (opts.runChannelsAdd ?? (await import("./add-BQmcrb4B.js")).channelsAddCommand)(handoff.channel ? { channel: handoff.channel } : {}, runtime, {
		hasFlags: false,
		beforePersistentEffect
	});
}
...
async function requireTuiVerifiedInference(opts) {
	...
	try {
		...
		if (route) {
			const [{ getPreparedModelCatalogSnapshot }, { resolveThinkingDefault }] = await Promise.all([import("./prepared-model-catalog-CDZ2A_-7.js"), import("./model-thinking-default-DiuF0wMx.js")]);
