...
import { a as resolveGatewayTokenSecretRefValue, i as resolveGatewayPasswordSecretRefValue, n as hasConfiguredGatewayAuthSecretInput } from "./auth-config-utils-n10ddJax.js";
...
async function ensureGatewayStartupAuth(params) {
	...
	const [resolvedTokenRefValue, resolvedPasswordRefValue] = await Promise.all([resolveGatewayTokenSecretRefValue({
		...
		hasPasswordOverride: hasGatewayPasswordOverrideCandidate({ authOverride: params.authOverride }),
		hasTokenFallback: Boolean(trimToUndefined(env.OPENCLAW_GATEWAY_TOKEN)),
		hasPasswordFallback: Boolean(trimToUndefined(env.OPENCLAW_GATEWAY_PASSWORD)) || hasConfiguredGatewayAuthSecretInput(params.cfg, "gateway.auth.password")
